...
AUTHORITY_SCHEMA = (
    "fin_ia_0_1_3_s1_08_tencent_wsa_exact_copy_ak_sk_r3_authority_v1_0"
)
RESULT_SCHEMA = (
    "fin_ia_0_1_3_s1_08_tencent_wsa_exact_copy_ak_sk_r3_result_v1_0"
)
CONTRACT_REF = "fin_0_1_3.S1_08.tencent_wsa_exact_copy_ak_sk_r3:v1"
RUN_SCOPE = "S1_08_PAID_BROAD_SEARCH_TENCENT_WSA_EXACT_COPY_AK_SK_R3_DIAGNOSTIC"
PREDECESSOR_ATTEMPT_ID = "fin013-s1-08-tencent-wsa-query-only-replacement-r2"


class TencentWSAExactCopyR3Error(RuntimeError):
    def __init__(self, code: str) -> None:
        self.code = code
        super().__init__(code)


def load_exact_copy_r3_authority(path: str | Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise TencentWSAExactCopyR3Error(
            "tencent_wsa_exact_copy_r3_authority_identity_invalid"
        )
    if body.get("predecessor_attempt_id") != PREDECESSOR_ATTEMPT_ID:
        raise TencentWSAExactCopyR3Error(
            "tencent_wsa_exact_copy_r3_predecessor_invalid"
        )
    ...
    if (
        ...
        raise TencentWSAExactCopyR3Error(
            "tencent_wsa_exact_copy_r3_authority_boundary_invalid"
        )
    compile_query_only_request(body.get("query") or {})
    return payload


def build_exact_copy_r3_terminal_result(
    *,
    admission_id: str,
    source_commit: str,
    status: str,
    ...
    if set((request_capture.get("request_body") or {}).keys()) != {"Query"}:
        raise TencentWSAExactCopyR3Error(
            "tencent_wsa_exact_copy_r3_terminal_request_not_exact"
        )
    if network_call_count not in {0, 1}:
        raise TencentWSAExactCopyR3Error(
            "tencent_wsa_exact_copy_r3_network_count_invalid"
        )
    body = {
