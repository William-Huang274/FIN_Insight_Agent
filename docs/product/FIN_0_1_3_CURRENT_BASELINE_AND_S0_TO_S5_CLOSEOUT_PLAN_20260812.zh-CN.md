...
25. **DELL 五单元动态完整案例（R1 容量缺陷已零调用关闭，12-node successor 待 live）**：有限 S2 回归已证明法说叙事不会越权成为 NumericFact；五个 RoleMethodPack、本案即时 GraphContextPack 和稳定 runner 均已资格化。自然 R1 的 Planner 和当前 S1/S2 成功，但 `value_capture` 合法形成 10 条 NumericFact，而 v1.3 静态上限为 8，故在 0 个 cell Judgment 时 fail closed。R1 永久保持失败。policy v1.4 已按五指标双期间关闭容量矛盾；同一 runner 的 successor 复用 immutable Planner/S1/S2，只允许五个分析、五个严格交卷和两次综合。scope decision 还显式绑定 `evidence_mode=immutable_dynamic_R1_planner_current_S1_S2_prefix_no_new_evidence`；两次相关复证均为 `83/83`、全仓 `420/420`。下一步仅为 clean preflight、fresh authority 和一次 successor live，之后依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容。一次完整运行不能自动等同 S3 acceptance。
