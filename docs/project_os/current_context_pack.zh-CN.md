...
RC-S1-024 的 provider-neutral v2 successor 已完成零调用工程复证：typed request 先编译独立 need，精确未映射业务词只获原词权限，同口径年份形成候选组，最终 review prefix 按 facet 有界轮转；旧 v1 合同和 R1 失败保持不可变，隐藏 test／holdout 未物化或读取。设计提交 `1fa65512` 和独立 R2 authority `32a2a673` 已 clean push；唯一一次 COST R2 label-blind candidate execution 已在 `cuda:0`／FP16 成功完成，5 个命题、113 个 RetrievalNeed、每个 reranker 1,440 对，0 CPU vector fallback／network／generation model／retry，evaluator reference 未加载，candidate public 已在 `7862c393` 冻结。分离 evaluator 继续使用 R1 的参考、业务模板和门槛，只增加向后兼容的 v1.1 schema；全仓 `631 passed`、active baseline 和 7,127-file secret scan 通过。下一步是 clean push evaluator design、独立 exact-once authority 和零向量评价；评价失败不进入 R3，评价通过也不等于 S1 资格。每个纵切都必须进入当前 Pack／Workbench，局部组件不能留到最终 big-bang integration。
