...
8. **S1-C1 query family、对象编译器与 typed fact route（已完成工程门）**：17 facet 已且仅映射到 11 类问题；混合请求拆成同 cell 的 narrative／fact sibling。1,805 个 current child 编译出 20,270 个去重 claim／metric-row／context 候选，2,433 个重叠切块重复已合并并保留 lineage；高管年龄表等 257 张非金融数值表已拒绝。24 类指标可路由到 typed fact request，但当前公司财务事实 mart 不存在，故返回 S2 typed gap；没有把表格行冒充 NumericFact。
9. **当前下一门——S1-C2/C3 多检索器与 Cross-Encoder 有界对照**：在本轮相同 20,270 个去重对象、同硬过滤、同预算上比较 BM25、BGE-M3 三模式和 Qwen Embedding；随后只在同一候选并集比较 BGE/Qwen Reranker。评测必须报告真实业务错因、独有有效候选、延迟和显存，不能只报 Recall/MRR。
