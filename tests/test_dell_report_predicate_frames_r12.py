...
def test_r12_candidate_surface_is_necessary_for_positive_frame(
    target_id: str,
    text: str,
) -> None:
...
def test_r12_no_comma_independent_events_split_and_cannot_union_roles(
    target_id: str,
    text: str,
) -> None:
...
def test_r12_compound_company_subject_keeps_one_shared_predicate_frame() -> None:
...
def test_r12_open_vocabulary_right_owner_splits_without_entity_allowlist(
    right_owner: str,
) -> None:
    text = (
...
def test_r12_fronted_functional_adjunct_proves_shared_subject(
    adjunct: str,
) -> None:
...
def test_r12_case_preserved_predicate_token_collision_is_owner_not_verb(
    collision_owner: str,
) -> None:
    text = (
        "Dell quoted support for USD 150 and "
...
def test_r12_unseen_material_clause_head_creates_ambiguity_barrier(
    unknown_head: str,
) -> None:
    text = (
...
def test_r12_casefold_length_alignment_failure_is_a_typed_role_barrier() -> None:
    text = (
...
def test_r12_shared_object_or_auxiliary_continuation_does_not_false_split(
    text: str,
) -> None:
    decisions = frame_boundary_decisions(text)
...
def test_r12_predicate_at_right_offset_zero_is_shared_subject_continuation() -> None:
    text = (
...
def test_r12_semantic_state_is_typed_and_bound_by_exact_scope_edge(
    text: str,
    ...
    assert edge.source_modifier_frame_id.startswith("MODIFIER::R12::")
...
def test_r12_unrelated_state_change_does_not_veto_independent_positive_frame() -> None:
    text = (
...
def test_r12_price_is_bound_to_typed_argument_group_not_record_global_value(
    text: str,
...
def test_r12_non_hardware_price_near_neighbors_fail_closed(text: str) -> None:
...
def test_r12_price_copresence_without_direct_attachment_never_admits_relation(
    text: str,
) -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
...
def test_r12_affirmative_price_attachment_has_traversable_span_proof(
    text: str,
    proof_type: str,
) -> None:
    frame = extract_predicate_frames(
...
def test_r12_cross_group_product_and_hardware_price_cannot_complete_asp() -> None:
    text = (
...
def test_r12_generic_hardware_price_has_no_specific_product_relation() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
...
def test_r12_multiple_product_price_relations_fail_closed() -> None:
    text = (
...
def test_r12_multiple_products_inside_one_price_group_fail_closed() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
        text=(
...
def test_r12_inherits_r8_fresh_negative_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
...
def test_r12_inherits_frozen_positive_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
    ...
    assert assessment["classification"] == "complete_bounded_target_package"
    assert assessment["accepted_frame_id"].startswith("FRAME::R12::")
...
def test_r12_inherits_all_63_r7_negative_semantic_attacks() -> None:
...
def test_r12_frame_records_and_bindings_are_deterministic_and_immutable() -> None:
    kwargs = {
...
def test_r12_open_vocabulary_and_lowercase_right_owner_isolates_roles(
    text: str,
) -> None:
    decisions = frame_boundary_decisions(text)
    assert len(decisions) == 1
...
def test_r12_unseen_structural_fronted_adjunct_preserves_shared_subject(
    adjunct: str,
) -> None:
    text = f"NVIDIA provides GPUs and {adjunct} ships them to Dell."
    decision = frame_boundary_decisions(text)[0]
...
def test_r12_intervening_governing_nominal_head_cannot_be_hardware_price(
    governing_head: str,
) -> None:
    text = (
...
def test_r12_direct_hardware_price_and_explicit_price_head_remain_positive() -> None:
    for text in (
