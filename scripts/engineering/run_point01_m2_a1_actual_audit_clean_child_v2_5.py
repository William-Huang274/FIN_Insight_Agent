"""Clean-process executor for a future v2.5 externally-admitted package.

It is frozen into v2.5 but receives no authority in Phase B0.2.  The actual
path remains exact-admission gated; ``--help`` and the isolation probe create
no authority, runtime, or transport resources.
"""
...
def _json(path: Path) -> Mapping[str, Any]:
    try:
        ...
        raise ValueError("m2_a1_json_input_unreadable") from exc
    except json.JSONDecodeError as exc:
        raise ValueError("m2_a1_json_input_invalid") from exc
...
def transport_isolation_probe() -> int:
    ...
    canary = M2A1AuditCanary(
        allowed_temporary_roots=(ROOT / ".phase_b0_2_probe_never_materialized",),
    ...
    print(json.dumps({
        "status": "v2_5_clean_child_canary_before_harness_import_pass",
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Consume one exact v2.5 M2-A1 receipt inside a clean Python child.")
