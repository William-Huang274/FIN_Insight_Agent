...
2026-09-15用户进一步提出并要求作为设计方向保留：独立复核可以发生在Lead最终判断之前；出现复核问题后，Lead应整理自己的上下文并形成任务的最终判断；Lead及时参与、主动发现与安排解决问题是自主runtime的基础，不能日常依赖人工发现错误和安排返修。本节更新5.2原来偏重成文后复核的建议，仍属设计记录，未实施。
