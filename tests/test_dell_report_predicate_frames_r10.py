...
def test_r10_no_comma_independent_events_split_and_cannot_union_roles(
    target_id: str,
    text: str,
) -> None:
...
def test_r10_compound_company_subject_keeps_one_shared_predicate_frame() -> None:
...
def test_r10_open_vocabulary_right_owner_splits_without_entity_allowlist(
    right_owner: str,
) -> None:
    text = (
...
def test_r10_predicate_at_right_offset_zero_is_shared_subject_continuation() -> None:
    text = (
...
def test_r10_semantic_state_is_typed_and_bound_by_exact_scope_edge(
    text: str,
    ...
    assert edge.source_modifier_frame_id.startswith("MODIFIER::R10::")
...
def test_r10_unrelated_state_change_does_not_veto_independent_positive_frame() -> None:
    text = (
...
def test_r10_price_is_bound_to_typed_argument_group_not_record_global_value(
    text: str,
...
def test_r10_non_hardware_price_near_neighbors_fail_closed(text: str) -> None:
...
def test_r10_cross_group_product_and_hardware_price_cannot_complete_asp() -> None:
    text = (
...
def test_r10_generic_hardware_price_has_no_specific_product_relation() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
...
def test_r10_multiple_product_price_relations_fail_closed() -> None:
    text = (
...
def test_r10_multiple_products_inside_one_price_group_fail_closed() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
        text=(
...
def test_r10_inherits_r8_fresh_negative_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
...
def test_r10_inherits_frozen_positive_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
    ...
    assert assessment["classification"] == "complete_bounded_target_package"
    assert assessment["accepted_frame_id"].startswith("FRAME::R10::")
...
def test_r10_inherits_all_63_r7_negative_semantic_attacks() -> None:
...
def test_r10_frame_records_and_bindings_are_deterministic_and_immutable() -> None:
    kwargs = {
