...
NVDA raw authority 已独立编译，仍未签发。第三案绑定同一 frozen blind input、runtime policy、DeepSeek Pro 参数与 10–12 call 动态链；NVDA model-visible case=`13 Evidence / 3 Numeric / 4 gaps`。Evaluator v1.3 仅作为返回后的统一评分器，未进入 prompt。一次 exact-once、0 retry/fallback；完成后停止在 NVDA supervision boundary，不自动调用 supervisor、生成 corrected candidate 或做产品晋升。

NVDA R1 已 exact-once 消费并在 Lead 后 terminal。唯一 Provider 输出为 `ok/stop`，但 `$5.36T` 被 execution-time numeric compiler 拒绝。该数值由冻结 NVDA_E13 `5359 USD_billion / 约 5.359 万亿美元` 合法换算而来；根因是本地只生成 billion suffix、未生成 trillion 等价与舍入表面。修复只扩展同一 typed authority 的单位换算，不改变 prompt、blind input 或未知数字门禁；immutable Lead replay=pass。R1 仍是不可改写的 incomplete failure，不能进入 hidden score。任何 replacement 都需新的独立 authority，且仍不得读取 DELL/MU correction 或 hidden Gold。

用户已单独批准 NVDA replacement R2。authority 绑定同一 NVDA model-visible digest、Prompt、DeepSeek Pro 参数、runtime policy 与 12-call/0-retry 上限；仅替换本地 numeric-scale validator 为已验证的等价单位 compiler。fresh root 与 nonce 禁止复用 R1；前两案 raw/correction、supervisor prompt 和 hidden Gold 仍隔离。R2 authority 零调用验证=`34 focused / 93 broad passed`，尚未签发。clean commit/push 与 scoped preflight 通过后仅可 exact-once 执行一次；新 L1 停止且不得自动 R3。

NVDA R2 已 exact-once 完成 Lead＋6 Specialist＋Synthesis＋Writer＋Verifier=`10/10 calls/captures`，tokens=`31,947/6,649/38,596`、USD=`0.0304715`、0 retry/fallback；R1 numeric-scale false positive 未复发。execution-time v1.3=`5 L1/2 L2/27 L3`；零调用审计发现条件清单中 cash-flow 与 P/E 共现造成 2 L1＋1 L2 误报，同时旧 evaluator 漏掉 Writer 将 Gap ID 写进 `evidence_ids` 的 citation-role 错误。v1.4 对三案统一做 path-aware bridge 与 typed ID-role replay，raw mutation=0，最终 DELL=`3/1/23`、MU=`8/2/14`、NVDA=`4/1/27`。三案 raw 均 complete/hidden-scoreable 但 quality fail；三案 deterministic supervision boundary=`27/24/32 rows`。S2-05 raw campaign 完成；supervisor model call、corrected candidate、formal hidden score 和业务晋升仍为 0，需另行统一裁决。

三案例统一 Supervisor authority decision 已完成，但 admission 尚不能签发。统一含义是同一 typed 协议和验收、三个 case-isolated execution，不是跨案共用 Prompt。每案最多 1 次 SupervisorPlan＋10 次 corrected graph call，campaign 最多 33 次、retry/fallback=0；Supervisor 只产出 correction plan，originating node 返工、下游依赖重算和 fresh corrected identity 必须由新的 governed runner 执行。逐案 `L1=0/L2=0`、空 counterevidence 与未校准 threshold 的 class-specific closure、Verifier prior-class coverage 是硬门；三案全部通过才可声明 recoverability proven。当前 case-qualified plan、coverage/citation owner、partial graph runner、candidate-freeze 后 scoring guard 四项均缺失，故只允许一个 zero-call shared implementation package；本轮 model/provider/network/admission=0。

上述唯一 zero-call shared implementation package 已工程通过。v1.2 boundary 使用 case-qualified correction ID；所有当前 v1.4 finding class 均有 typed owner/action。SupervisorPlan 由单一 spec 同时生成 prompt、schema、validator 和 fake，并按节点归并 correction，只允许选择本案 Evidence/Numeric/Gap alias，不产出报告或新金融事实。corrected runner 计算 originating node 与完整 downstream closure，强制 fresh identity、capture-first、0 retry/fallback、raw immutable 和 candidate freeze 后评分。三案 full-fake 最坏路径均为 `1+10=11 calls`；真实 frozen raw 零调用预检为 DELL=`8 calls/33,590 chars`、MU=`10/28,104`、NVDA=`10/35,650`，低于硬上限。本结果仅为 engineering/fixture/capacity proof；自然 SupervisorPlan、corrected candidate、L1/L2 修复和内容增益仍未测量，下一步必须先做 clean-commit independent fresh proof，再单独决定 admission。

clean-commit independent fresh proof 已通过。proof 固定在 clean/synced commit `60b66bc9...0ef`，由两个 Git archive、两个独立 Python process 和两份按字节隔离注入的受限 raw 输入组成；归一化输出完全一致，24 个 supervision/runtime 测试均通过。三案当前 evaluator digest、finding/correction 数、Supervisor request chars 与 corrected graph capacity 均重新计算并与冻结值一致：DELL=`27/27 findings/corrections, 33,590 chars, 8 calls`，MU=`24/24, 28,104, 10`，NVDA=`32/32, 35,650, 10`。源 raw 与目标 supervision tree 前后 digest 不变，model/provider/network/admission/candidate/score/promotion 均为 0。复证同时暴露一个 release-portability 债务：历史 blind-input policy 绑定 Windows CRLF 工作树 SHA，而 Git blob 为 LF；proof 只在“Git clean＋CRLF→LF 后逐字节等于 blob”的严格条件下投影这一份输入，不放宽语义或权限。该债务后传 S5 portability，当前同主机 admission 仍必须另行裁决；DeepSeek 自然 SupervisorPlan、L1/L2 修复与内容质量仍完全未证明。
