...
def _verify_predecessor_bindings(program: Mapping[str, Any]) -> None:
    ...
    for ref_key, sha_key in (
        ...
        ("audit_failure_ref", "audit_failure_sha256"),
    ):
...
def _artifact_state(path: Path, *, model_id: str, kind: str) -> dict[str, Any]:
    ...
    try:
        identity = (
            local_embedding_model_identity_v3(path, model_id)
            if kind == "embedding"
            else local_cross_encoder_model_identity_v3(path, model_id=model_id)
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize the zero-call S1 large-model challenger v2 preflight "
            "under the recursive identity-v3 contract."
        )
    )
    parser.add_argument(
        "--program",
        default=(
            "configs/retrieval/"
            "fin_ia_0_1_3_s1_large_model_challenger_program_v1_1.json"
        ),
    )
    ...
    parser.add_argument(
        ...
        default=(
            "configs/retrieval/"
            "fin_ia_0_1_3_s1_large_model_challenger_preflight_result_v1_1.json"
        ),
    )
...
def main() -> int:
    ...
    output = {
        "schema_version": (
            "fin_ia_s1_large_model_challenger_preflight_result_v1_1"
        ),
        ...
        "implementation_refs": [
            ...
            _bound_ref(
                ...
                "materialize_s1_large_model_challenger_preflight_v2.py"
            ),
        ],
        **gate,
        "known_boundary": (
            "This zero-call preflight does not download or load a model, "
            "compute vectors or scores, inspect forbidden/hidden references, "
            "grant Evidence or NumericFact authority, qualify S1, or promote "
            "a runtime. A suitable host must still open a new recorded attempt."
        ),
    }
