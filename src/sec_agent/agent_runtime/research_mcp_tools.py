"""Adapt scoped research tool requests to MCP calls and typed results."""
...
from .source_family_compiler import (
...
class _MCPTransportError(MCPToolAdapterError):
    pass
...
def compose_mcp_graph_run(
    ...
    *,
    ...
    for branch_id in selected:
        ...
        branch_package = project_research_method(foundation, (branch_id,))
...
class MCPToolLaneAdapter(AbstractContextManager["MCPToolLaneAdapter"]):
    ...
    def __enter__(self) -> "MCPToolLaneAdapter":
        with self._lock:
            if self._portal is not None:
                raise MCPToolAdapterError("mcp_adapter_already_open")
            try:
                from mcp import Client
            except ImportError as exc:  # pragma: no cover - optional extra
                raise MCPToolAdapterError("mcp_v2_dependency_missing") from exc
    ...
    def _execute(self, value: Mapping[str, Any], lane: str) -> ToolLaneResult:
        ...
        try:
            ...
            if task.lane != lane:
                raise MCPToolAdapterError("mcp_lane_callable_mismatch")
            ...
            if task is None:
                raise MCPToolAdapterError("mcp_tool_lane_task_invalid") from exc
            owner = (
                "tool_transport"
    ...
    def _validate_binding(self, lane_task: ToolLaneTask) -> None:
        ...
        for code, valid in checks.items():
            if not valid:
                raise MCPToolAdapterError(code)
    ...
    def _scope(
        ...
        if call.error or call.content is None:
            raise MCPToolAdapterError("mcp_method_binding_failed")
        scope = ResearchRunScope.model_validate(call.content.get("run_scope"))
        package = call.content.get("method_package")
        method = package.get("method") if isinstance(package, Mapping) else None
        if not isinstance(method, Mapping):
            raise MCPToolAdapterError("mcp_method_projection_missing")
        if (
            ...
            raise MCPToolAdapterError("mcp_method_run_scope_binding_mismatch")
        return scope
        ...
        for request_index, raw_request in enumerate(
            ...
            if "source_document" in raw_request:
                if not self._source_read_enabled or set(raw_request) != {"source_document"}:
                    raise MCPToolAdapterError("source_document_read_not_authorized")
            ...
            if not isinstance(hits, list):
                raise MCPToolAdapterError("mcp_evidence_search_hits_invalid")
            ids = [
                str(hit["evidence_id"])
                for hit in hits
            ...
            if ids:
                ...
                if not isinstance(evidence, list):
                    raise MCPToolAdapterError("mcp_evidence_read_items_invalid")
                for row in evidence:
                    if not isinstance(row, Mapping) or row.get("writer_citable") is not True:
                        raise MCPToolAdapterError("mcp_evidence_authority_failed")
                    items.append(
                        {
    ...
    def _compiled_evidence(
        ...
        *,
        ...
        if compiler is None:  # pragma: no cover - guarded by caller
            raise MCPToolAdapterError("mcp_source_family_compiler_required")
    ...
    def _reviewed(
        ...
        *,
        ...
        if target.reviewed_index_digest != index.index_digest:
            raise MCPToolAdapterError("mcp_reviewed_index_binding_mismatch")
        eligible = {
            row.evidence_id: row
        ...
        for hit in hits:
            if not isinstance(hit, Mapping):
                raise MCPToolAdapterError("mcp_evidence_search_hit_invalid")
    ...
    def _local(
        ...
        *,
        ...
        if not isinstance(candidates, list):
            raise MCPToolAdapterError("mcp_local_candidates_invalid")
        requested_scope = {
        ...
        if (
            ...
            raise MCPToolAdapterError("mcp_local_scope_binding_failed")
        ...
        for row in candidates:
            if (
                ...
                raise MCPToolAdapterError("mcp_candidate_authority_failed")
            candidate_values = {
    ...
    def _external(
        ...
        *,
        ...
        if status not in {"ok", "zero_results"} or not isinstance(candidates, list):
            raise MCPToolAdapterError("mcp_external_discovery_invalid")
        if len(candidates) > request_limit:
            raise MCPToolAdapterError("mcp_external_discovery_limit_exceeded")

        for row in candidates:
            if not isinstance(row, Mapping) or row.get("candidate_is_not_evidence") is not True:
                raise MCPToolAdapterError("mcp_external_candidate_authority_failed")
            items.append(
                {
                    **row,
                    "result_state": "retrieval_candidate",
        ...
        for row in candidates[:capture_limit]:
            ...
            if (
                ...
                raise MCPToolAdapterError("mcp_external_capture_authority_failed")
            items.append(
                {
                    **capture.content,
        ...
        for request in lane_task.task.fact_requests:
            if "calculation" in request:
                if set(request) != {"calculation"}:
                    raise MCPToolAdapterError("mcp_calculation_request_fields_invalid")
                ...
                if (result.get("arithmetic_verified") is not True or result.get("numeric_fact_authority") is not False
                    ...
                    raise MCPToolAdapterError("mcp_calculation_response_invalid")
            ...
            if not isinstance(results, list):
                raise MCPToolAdapterError("mcp_financial_results_invalid")
            for row in results:
                if not isinstance(row, Mapping):
                    raise MCPToolAdapterError("mcp_financial_result_invalid")
                if row.get("status") == "resolved":
                    for fact in row.get("facts", []):
                        if (
                            ...
                            raise MCPToolAdapterError("mcp_numeric_fact_authority_failed")
                        items.append(
                            {
                    ...
                    raise MCPToolAdapterError("mcp_financial_status_invalid")
    ...
    def _call(self, name: str, arguments: Mapping[str, Any]) -> _Call:
        if self._portal is None or self._client is None or self._discovery_digest is None:
            raise MCPToolAdapterError("mcp_adapter_not_open")
    ...
    def _keys(request: Mapping[str, Any], allowed: frozenset[str], label: str) -> None:
        ...
        if unexpected:
            raise MCPToolAdapterError(f"{label}_keys_invalid:{','.join(unexpected)}")
...
def _iso_date(value: Any, label: str) -> date:
    try:
        ...
        raise MCPToolAdapterError(f"mcp_{label}_invalid") from exc
...
def _as_of(value: Any) -> str:
    ...
    if parsed.tzinfo is None or parsed.utcoffset() is None:
        raise MCPToolAdapterError("mcp_research_as_of_timezone_required")
