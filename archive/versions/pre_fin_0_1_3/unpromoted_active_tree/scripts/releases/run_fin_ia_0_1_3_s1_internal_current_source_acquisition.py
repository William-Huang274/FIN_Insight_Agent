...
DEFAULT_ADMISSION = ROOT / (
    ".codex_runtime/fin013_s1_internal_current_source_acquisition/admission.json"
)
DEFAULT_RUNTIME = ROOT / (
    "data/workbench_private/fin_0_1_3_s1_internal_current_source_acquisition/live-r1"
)
DEFAULT_LEDGER = ROOT / (
    ".codex_runtime/shared/fin013_s1_internal_source_acquisitions.sqlite3"
)
DEFAULT_RESULT = ROOT / (
    "configs/releases/fin_ia_0_1_3_s1_internal_"
    "current_source_acquisition_result_v1_0.json"
)
MODULE_PATH = ROOT / "src/sec_agent/s1_internal_current_source_acquisition.py"
_EMAIL_RE = re.compile(r"^[^\s@]+@[^\s@]+\.[^\s@]+$")


class InternalSourceAcquisitionRunnerError(RuntimeError):
    pass
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise InternalSourceAcquisitionRunnerError(
...
def _require_clean_synced() -> str:
    if _git("status", "--porcelain"):
        raise InternalSourceAcquisitionRunnerError(
            "internal_source_acquisition_clean_worktree_required"
        )
    head = _git("rev-parse", "HEAD")
    upstream = _git("rev-parse", "@{u}")
    if head != upstream:
        raise InternalSourceAcquisitionRunnerError(
            "internal_source_acquisition_synced_head_required"
        )
    return head
...
def _dns_guard() -> dict[str, Any]:
    ...
    for host in ("data.sec.gov", "www.sec.gov"):
        ...
        if not addresses:
            raise InternalSourceAcquisitionRunnerError(
                "internal_source_acquisition_dns_empty"
            )
        forbidden = []
        ...
        if any(address not in synthetic for address in forbidden):
            raise InternalSourceAcquisitionRunnerError(
                "internal_source_acquisition_dns_forbidden_address"
            )
        ...
        rows.append(
            {
                ...
                "controlled_synthetic_range_observed": uses_synthetic,
            }
        )
    body = {
...
def _preflight(*, policy_path: Path, proof_path: Path) -> dict[str, Any]:
    ...
    valid = (
        ...
        and int(policy["budgets"]["network_call_ceiling"]) == 8
        and int(policy["budgets"]["retry_ceiling"]) == 0
    )
    body = {
        "schema_version": (
            "fin_ia_0_1_3_s1_internal_current_source_acquisition_preflight_v1_0"
        ),
        ...
        *,
        ...
        if remaining <= 0:
            raise OfficialSourceAttemptError(
                "internal_source_acquisition_overall_timeout_exceeded"
            )
...
def execute(
    *,
    ...
    if runtime_root.exists() or output_path.exists():
        raise InternalSourceAcquisitionRunnerError(
            "internal_source_acquisition_exact_once_output_exists"
        )
    commit = _require_clean_synced()
    preflight = _preflight(policy_path=policy_path, proof_path=proof_path)
    if preflight["status"] != "pass":
    ...
    if admission.get("implementation_commit") != commit:
        raise InternalSourceAcquisitionRunnerError(
            "internal_source_acquisition_execution_commit_binding_invalid"
        )
    ...
    try:
        terminal = execute_internal_source_acquisition_guarded(
            policy=policy,
            admission=admission,
            runtime_root=runtime_root,
            ledger=ledger,
            transport=_DeadlineTransport(deadline=time.monotonic() + 180),
            observed_at=observed_at,
        )
    finally:
        if previous is None:
