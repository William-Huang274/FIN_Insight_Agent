"""Review working papers and bind findings to their supporting sources."""
...
async def open_case_review_composition(*, authority, model_config, api_key, public_sink, private_sink):
    ...
    from .model_execution_policy import file_sha256, require_data_authority_binding
    ...
    artifacts = CaseArtifacts(seed["papers"])
    ...
    with open_approved_data_composition(run_invocation_id=authority.run_invocation_id,
        ...
        async with Client(data.mcp_server, raise_exceptions=False, read_timeout_seconds=120) as client:
            ...
            if authority.case_convergence_scope is not None:
                from .report_synthesis_agent import build_case_output_agent, build_case_convergence_graph
