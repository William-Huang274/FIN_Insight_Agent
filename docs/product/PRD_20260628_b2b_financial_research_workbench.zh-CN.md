...
Owner 随后批准在同一 S1 边界内继续零网络修复。successor 不修改历史 v1 policy／result，而是采用新的 `literal_phrase_groups_v1` 合同：金额、backlog、供需方向和时间范围分别作为短 literal anchor，由最小窗口负责组合；任意 `required_patterns` regex surface 在读取正文前静态拒绝。两个 clean Git archive／fresh process 已对两份 immutable Reader capture 独立重放并获得相同结果：Dell `3/3`、Micron `2/2` fragments，Pack 由 `22→27 Evidence／15→14 gaps`，三门 `core／supplier／valuation-input=true`，网络／模型／retry=`0/0/0`，proof=`b8a9b04d...74bd04`。仍须持久化 corrected Pack 和公开 result，之后才可签发一次 changed-input DeepSeek comparison；clean replay 不能直接写成报告质量通过。
