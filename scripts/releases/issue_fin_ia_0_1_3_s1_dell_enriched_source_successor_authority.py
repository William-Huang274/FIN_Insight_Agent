...
RUNNER_PATH = ROOT / (
    "scripts/releases/"
    "run_fin_ia_0_1_3_s1_dell_enriched_source_successor.py"
)
DEFAULT_OUTPUT = ROOT / (
    "configs/releases/"
...
def main() -> int:
    ...
    run_id = "fin013_s1_dell_enriched_source_" + canonical_digest(
        {
    ...
    binding_paths = (
        ...
        PROOF_PATH,
        RUNNER_PATH,
        ROOT / "src/sec_agent/s1_dell_enriched_source_successor.py",
        ROOT / "src/sec_agent/market_data_adapter.py",
        ROOT / "src/sec_agent/official_source_attempt_program.py",
        ROOT / "src/sec_agent/s1_dell_targeted_source_supplement.py",
        ROOT / "src/sec_agent/s1_dell_targeted_source_recovery.py",
    ...
    body = {
        ...
        "implementation_commit": head,
        "user_authority": (
            "annotation_1_owner_approved_steps_1_to_5_with_alpha_vantage_primary_and_akshare_shadow"
        ),
        ...
        "file_bindings": {_relative(path): file_sha256(path) for path in binding_paths},
        ...
        "known_boundary": (
            "This admission authorizes exactly two official-source attempts, one "
            "Alpha Vantage primary invocation and one non-promoting AKShare shadow "
            "invocation. It authorizes no retry, model call, valuation conclusion, "
            "recommendation, business promotion or release."
        ),
    }
