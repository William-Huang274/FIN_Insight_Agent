...
def test_checked_in_owner_decision_is_externally_anchored_and_exact() -> None:
    decision = load_owner_data_gate_decision()
...
def test_schema_valid_resigned_decision_cannot_self_authorize() -> None:
    ...
    resigned = DataAccessPolicyDecision.model_validate_json(json.dumps(raw))

    with pytest.raises(
        DataAccessPolicyError,
        match="owner_data_gate_decision_untrusted_digest",
    ):
        validate_trusted_owner_data_gate_decision(resigned)
