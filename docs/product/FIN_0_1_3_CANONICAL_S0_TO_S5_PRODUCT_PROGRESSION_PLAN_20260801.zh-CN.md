...
状态：`accepted planning normalization / S0 exit-contract v2 reference-role implementation engineering pass / one host proof authorized not executed`
...
| --- | --- | --- | --- | --- |
| S0 可信基础 | 同一个研究任务在宿主机和干净环境中使用同一套代码、合同和资源，不因隐藏文件或机器路径产生假通过 | Runtime resource、typed reference role、typed environment、active tests、capture/terminal result、两套 disposable parity | collect-all 零漏分；host engineering proof 全绿；独立双-disposable formal proof 全绿；RC-P36-090–094 可关闭 | 模型比较、真实金融结论、DELL/MU/NVDA 产品验收 |
...
未知角色继续 fail closed。实现必须先做 collect-all 并一次报告全部未分类角色，不能再按字段逐个补例外。旧 T03 的 `1/1` 预算不重置；v2 仅获得一个实现包、一个 host engineering proof 和一个独立双-disposable formal proof，均无自动 retry 或 replacement。
