...
ISSUANCE_TEST = ROOT / (
    "tests/contract/test_fin_0_1_s4_t05_dell_r9_specialist_validated_"
    "segment_union_capacity_fresh_exact_admission_issuance.py"
)
NEXT_ACTION = (
...
class S4T05DellR9CapacityIssuanceError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T05DellR9CapacityIssuanceError(code)
...
def build_issuance() -> tuple[dict[str, Any], dict[str, Any]]:
    _require(not ADMISSION.exists(), "R9_capacity_admission_already_exists")
    _require(not ISSUANCE.exists(), "R9_capacity_issuance_already_exists")
    proof = _load(DECISION)
    _require(
        proof["status"]
        == "pass_zero_call_independent_profile_v3_capacity_fresh_proof_"
        "R9_admission_issuance_pending",
        "R9_capacity_proof_status_invalid",
    )
    _require(
        build_decision() == proof,
        "R9_capacity_frozen_proof_not_reproducible",
    )
    ...
    _require(
        ...
        "R9_capacity_proof_code_or_test_binding_drift",
    )
    ...
    _require(
        ...
        "R9_capacity_admission_digest_mismatch",
    )
    _require(
        admission.research_profile_ref
        == S4_DELL_THREE_CELL_RESEARCH_PROFILE_V3_REF,
        "R9_capacity_profile_v3_missing",
    )
    ...
    _require(
        ...
        == (6000, 8192, 24576),
        "R9_capacity_resolver_binding_mismatch",
    )
    ...
    def _forbidden_provider(**_: Any) -> dict[str, Any]:
        ...
        provider_calls += 1
        raise AssertionError("provider_forbidden_during_R9_capacity_issuance")
    ...
    _require(
        provider_calls == 0,
        "provider_called_during_R9_capacity_issuance",
    )
    ...
    _require(
        ...
        "R9_capacity_identity_consumed_before_issuance",
    )
    _require(
        database_sha == _sha256(database_path)
        and object_sha == _tree_digest(object_root),
        "R9_capacity_issuance_preflight_changed_runtime",
    )

    issuance = {
        "schema_version": (
            "fin_ia_0_1_s4_t05_dell_r9_specialist_validated_segment_"
            "union_capacity_fresh_exact_admission_issuance_v1_0"
        ),
        "issuance_id": (
            "S4-T05-DELL-R9-SPECIALIST-VALIDATED-SEGMENT-UNION-"
            "CAPACITY-FRESH-EXACT-ADMISSION-ISSUANCE-R1"
        ),
        "issued_at": "2026-07-28T04:00:00+08:00",
        ...
        "zero_call_preflight": {
            ...
            "capacity_resolver_recomputed": True,
        ...
        "stage_acceptance": {
            "RC_P36_065": (
                "R9_admission_issued_unconsumed_exact_live_"
