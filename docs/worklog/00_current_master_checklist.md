...
- [x] 两项只读本地审计完成：DELL 2023 accession目录仅有主HTML+metadata，完整DTS/company linkbases/accession archive与EFM/taxonomy profile均不存在，Arelle继续`HOLD_SOURCE_CAPTURE_PACKAGE`；DELL FY26同一9页PDF没有既存legacy artifact。先用现有`official_pdf.py`跑低内存pypdf基线，layout-rich基线继续复用`pdf_layout.py`而不另造harness；后者当前依赖未就绪，不伪称已跑。
