...
For isolated worktrees, `FINSIGHT_E2E_PYTHON` may select an existing qualification interpreter; the fixture explicitly imports the worktree's Python sources. After building, `FINSIGHT_E2E_DIRECT_BFF=1` serves the built frontend directly from the test BFF and does not start Vite on 5173. The project-library spec also checks the actual project-filtered catalogue endpoint and reader return state.
