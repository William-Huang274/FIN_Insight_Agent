...
Research Context Closure 的结构门和当前 profile 容量门已通过，但唯一 Chat R1 已以 `IncompleteRead` terminal failure 消费，不能自动重跑。Owner 已授权先修 partial capture、做零调用回放，再执行一次真实 replacement。共享修复已提交并由 formal zero-call replay R1 通过：两个 fresh targeted process 各 10 passed，全仓 280 passed，compileall、active baseline 与 secret scan 同时通过；旧 R1 保持不可变，旧 partial 不可恢复。当前下一步是把该 replay 与新的 replacement scope decision／runner gate 一起提交并推送，再签发唯一一次新的 Chat Run／Attempt。第 7 步五单元、其他 RoleMethodPack、Responses 和完整图 handler仍不自动开始。
