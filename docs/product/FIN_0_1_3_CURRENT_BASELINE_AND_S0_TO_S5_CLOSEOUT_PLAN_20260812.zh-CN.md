...
日期：2026-08-12
状态：`repository_baseline_complete / S1_VS1_to_VS3_vertical_slices_integrated / S1_VS4_VS5_and_qualification_open / S2_company_fact_mart_pass / S2_product_bridge_open / fixed_pack_and_dynamic_single_cell_accepted / DELL_R7_complete_five_cell_report_contract_pass_truth_reconciliation_fail / full_product_chain_blocked_until_S1_qualified / product_iteration_not_closed`
## 1. 这份文件拥有哪项真值
...
1. **spine／覆盖矩阵／评测程序基础与 VS1／VS2／VS3 Runtime 迁移已完成，隐藏资格集仍开放。** machine-readable spine、A–J 矩阵和开发资产已进入当前 R17 Runtime；valid／frozen test／heterogeneous holdout 现在可以在 VS3 合同稳定后另行预注册，不能把已观察案例改名为隐藏集。R16 的 result-local ref 门继续有效；旧 R14／R15、VS3 v1.6／v1.7 失败均保持历史证据，不被原地改写。
...
5. **VS4：当前下一项，Coverage 驱动的第二轮补证纵切。** 用 DELL 营运资金、发行人反方、上游反方验证 residual gap→counter-hypothesis→route→capture→decision→Coverage delta→typed stop／ready；补证必须由现有账本证明属于未执行路线，而不是本地对象、解析、召回或排序故障。随后让 MU／NVDA 从自然问题走同核心。案例用于暴露范式缺陷，不用于刷案例通过率。
