...
def test_issuance_loads_in_runner_without_provider_or_execution_call() -> None:
    ...
    assert target.research_run_id == "research_run_fin01_641650afe6bb1062f9ae135e"
    ...
    assert target.research_run_id not in set(
        _load(DECISION)["freshness_and_nonreuse"].get(
            "prior_research_run_ids", []
        )
    )


def test_issuance_budget_nonreuse_and_authority_remain_closed() -> None:
    decision = _load(DECISION)
    issuance = _load(ISSUANCE)
    ...
    assert envelope["lead_max_output_tokens"] == 1800
...
def test_issuance_stays_frozen_and_project_os_advances_after_exact_execution() -> None:
    issuance = _load(ISSUANCE)
    backlog = _load(BACKLOG)
    assert issuance["next_action"] == (
        "S3-T09-OWNER-GRADE-V3-RESEARCH-LEAD-V2-FRESH-EXACT-LIVE-EXECUTION"
    )
