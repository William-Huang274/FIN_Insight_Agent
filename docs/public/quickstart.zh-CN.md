...
0.1.4 开发分支增加 `/workspace/assets` 独立资产工作区：默认浏览已配置的知识库，也可切换财务数据库和项目资料；研究页顶部“项目资料”可打开右侧阅读面板，进入完整编辑页后返回原任务。资产区沿用研究区字体、配色和明暗主题。可阅读和修订项目资料，维护个人偏好，保存问题并携带所选版本进入助手对话或研究草稿。准备草稿不调用模型；实际发送或启动才进入原有运行流程。首版仍有本地容量限制，记忆由用户主动维护，运行中不会自动采用资产新版。协议、数据库关系与项目库备份恢复命令见[资产工作区说明](../architecture/asset_workspace_protocol.zh-CN.md)。
