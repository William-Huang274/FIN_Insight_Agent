...
[交互与模式测试题](eval_sets/workbench_execution_modes.json)覆盖简短追问、指定专家、跨公司单 Agent 研究、节点微修订和自由分派。模式可选择不代表成本已经最优：本地同题追问为 44,069 → 25,495 tokens，但该自由模式小题仍用了 41 次调用、506,744 tokens；多层复核的投入仍待优化。这是单次工程资格记录，不是通用节费承诺。
