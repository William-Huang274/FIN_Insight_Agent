...
export function WorkingNotes({ endpoint }: { endpoint:string }) {
  ...
  async function list(offset=0) {
    ...
    try {const data=await get({query,offset:String(offset)});setItems(old=>offset?[...old,...(data.items||[])]:data.items||[]);setNext(data.next_offset??null);setNotice(data.enabled===false?data.notice:"");}
