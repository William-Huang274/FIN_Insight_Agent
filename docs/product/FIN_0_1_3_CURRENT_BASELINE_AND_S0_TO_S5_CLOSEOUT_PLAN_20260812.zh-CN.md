...
25. **DELL 五单元动态完整案例（R2 局部成功，紧凑 partial successor 正在资格化）**：有限 S2 回归已证明法说叙事不会越权成为 NumericFact；五个 RoleMethodPack、本案即时 GraphContextPack 和稳定 runner 均已资格化。自然 R1 的 Planner 和当前 S1/S2 成功，但 `value_capture` 合法形成 10 条 NumericFact，而 v1.3 静态上限为 8，故在 0 个 cell Judgment 时 fail closed；policy v1.4 已按五指标双期间关闭容量矛盾。R2 精确复用 R1 前缀并尝试剩余节点：需求质量与经营表现通过本地 Judgment，且未发现新金融 L1；价值获取、现金转换和反方证据在分析阶段用尽 8,000 completion token，综合未执行。三次均为 HTTP 200、0 retry/fallback，不是网络、S1/S2、Skill、Graph 或 Tool 交卷失败。当前最早责任层是重复且携带不可执行交卷信息的分析视图，以及隐藏推理／可见草案共用预算。下一步只允许一份 provider-neutral analysis-only 紧凑投影和同一 runner 的节点恢复：重新验证并复用两个有效 Judgment，只执行三个失败单元和综合，最多 8 次新调用。零调用 proof、clean preflight 和 fresh authority 均通过后才执行；若同类预算失败再次出现，不再自动加 token 或扩建 DeepSeek 专用分支，而转为模型 profile／动作面／提交职责决策。即使运行成功，也必须依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容；一次完整运行不能自动等同 S3 acceptance。
