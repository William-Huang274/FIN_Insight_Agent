...
from scripts.releases.prepare_fin_ia_0_1_s4_t06_mu_canonical_case_surface_and_fresh_exact_admission_preparation import (  # noqa: E402
    DECISION_PATH,
...
EXPECTED_SOURCE_PACK_SHA256 = (
    "c97add3afee69278084c090b05e114436f426266c40d91c8e4ce916774e33d09"
)
EXPECTED_ADMISSION_DIGEST = (
    "56005ffb1227e9ec1ead1b73b780342dfeaeef06bbdb0eff01592d7cdc19c891"
)
NEXT_ACTION = (
...
CODE_BINDING_PATHS = (
    ...
    Path(
        "scripts/releases/"
        "prepare_fin_ia_0_1_s4_t06_mu_canonical_case_surface_and_"
        "fresh_exact_admission_preparation.py"
    ),
    Path(
        "scripts/releases/"
    ...
    Path(
        "scripts/releases/"
        "issue_fin_ia_0_1_s4_t06_mu_fresh_exact_admission.py"
    ),
)


class S4MuExactAdmissionIssuanceError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4MuExactAdmissionIssuanceError(code)
...
def _assert_frozen_proof_reprepared(
    ...
    _require(
        regenerated == frozen,
        "s4_mu_frozen_proof_reprepare_mismatch",
    )
    _require(
        regenerated.get("status") == EXPECTED_DECISION_STATUS,
        "s4_mu_frozen_proof_status_mismatch",
    )
    ...
    _require(
        ...
        "s4_mu_materialization_not_logically_idempotent",
    )
    ...
    _require(
        ...
        "s4_mu_fresh_proof_not_issuable",
    )


def render_issuance() -> tuple[dict[str, Any], dict[str, Any]]:
    _require(not ADMISSION.exists(), "s4_mu_admission_already_exists")
    _require(not ISSUANCE.exists(), "s4_mu_issuance_already_exists")
    _require(
        _sha256(SOURCE_PACK_PATH) == EXPECTED_SOURCE_PACK_SHA256,
        "s4_mu_source_pack_digest_drift",
    )
    ...
    _require(
        ...
        "s4_mu_frozen_decision_byte_drift",
    )
    ...
    _require(
        ...
        == "exact_live_s4_mu_source_grounded_three_cell_r1"
        ...
        "s4_mu_admission_contract_binding_mismatch",
    )
    ...
    _require(
        provider_callback_calls == 0,
        "s4_mu_provider_called_during_issuance",
    )
    ...
    _require(
        ...
        "s4_mu_fresh_identity_reused_before_issuance",
    )
    ...
    _require(
        ...
        "s4_mu_execution_state_exists_before_issuance",
    )
    ...
    issuance = {
        "schema_version": (
            "fin_ia_0_1_s4_t06_mu_fresh_exact_admission_issuance_v1_0"
        ),
        "issuance_id": "S4-T06-MU-FRESH-EXACT-ADMISSION-ISSUANCE-R1",
    ...
    _require(
        ...
        "s4_mu_issuance_changed_canonical_runtime",
    )
...
def _write_and_validate(
    ...
    try:
        ...
        _require(
            ...
            "s4_mu_runner_load_admission_id_mismatch",
        )
