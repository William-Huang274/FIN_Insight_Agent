...
T07-B 已完成该内部 session 的工程实现：离线签发、digest-only credential store、identity/role allowlist、expiry/revocation、exact scope 和 append-only decision control 均通过后端与浏览器 mutation。默认真实 store 保持零 session/零 decision，因此当前仍是“机制可用”，不是“Human Review 已完成”。T07-C 只剩用户真实签发、审阅 exact packet 并明确 accept/return；生产 OIDC/SSO 边界不变。
