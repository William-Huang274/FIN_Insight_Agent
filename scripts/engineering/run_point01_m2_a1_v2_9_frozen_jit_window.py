"""Frozen v2.9 executable JIT entry for one future exact human approval.

The approval remains package-external.  This source is already part of the
v2.9 package before any future approval can exist, so an approval activates
reviewed bytes rather than a post-approval script edit.  No approval means no
authority root, receipt, runtime directory, child process, or actual run.
"""
...
PACKAGE_GATE_PATH = ROOT / "data/manifests/point01_m2_a1_execution_ready_audit_package_freeze_gate_result_v2_9.json"
PLAN_PATH = ROOT / "data/manifests/point01_m2_a1_receipt_execution_plan_v1_6_executable_authority.json"
PLAN_GATE_PATH = ROOT / "data/manifests/point01_m2_a1_receipt_execution_plan_v1_6_executable_authority_gate.json"
BLUEPRINT_PATH = ROOT / "data/manifests/point01_m2_a1_baseline_authority_blueprint_v1_6_executable_authority.json"
BLUEPRINT_GATE_PATH = ROOT / "data/manifests/point01_m2_a1_baseline_authority_blueprint_v1_6_executable_authority_gate.json"
PARENT = ROOT / "scripts/engineering/run_point01_m2_a1_actual_audit_v2_9.py"
REGISTRAR = ROOT / "scripts/engineering/run_point01_m2_a1_receipt_registrar_v2_9.py"
...
def _load(path: Path) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise ValueError("m2_a1_v2_9_mapping_required")
    return value
...
def _artifacts() -> tuple[Mapping[str, Any], ...]:
    ...
    if package_gate.get("status") != "pass" or plan_gate.get("status") != "pass" or blueprint_gate.get("status") != "pass":
        raise ValueError("m2_a1_v2_9_gate_not_pass")
    if package_gate.get("package_digest") != package.get("package_digest") or plan_gate.get("target_digest") != plan.get("plan_digest") or blueprint_gate.get("target_digest") != blueprint.get("blueprint_digest"):
        raise ValueError("m2_a1_v2_9_cross_gate_binding_invalid")
    return artifacts
...
def _approval_preflight(path: Path) -> tuple[Any, tuple[Mapping[str, Any], ...]]:
    ...
    try:
        preflight_exact_execution(package, None, repository_root=ROOT, receipt_id="v2-9-dry-run-no-receipt", scenario_id=approval.scenario_id, human_approval_digest=approval.approval_digest)
    except M2A1ExecutionPreflightError as exc:
        if str(exc) != "package_admission_required":
            raise ValueError(f"m2_a1_v2_9_package_preflight_invalid:{exc}") from exc
    else:
        raise ValueError("m2_a1_v2_9_missing_admission_unexpected_pass")
...
def dry_run(approval_path: Path) -> int:
    try:
        ...
        print(json.dumps({"status": "m2_a1_v2_9_approval_preflight_fail_closed", "new_admission": 0, "new_receipt": 0, "namespace": 0, "runtime": 0, "actual": 0}, sort_keys=True))
        return 2
    package, package_gate, plan, plan_gate, blueprint, blueprint_gate = artifacts
    print(json.dumps({"status": "m2_a1_v2_9_approval_preflight_pass_no_side_effects", "approval_digest": approval.approval_digest, "package_digest": package["package_digest"], "package_gate_digest": package_gate["gate_digest"], "plan_digest": plan["plan_digest"], "plan_gate_digest": plan_gate["gate_digest"], "blueprint_digest": blueprint["blueprint_digest"], "blueprint_gate_digest": blueprint_gate["gate_digest"], "new_admission": 0, "new_receipt": 0, "namespace": 0, "runtime": 0, "actual": 0}, sort_keys=True))
    return 0


def _issue_authority(approval: Any, package: Mapping[str, Any]) -> tuple[Any, Any]:
    from sec_agent.canonical_runtime.m2_a1_execution_receipt import M2A1ExecutionReceipt, M2A1ExternalPackageAdmission, V2_9_ADMISSION_SCHEMA, V2_9_RECEIPT_SCHEMA
    ...
    if receipt_expiry <= current:
        raise ValueError("m2_a1_v2_9_expired_before_issue")
    admission = M2A1ExternalPackageAdmission.create(
        admission_ref=approval.approval_ref,
        admission_id=f"{approval.approval_id}:admission:v1",
        admission_version=1,
        reviewer_identity=approval.reviewer_identity,
        package_ref=approval.package_ref,
        executable_package_digest=approval.package_digest,
        scope=str(package["scope"]),
        authority_boundary=approval.authority_boundary,
        execution_staging_namespace_id=approval.execution_staging_namespace_id,
        expires_at=admission_expiry,
        schema_version=V2_9_ADMISSION_SCHEMA,
    ...
    receipt = M2A1ExecutionReceipt.create(
        ...
        schema_version=V2_9_RECEIPT_SCHEMA,
        human_approval_digest=approval.approval_digest,
    )
...
def execute(approval_path: Path) -> int:
    """Run the only future active path.  It is never used by this B0.6 repair."""
    ...
    try:
        ...
        _write(admission_path, admission.model_dump(mode="json"))
        _write(receipt_path, receipt.model_dump(mode="json"))
        ...
        if registered.returncode != 0:
            raise ValueError("m2_a1_v2_9_registration_failed")
        ...
        if consumed is None:
            raise ValueError("m2_a1_v2_9_consumed_receipt_missing")
        ...
        reviewer = review_future_actual(package=package, actual_results=(actual,), oracle_evaluations=(oracle,), expected_scenario_ids=(approval.scenario_id,), admission=admission, consumed_receipt=consumed, receipt_ledger_state=state, receipt_terminal_event_digest=None, require_terminal_event=False, expected_human_approval_digest=approval.approval_digest)
        ...
        ledger.verify_terminal_event(receipt.receipt_id, expected_human_approval_digest=approval.approval_digest, expected_actual_result_digest=actual.actual_result_digest, expected_oracle_evaluation_digest=oracle.evaluation_digest, expected_reviewer_gate_digest=reviewer.gate_digest)
        _write(preflight.output_path.parent / "oracle_evaluation.json", oracle.model_dump(mode="json"))
        _write(preflight.output_path.parent / "reviewer_gate.json", reviewer.model_dump(mode="json"))
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Frozen M2-A1 v2.9 JIT entry; exact external approval is mandatory.")
