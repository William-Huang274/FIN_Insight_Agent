...
from scripts.releases.prepare_fin_ia_0_1_s4_t06_mu_current_case_aware_delivery_identity_boundary_fresh_proof import (
...
ISSUANCE = ROOT / (
    ...
    "delivery_identity_boundary_fresh_exact_admission_r4_issuance_v1_0.json"
)
EXPECTED_PROOF_SHA256 = (
    "046bc774a9e46f563921d20413f528ef28c89476e14743e74617d6ac18bdb642"
)
EXPECTED_ADMISSION_DIGEST = (
    "a1e37a09d87250fc6c8cfd448cd3dabacea02a3041e68735448485257df4da04"
)
...
CODE_BINDING_PATHS = (
    ...
    Path(
        "scripts/releases/prepare_fin_ia_0_1_s4_t06_mu_current_case_"
        "aware_delivery_identity_boundary_fresh_proof.py"
    ),
    Path(
    ...
    Path(
        "scripts/releases/issue_fin_ia_0_1_s4_t06_mu_current_case_"
        "aware_delivery_identity_boundary_fresh_exact_admission_r4.py"
    ),
)


class S4T06MuIdentityBoundaryR4AdmissionIssuanceError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T06MuIdentityBoundaryR4AdmissionIssuanceError(code)
...
def render_issuance() -> tuple[dict[str, Any], dict[str, Any]]:
    _require(not ADMISSION.exists(), "s4_t06_mu_R4_admission_already_exists")
    _require(not ISSUANCE.exists(), "s4_t06_mu_R4_issuance_already_exists")
    _require(
        _sha256(PROOF_DECISION) == EXPECTED_PROOF_SHA256,
        "s4_t06_mu_R4_frozen_proof_byte_drift",
    )
    frozen = _load(PROOF_DECISION)
    _require(
        build_decision() == frozen,
        "s4_t06_mu_R4_frozen_proof_regeneration_mismatch",
    )
    ...
    _require(
        ...
        == prospective["digest"]
        == EXPECTED_ADMISSION_DIGEST,
        "s4_t06_mu_R4_admission_digest_mismatch",
    )
    _require(
        ...
        "s4_t06_mu_R4_admission_binding_mismatch",
    )
    provider_calls = 0
    ...
    def _must_not_call_provider(**_: Any) -> dict[str, Any]:
        ...
        provider_calls += 1
        raise AssertionError("provider_forbidden_during_R4_admission_issuance")

    build_s3_three_cell_bounded_agent_executor_for_admission(
        admission,
        chat_completion_fn=_must_not_call_provider,
    )
    _require(provider_calls == 0, "s4_t06_mu_R4_provider_called")
    ...
    _require(
        ...
        "s4_t06_mu_R4_identity_not_fresh",
    )
    ...
    issuance = {
        "schema_version": (
            ...
            "boundary_fresh_exact_admission_r4_issuance_v1_0"
        ),
        "issuance_id": (
            "S4-T06-MU-CURRENT-CASE-AWARE-DELIVERY-IDENTITY-BOUNDARY-"
            "FRESH-EXACT-ADMISSION-ISSUANCE-R4"
        ),
        "issued_at": "2026-07-29T23:25:00+08:00",
        ...
        "authority": {
            ...
            "retry_fallback_patch_R5_or_rerun_authorized": False,
        },
        ...
        "execution_envelope": {
            ...
            "automatic_retry_repair_fallback_R5_or_rerun_allowed": False,
        ...
        "proof_reverification": {
            ...
            "three_case_current_ticker_and_nonlocal_mutation_reproof": True,
    ...
    _require(
        ...
        == snapshot_before,
        "s4_t06_mu_R4_issuance_changed_target_runtime",
    )
...
def _write_and_validate(
    ...
    try:
        ...
        _require(
            ...
            == EXPECTED_ADMISSION_DIGEST,
            "s4_t06_mu_R4_runner_load_payload_mismatch",
        )
...
def verify_issued_admission() -> dict[str, Any]:
    ...
    _require(
        ...
        "s4_t06_mu_R4_issued_payload_not_frozen_payload",
    )
    admission = S3ThreeCellBoundedAgentAdmission.model_validate(payload)
    digest = canonical_digest(admission.digest_payload())
    _require(
        digest == EXPECTED_ADMISSION_DIGEST,
        "s4_t06_mu_R4_issued_admission_digest_mismatch",
    )
    ...
    _require(
        ...
        "s4_t06_mu_R4_identity_consumed_before_execution",
    )
    return {
        "status": EXPECTED_ISSUANCE_STATUS,
        "admission_id": admission.admission_id,
