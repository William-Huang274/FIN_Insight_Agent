...
2026-09-04 r8 fresh attempt2=`20260904T052346+0800-zero-model-r8` 已在 clean pushed `587853944cf6610d9f38edcf3c65be2dd7b8aa1c`、独立 project=`finsight-dell-qualification-20260904-r8a2`、loopback `18129` 与 fresh volume 上真实执行并保留为 immutable failed，不是 PASS。它已 phase-local 完成 START、API restart、Redis process restart、同 project/full-stack stop-start、RESUME 与 final exact replay；三容器 healthy，真实固定 Q1 zero-model Evidence/Finance MCP、native interrupt/checkpoint 与 SSE 都实际运行。但最后 LangSmith phase 在 `62.719s` 以 `r8_langsmith_query_failed` 失败，host 最终 cross-phase/PASS receipt 尚未执行，故只能登记六个 completed phases，不能把完整 r8 写成通过。attempt2 manifest SHA=`e5e54964e574c3c156c3599200062d4f92c7385057d5aec3a5b9759eefa42bb7`，failure receipt SHA=`04986641acb8a922710fea39ffb100d0051cad2bbae6def2fba85c9447c9801d`；attempt1/attempt2 project、container、volume 与 receipt 均未清理或覆盖，模型/付费 authority=false。只读诊断排除梯子/代理：Docker 仍经 `http.docker.internal:3128` 完成 build，Agent Server 已上传 LangSmith metadata，同容器 DNS/认证/合法 query 均成功；`limit=100` 成功、`101/500` 稳定 HTTP 400。最早责任层是 harness 把 pinned `langsmith==0.12.1` 的 legacy `/runs/query` limit 误设为 500，并重复 deterministic 400。现场完整 cursor 查询只有 START=`5` spans、RESUME=`3` spans；START 唯一 error 是 native `qualification_interrupt` 的预期 `GraphInterrupt` 控制流，固定 LangGraph 明确它不是运行失败。当前同一 R8 correction candidate 已改为 public cursor iterator + 2-root/100-span `maximum+1` overflow sentinel、trace/parent closure、永久错误立即失败/仅瞬态错误重试，以及 START 恰好一个受控 GraphInterrupt；claim 收窄为 exact trace、input/output hiding、credential/host-data-locator scan 和 unexpected error=0，不再冒充全部 traceback/metadata privacy。作者分离只读终审已为 `P0=0 / P1=0`；successor 必须先形成 clean commit/push，再使用 fresh project=`finsight-dell-qualification-20260904-r8a3`、loopback `18130` 与 fresh volume 创建 attempt3。Project OS 既存 sealed SHA drift 仍使 full repository green=false。R7 bounded baseline 仍是 current；durable orphan lifecycle、unknown-outcome 自动重试、distributed exactly-once、profile 入库、guard/binder 分离、完整 multi-agent、模型/DeepSeek/实时外源/付费、Evidence admission、S2 write、HITL、报告和 production 仍为 false。
