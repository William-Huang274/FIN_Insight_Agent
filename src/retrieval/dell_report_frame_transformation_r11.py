...
class FrameTransformationBindingR11Error(ValueError):
    pass
...
def _require_identifier(value: str, *, field: str) -> str:
    if not value or value.strip() != value or re.search(r"[\x00-\x1f]", value):
        raise FrameTransformationBindingR11Error(
            f"R11_transformation_invalid_identifier:{field}"
        )
    return value
...
def validate_predicate_frame_r11_integrity(frame: PredicateFrame) -> None:
    for proof in frame.clause_ownership_proofs:
        ...
        if observed_proof_digest != canonical_digest(proof_body):
            raise FrameTransformationBindingR11Error(
                "R11_transformation_clause_ownership_proof_digest_invalid"
            )
    for group in frame.argument_groups:
        ...
        if group.proof_digest != canonical_digest(proof_body):
            raise FrameTransformationBindingR11Error(
                "R11_transformation_price_attachment_proof_digest_invalid"
            )
        ...
        if observed_group_digest != canonical_digest(group_body):
            raise FrameTransformationBindingR11Error(
                "R11_transformation_argument_group_digest_invalid"
            )
        if (
            ...
            raise FrameTransformationBindingR11Error(
                "R11_transformation_price_attachment_proof_structure_invalid"
            )
    ...
    if frame.semantic_signature_digest != expected_semantic:
        raise FrameTransformationBindingR11Error(
            "R11_transformation_source_or_compiled_semantic_digest_invalid"
        )
    if (
        frame.frame_digest != expected_representation
        or frame.representation_frame_digest != expected_representation
        or frame.frame_id
        != f"FRAME::R11::{expected_representation[:24].upper()}"
    ):
        raise FrameTransformationBindingR11Error(
            "R11_transformation_source_or_compiled_representation_digest_invalid"
        )
...
def _mapping(
    ...
    mapping_id = f"ROLEMAP::R11::{canonical_digest(core)[:24].upper()}"
...
def build_frame_transformation_binding_r11(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    validate_predicate_frame_r11_integrity(source_frame)
    validate_predicate_frame_r11_integrity(compiled_frame)
    ...
    binding_id = f"FRAMEBIND::R11::{canonical_digest(core)[:24].upper()}"
    ...
    if require_lossless and not binding.binding_accepted:
        ...
        raise FrameTransformationBindingR11Error(
            "R11_transformation_not_lossless:" + "|".join(findings)
        )
    return binding


def build_missing_compiled_frame_binding_r11(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    core = {
        ...
        "compiled_object_ids": (),
        "compiled_window_ids": (),
        "compiled_frame_id": "MISSING::R11::COMPILED_FRAME",
...
def transformation_binding_digest_r11(
...
def validate_frame_transformation_binding_r11(
    ...
    if observed != canonical_digest(body):
        raise FrameTransformationBindingR11Error(
            "R11_transformation_binding_self_digest_invalid"
        )
    if value.get("binding_accepted") is not True:
        raise FrameTransformationBindingR11Error(
            "R11_transformation_binding_not_accepted"
        )
    if value.get("semantic_signature_equal") is not True:
        raise FrameTransformationBindingR11Error(
            "R11_transformation_binding_semantic_signature_mismatch"
        )
    if any(value.get(field) for field in (
        ...
        raise FrameTransformationBindingR11Error(
            "R11_transformation_binding_contains_material_flags"
        )
