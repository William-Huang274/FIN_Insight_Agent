...
6. [ ] `S2_MINIMAL_NATURAL_CANARY_AND_FORMAL_RERUN_DECISION`：6A 零调用选型已完成，选定 DELL demand-authenticity atom，只给 E022/E018/E023，定向测试 `$16.1B`、超过 5,000 客户、订单／backlog 的合法展示和 issuer／read-through 角色；当前只授权 zero-call runner／fake／mutation／clean proof 实现，尚未签 1-call live authority。未来 canary 也只决定是否值得重新运行 DELL，不自动授权 paid report、paired／Owner 或其他案例。

`FIN-0.1.3-S2-SELECTED-EVIDENCE-NUMERIC-CANDIDATE-COCOMPILATION-MINIMUM-ZERO-CALL-IMPLEMENTATION` 现已达到 clean independent engineering proof。implementation artifact 保留其历史 `working_tree...pending` 状态，successor proof=`configs/releases/fin_ia_0_1_3_s2_selected_evidence_numeric_candidate_cocompilation_clean_independent_proof_v1_0.json`。第 6 项的选型 decision=`configs/releases/fin_ia_0_1_3_s2_selected_evidence_numeric_natural_node_canary_zero_call_authority_decision_v1_0.json` 已完成；current next=`IMPLEMENT_AND_CLEAN_PROVE_ONE_DELL_DEMAND_AUTHENTICITY_NUMERIC_VIEW_ATOM_CANARY_ZERO_CALL`，自然模型调用和 DELL rerun仍未授权。
