...
def build_source_capability_router(state: Mapping[str, Any]) -> dict[str, Any]:
    ...
    family_set = sorted(set(authority) | available_families | allowed_families | route_families | requested_families)
    ...
    decisions = [
        _route_decision(route, capability_by_family=capability_by_family, allowed_families=allowed_families)
        for route in routes
    ]
    payload = {
        "schema_version": SOURCE_CAPABILITY_ROUTER_SCHEMA_VERSION,
        "policy": "explicit_source_capability_router_no_weak_proxy_fallback_v0_1",
...
def _source_capability(
    ...
    *,
    ...
    context_only = bool(auth.get("context_only")) or exact is False
    return {
        ...
        "allowed_claim_scope": str(auth.get("allowed_claim_scope") or ""),
        "gap_policy": _gap_policy_for_family(family, available=available, allowed_by_activation=family in allowed_families if allowed_families else True),
    }
