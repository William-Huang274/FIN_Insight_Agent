"""Authority-only registrar for one exact M2-A1 execution receipt.

The registrar has no M2/compiler/canary imports.  It may create only the
package-derived authority root and SQLite receipt ledger.  It cannot create a
runtime/output root, issue an admission, or execute a scenario.
"""
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Register one exact package-bound M2-A1 receipt without creating runtime resources.")
    ...
    try:
        ...
        ledger = M2A1ReceiptLedger.create_for_registration(
