...
DECISION_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_nvda_raw_experiment_a_authority_v1_0.json"
)
DELL_RESULT_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_dell_layered_replacement_exact_live_result_v1_0.json"
)
MU_RESULT_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_mu_raw_exact_live_and_s2_06_boundary_result_v1_0.json"
)
RUNNER_REF = "src/sec_agent/s2_same_evidence_experiment_runtime.py"
DEFAULT_AUTHORITY_ROOT = (
    ROOT / ".codex_runtime" / "fin013_s2_05" / "authorities" / "NVDA_RAW"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Issue one NVDA raw same-evidence Experiment A admission"
    )
    ...
    dell_result = _read_json(ROOT / DELL_RESULT_REF)
    mu_result = _read_json(ROOT / MU_RESULT_REF)
    _validate_decision(decision, dell_result, mu_result)
    ...
    if (
        ...
        raise RuntimeError("experiment_a_nvda_frozen_visible_input_mismatch")
    ...
    output = {
        ...
        "DELL_or_MU_correction_raw_or_hidden_gold_read": False,
    }
    if not args.dry_run:
        authority_root = args.authority_root.resolve()
        if authority_root != DEFAULT_AUTHORITY_ROOT.resolve():
            raise RuntimeError("experiment_a_nvda_authority_root_invalid")
        authority_root.mkdir(parents=True, exist_ok=True)
        if list(authority_root.glob("*.json")):
            raise RuntimeError("experiment_a_nvda_admission_already_exists")
...
def _validate_decision(
    ...
    dell_result: dict[str, Any],
    mu_result: dict[str, Any],
) -> None:
    ...
    if (
        ...
        != "NVDA_raw_admission_and_execution_authorized_not_issued"
        ...
        or fairness.get("same_model_visible_contract_as_DELL_and_MU") is not True
        or fairness.get("DELL_or_MU_correction_visible") is not False
        ...
        != ".codex_runtime/fin013_s2_05/authorities/NVDA_RAW"
    ):
        raise RuntimeError("experiment_a_nvda_raw_authority_invalid")


def _validate_repository(decision: dict[str, Any]) -> None:
    if _git("status", "--porcelain"):
        raise RuntimeError("experiment_a_nvda_repository_not_clean")
    head = _git("rev-parse", "HEAD")
    audited = str(decision["audited_repository_state"]["head"])
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", audited, head], cwd=ROOT, check=False
    ).returncode != 0:
        raise RuntimeError("experiment_a_nvda_head_not_descendant")
    upstream = _git("rev-parse", "--abbrev-ref", "@{upstream}")
    if _git("rev-list", "--left-right", "--count", f"{upstream}...HEAD").split() != ["0", "0"]:
        raise RuntimeError("experiment_a_nvda_repository_not_synced")
...
def _validate_frozen_bindings(decision: dict[str, Any]) -> None:
    for binding in decision["frozen_bindings"].values():
        ...
        if not ref or not expected or _sha256(ROOT / ref) != expected:
            raise RuntimeError("experiment_a_nvda_frozen_binding_mismatch")


def _read_json(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("experiment_a_nvda_decision_json_invalid")
    return value
