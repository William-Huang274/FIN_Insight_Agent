...
import {AssetCaptureDialog,type CaptureSelection} from "./AssetCaptureDialog";
...
type Financial = { selection_id:string; ticker:string; legal_name:string; metric_id:string; value_decimal:string; unit:string; period_start:string; period_end:string; fiscal_year:number; fiscal_period:string; filed_at:string; citation_url:string; superseded_by_observation_id:string|null };
type Result = { snapshot:string;selection_query:Record<string,unknown>;items:(Source|Financial)[]; total:number; tickers:string[]; years:(string|number)[]; kinds?:string[]; metrics?:string[]; metric_labels?:Record<string,string>; periods?:string[]; notice:string };
const labels:Record<string,string> = {issuer_earnings_release:"业绩新闻稿",issuer_earnings_call:"业绩电话会",hyperscaler_demand_primary:"云厂商披露",industry_benchmark_primary:"行业资料",issuer_filing_narrative:"公司申报文件",issuer_management_disclosure:"公司业绩披露",model_provider_primary:"模型厂商资料",named_deployment_primary:"产品部署资料",peer_financial_disclosure:"同业财务披露",peer_management_disclosure:"同业业绩披露",regulator_primary:"监管资料",supplier_management_disclosure:"供应商业绩披露",supplier_platform_primary:"供应商产品资料","10-K":"年度报告 · 10-K","10-Q":"季度报告 · 10-Q","8-K":"重要事项 · 8-K",earnings_release:"业绩新闻稿",revenue:"营业收入",operating_cash_flow:"经营现金流",capital_expenditures:"资本开支",net_income:"净利润"};
export function DataLibrary({page,navigate,onSupplement}:{page:string;navigate:(page:string,id?:string)=>void;onSupplement:(question:string)=>void}) {
  ...
  const [query,setQuery]=useState(initial.get('query')||""); const [ticker,setTicker]=useState(initial.get('ticker')||""); const [year,setYear]=useState(initial.get('fiscal_year')||"");
  const [kind,setKind]=useState(initial.get('metric')||""); const [offset,setOffset]=useState(0); const [submitted,setSubmitted]=useState(initial.get('query')||"");
  const [asOf,setAsOf]=useState(initial.get('as_of')||""); const [period,setPeriod]=useState(initial.get('period')||"");
  ...
  const [selected,setSelected]=useState<Source|null>(null); const [sections,setSections]=useState<{title:string;text:string;selection_id:string}[]>([]);
  ...
  const [chosenRows,setChosenRows]=useState<string[]>([]),[chosenSections,setChosenSections]=useState<string[]>([]),[readSnapshot,setReadSnapshot]=useState('');
  const [capture,setCapture]=useState<{selection:CaptureSelection;title:string;summary:string}|null>(null);
  const toggle=(values:string[],id:string)=>values.includes(id)?values.filter(v=>v!==id):[...values,id];
  useEffect(()=>{const source=initial.get('source');const controller=new AbortController();if(!financial&&source)void fetch(`/api/v1/data-library/sources/${encodeURIComponent(source)}`,{signal:controller.signal}).then(r=>{if(!r.ok)throw new Error('原来源已不在当前知识库中，项目保存的快照仍可阅读。');return r.json();}).then(r=>setSelected(r.document)).catch(e=>{if(e.name!=='AbortError')setError(e.message);});return()=>controller.abort();},[]);
  useEffect(()=>{setChosenRows([]);},[financial,submitted,ticker,year,kind,offset,asOf,period]);
  useEffect(()=>{setChosenSections([]);setReadSnapshot('');},[selected?.document_id]);
  useEffect(()=>{
  ...
  useEffect(()=>{
    ...
    fetch(`/api/v1/data-library/sources/${encodeURIComponent(selected.document_id)}?offset=${readOffset}`,{signal:controller.signal})
      .then(async r=>{if(!r.ok)throw new Error("原文暂不可用");return r.json();}).then(r=>{setSections(r.sections);setSectionTotal(r.total);setReadSnapshot(previous=>{if(previous&&previous!==r.snapshot)setChosenSections([]);return r.snapshot;});})
      .catch(e=>{if(e.name!=="AbortError")setReadError(e.message);});return ()=>controller.abort();
  },[selected,readOffset]);
  return <section className="fs-page dl-page">
    <header className="dl-heading"><div><span className="fs-kicker">ASSET LIBRARY</span><h1>{financial?"财务数据库":"知识库"}</h1><p>{financial?"直接查阅已保存的财务数字，核对期间、单位与披露版本。":"先看看有哪些资料，再开始有依据的研究。"}</p></div><button onClick={()=>navigate(financial?"library":"financial-data")}><Database size={17}/>{financial?"浏览公司资料":"查询财务数据"}<ArrowRight size={16}/></button></header>
    ...
    {financial&&!!data?.items.length&&<div className="dl-selection"><label><input type="checkbox" aria-label="选择本页财务记录" disabled={loading} checked={chosenRows.length===data.items.length} onChange={e=>setChosenRows(e.target.checked?(data.items as Financial[]).map(r=>r.selection_id):[])}/>本页全选</label><span>已选 {chosenRows.length} 条，仅保存所选记录</span><button disabled={loading||!!error||!chosenRows.length} onClick={()=>setCapture({selection:{kind:'financial',query:data.selection_query,snapshot:data.snapshot,row_ids:chosenRows},title:`${ticker||'财务'} · ${data.metric_labels?.[kind]||labels[kind]||'数据'}记录`,summary:`本次保存 ${chosenRows.length} 条财务记录；筛选共 ${data.total} 条。`})}>保存所选记录</button></div>}
    {financial&&!!data?.items.length&&<div className="dl-table"><table><thead><tr>{["选择","公司 / 财年","指标","数值 / 来源单位","期间","披露版本","原始披露"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{(data.items as Financial[]).map((r,i)=><tr key={r.selection_id}><td><input type="checkbox" aria-label={`选择财务记录 ${i+1}`} disabled={loading} checked={chosenRows.includes(r.selection_id)} onChange={()=>setChosenRows(toggle(chosenRows,r.selection_id))}/></td><td><b>{r.ticker}</b><br/>FY{r.fiscal_year} {r.fiscal_period}</td><td>{data?.metric_labels?.[r.metric_id]||labels[r.metric_id]||r.metric_id.replaceAll('_',' ')}</td><td><strong>{r.value_decimal}</strong><br/>{r.unit}</td><td>{r.period_start||"时点"}<br/>{r.period_end}</td><td>{r.filed_at}<br/>{r.superseded_by_observation_id?"有后续披露版本":"保留版本"}</td><td><a href={r.citation_url} target="_blank" rel="noreferrer">查看来源 <ExternalLink size={14}/></a></td></tr>)}</tbody></table></div>}
    ...
    {selected&&<dialog ref={reader} className="dl-overlay" aria-label="公共资料阅读" onCancel={()=>setSelected(null)}><article className="dl-reader"><header><div><small>{selected.company} · {selected.publication_date}</small><h2>{selected.title}</h2><a href={selected.stable_url} target="_blank" rel="noreferrer">打开官方原文 <ExternalLink size={14}/></a></div><button autoFocus aria-label="关闭公共资料" onClick={()=>setSelected(null)}><X/></button></header><div className="dl-selection"><button disabled={!sections.length||!!readError} onClick={()=>setChosenSections([...new Set([...chosenSections,...sections.map(s=>s.selection_id)])])}>选择本页章节</button><span>已选 {chosenSections.length} 章</span><button disabled={!chosenSections.length||!!readError||!readSnapshot} onClick={()=>setCapture({selection:{kind:'library',document_id:selected.document_id,snapshot:readSnapshot,section_ids:chosenSections},title:selected.title,summary:`保存《${selected.title}》所选 ${chosenSections.length} 个章节。`})}>将所选章节加入项目</button></div><div className="dl-reading">{readError&&<p role="alert">{readError}</p>}{!sections.length&&!readError&&<p>正在读取正文…</p>}{sections.map((s,i)=><section key={s.selection_id}><label className="dl-section-choice"><input type="checkbox" aria-label={`选择章节 ${readOffset+i+1}`} checked={chosenSections.includes(s.selection_id)} onChange={()=>setChosenSections(toggle(chosenSections,s.selection_id))}/>选择本章</label><h3>{s.title}</h3><ReactMarkdown remarkPlugins={[remarkGfm]}>{s.text}</ReactMarkdown></section>)}</div><footer><button disabled={readOffset===0} onClick={()=>setReadOffset(Math.max(0,readOffset-5))}>前五章</button><span>{readOffset+1}–{Math.min(readOffset+5,sectionTotal)} / {sectionTotal} 章</span><button disabled={readOffset+5>=sectionTotal} onClick={()=>setReadOffset(readOffset+5)}>后五章</button></footer></article></dialog>}
    {capture&&<AssetCaptureDialog selection={capture.selection} initialTitle={capture.title} summary={capture.summary} onClose={()=>setCapture(null)}/>}
  </section>;
}
