"""Compile configured source families into bounded retrieval routes."""
...
REQUIRED_SOURCE_FAMILIES_BY_COVERAGE: tuple[
    tuple[str, tuple[str, ...]], ...
] = (
...
_REQUIRED_FAMILY_MAP = dict(REQUIRED_SOURCE_FAMILIES_BY_COVERAGE)
_EXPECTED_SOURCE_FAMILY_REFS = frozenset(
    family_ref
    for _, family_refs in REQUIRED_SOURCE_FAMILIES_BY_COVERAGE
...
class SourceFamilyCatalog(_StrictFrozenModel):
    ...
    def validate_catalog(self) -> "SourceFamilyCatalog":
        ...
        actual_by_coverage = {
            ...
            for coverage_id in COVERAGE_OBLIGATION_IDS
        }
        for coverage_id, expected in REQUIRED_SOURCE_FAMILIES_BY_COVERAGE:
...
class LocalInventoryRecord(_StrictFrozenModel):
    ...
    def validate_record(self) -> "LocalInventoryRecord":
        ...
        if not set(self.branch_refs).issubset(COVERAGE_OBLIGATION_IDS):
...
class HostOwnedBaselineSourcePlan(_StrictFrozenModel):
    ...
    def validate_authority(self) -> "HostOwnedBaselineSourcePlan":
        ...
        if coverage != COVERAGE_OBLIGATION_IDS:
...
def _validate_exact_required_source_families(
    ...
    actual: dict[str, set[str]] = {
        coverage_id: set() for coverage_id in COVERAGE_OBLIGATION_IDS
    }
...
def build_host_owned_baseline_source_plan(
    *,
    ...
    plan_body = {
        ...
        "coverage_obligation_ids": COVERAGE_OBLIGATION_IDS,
...
class SourceFamilyCompiler:
    ...
    def provider_route_catalog(
        ...
        *,
        ...
        selected = set(branch_ids or COVERAGE_OBLIGATION_IDS)
        if not selected.issubset(COVERAGE_OBLIGATION_IDS):
