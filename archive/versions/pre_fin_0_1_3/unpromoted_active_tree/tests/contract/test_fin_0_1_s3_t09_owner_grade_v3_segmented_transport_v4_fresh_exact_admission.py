...
ADMISSION = RELEASES / (
    ...
    "transport_v4_exact_admission_v1_0.json"
)
ISSUANCE = RELEASES / (
    "fin_ia_0_1_s3_t09_owner_grade_v3_segmented_transport_v4_"
...
def test_v4_decision_and_issuance_are_exact_and_capture_bound() -> None:
    ...
    assert admission.transport_ref == S3_OWNER_GRADE_SEGMENTED_SPECIALIST_TRANSPORT_V4_REF
...
def test_v4_issuance_loads_in_existing_runner_without_provider_call() -> None:
    target = load_execution_target(ISSUANCE)
    assert target.research_run_id == "research_run_fin01_0e2b6e9698ebbf61288708a9"
...
def test_v4_budget_nonreuse_and_stop_contract_remain_closed() -> None:
    decision = _load(DECISION)
    issuance = _load(ISSUANCE)
    assert decision["freshness_and_nonreuse"]["prior_research_run_count"] == 7
...
def test_v4_contracts_do_not_persist_plaintext_credentials() -> None:
    rendered = "\n".join(
