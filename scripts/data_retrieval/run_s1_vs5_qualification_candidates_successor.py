...
from retrieval.query_plan_v2 import (  # noqa: E402
    canonical_digest,
...
POLICY_SCHEMA = "fin_ia_s1_vs5_candidate_execution_policy_v1_1"
AUTHORITY_SCHEMA = "fin_ia_s1_vs5_candidate_execution_authority_v1_1"
RAW_SCHEMA = "fin_ia_s1_vs5_candidate_execution_raw_v1_1"
RESULT_SCHEMA = "fin_ia_s1_vs5_candidate_execution_result_v1_1"
...
def _validate_authority(
    *, policy_path: Path, policy: Mapping[str, Any], authority_path: Path
    ...
    if (
        ...
        != "authorized_exact_once_valid_temporal_successor"
    ...
    if policy.get("status") != "frozen_before_successor_qualification_ranking":
...
def _load_split_inputs(
    *, runtime_inputs_result_path: Path, split: str
) -> list[EvaluationInput]:
    result = read_json(runtime_inputs_result_path)
    if (
        result.get("status")
        != "qualification_runtime_inputs_materialized_references_separate"
    ):
    ...
    if len(outputs) != 1:
        raise ValueError("qualification_candidate_split_catalog_invalid")
    asset = outputs[0]
    path = _resolve(str(asset.get("ref") or ""))
...
def _run(
    *,
    ...
    runtime_inputs_result_path = _validate_bound_ref(
        bound, "runtime_inputs_result"
    )
    ...
    _validate_bound_ref(bound, "query_plan_runtime")
    _validate_bound_ref(bound, "retrieval_need_runtime")
    _validate_bound_ref(bound, "financial_intent_runtime")
    _validate_bound_ref(bound, "evidence_role_runtime")
    _validate_bound_ref(bound, "financial_shortlist_runtime")
    _validate_bound_ref(bound, "candidate_ranking_runtime")
    ...
    inputs = _load_split_inputs(
        runtime_inputs_result_path=runtime_inputs_result_path,
        split=split,
    )
    ...
    for proposition, pair_slice in zip(proposition_runtime, pair_slices):
        ...
        final_rows = fuse_lane_rankings_with_balanced_review_prefix(
            lane_shortlists,
            maximum=len(rerank_ids),
            review_k=int(contract["candidate_review_k"]),
            reciprocal_rank_constant=int(contract["reciprocal_rank_constant"]),
        )
    ...
    raw = {
        ...
        "status": "candidate_generation_successor_complete_labels_not_loaded",
    ...
    public = {
        ...
        "status": "candidate_generation_successor_complete_evaluation_pending",
        "recorded_at": "2026-08-18",
        "attempt_id": authority["attempt_id"],
        "split": split,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run label-blind S1 VS5 successor candidate generation."
    )
