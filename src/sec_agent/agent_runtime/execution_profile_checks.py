"""Validate execution profiles and the configured no-model diagnostic task."""
...
ExecutionProfile = Literal["product", "zero_model_control_plane_v1"]

_QUALIFICATION_BRANCH_ID = "Q1_ISSUER_TRUTH"
_QUALIFICATION_ROUTE_ID = "route:Q1_ISSUER_TRUTH:required-reviewed"


class ExecutionProfileCheckError(ValueError):
...
def require_execution_profile(value: Any) -> ExecutionProfile:
    ...
    raise ExecutionProfileCheckError("dell_execution_profile_invalid")
...
def build_zero_model_qualification_tasks(
    *,
    ...
    if len(methods) != 1:
        raise ExecutionProfileCheckError(
