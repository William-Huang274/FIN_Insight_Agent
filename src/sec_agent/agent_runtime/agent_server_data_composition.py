"""Open validated data sources for a configured research task."""
...
from .data_access_policy import load_owner_data_gate_decision
from .research_graph_contracts import CaseFoundationBinding
from .research_graph import ResearchGraphDependencies
from .research_mcp_tools import (
    MCPToolLaneAdapter,
    compose_mcp_graph_run,
)
from .reviewed_evidence_inventory import (
...
APPROVED_RESEARCH_AS_OF = "2026-09-02T00:00:00Z"


def task_research_as_of(environment):
    """A task cutoff is separate from frozen historical inventory dates."""
    value = environment.get('FINSIGHT_RESEARCH_AS_OF', APPROVED_RESEARCH_AS_OF)
...
class ApprovedDataCompositionError(RuntimeError):
...
def _repository_root(environment: Mapping[str, str]) -> Path:
    ...
    if not isinstance(value, str) or not value.strip():
        raise ApprovedDataCompositionError("approved_repository_root_missing")
    try:
    ...
    if not root.is_dir():
        raise ApprovedDataCompositionError("approved_repository_root_not_directory")
    return root
...
def _model_execution_not_authorized(*_args: Any, **_kwargs: Any) -> Any:
    raise ApprovedDataCompositionError("model_execution_not_authorized")


@dataclass(frozen=True)
class ApprovedDataComposition:
...
@contextmanager
def open_approved_data_composition(
    *,
    ...
    if not isinstance(run_invocation_id, str) or not run_invocation_id.strip():
        raise ApprovedDataCompositionError("run_invocation_id_required")
    ...
    try:
        ...
        if catalog.research_as_of != APPROVED_RESEARCH_AS_OF[:10]:
            raise ApprovedDataCompositionError(
        ...
        if live_web_read_enabled:
            if not source_read_enabled:
                raise ApprovedDataCompositionError("live_web_requires_source_read_capability")
            from sec_agent.research_foundation.external_sources import (
