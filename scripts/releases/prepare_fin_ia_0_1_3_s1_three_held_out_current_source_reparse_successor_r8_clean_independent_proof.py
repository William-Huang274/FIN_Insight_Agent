...
ATTEMPT_ID = "S1-THREE-HELD-OUT-CURRENT-SOURCE-REPARSE-R8-CLEAN-PROOF-A1"
...
class CleanReparseR8ProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise CleanReparseR8ProofError(code)
    ...
    with tarfile.open(archive_path, "r") as archive:
        for member in archive.getmembers():
            ...
            try:
                ...
                raise CleanReparseR8ProofError(
...
def _blocked_network(*args: Any, **kwargs: Any) -> Any:
    raise CleanReparseR8ProofError("clean_reparse_r8_proof_network_forbidden")
...
def _worker_payload(runtime_root: Path) -> dict[str, Any]:
    ...
    execution_root = runtime_root / (
        ...
        "fin_0_1_3_s1_three_held_out_current_source_reparse/clean-proof-r8"
    )
...
def build_result() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin013-s1-held-out-reparse-r8-clean-proof-"
    ) as parent:
        parent_path = Path(parent).resolve()
        _require(
            parent_path.name.startswith(
                "fin013-s1-held-out-reparse-r8-clean-proof-"
            ),
    ...
    body = {
        "schema_version": (
            "fin_ia_0_1_3_s1_three_held_out_current_source_reparse_"
            "successor_r8_clean_independent_proof_v1_0"
        ),
        "contract_ref": (
            "fin_0_1_3.S1.three_held_out_current_source_reparse_"
            "successor_r8_clean_independent_proof:v1"
        ),
        ...
        "decision_zh": (
            ...
            "原始响应，均逐字节重现 R8 result。R5-R7 失败继续保留；下一步只准入"
            "把 CandidateBundle-only manifest 改绑到 R8，不准入真实 BGE、Milvus、"
            "ranking、Evidence、外源补源或 DeepSeek。"
        ),
        "known_boundary": (
            "This proof establishes clean reproducibility of the R8 flow-stock and "
            "business-coordinate successor only. It does not establish a physical "
