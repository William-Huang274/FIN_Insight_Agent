...
import { ArrowUp, Layers, MessageSquare, Plus, Square } from "lucide-react";
import { ContextUsage } from "./ContextUsage";
import type { Event, Session } from "../api/reportSessions";
import "./research-session.css";
import "./workspace-design.css";

type Conversation = { thread_id: string; title: string; status: string; messages: {id: string; role: string; content: string}[]; events: Event[]; runs: NonNullable<Session["runs"]>; permissions_notice: string };
...
export default function ConversationWorkspace() {
  ...
  const send = async () => { if (!text.trim() || sending || active) return; setSending(true); setError("");
    try { const result = await request<{thread_id:string}>(id ? `/${id}/messages` : "", {message:text, model, permission_mode:permission}); setText(""); setLive([]); if (!id) setParams({thread:result.thread_id}); else await refresh(); }
    catch(e) { setError((e as Error).message); } finally { setSending(false); }
  };
  return <div className={`rs-shell fs-workspace fs-assistant-shell ${navOpen ? "nav-open" : ""}`}><aside className="fs-assistant-nav"><a className="fs-brand" href="/workspace"><Layers size={24}/><strong>FinSight</strong></a><button className="fs-primary" onClick={() => { setParams({}); setText(""); setNavOpen(false); }}><Plus size={17}/>新对话</button><a href="/workspace">研究工作台 →</a><nav aria-label="已保存对话">{threads.map(t => <button key={t.thread_id} aria-current={id===t.thread_id ? "page":undefined} onClick={() => {setParams({thread:t.thread_id}); setNavOpen(false);}}><MessageSquare size={15}/><span>{t.title || "未命名对话"}</span></button>)}</nav></aside>
    <main className="fs-assistant-main"><header><button className="fs-assistant-menu" aria-expanded={navOpen} onClick={()=>setNavOpen(!navOpen)}>对话列表</button><div><small>FINSIGHT · ASSISTANT</small><h1>{session?.title || "从一个问题开始"}</h1></div><a href="/workspace">返回研究</a></header>
      ...
      <footer className="fs-assistant-compose">{session?.runs[0] && <ContextUsage usage={session.runs[0].context_usage} nodeName={() => "当前助理"}/>}
        {error && <p role="alert">{error}</p>}<label className="fs-sr-only" htmlFor="assistant-message">发送消息</label><textarea id="assistant-message" value={text} onChange={e=>setText(e.target.value)} rows={3} maxLength={16000} placeholder="输入问题，或继续修改刚才的回答…"/>
