...
DEFAULT_OUTPUT = ROOT / (
    "configs/releases/fin_ia_0_1_3_s2_fixed_pack_dell_canary_result_v1_0.json"
)
PRIVATE_ROOT = ROOT / (
    "data/workbench_private/fin_0_1_3_s2_fixed_pack_research/live"
)
LEDGER_PATH = PRIVATE_ROOT / "shared/admission_consumption.sqlite"


class FixedPackDellRunnerError(RuntimeError):
    pass
...
def validate_repository() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise FixedPackDellRunnerError(
            "fixed_pack_dell_runner_requires_clean_worktree"
        )
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise FixedPackDellRunnerError(
            "fixed_pack_dell_runner_requires_synced_branch"
        )
    return head
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise FixedPackDellRunnerError("fixed_pack_dell_runner_json_invalid")
    return value
...
def preflight() -> dict[str, Any]:
    ...
    if not os.environ.get(env_name, "").strip():
        raise FixedPackDellRunnerError("fixed_pack_dell_runner_credential_missing")
    ...
    if ancestor.returncode != 0:
        raise FixedPackDellRunnerError(
            "fixed_pack_dell_runner_implementation_not_ancestor"
        )
    ...
    if project_os.get("status") != "pass":
        raise FixedPackDellRunnerError(
            "fixed_pack_dell_runner_project_os_blocked:"
    ...
    if runtime_root.exists():
        raise FixedPackDellRunnerError(
            "fixed_pack_dell_runner_attempt_root_already_exists"
        )
    return {
        "head": head,
        "authority": authority,
...
def execute(*, output_path: Path) -> dict[str, Any]:
    if output_path.exists():
        raise FixedPackDellRunnerError("fixed_pack_dell_runner_result_already_exists")
    state = preflight()
            ...
            json.dumps(
                {
                    ...
                    "provider_call_ceiling": authority["execution_ceiling"][
                        "provider_calls"
                    ],
                    "retry_count": 0,
                    "credential_present": True,
        ...
        json.dumps(
            {
                ...
                "provider_calls": result["observed_counts"]["provider_calls"],
