...
6. [ ] `S2_MINIMAL_NATURAL_CANARY_AND_FORMAL_RERUN_DECISION`：唯一 DELL demand-authenticity DeepSeek Pro atom 已 exact-once 消费，`1 call／3,664 tokens／0 retry`。内容正确使用 E022、4 个 NUM、E018 read-through、E023 pull-forward 和兑现／利润边界；formal terminal 仅因 `surpassing` 未逐字匹配 canonical `surpassed` 而 failed，一词零调用反事实通过全部后续 gate。终态不改标、不重跑；先完成 provider-neutral presentation equivalence／local renderer 零调用处置及负向 mutation replay，再另行决定能否以保存 capture 完成 S2 bounded closeout。完整 DELL、paired／Owner、其他案例仍未授权。

`FIN-0.1.3-S2-SELECTED-EVIDENCE-NUMERIC-CANDIDATE-COCOMPILATION-MINIMUM-ZERO-CALL-IMPLEMENTATION` 已达到 clean independent engineering proof；第 6 项的自然观察已执行并保存。current next=`ZERO_CALL_PROVIDER_NEUTRAL_NUMERIC_PRESENTATION_EQUIVALENCE_AND_LOCAL_RENDERER_DISPOSITION_USING_IMMUTABLE_CAPTURE_NO_NEW_MODEL_CALL`。目标不是给 `surpassing` 加白名单，而是把 NUM authority／qualifier 与最终语法展示分开，并保持 negation、below-threshold、entity、period、unit fail closed。该处置通过前，formal canary、DELL report 与 S2 均不记 pass。
