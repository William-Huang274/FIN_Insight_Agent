...
CURRENT_DYNAMIC_MULTI_AGENT_CONTENT_REPAIR_SUBMISSION_RESUME_DECISION_STATUS = (
...
def validate_multi_agent_preview_generic_successor_scope_decision(
    *,
    ...
    terminal_writer_submission_fields = {
        "cross_role_evaluation_checkpoint_ref",
        "cross_role_evaluation_checkpoint_sha256",
        "cross_role_evaluation_checkpoint_digest",
    ...
    supplied_terminal_writer_fields = (
        set(decision) & terminal_writer_fields
        if "writer_analysis_fragment_checkpoint_ref" in decision
        else set()
    )
    supplied_terminal_writer_submission_fields = (
        set(decision) & terminal_writer_submission_fields
        if "writer_analysis_completion_checkpoint_ref" in decision
        else set()
    )
    ...
    if supplied_role_evaluation_checkpoint_fields not in (
        set(),
        role_evaluation_checkpoint_fields,
    ):
        raise ValueError(
    ...
    if supplied_terminal_writer_submission_fields not in (
        set(),
        terminal_writer_submission_fields,
    ):
        raise ValueError(
            "project_os_multi_agent_generic_successor_terminal_writer_"
            "submission_binding_incomplete"
        )
    ...
    if terminal_writer_successor and terminal_writer_submission_successor:
        raise ValueError(
            "project_os_multi_agent_generic_successor_two_terminal_modes_forbidden"
        )
    ...
    if frontier.get("evaluation_strategy") == HIERARCHICAL_EVALUATION_STRATEGY:
        ...
        if supplied_evaluator_profile_fields:
            ...
            expected_evaluator_defaults = (
                ...
                if supplied_role_evaluation_checkpoint_fields
                else {
                    "max_tokens": 10000,
                    "stream": False,
                    "thinking": {"type": "enabled"},
            ...
            if not (
                ...
                and evaluator_defaults == expected_evaluator_defaults
        ...
        if supplied_role_evaluation_checkpoint_fields:
            if supplied_evaluator_profile_fields != evaluator_profile_fields:
                raise ValueError(
                    "project_os_multi_agent_generic_successor_role_"
                    "evaluation_checkpoint_profile_required"
                )
            checkpoint_path, role_evaluation_checkpoint = (
                _validate_artifact_binding(
                    root=root,
                    decision=decision,
                    ref_field="role_evaluation_progress_checkpoint_ref",
                    sha_field="role_evaluation_progress_checkpoint_sha256",
                )
            )
            checkpoint_body = {
                key: value
        ...
        elif frontier.get("completed_role_evaluation_agent_ids"):
            raise ValueError(
                "project_os_multi_agent_generic_successor_role_evaluation_"
                "checkpoint_required"
            )
    ...
    if role_evaluation_checkpoint is not None:
        ...
        if not (
            ...
            == source_evaluator_binding.get("ref")
            and role_evaluation_checkpoint.get(
                "evaluator_analysis_profile_sha256"
            )
    ...
    if evaluator_profile is not None:
        ...
        if predecessor_evaluator_ref:
            if role_evaluation_checkpoint is None and (
    ...
    if terminal_writer_successor:
        ...
        if frontier.get("schema_version") != (
            ...
            raise ValueError(
                "project_os_multi_agent_terminal_writer_submission_frontier_invalid"
            )
        _, cross_role_checkpoint = _validate_artifact_binding(
            root=root,
            decision=decision,
        ...
        if not (
            ...
            == frontier["cross_role_evaluation_checkpoint_digest"]
            == canonical_digest(cross_role_body)
            and decision["cross_role_evaluation_checkpoint_ref"]
            == predecessor_scope.get("cross_role_evaluation_checkpoint_ref")
            and decision["cross_role_evaluation_checkpoint_sha256"]
            == predecessor_scope.get("cross_role_evaluation_checkpoint_sha256")
            and cross_role_checkpoint.get("role_evaluation_checkpoint_digest")
            == decision["role_evaluation_progress_checkpoint_digest"]
            and (cross_role_checkpoint.get("final_evaluation") or {}).get(
                "report_may_proceed"
            ...
            raise ValueError(
                "project_os_multi_agent_terminal_writer_submission_cross_role_invalid"
            )
        completion_path, completion_raw = _validate_artifact_binding(
            root=root,
            decision=decision,
            ref_field="writer_analysis_completion_checkpoint_ref",
            sha_field="writer_analysis_completion_checkpoint_sha256",
        )
        completion = validate_analysis_completion_checkpoint(completion_raw)
        fragment_path = _repo_path(
            root, str(completion["fragment_checkpoint_ref"])
        )
        request_path = _repo_path(
            root, str(completion["continuation_request_capture_ref"])
        )
        response_path = _repo_path(
            root, str(completion["continuation_response_capture_ref"])
        )
        fragment = (
            validate_analysis_fragment_checkpoint(_load_json(fragment_path))
            if fragment_path.is_file()
            else {}
        )
        if not (
            completion_path.relative_to(root).as_posix()
            == decision["writer_analysis_completion_checkpoint_ref"]
            and completion.get("checkpoint_digest")
            == decision["writer_analysis_completion_checkpoint_digest"]
            == frontier["writer_analysis_completion_checkpoint_digest"]
            and completion.get("node_id") == "AGENT::WRITER::REPORT_DRAFT"
            and completion.get("source_continuation_authority_ref")
            == decision["predecessor_live_authority_ref"]
            and completion.get("source_continuation_authority_sha256")
            == decision["predecessor_live_authority_sha256"]
            and completion.get("source_continuation_result_ref")
            == decision["predecessor_live_result_ref"]
            and completion.get("source_continuation_result_sha256")
            == decision["predecessor_live_result_sha256"]
            and completion.get("source_continuation_result_digest")
            == failed_result.get("result_digest")
            and fragment_path.is_file()
            and _sha256(fragment_path)
            == completion.get("fragment_checkpoint_sha256")
            and fragment.get("checkpoint_digest")
            == completion.get("fragment_checkpoint_digest")
            == frontier["writer_analysis_fragment_checkpoint_digest"]
            and request_path.is_file()
            and _sha256(request_path)
            == completion.get("continuation_request_capture_sha256")
            and response_path.is_file()
            and _sha256(response_path)
            == completion.get("continuation_response_capture_sha256")
            and (completion.get("submission_policy") or {}).get(
                "analysis_rerun_forbidden"
            )
            is True
            and (completion.get("submission_policy") or {}).get(
                "continuation_rerun_forbidden"
            )
            is True
        ):
            raise ValueError(
                "project_os_multi_agent_terminal_writer_completion_invalid"
            )
        predecessor_frontier_path = _repo_path(
            root, str(predecessor_scope["successor_execution_frontier_ref"])
        )
        predecessor_frontier = _load_json(predecessor_frontier_path)
        _, submission_proof_raw = _validate_artifact_binding(
            root=root,
            decision=decision,
            ref_field=(
                "writer_terminal_submission_successor_zero_call_proof_ref"
        ...
        profile_path = _repo_path(
            root,
            str(submission_proof_raw.get("writer_submission_profile_ref") or ""),
        )
        profile_raw = _load_json(profile_path) if profile_path.is_file() else {}
        profile = (
            load_chat_completion_profile(profile_raw) if profile_raw else None
        )
        if not (
            profile is not None
            and _sha256(profile_path)
            == submission_proof_raw.get("writer_submission_profile_sha256")
            ...
            raise ValueError(
                "project_os_multi_agent_terminal_writer_submission_profile_invalid"
            )
        submission_proof = (
            validate_terminal_submission_successor_zero_call_proof(
                submission_proof_raw,
                predecessor_frontier=predecessor_frontier,
                submission_frontier=frontier,
                writer_completion_checkpoint=completion,
                writer_submission_profile=profile_raw,
            )
        )
        if not (
            submission_proof["submission_frontier_ref"]
            == decision["successor_execution_frontier_ref"]
            and submission_proof["submission_frontier_sha256"]
            == decision["successor_execution_frontier_sha256"]
            and submission_proof["writer_completion_checkpoint_ref"]
            == decision["writer_analysis_completion_checkpoint_ref"]
            and failed_result.get("failure_code")
            == "multi_agent_analysis_continuation_semantically_incomplete"
        ):
            raise ValueError(
                "project_os_multi_agent_terminal_writer_submission_proof_invalid"
            )
    expected_constraints = {
    ...
    if terminal_writer_submission_successor:
        expected_budget_policy.update(
            {
                "reused_cross_role_evaluation_has_no_new_token_budget": True,
                "writer_completed_analysis_reuse_has_no_new_token_budget": True,
            }
        )
    if decision.get("token_budget_basis_policy") != expected_budget_policy:
        raise ValueError(
    ...
    return {
        ...
        "evaluator_analysis_reasoning_effort": (
            (
                evaluator_profile.get("request_defaults", {}).get(
                    "reasoning_effort"
                )
                if evaluator_profile is not None
                else None
        ...
        "evaluator_analysis_thinking_mode": (
        ...
        "role_evaluation_progress_checkpoint_ref": (
            decision.get("role_evaluation_progress_checkpoint_ref")
            if role_evaluation_checkpoint is not None
            else None
        ),
        "role_evaluation_progress_checkpoint_sha256": (
            decision.get("role_evaluation_progress_checkpoint_sha256")
            if role_evaluation_checkpoint is not None
            else None
        ),
        "role_evaluation_progress_checkpoint_digest": (
            decision.get("role_evaluation_progress_checkpoint_digest")
            if role_evaluation_checkpoint is not None
            else None
        ),
        ...
        "writer_analysis_completion_checkpoint_ref": (
            decision.get("writer_analysis_completion_checkpoint_ref")
            if terminal_writer_submission_successor
            else None
        ),
        "writer_terminal_submission_successor_zero_call_proof_status": (
            submission_proof["status"]
            if terminal_writer_submission_successor
            else None
        ),
        "maximum_new_lead_plan_model_calls": 0,
...
def _validate_current_dynamic_multi_agent_content_repair_submission_resume_decision(
    *, root: Path, decision: Mapping[str, Any]
) -> dict[str, Any]:
    expected_equal = {
        "schema_version": (
        ...
        "evidence_mode": (
            "immutable_R5_workpapers_R7_five_reusable_nodes_zero_new_evidence"
        ),
        "next_authorized_scope": (
            CURRENT_DYNAMIC_MULTI_AGENT_CONTENT_REPAIR_SUBMISSION_RESUME_SCOPE
        ),
        ...
        "failed_R7_required": True,
        "five_reusable_semantic_nodes_required": True,
        "invalid_Demand_submission_reuse_forbidden": True,
        "typed_validation_feedback_required": True,
        "fresh_R8_authority_required": True,
        "exact_three_remaining_role_repairs_required": True,
    ...
    if any(decision.get(key) != value for key, value in expected_equal.items()):
        raise ValueError(
            ...
            "submission_resume_identity_invalid"
        )
    expected_budget = expected_content_repair_submission_resume_budget()
    if dict(decision.get("execution_budget") or {}) != expected_budget:
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "submission_resume_budget_invalid"
        )
    if tuple(decision.get("new_repair_agent_ids") or ()) != (
        "AGENT::DEMAND_QUALITY",
        "AGENT::OPERATING_PERFORMANCE",
        "AGENT::VALUE_CAPTURE",
    ) or tuple(decision.get("reused_repair_agent_ids") or ()) != (
    ...
    predecessor_private_path, predecessor_private = _validate_artifact_binding(
        root=root,
        decision=decision,
    ...
    _, failed_authority = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_authority_ref",
        sha_field="failed_R7_authority_sha256",
    )
    _, failed_public = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_public_ref",
        sha_field="failed_R7_public_sha256",
        digest_field="failed_R7_public_result_digest",
    )
    failed_private_path, failed_private = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_private_ref",
        sha_field="failed_R7_private_sha256",
        digest_field="failed_R7_private_full_result_digest",
        artifact_digest_field="full_result_digest",
    )
    ...
    if not (
        ...
        == str(decision["failed_R7_authority_sha256"])
        ...
        == 6
        and (failed_private.get("execution") or {}).get("new_provider_http_200")
        == 6
        and (failed_private.get("execution") or {}).get(
        ...
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "submission_resume_chain_invalid"
        )
    for ref_field, sha_field in (
        ...
        if _sha256(bound_path) != str(decision.get(sha_field) or ""):
            raise ValueError(
                "project_os_current_dynamic_multi_agent_content_repair_"
                f"submission_resume_runtime_sha_drift:{ref_field}"
            )
    _, profile = _validate_artifact_binding(
        root=root,
        decision=decision,
    ...
    if not (
        ...
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "submission_resume_profile_invalid"
        )
    token_basis = decision.get("token_budget_basis")
    required_nodes = {
        "role_repair_analysis": 2,
        "role_repair_submission": 3,
    ...
    if not (
        ...
        and sum(required_nodes.values()) == expected_budget["maximum_new_model_calls"]
    ):
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "submission_resume_token_basis_invalid"
        )
    return {
        "clean_proof_status": zero["status"],
        "provider_id": profile["provider_id"],
        ...
        "current_dynamic_multi_agent_content_repair_submission_resume": True,
        "run_scope_id": decision["run_scope_id"],
        "execution_limits": expected_budget,
        "node_profiles": token_basis,
        "failed_R7_code": failure_code,
        "failed_R7_provider_calls": 6,
        "submission_resume_zero_call_status": zero["status"],
    }


def _validate_current_dynamic_multi_agent_content_repair_authority_ceiling_resume_decision(
    *, root: Path, decision: Mapping[str, Any]
) -> dict[str, Any]:
    expected_equal = {
        "schema_version": (
        ...
        "evidence_mode": (
            "immutable_R5_R7_R8_and_digest_bound_Demand_authority_ceiling_zero_new_evidence"
        ),
        "next_authorized_scope": (
            CURRENT_DYNAMIC_MULTI_AGENT_CONTENT_REPAIR_AUTHORITY_CEILING_RESUME_SCOPE
        ),
        ...
        "immutable_R5_predecessor_required": True,
        "failed_R7_required": True,
        "failed_R8_required": True,
        "R8_one_provider_zero_retry_required": True,
        "digest_bound_authority_ceiling_required": True,
        "authority_ceiling_zero_call_proof_required": True,
        "fresh_R9_authority_required": True,
        "exact_two_remaining_role_repairs_required": True,
        "demand_local_revalidation_reuse_required": True,
        "supply_workpaper_reuse_required": True,
        "authority_sets_unchanged_required": True,
        "one_natural_lead_recheck_required": True,
        "independent_reassessment_required": True,
    ...
    if any(decision.get(key) != value for key, value in expected_equal.items()):
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "authority_ceiling_resume_identity_invalid"
        )
    expected_budget = expected_content_repair_authority_ceiling_resume_budget()
    if dict(decision.get("execution_budget") or {}) != expected_budget:
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
    ...
    if tuple(decision.get("new_repair_agent_ids") or ()) != (
        ...
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "authority_ceiling_resume_target_invalid"
        )
    _, zero = _validate_artifact_binding(
        root=root,
        decision=decision,
    ...
    _, failed_r7_authority = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_authority_ref",
        sha_field="failed_R7_authority_sha256",
    )
    _, failed_r7_public = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_public_ref",
        sha_field="failed_R7_public_sha256",
    ...
    failed_r7_private_path, failed_r7_private = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R7_private_ref",
        sha_field="failed_R7_private_sha256",
    ...
    _, failed_r8_authority = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R8_authority_ref",
        sha_field="failed_R8_authority_sha256",
    )
    _, failed_r8_public = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R8_public_ref",
        sha_field="failed_R8_public_sha256",
        digest_field="failed_R8_public_result_digest",
    )
    failed_r8_private_path, failed_r8_private = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R8_private_ref",
        sha_field="failed_R8_private_sha256",
        digest_field="failed_R8_private_full_result_digest",
        artifact_digest_field="full_result_digest",
    )
    _, failure_assessment = _validate_artifact_binding(
        root=root,
        decision=decision,
        ref_field="failed_R8_assessment_ref",
        sha_field="failed_R8_assessment_sha256",
    )
    failure_code = "multi_agent_workpaper_claim_unbound"
    ...
    if not (
        ...
        == "dynamic_multi_agent_contract_pass_financial_truth_and_evidence_authority_fail_writer_not_eligible"
        and len(assessment.get("material_findings") or ()) == 7
        and failed_r7_authority.get("schema_version")
        == "fin_ia_s3_current_dynamic_multi_agent_content_repair_authority_v1_0"
        ...
        == _sha256(failed_r7_private_path)
        ...
        == 5
        and failed_r8_authority.get("schema_version")
        ...
        == str(decision["failed_R8_authority_sha256"])
        ...
        and (failure_assessment.get("rejected_claim") or {}).get(
            "claim_digest"
        )
        ...
        and dict(zero.get("execution_budget") or {}) == expected_budget
        and (zero.get("demand_submission_receipt") or {}).get(
            "model_authority_label_ceiling_applied"
        )
        is True
    ):
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "authority_ceiling_resume_chain_invalid"
        )
    for ref_field, sha_field in (
        ("runner_ref", "runner_sha256"),
        ("content_repair_runtime_ref", "content_repair_runtime_sha256"),
        ...
        if _sha256(bound_path) != str(decision.get(sha_field) or ""):
            raise ValueError(
                "project_os_current_dynamic_multi_agent_content_repair_"
                f"authority_ceiling_resume_runtime_sha_drift:{ref_field}"
            )
    _, profile = _validate_artifact_binding(
        root=root,
        decision=decision,
    ...
    if not (
        ...
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "authority_ceiling_resume_profile_invalid"
        )
    token_basis = decision.get("token_budget_basis")
    required_nodes = {
        "remaining_role_repair_analysis": 2,
        "remaining_role_repair_submission": 2,
        "lead_recheck_analysis": 1,
        "lead_recheck_submission": 1,
    }
    required_fields = {
    ...
    if not (
        ...
        raise ValueError(
            "project_os_current_dynamic_multi_agent_content_repair_"
            "authority_ceiling_resume_token_basis_invalid"
        )
    return {
        "clean_proof_status": zero["status"],
        "provider_id": profile["provider_id"],
        "provider_model": profile["model"],
        "api_key_env": profile["api_key_env"],
        "recent_provider_steps": 7,
        "current_dynamic_multi_agent_content_repair": True,
        "current_dynamic_multi_agent_content_repair_authority_ceiling_resume": True,
        "run_scope_id": decision["run_scope_id"],
        "execution_limits": expected_budget,
        "node_profiles": token_basis,
        "failed_R8_code": failure_code,
        "failed_R8_provider_calls": 1,
        "authority_ceiling_resume_zero_call_status": zero["status"],
    }
...
def _validate_current_dynamic_multi_agent_content_repair_decision(
    *, root: Path, decision: Mapping[str, Any]
    ...
    if (
        ...
        return _validate_current_dynamic_multi_agent_content_repair_authority_ceiling_resume_decision(
            root=root,
            decision=decision,
        )
    if (
        ...
        return _validate_current_dynamic_multi_agent_content_repair_submission_resume_decision(
            root=root,
            decision=decision,
        )
    successor = (
    ...
    *,
    ...
    if decision_projection.get("multi_agent_preview_generic_successor") is True:
        ...
        if (
            ...
            and not _issue_explicitly_allows(
                root=root,
                issue_id=(
                    "RC-AR-024-low-reasoning-evaluator-has-no-visible-"
                    "output-reserve-or-node-checkpoint"
                ),
                allowed_scope=MULTI_AGENT_PREVIEW_GENERIC_SUCCESSOR_SCOPE,
            )
        ):
    ...
    if (
        ...
        if decision_projection.get(
            ...
            known_boundary = (
                "This current-baseline preflight preserves immutable R5, the "
                "consumed R7 six-call terminal and the consumed R8 one-call "
                "terminal. R8 received typed feedback but repeated the exact "
                "unbound Demand claim. A zero-call successor binds that unique "
                "claim digest to its existing gap and lowers only effective "
                "authority from bounded_inference to not_inferable; claim text, "
                "Evidence, NumericFact, Relation and gap refs remain unchanged. "
                "Cash, Counterevidence and Demand repairs are reusable. It "
                "authorizes only Operating and Value analysis plus strict "
                "submission pairs and one Lead recheck pair: at most 6 new "
                "Provider calls. It permits zero new S1/S2 request, retrieval "
                "round, external source call, retry, fallback, Candidate "
                "promotion or product-pointer mutation. Success still requires "
                "independent L1/L2 and content reassessment and does not "
                "authorize Writer, S3 acceptance, heterogeneous generalization, "
                "Workbench publication or release."
            )
            required_terms = (
                "consumed R8 one-call terminal",
                "lowers only effective authority",
                "at most 6 new Provider calls",
            ...
            known_boundary = (
                "This current-baseline preflight preserves immutable R5 and the "
                "consumed R7 terminal with six HTTP-200 calls. Cash and "
                "Counterevidence strict repairs plus the Demand natural draft "
                "are reusable; the rejected Demand strict submission is not. "
                "It authorizes exactly one feedback-bound Demand strict "
                "resubmission, two remaining Operating and Value analysis plus "
                "strict-submission pairs, and exactly one Lead recheck pair: at "
                "most 7 new Provider calls. All Evidence, NumericFact, Relation "
                ...
                "Success still requires independent L1/L2 and content "
                "reassessment and does not authorize Writer, S3 acceptance, "
                "heterogeneous generalization, Workbench publication or release."
            )
            required_terms = (
                "rejected Demand strict submission is not",
                "exactly one feedback-bound Demand strict resubmission",
                "at most 7 new Provider calls",
                "zero new S1/S2 request",
                "does not authorize Writer",
            )
        else:
