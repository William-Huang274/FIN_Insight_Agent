"""Follow up on saved reports while retaining source and version bindings."""
...
from .report_synthesis_agent import build_case_output_agent, report_citations, report_model_view, CaseReport, ReportReview
from .case_review_agent import CaseModelAudit, case_chat_model, case_mcp_tools
...
def load_session_materials(settings):
    from .model_execution_policy import file_sha256
    ...
    artifacts = CaseArtifacts(bundle["papers"])
...
def _schema_graph():
    ...
    return build_report_session_graph(**agents, artifacts=None, initial={}).compile(name='report_session')
...
@asynccontextmanager
async def report_session_graph(config: RunnableConfig, runtime: ServerRuntime):
    ...
    with open_approved_data_composition(run_invocation_id=invocation, source_read_enabled=True,
        ...
        async with Client(data.mcp_server, raise_exceptions=False, read_timeout_seconds=120) as client:
            ...
            with native_memory_scope(thread.get('metadata', {}).get('owner_id', 'local-pilot'), thread_id):
                yield build_report_session_graph(**agents, artifacts=artifacts, initial=initial, audits=audits).compile(
                    name='report_session').with_config({"recursion_limit": 240})
