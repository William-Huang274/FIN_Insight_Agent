...
- `thesis_path`, `judgment_cards`, and any upstream `judgment_candidates`: primary writer-ready material when present. They represent analyst judgment units, not raw evidence inventory.
...
- Treat JudgmentCards as the main units of analysis: write the judgment, then the evidence bridge, then the business mechanism, then the counter-read and what would change the view.
- Treat `supported_claims` as fallback verified ClaimCard observations only when a thesis pack is absent.
- If a required item lacks a JudgmentCard or typed gap, fail that section or mark it as traceable gap; do not fill with generic “insufficient data” prose.
...
- Do not collapse product analysis to “no SKU revenue.” If product/spec/architecture, deployment, supply-chain, or competitive evidence exists, write the bounded product judgment and clearly state what it cannot infer.
