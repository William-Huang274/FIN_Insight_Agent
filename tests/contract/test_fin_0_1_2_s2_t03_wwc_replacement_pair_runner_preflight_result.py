...
def test_persisted_result_preserves_historical_zero_call_preflight() -> None:
    result = _load(RESULT)
...
def test_historical_projection_stops_at_exact_pair_execution_and_current_advances() -> None:
    result = _load(RESULT)
    ...
    assert current["current_projection_ref"].endswith(
        "fin_ia_0_1_2_current_program_projection_v2_18.json"
    )
    ...
    assert current["S2_T03_replacement_pair_execution_started"] is True
...
def test_project_os_preserves_preflight_open_state_as_history() -> None:
    ...
    for issue_id in (
        ...
        preflight_issue = [
            ...
            and row.get("recorded_at") == "2026-08-03T11:30:58+08:00"
        ][-1]
        assert preflight_issue["status"] == "open"
        assert preflight_issue["allowed_run_scopes"] == [
            "FIN-0.1.2-S2-T03-MU-WWC-V1.2-FLASH-STABLE-VS-PRO-PREVIEW-REPLACEMENT-PAIR-EXACT-EXECUTION",
            "restricted_audit_evidence_review",
            "repository_and_git_hygiene",
        ]
        assert preflight_issue["verification"]["replacement_execution_started"] is False
