...
from . import dell_report_internal_chain_ceiling_r9 as r9
from .dell_report_frame_transformation_r10 import (
    FrameTransformationBinding,
    build_frame_transformation_binding_r10,
    build_missing_compiled_frame_binding_r10,
    transformation_binding_digest_r10,
)
from .dell_report_predicate_frames_r10 import (
    TARGET_IDS,
    PredicateFrame,
    classify_package as _classify_predicate_frame_r10,
...
POLICY_SCHEMA_VERSION = "fin_ia_dell_report_internal_chain_ceiling_policy_v1_9"
PRIVATE_RESULT_SCHEMA_VERSION = (
    "fin_ia_dell_report_internal_chain_candidate_ceiling_private_result_v1_9"
)
PUBLIC_RESULT_SCHEMA_VERSION = (
    "fin_ia_dell_report_internal_chain_candidate_ceiling_result_v1_9"
)
ATTEMPT_ID = "dell-rsq-03b-internal-chain-r10"
PROGRAM_ID = "FIN-0.1.3-S1-DELL-RSQ-03B-R10"
BRANCH = r9.BRANCH
POLICY_REF = (
    "configs/retrieval/"
    "fin_ia_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling_policy_v1_9.json"
)
PUBLIC_REF = (
    "configs/retrieval/"
    "fin_ia_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling_result_v1_9.json"
)
PRIVATE_REF = (
...
MIN_FREE_BYTES_BEFORE_ATTEMPT = r9.MIN_FREE_BYTES_BEFORE_ATTEMPT
ZERO_EXECUTION_FIELDS = r9.ZERO_EXECUTION_FIELDS
EXECUTION_CONTRACT = dict(r9.EXECUTION_CONTRACT)
AUTHORITY = dict(r9.AUTHORITY)
SEMANTIC_CONTRACT = {
    **dict(r9.SEMANTIC_CONTRACT),
    "predicate_frame_mode": (
        "open_vocabulary_structural_boundary_typed_scope_argument_group_"
        "frame_v6"
    ),
    "scope_mode": (
    ...
    "material_coverage_mode": (
        "relational_semantic_signature_and_lossless_frame_"
        "transformation_binding_v2"
    ),
}
EXPECTED_IMPLEMENTATION_PATHS = frozenset(
    set(r9.EXPECTED_IMPLEMENTATION_PATHS)
    | {
        "src/retrieval/dell_report_predicate_frames_r10.py",
        "src/retrieval/dell_report_frame_transformation_r10.py",
        "src/retrieval/dell_report_internal_chain_ceiling_r10.py",
        "scripts/data_retrieval/run_dell_report_internal_chain_ceiling_r10.py",
    }
)
EXPECTED_BOUND_INPUT_IDS = frozenset(
    {
        ...
        "R9_private",
        "R9_attempt_receipt",
        "R9_raw_execution_capture",
        "R9_fresh_audit",
        "R9_fixed_audit_manifest",
        "R17_report_audit",
...
class DellReportInternalChainCeilingR10Error(ValueError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise DellReportInternalChainCeilingR10Error(code)
...
def validate_dell_report_internal_chain_ceiling_r10_policy(
    ...
    **bound_values: Mapping[str, Any],
    ...
    _require(
        ...
        "dell_03B_R10_policy_identity_or_digest_invalid",
    )
    _require(
        value.get("execution_contract") == EXECUTION_CONTRACT
        and value.get("semantic_contract") == SEMANTIC_CONTRACT,
        "dell_03B_R10_policy_execution_or_semantic_contract_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R10_policy_output_contract_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R10_policy_bound_input_population_invalid",
    )
    for binding_id, raw in raw_bound.items():
        ...
        _require(
            ...
            f"dell_03B_R10_policy_bound_input_invalid:{binding_id}",
        )
        supplied = dict(bound_values[binding_id])
        if "result_digest" in row:
            _require(
                supplied.get("result_digest") == row.get("result_digest"),
                f"dell_03B_R10_policy_bound_result_digest_drift:{binding_id}",
            )

    r9_private = _validate_r9_predecessor_result(bound_values["R9_private"])
    r9_policy = dict(bound_values["R9_policy"])
    r9_public = dict(bound_values["R9_public"])
    r9_attempt = dict(bound_values["R9_attempt_receipt"])
    r9_raw = dict(bound_values["R9_raw_execution_capture"])
    _require(
        _self_digest(r9_policy)
        and _self_digest(r9_public)
        and _self_digest(r9_attempt)
        and _self_digest(r9_raw)
        and r9_policy.get("result_digest") == r9_private.get("policy_digest")
        and r9_public.get("private_result_digest")
        == r9_private.get("result_digest")
        and r9_attempt.get("attempt_id") == r9.ATTEMPT_ID
        and r9_attempt.get("policy_digest") == r9_private.get("policy_digest")
        and r9_raw.get("attempt_id") == r9.ATTEMPT_ID
        and r9_raw.get("policy_digest") == r9_private.get("policy_digest")
        and r9_raw.get("raw_execution_sha256")
        == r9_private.get("raw_execution_sha256"),
        "dell_03B_R10_R9_predecessor_chain_invalid",
    )
    r9_audit = dict(bound_values["R9_fresh_audit"])
    r9_manifest = dict(bound_values["R9_fixed_audit_manifest"])
    r17_audit = dict(bound_values["R17_report_audit"])
    engineering_verdict = dict(r9_audit.get("R9_engineering_verdict") or {})
    report_verdict = dict(r9_audit.get("R17_report_quality") or {})
    _require(
        _self_digest(r9_audit)
        and _self_digest(r9_manifest)
        and r9_audit.get("audit_manifest", {}).get("result_digest")
        == r9_manifest.get("result_digest")
        and r9_audit.get("audit_id") == r9_manifest.get("audit_id")
        and engineering_verdict.get("verdict") == "FAIL"
        and engineering_verdict.get("new_severity_counts")
        == {"P0": 0, "P1": 0, "P2": 2, "P3": 0}
        and {
            str(row.get("finding_id") or "")
            for row in r9_audit.get("R9_material_findings") or ()
            if isinstance(row, Mapping)
        }
        == {
            "R9-P2-OPEN-VOCABULARY-RIGHT-SUBJECT-BYPASS",
            "R9-P2-PRODUCT-PRICE-RELATION-OMITTED",
        }
    ...
    _require(
        ...
        "dell_03B_R10_policy_execution_identity_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R10_policy_implementation_binding_population_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R10_policy_TokenBudgetBasis_invalid",
    )
    _require(
        value.get("authority") == _POLICY_AUTHORITY,
        "dell_03B_R10_policy_authority_invalid",
    )
    return r9_private


def classify_dell_report_internal_chain_r10_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
) -> dict[str, Any]:
    return _classify_predicate_frame_r10(
        target_id=target_id,
        text=text,
        metadata=metadata,
    )
...
def build_dell_report_internal_chain_r10_corpus_index(
    *,
    source_rows: Sequence[Mapping[str, Any]],
    object_rows: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    return r9.build_dell_report_internal_chain_r9_corpus_index(
        source_rows=source_rows,
        object_rows=object_rows,
    )


def _package_windows_r10(
    *,
    target_id: str,
    ...
    for unit in units:
        ...
        if (
            ...
            continue
        assessment = classify_dell_report_internal_chain_r10_package(
            target_id=target_id,
            text=text,
            metadata=metadata,
        )
...
def _best_package_r10(
    ...
    *,
    ...
    if windows:
        ...
        value = classify_dell_report_internal_chain_r10_package(
...
def _family_transformation_binding_r10(
    *,
    target_id: str,
    family_id: str,
    metadata: Mapping[str, Any],
    ...
    if (
        ...
        return None
    source_frame = source_frame or _selected_frame_r10(
        target_id=target_id,
        package=source_package,
        metadata=metadata,
    )
    compiled_frame = compiled_frame or _selected_frame_r10(
        target_id=target_id,
    ...
    return build_frame_transformation_binding_r10(
        ...
        compiled_window_ids=[f"WINDOW::R10::{value}" for value in object_ids],
...
def _matching_compiled_frame_r10(
    *,
    target_id: str,
...
def _coverage_gap_r10(
    *,
    ...
    return {
        ...
        "anchor_mode": "relational_semantic_signature_transformation_bound_v6",
        "reason": (
...
def assess_dell_report_internal_chain_r10_packages(
    *,
    target_id: str,
    source_rows: Sequence[Mapping[str, Any]],
    object_rows: Sequence[Mapping[str, Any]],
    ...
    index = (
        ...
        else build_dell_report_internal_chain_r10_corpus_index(
            source_rows=source_rows,
            object_rows=object_rows,
        )
    )
    _require(
        ...
        "dell_03B_R10_corpus_index_population_or_position_drift",
    )
    ...
    for family_id in family_ids:
        ...
        if selected is None:
            source_windows = _package_windows_r10(
                target_id=target_id,
            ...
            source_package = _best_package_r10(
                source_windows,
                family_id=family_id,
                metadata=metadata,
        ...
        object_windows = _package_windows_r10(
            target_id=target_id,
        ...
        compiled_package = _best_package_r10(
            object_windows,
            family_id=family_id,
            metadata=metadata,
            object_package=True,
        ...
        if source_package is not None:
            source_frame = _selected_frame_r10(
                target_id=target_id,
                package=source_package,
                metadata=metadata,
            )
            matching_compiled = (
                _matching_compiled_frame_r10(
                    target_id=target_id,
                    source_frame=source_frame,
            ...
            binding = _family_transformation_binding_r10(
                target_id=target_id,
                family_id=family_id,
                metadata=metadata,
            ...
            if binding is None and source_frame is not None:
                binding = build_missing_compiled_frame_binding_r10(
                    canonical_source_family_id=family_id,
    ...
    return {
        ...
        "frame_transformation_binding_digest": (
            transformation_binding_digest_r10(bindings)
        ),
...
def _validate_r9_predecessor_result(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    result = dict(value)
    _require(
        result.get("schema_version") == r9.PRIVATE_RESULT_SCHEMA_VERSION
        and result.get("attempt_id") == r9.ATTEMPT_ID
        and result.get("status")
        == "dell_03B_R9_typed_frame_transformation_ceiling_executed"
        and result.get("case_key") == "DELL"
        and _self_digest(result),
        "dell_03B_R10_R9_predecessor_private_identity_invalid",
    )
    return result


def compile_dell_report_internal_chain_ceiling_r10_result(
    *,
    r9_private_result: Mapping[str, Any],
    r10_policy: Mapping[str, Any],
    ...
    """Compile one fresh or explicitly saved execution through the R10 IR."""

    predecessor = _validate_r9_predecessor_result(r9_private_result)
    predecessor_targets = {
    ...
    _require(
        ...
        "dell_03B_R10_R9_predecessor_target_population_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R10_raw_execution_sha_mismatch",
    )
    request_by_id = validated["request_results_by_id"]
    corpus_index = build_dell_report_internal_chain_r10_corpus_index(
        source_rows=source_rows,
        object_rows=object_rows,
    )
    object_ids = {
    ...
    for target_id in sorted(TARGET_IDS):
        ...
        _require(
            ...
            f"dell_03B_R10_candidate_object_binding_invalid:{target_id}",
        )
        total_union_occurrences += len(union_ids)
        corpus = assess_dell_report_internal_chain_r10_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        union = assess_dell_report_internal_chain_r10_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        final = assess_dell_report_internal_chain_r10_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        target_results.append(
            {
                ...
                "semantic_evidence_unit": (
                    "one_typed_R10_assertion_frame_with_scope_edges_argument_"
                    "groups_and_lossless_source_compiled_binding"
                ),
                "candidate_ceiling": {
                    ...
                    "material_anchor_mode": (
                        "relational_semantic_signature_transformation_bound_v6"
                    ),
    ...
    body = {
        ...
        "known_boundary": (
            "R10 formal authority permits exactly one fresh local 0.6B query "
            "embedding batch; preview alone may recompile immutable R9 raw "
            "inputs with zero calls. R10 adds open-vocabulary structural event "
            "boundaries and same-group product-price relational completion plus "
            "relation-aware source-to-compiled transformation bindings. "
            "It grants no external capture, 4B embedding, reranker, Evidence, "
            "report or product authority."
        ),
        "policy_digest": r10_policy.get("result_digest"),
    }
    return {**body, "result_digest": canonical_digest(body)}


_R10_AUTHORITY_KEYS = frozenset(
    {
        "03B_R10_execution_consumed",
...
_R10_PUBLIC_BINDING_IDS = frozenset(
...
def _public_input_bindings_r10(value: Any) -> dict[str, Any]:
    bindings = _exact_mapping(
        value,
        _R10_PUBLIC_BINDING_IDS,
        "dell_03B_R10_public_input_bindings",
    )
    output: dict[str, Any] = {}
    for binding_id, raw in bindings.items():
        if binding_id == "git_identity":
            row = _exact_mapping(
                ...
                "dell_03B_R10_public_git_identity",
            )
            _require(
                ...
                "dell_03B_R10_public_git_identity_value_invalid",
            )
            ...
            row = _exact_mapping(
                ...
                "dell_03B_R10_public_disk_preflight",
            )
            _require(
                ...
                == MIN_FREE_BYTES_BEFORE_ATTEMPT,
                "dell_03B_R10_public_disk_preflight_value_invalid",
            )
            output[binding_id] = row
        else:
            _require(
                isinstance(raw, Mapping),
                f"dell_03B_R10_public_binding_not_mapping:{binding_id}",
            )
            row = dict(raw)
            _require(
                {"ref", "sha256"}.issubset(row)
                and set(row).issubset(r7._STANDARD_BINDING_KEYS),  # noqa: SLF001
                f"dell_03B_R10_public_binding_unknown_or_missing_key:{binding_id}",
            )
            _require(
                ...
                f"dell_03B_R10_public_binding_value_invalid:{binding_id}",
            )
            output[binding_id] = row
    return output


def build_dell_report_internal_chain_ceiling_r10_public_projection(
    *,
    ...
    _require(
        ...
        "dell_03B_R10_private_projection_identity_invalid",
    )
    _require(
        private_ref == PRIVATE_REF
        and bool(re.fullmatch(r"[0-9a-f]{64}", private_sha256)),
        "dell_03B_R10_private_projection_binding_invalid",
    )
    try:
        target_results = []
        for raw_row in private.get("target_results") or ():
            _require(
                ...
                "dell_03B_R10_public_target_not_mapping",
            )
            row = dict(raw_row)
            ...
            _require(
                ...
                "dell_03B_R10_private_transformation_surface_invalid",
            )
            target_results.append(
                r7._public_target_row(row)  # noqa: SLF001
            )
    except r7.DellReportInternalChainCeilingR7Error as exc:
        raise DellReportInternalChainCeilingR10Error(
            str(exc).replace("dell_03B_R7_", "dell_03B_R10_", 1)
        ) from exc
    _require(
        len(target_results) == len(TARGET_IDS)
        and {str(row["target_id"]) for row in target_results} == set(TARGET_IDS),
        "dell_03B_R10_public_target_population_invalid",
    )
    authority = _exact_mapping(
        private["authority"],
        _R10_AUTHORITY_KEYS,
        "dell_03B_R10_public_authority",
    )
    _require(
        authority.get("03B_R10_execution_consumed") is True
        and all(
            authority.get(key) is False
            for key in _R10_AUTHORITY_KEYS - {"03B_R10_execution_consumed"}
        ),
        "dell_03B_R10_public_authority_value_invalid",
    )
    body = {
        ...
        "input_bindings": _public_input_bindings_r10(private["input_bindings"]),
        "runtime_registry": _exact_mapping(
            private["runtime_registry"],
            r7._RUNTIME_REGISTRY_KEYS,  # noqa: SLF001
            "dell_03B_R10_public_runtime_registry",
        ),
        ...
        "execution_summary": _exact_mapping(
            ...
            "dell_03B_R10_public_execution_summary",
        ),
        "target_results": target_results,
        "summary": _exact_mapping(
            private["summary"],
            r7._SUMMARY_KEYS,  # noqa: SLF001
            "dell_03B_R10_public_summary",
        ),
    ...
    validate_r10_public_tree(body)
    return {**body, "result_digest": canonical_digest(body)}


def validate_r10_public_tree(value: Any) -> None:
