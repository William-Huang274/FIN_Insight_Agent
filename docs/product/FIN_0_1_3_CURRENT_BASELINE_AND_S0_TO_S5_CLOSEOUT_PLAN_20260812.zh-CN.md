...
25. **DELL 五单元动态完整案例（R1 已在 cell 前暴露容量合同缺陷）**：有限 S2 回归已证明法说叙事不会越权成为 NumericFact；consumer policy v1.3 为需求真实性、经营表现、价值捕获、现金／资产负债表、瓶颈／反方／WWC 五单元建立 RoleMethodPack 和本案即时 GraphContextPack。formal context／runner zero-call 与 clean authority 均已通过。自然 R1 的 Planner 和当前 S1/S2 也已成功，但 `value_capture` 合法形成 10 条 NumericFact，而 v1.3 静态上限为 8，故在 0 个 cell Judgment 时 fail closed。R1 永久保持失败，不能重跑 Planner/S1/S2，也不能把问题归因于 DeepSeek。下一步只做 policy v1.4、10／11 容量 mutation、R1 immutable replay 和稳定 runner 的 12-node successor；通过后才依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容。一次完整运行不能自动等同 S3 acceptance。
