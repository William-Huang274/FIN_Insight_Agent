...
from test_fin_0_1_s4_t05_case_numeric_authority_and_delivery_identity_zero_call_implementation import (
    test_three_case_full_fake_reaches_twelve_callbacks_nine_artifacts_and_local_truth,
)
from test_fin_0_1_s4_t06_mu_case_runtime_mandatory_material_truth_identity_safety_closure_zero_call_implementation import (
    ...
    test_final_numeric_fact_statement_mutation_fails_exact_correspondence,
    test_lead_v7_composes_safety_without_inheriting_v6_gap_atoms,
    test_mu_s4_path_reaches_twelve_calls_nine_final_artifacts_with_safety_markers,
)
...
DECISION = (
    ...
    "closure_fresh_agent_proof_decision_v1_0.json"
)
PROSPECTIVE_ADMISSION = (
    ROOT
    / "configs/releases/"
    "fin_ia_0_1_s4_t06_mu_mandatory_material_truth_identity_safety_"
    "closure_fresh_exact_admission_r3.json"
)
EXECUTION_IDENTITY = (
    "fin01-s4-t06-mu-mandatory-material-truth-identity-safety-"
    "closure-exact-live-r3"
)
PROSPECTIVE_ADMISSION_ID = (
    "fin01-s4-t06-mu-mandatory-material-truth-identity-safety-"
    "closure-fresh-exact-admission-r3"
)
EXECUTION_MODE = (
    "exact_live_s4_mu_mandatory_material_truth_identity_safety_"
    "closure_r3"
)
NEXT_ACTION = (
    "S4-T06-MU-MANDATORY-MATERIAL-TRUTH-AND-IDENTITY-SAFETY-"
    "CLOSURE-FRESH-EXACT-ADMISSION-ISSUANCE"
)


class S4T06MuSafetyFreshProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T06MuSafetyFreshProofError(code)
...
def _run_fixture_reproof() -> dict[str, Any]:
    ...
    return {
        "three_case_final_path": case_results,
        "mandatory_safety_marker_recomputed": True,
        "lead_v7_capability_composition_recomputed": True,
        "negative_mutations_rejected": [
            "missing_safety_profile_marker",
            "numeric_projection_exact_value_mutation",
            "cross_case_report_title",
            "nonlocal_report_numeric_token",
            "cross_case_review_label",
            "canonical_numeric_fact_statement_mutation",
        ],
    }
...
def prepare(
    *,
    ...
    source_success_path: Path = SOURCE_SUCCESS,
    source_paired_path: Path = SOURCE_PAIRED,
    ...
    source_success = _load(source_success_path)
    source_paired = _load(source_paired_path)
    ...
    _require(
        source_success["status"]
        == "terminal_succeeded_exact_once_nine_artifacts_"
        "success_only_paired_assessment_eligible",
        "immutable_R2_success_record_status_mismatch",
    )
    _require(
        source_paired["four_layer_assessment"]["L1_hard_integrity"][
            "status"
        ]
        == "fail",
        "immutable_R2_paired_L1_failure_missing",
    )
    _require(
        source_admission.research_lead_transport_ref
        == S3_OWNER_GRADE_RESEARCH_LEAD_TRANSPORT_V7_REF,
        "source_R2_lead_transport_is_not_v7",
    )
    _require(
        source_admission.transport_ref
        == S3_OWNER_GRADE_SEGMENTED_SPECIALIST_TRANSPORT_V7_REF,
        "source_R2_specialist_transport_is_not_v7",
    )
    _require(
        source_admission.case_numeric_authority_policy_ref is None
        and source_admission.case_delivery_identity_policy_ref is None,
        "immutable_R2_safety_pair_history_changed",
    )
    ...
    _require(
        lead_contract.case_material_truth_identity_safety_composable,
        "lead_v7_safety_capability_missing",
    )
    _require(
        not lead_contract.gap_atom_deterministic_projection,
        "lead_v7_incorrectly_inherited_gap_atom_projection",
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin01-s4-t06-mu-safety-fresh-proof-"
    ) as temp_dir:
    ...
    _require(
        ...
        == S4_CASE_DELIVERY_IDENTITY_POLICY_REF,
        "delivery_identity_policy_not_bound",
    )
    callback_calls = 0
    ...
    return {
        "schema_version": (
            ...
            "safety_closure_fresh_agent_proof_decision_v1_0"
        ),
        "decision_id": (
            "S4-T06-MU-CASE-RUNTIME-MANDATORY-MATERIAL-TRUTH-AND-"
            "IDENTITY-SAFETY-CLOSURE-FRESH-AGENT-PROOF-DECISION"
        ),
        "recorded_at": "2026-07-29T23:45:00+08:00",
        "status": (
            "pass_zero_call_independent_fresh_proof_contract_frozen_"
            "admission_issuance_authorized_by_user"
        ),
        "authority": {
        ...
        "source_refs": {
            ...
            "immutable_consumed_R2_admission": _display_path(
                source_admission_path
            ),
            "immutable_R2_success": _display_path(source_success_path),
            "immutable_R2_paired_L1_failure": _display_path(
                source_paired_path
            ),
        },
        "implementation_reaudit": {
            ...
            "lead_v7_safety_composable": True,
            "lead_v7_gap_atom_inherited": False,
        },
        ...
        "stop_rule": {
            "second_runtime_repair_bundle_allowed": False,
            "first_new_L1_failure_stops_sequence": True,
            "automatic_R4_or_micro_patch_allowed": False,
        },
...
def build_decision(
    *,
    ...
    kwargs = {
        ...
        "source_success_path": source_success_path,
        "source_paired_path": source_paired_path,
