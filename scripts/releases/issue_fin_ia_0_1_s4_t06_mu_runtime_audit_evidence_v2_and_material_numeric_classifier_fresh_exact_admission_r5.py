...
EXPECTED_AUTHORITY_SHA256 = (
    "173db0012268c82773dffe27fe319ac5dcbef5ec52e21de0993a25d35ab76e17"
)
EXPECTED_PROOF_SHA256 = (
    "caf2f237e58b106753510996a20c5a18a7812436008fd7be4f3fb17315733d11"
)
EXPECTED_IMPLEMENTATION_SHA256 = (
    "3cd1d82013ef521789657764845e609bd5992e08286cf6fe73099bdd9ee2f6ee"
)
...
class S4T06MuR5AdmissionIssuanceError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T06MuR5AdmissionIssuanceError(code)
...
def render_issuance() -> tuple[dict[str, Any], dict[str, Any]]:
    _require(not ADMISSION.exists(), "s4_t06_mu_R5_admission_already_exists")
    _require(not ISSUANCE.exists(), "s4_t06_mu_R5_issuance_already_exists")
    _require(
        _sha256(AUTHORITY) == EXPECTED_AUTHORITY_SHA256,
        "s4_t06_mu_R5_authority_byte_drift",
    )
    _require(
        _sha256(PROOF_DECISION) == EXPECTED_PROOF_SHA256,
        "s4_t06_mu_R5_proof_byte_drift",
    )
    _require(
        _sha256(IMPLEMENTATION) == EXPECTED_IMPLEMENTATION_SHA256,
        "s4_t06_mu_R5_implementation_byte_drift",
    )
    _require(
        _sha256(R4_ADMISSION) == EXPECTED_R4_ADMISSION_SHA256
        and _sha256(R4_FAILURE) == EXPECTED_R4_FAILURE_SHA256,
        "s4_t06_mu_R5_immutable_R4_history_drift",
    )
    ...
    _require(
        ...
        and authority["authority"][
            "future_exact_R5_admission_issuance_authorized"
        ]
        is True
        and authority["authority"]["admission_consumption_authorized"]
        is False
        and authority["authority"]["R5_exact_live_execution_authorized"]
        is False
        and authority["next_action_authorized"] is True,
        "s4_t06_mu_R5_issuance_not_authorized",
    )

    frozen_proof = _load(PROOF_DECISION)
    _require(
        build_decision() == frozen_proof,
        "s4_t06_mu_R5_frozen_proof_regeneration_mismatch",
    )
    prospective = frozen_proof["prospective_R5_admission"]
    ...
    _require(
        digest
        == prospective["digest"]
        == authority["frozen_R5_admission"]["admission_digest"]
        == EXPECTED_ADMISSION_DIGEST,
        "s4_t06_mu_R5_admission_digest_mismatch",
    )
    _require(
        payload == admission.digest_payload(),
        "s4_t06_mu_R5_persisted_payload_not_digest_payload",
    )

    frozen = authority["frozen_R5_admission"]
    ...
    _require(
        ...
        "s4_t06_mu_R5_admission_binding_mismatch",
    )
    ...
    def _must_not_call_provider(**_: Any) -> dict[str, Any]:
        ...
        provider_calls += 1
        raise AssertionError("provider_forbidden_during_R5_admission_issuance")

    build_s3_three_cell_bounded_agent_executor_for_admission(
        admission,
        chat_completion_fn=_must_not_call_provider,
    )
    _require(provider_calls == 0, "s4_t06_mu_R5_provider_called")
    ...
    _require(
        ...
        "s4_t06_mu_R5_identity_not_fresh",
    )
    ...
    issuance = {
        "schema_version": (
            "fin_ia_0_1_s4_t06_mu_runtime_audit_evidence_v2_and_"
            "material_numeric_classifier_fresh_exact_admission_r5_"
            "issuance_v1_0"
        ),
        "issuance_id": (
            "S4-T06-MU-RUNTIME-AUDIT-EVIDENCE-V2-AND-MATERIAL-"
            "NUMERIC-CLASSIFIER-FRESH-EXACT-ADMISSION-R5-ISSUANCE"
        ),
        "issued_at": "2026-07-30T10:30:00+08:00",
        "status": EXPECTED_ISSUANCE_STATUS,
        "authority": {
            "user_instruction": "继续",
            "fresh_exact_R5_admission_issuance_authorized": True,
            ...
            "automatic_retry_fallback_replay_relaunch_R6_or_rerun_authorized": (
                False
            ),
        },
        ...
        "execution_envelope": {
            ...
            "automatic_retry_fallback_replay_relaunch_R6_or_rerun_allowed": (
                False
            ),
            "first_new_L1_failure": "terminal_fail_closed_stop_without_R6",
        },
        "proof_reverification": {
            ...
            "implementation_runtime_code_bindings_match": 4,
            "admission_schema_and_profile_admissible": True,
            "admission_roundtrip_digest_match": True,
            "R4_admission_and_failure_immutable": True,
    ...
    _require(
        ...
        == snapshot_before,
        "s4_t06_mu_R5_issuance_changed_target_runtime",
    )
...
def _write_and_validate(
    ...
    try:
        ...
        _require(
            ...
            == EXPECTED_ADMISSION_DIGEST,
            "s4_t06_mu_R5_runner_load_payload_mismatch",
        )
...
def verify_issued_admission() -> dict[str, Any]:
    ...
    _require(
        ...
        "s4_t06_mu_R5_issued_payload_not_frozen_payload",
    )
    admission = S3ThreeCellBoundedAgentAdmission.model_validate(payload)
    admission.assert_profile_admissible()
    digest = canonical_digest(admission.digest_payload())
    _require(
        ...
        "s4_t06_mu_R5_issued_admission_digest_mismatch",
    )
    ...
    return {
        ...
        "admission_digest": digest,
        "fresh_identity_rows": fresh_counts,
        "capture_v2_numeric_classifier_v2_identity_v2_bound": True,
