...
当前执行入口：§0与S3/190文末最新状态；2026-09-06 21:50Z，本次全链交付候选为v3，7,281字符/42引用/3图，真实四格式已下载并渲染。run `01a0787d-961c-7f21-a590-53887cfe2c75`于21:40:15Z成功停止，session为needs_revision：仍1条material指向P02需求底稿的“非泡沫/相当部分”未与已修正文稿同步。工作包1–4及上传/视觉/图表/导出/公开EN-CN准备已落地；第5包真实运行已发生，但最终质量门和Owner验收未完成。6run共265请求/264已知/17,060,539tokens/估28.092715元，原Q4未知另列，停止新增paid；不重跑九主题、不增案例。源码至`19e57b4a`（含58da9533轮询和PPT显示修正）；API18165/原PG-Redis不变，BFF27908/session97515、8766。交付文件在`D:/temp/finsight-dell-final-20260907-a1`，PPT用formatted版本。下一只应同步P02相关主张并定向复核；先确认额外费用与有界路径，不默认再付费重写整稿。公开发布未授权。以下逐日期记录均历史，不能据旧“未部署/0模型”重启整案。
...
2026-09-06 18:20Z实时进展：新入口已实际发起完整Dell研究（thread `01a077d8-a47c-7280-98f5-3df94b219488`，run `01a077d8-a486-7150-b14c-646b884f914f`），非固定两专家、非旧稿改写；当前仍运行，最近可核58调用约5.4221元，Q4一次连接失败保留未知用量，已交稿不重跑。真实MCP Flash vision单次423tokens/2.801秒，第二次同对象缓存，无测试图混入研究。上传/图表/四格式导出/运行意见及公开中英文说明已落地，视觉与费用跟随真实验收；实时任务事件刷新丢历史的本地显示修正不改变正在运行的模型图。详细真实ID、费用、失败、未完成门见S3/190最新节。本段取代下方早期“0新DS/未部署”的当前性；报告和Owner验收仍未通过，不新增运行协议或产品版本。

2026-09-06 17:38Z实施检查点：五包及新增功能仍同一批准范围。任务上传/成熟解析/按需Flash vision、图表与四格式导出已编码，新研究BFF及固定容器已实际部署到可见入口；最终源码增量/浏览器及文档视觉/vision工具/完整新Dell验收继续进行，不宣称产品PASS。运行中意见沿原生metadata在阶段交接读取，不中断正在生成的单次回复；原生未完成研究确认不接受报告或自动重试。最新57近邻通过，0新DS。完整费用重算12–20元，新增综合/复核不再遗漏，约20元授权下监控实际成本。详细增量、兼容问题与下一动作见S3/190当前进行中检查点；不新增框架/阶段协议。

### 2026-09-06 09:50Z 当前执行计划：新题目、方法与费用纠偏 → 完整 Dell 产品纵切

本节是本次 Owner 要求的合并实施计划，取代本节上一版“先付费修好旧报告、再连接新入口”的顺序；本文件后续按日期记录的状态只保留历史。仍属于 FIN 0.1.3 当前 Dell 纵切，不重开 Phase 0–7，不恢复 R14，不创建产品版本或执行协议。计划可以随实测修正，但变更须说明新证据与影响，不能边跑边降低“完整研究”的定义。

本次只落计划。起始分支 `codex/fin013-dell-s1-s2-product-bridge`、HEAD `82c88378be510656daf7cde9c94beeb447e223ea`；继承上轮五份已说明的文档/Project OS 改动。没有新 DeepSeek 调用、运行代码或方法配置修改、原报告覆盖、SQL 写入或部署变更。

#### A. 目标、真实起点与范围

2026-09-06 Owner 已批准工作包 1–5 开始实施，并增加资料按需扩展、用户上传文件/图片在线解析进入任务 RAG、Flash 视觉工具、图表及 MD/PDF/Word/PPT 导出、对外仓库与中英文文档整理。第 6 包新案例仍为后续候选，不把本次审批扩大到自动执行。执行顺序收敛为：①方法/新题目与 context 共用修正；②必要数据/工具接缝；③原生完整父图；④真实前端任务/过程；⑤受控上传/解析/视觉工具；⑥来源绑定图表和多格式导出；⑦原工作包 5 的新题目完整 Dell 实测/内容与交互验收；随后按实证整理公开展示面。各项本地近邻验证后必要时少量 paid 工具测试，最后一次完整研究覆盖已接入能力，不重新大范围规划。

新增边界：Owner 允许为新题目补充本地资料、RAG 和外源通道，不将旧上下文实体名单或旧搜索次数当研究权限上限；保留身份/as-of/来源正确性。Agent 只接触任务级对象 ID 和受控服务，不给任意宿主路径、shell、用户文件写入、凭据或系统提权。上传服务仅处理用户明确提交的副本，保存在隔离任务存储，默认不进入共享知识库/公开 Git；类型/大小/解压资源限制、跨会话隔离、来源中的指令不执行，均须本地测试。公开准备不等于已获公开发布/改仓库可见性授权；先明确 code/docs/fixtures/demo 可公开范围，排除 secrets、上传资料、原始私有模型记录及大语料。

视觉采用官网当前独立模型 `deepseek-v4-flash-vision-exp`（实验版），不是普通 Flash 别名；作为 Pro 的只读图片理解工具，保留原图/页码定位，提取结果不自动晋升为 SQL 财务权威。先亲测再开放给研究 Agent。官方依据：https://api-docs.deepseek.com/zh-cn/guides/vision/ 。图表使用成熟渲染/导出库，数据关联已观察来源/计算，不能允许模型执行任意代码或伪造图中数字。公开命名按职责提取通用模块、保留 Dell 示例/回归/兼容入口，不一次性全仓改名。新增上传/视觉/导出测试成本与原完整文本研究预估分别记录。

**交付目标：用户从前端提出一个真正的 Dell 研究问题，系统自主拆解、研究、补证、交叉质疑、必要修订，交付有判断、有依据、能追问的报告，并能展示全过程、实际成本和故障。**

下一次真实运行的问题为：

> 截至本案例约定的研究时点，评估 Dell AI 基础设施业务所推动的增长质量，以及这种增长向利润和现金流兑现的程度。结合客户需求、竞争格局、产品架构与供应条件，分析未来几个季度完成公司经营目标所面临的执行压力。给出明确但有条件的核心判断、关键依据、最强反证，以及哪些后续变化会使你调整判断。必要时自主补查已开放的内外源资料。区分已披露事实、计算结果、研究推断和情景假设。

“有判断”不等于预设看多或看空；“现有证据”不禁止已开放的外源搜索、同行和产业链核对。结论允许不确定，但不能用一页页边界说明代替已有资料足以支持的分析。本轮不是估值/目标价项目，也不扩展到任意公司、生产多租户或投资建议。

已具备：真实多轮 Specialist、Lead 动态任务分工和并行子图、Counter/Verifier、责任作者修订与 Writer 的分段执行；真实 S1/S2/MCP、外源读取、计算；Agent Server/PostgreSQL/Redis/LangSmith；报告 v3 及审阅/追问/定向修订界面。v3 仍为 0 material / 2 advisory 的模型审查结果，未获 Owner 产品验收。

尚缺：新问题通过前端一次启动完整链；旧角色方法的有效迁移/渐进披露；独立 convergence 终审的大引用对象去重；若干来源/计算/审查误判修正；更清楚的真实运行界面；新案例和完整请求级验收。不能把已有分段成功加总成这些能力已经完成。

旧 v3、底稿和失败继续保留，作为诊断与回归材料。此次完整新研究复用原始文档、SQL、索引和方法，不默认复用旧专家结论、旧完整答案或 Q1 seed。显式续研可以复用底稿，但必须单独标识，不能代替新研究验收；Dell 是已开发案例，不包装成盲测。

#### B. 不再变动的技术责任分工

2026-09-06 Owner 批准父图纠偏，其他工作包不变：此前“两个专家”只是两任务 scripted 接线测试，不是正式配置或 DeepSeek 自主研究证明。正式 case 为九研究面、动态任务/依赖及追加任务，当前最多十二任务、并发二；并发数不等于总专家数。补一项使用实际 case 配置的九面、多波次、依赖及追加任务测试，不以两任务测试代替。

本段实施纠偏：Counter/研究 Verifier → 必要责任作者修订 → Lead 按当前成果重新综合判断 → 独立研究复核 → Writer → 终审。终审以明确责任字段区分 Writer、研究底稿、数据/工具和人工；本地只验证 schema、目标/引文和路由，不写自然语言语义判定规则。写作问题不重跑研究；研究问题只回有关作者，再 Lead 综合/独立复核/Writer/终审；已证工具故障或无法定位的未决问题交人工，不让 Writer 掩盖。自动定向纠偏全程最多一轮（不含最初跨稿审查触发的作者响应），再次重大失败即保留当前产物/意见并交接，不反复跑到 PASS。沿用 native 子图/checkpoint，原始与每轮公开产物保留；不同 Agent 不共享私有消息。新增 Lead 综合使用独立任务预算，不沿用廉价调度预算假装复杂研究；最终 full-case 前重新核成本。上传、视觉、图表/导出、真实前端与新题目实测和公开准备全部仍在原范围内。

| 能力 | 本轮使用什么 | FIN 只补什么 |
| --- | --- | --- |
| 多轮 Agent 与任务流转 | 现有 LangChain create_agent、LangGraph 子图/Send、Agent Server | 研究任务和产物交接、责任作者定位、领域工具/输出的薄适配 |
| 会话、checkpoint、人工点 | 现有 Agent Server、PostgreSQL、Redis | 当前任务/报告版本的产品映射；不自建队列、恢复协议或会话数据库 |
| 可观测性 | LangSmith + 已有本地用量/来源记录 | 同一用户请求下的角色、阶段、费用和产物投影；不另起 tracing fallback |
| 数据与工具 | 当前已实测的本地解析/分块/检索、S2 SQL、外源工具、官方 MCP 客户端与薄适配 | 具体失败的定位修复、来源上下文/数字/计算绑定；不重建整库或重选 RAG 栈 |
| 前端与交互 | 现有 React、Markdown、BFF、原生 stream | 新任务入口、任务/依赖/回派显示、易读来源和报告、人工反馈 |
| 金融研究方法 | 选择性迁移旧角色方法，通过现有方法 MCP 按需读取 | 题目、研究方法、口径和来源约束；不迁回旧庞大 ClaimCard/Memolet 壳 |

不新增通用 Skill 执行器、自动预算审批机、记忆平台、模型路由模型或通用 NLP 语义规则集。使用当前可运行部署和版本锁；只有发现本轮真实需求无法满足时，才提出有实测依据的窄变更。

#### C. 六个工作包与实施顺序

**工作包 1：改正研究任务，并让各角色实际获得合适的方法。**

接入点：`configs/research/fin_ia_0_1_3_dell_reference_vertical_foundation_v1_0.json` 的当前任务来源、`research_foundation/contracts.py`、`research_foundation/mcp_server.py`、`dell_specialist_agentic_composition.py`，以及 Lead/review/convergence 的输入装配。

- 从现有配置派生本次新研究的问题/方法修订，保留旧冻结配置及旧会话的绑定，不就地改写历史 digest，也不把配置修订当产品新版本。研究 as-of 和数据快照由后端绑定并在 UI 显示；不把旧快照伪装成今天实时全量数据。
- 旧 20 份 Markdown 先按方法价值去重，首包只迁六组实际需要的内容：Lead 的问题拆解/依赖；财务的收入—利润—营运资本—现金；行业/产品/供给的机制与同业比较；风险/Counter 的竞争解释与改变判断条件；Writer 的综合/重要性/条件判断；Verifier 的证据上下文/计算/遗漏检查。Valuation、旧 renderer/aggregator 以及重复版本暂不引入。
- 方法只包含步骤、公式及适用条件，不含本案结论、答案数字或强制观点。移除旧版“专家不能调工具”“Writer 不能补查”“固定 cell 壳”这类与当前 agentic 方向冲突的内容。
- 初始输入只放最小职责、工具/数据目录和方法简介。模型按任务读取需要的分支/角色方法、文档目录和来源窗口；已有方法 MCP 做小范围扩展并真正接入 Specialist。不能把目前仍 unavailable 的通用 disclosure 标成已启用，也不必为了启用它另造通用平台。
- 方法进入模型的实际输入/工具返回、是否据此核算或改变分析，将在同一次完整 Dell 的 trace/底稿中检查；“文件存在”或“调用了方法工具”单独都不算质量通过。

交付：新题目配置/方法资源及现有 MCP 接入；旧新方法的简短 retain/adapt/hold 对照。先用本地输入装配与 MCP 测试验证，不为每个 Skill 另外买一次模型资格运行。

**工作包 2：补齐具体研究能力缺口，同时修掉已知上下文浪费。**

接入点：现有来源读取/数字计算工具，`dell_case_convergence_agent.py` 的 Writer/Verifier seed 与引用解析，复用 `dell_report_session.py`、`dell_case_review_agent.py` 已有轻量上下文和工具路径。

- 用原文复核少数已定位问题：全年指引与已实现业绩未转成执行要求；收入与现金流缺少可比期分析；HPE 同期间一手来源可替换二手引用；Q1 AI 收入基数已有直接披露却被错误降为“只能倒算”；旧工具不足被沿用为公开信息 gap。保留旧错误与原报告，修正新输入/责任材料，不篡改历史结果。
- 对新外源或有故障的来源，先用将交给模型的实际工具取得正文/页码窗口，确认标题、期间、关键表格/数字可读；再交模型。复用已经亲测的外源通道，不逐站新建爬虫。新闻/帖子等可以使用，但在结论中说明来源性质和影响，不伪装成权威统计。
- SQL 已覆盖且适用的标准财务事实优先从 S2 取。S2 未覆盖的发行人披露/外部原文可由模型使用并计算，记录原文、定位、输入、公式、期间及权威性；既不冒充 SQL NumericFact，也不因“不是 SQL”一概禁止分析。缺销量/ASP 时不强算无法识别的量价分解。
- 将已在短问答验证的新观察 NUMFACT/CALC 引用路径薄接到报告和需要它的研究角色，不再要求为引用一个新 SQL 结果先补旧归档底稿。未实际观察、跨任务、错公司/期间/单位的引用继续拒绝，返回可操作错误。
- convergence 终审初始上下文改为问题、报告正文、底稿/来源目录、实际修订摘要；完整 citations 元数据仍保存在产物/前端按需可读，不整包重复注入模型。来源窗口和相关底稿通过现有工具读取，不通过裁掉所需证据来降费。
- 每位 Agent 保留自己的原生多轮状态；跨 Agent 传任务、底稿、来源引用和简明依据，不复制彼此完整对话/私有 reasoning。续问只加载所需材料，局部修订用现有 edits/diff 和相关审查。暂不加入新摘要模型；若真实运行仍有上下文压力，再评估框架原生压缩，不先建记忆系统。
- 审查同时检查错误、过强归因和被遗漏的关键分析。作者可以用原文/核算反驳 Counter 或 Verifier，不能默认审查意见正确。自然语言正文不再靠固定模板或边界词计数验收。

交付：工具/来源/引用和 context 接缝修复、相邻回归、实际输入投影对比。一般补丁只测受影响路径；这是省去重复输入，不预先宣称等质节省百分比。旧报告可以做零模型读取/计算回归，不以付费润色旧稿作为工作包 3 的门。

工作包2当前切片状态（2026-09-06 12:58Z）：

- [x] convergence轻量输入及SQL/CALC直接引文；本轮补新原文PASSAGE直接引文与实际嵌套SQL观察投影，未观察/错误工具/检索预览仍不可引用。
- [x] Specialist使用同一MCP来源计算器、原生ToolNode派发，计算主张需实际CALC结果；不另起计算器、调度或来源数据库。标准S2派生值仍沿旧规则，普通计算不晋升S2。
- [x] CALC和原始输入随本次底稿进入Lead/审查/写作的只读视图；保留期间、单位、引句、locator与原始引用ID，别名只用于展示导航。
- [x] 取消专家SQL层DELL硬编码，使用同一typed ticker规则；实际NVDA查数成功、HPE当前库缺数明确保留，不能把可查询误称全公司已入库。
- [x] 宿主通过模型将用的源工具看HPE财报第8页和Dell最新Exhibit现金流表；HPE同一表格两项原文数值计算得到披露的一位小数，仍非S2。少数特殊符号乱码不等于已全部解析无误，未自动改写冻结资料。
- [ ] 新题目真实研究中验证指引执行要求、现金转化分析、同业期间比较、关键原文/脚注和模型方法应用。必要时补具体SQL公司/资料，不以本轮接线测试替代研究验收。

**工作包 3：把已存在的 Agent 接成同一个真实研究任务。**

接入点：`dell_lead_research_graph.py` → `dell_case_review_agent.py` → `dell_case_convergence_agent.py` → `dell_report_session.py` 及现有 Agent Server graph 注册/配置。

流程为：用户问题 → Lead 动态计划 → 多轮 Specialists（可并行、有依赖）→ Counter/Verifier 跨稿审查 → 必要的责任作者修订 → Writer → 报告终审 → 人工点/交付与追问。

- Lead 根据题目和已有结果建立/调整任务，而非固定“一位 Lead + 两位专家”或每主题必配一个 Agent。初始并发沿用已证本地配置 2；这是资源并发，不是总专家数限制。
- Specialist、Counter、Verifier、Writer 继续使用各自 agentic 多轮循环和职责内工具；图只管理阶段、依赖、持久化和交接，不替模型写研究结论。
- 当次底稿、来源目录、审查发现和作者修订动态进入下游，不读静态旧十稿 bundle；Writer 不再硬编码必须读取十份稿。回派由实际发现决定，支持零作者、单作者、多作者，完成的无关稿不重跑。
- 已知本地工具/schema 问题给责任模型清楚反馈；需改宿主时先本地修后定向验证。失败和已完成子产物均保留，不把失败改成成功或在统计中藏掉。网络/代理检查优先用不付费连通性诊断；远端是否执行不明的请求不能盲目重复付费。
- 不把旧单次资格 runner 的一次性 Compose/端口/volume/authority 搬进交互产品。使用当前固定部署和原生 thread/run；新任务与续研区分，必要的续跑必须明确其复用，不冒充 fresh 全链。
- 本地覆盖真实接缝：动态产物、零/单/多作者、工具非法数据反馈、子任务失败保全、引用隔离、旧审阅会话兼容、未审新稿不能跳过终审。用原生组件/测试替身验证，不能将测试替身计为 DeepSeek 成功。

交付：从新问题到报告/人工点的可调用完整后端，先零模型和近邻集成验证；不额外建一套“证明可以执行”的状态机。

**工作包 4：让前端真正发起任务，并能看懂、介入和继续使用。**

接入点：`apps/workbench/backend/api/v1/report_sessions.py`、`apps/workbench/frontend/vite/src/app/ResearchSession.tsx` 和现有来源/报告组件。与工作包 3 配合实现；没有真实后端状态时不能用假进度顶替。

- 入口明确分为“新建研究”“打开已有研究”；已有任务内再有快速问答、深入追问、定向修订。用户输入问题；后端决定已授权工具、数据范围、模型配置和 as-of，不接收任意文件路径/模型权限覆盖。
- 展示谁在做什么、等待谁、查了什么、为什么回派、改了哪里；用实际任务/子图事件构成列表和依赖关系，不编造百分比或静态多 Agent 动画。
- 模型私有 reasoning 可按现有授权审计路径保留；普通前端显示行动、简短判断依据、来源和审查意见，不原样公开或跨 Agent 传递私有思维链。显示依据不代表已经语义正确，最终仍须内容复核。
- 报告正文与审查建议分层阅读，必要处用比较表帮助理解而非强制表格配额。来源卡片默认给机构、标题、日期、页码/段落、相关原文及上下文；数字可展开原值/公式/使用限制，原始 JSON 放调试折叠层。
- 沿用已证停止、原生 checkpoint 和人工点反馈。运行中提交意见可在安全交接点消费，并明确是否已应用；不冒称随时打断模型当前推理且无损续接。审阅中可追问、修订、比较版本，取消不覆盖已接受报告。
- 显示当前请求和会话累计的调用、input/output/cache、费用估算、耗时、错误。未知 usage 标未知；取消不等于远端免费/立即停费。
- 沿用固定 Compose/PostgreSQL volume/Redis/LangSmith；部署前后保留旧会话和版本，记录当前镜像/配置及回滚方式。不删旧数据，不做多租户/HA/Kubernetes 扩张。

交付：能启动真实研究的 UI、真实过程和可读报告/来源。先前端 build、BFF/事件适配测试、浏览器零模型检查；不要为 CSS、文案或普通组件修补跑全仓/整案模型。

**工作包 5：从前端执行一次修正后的完整 Dell 验收。**

工作包 1–4 就绪后，先按现有 Project OS 与 provider/data/budget preflight 做一次与实际运行风险相称的检查，不为小补丁另造哈希/审批体系。使用新任务/执行身份，明确研究时点、可用内外源、实际配置与预算。第一次付费完整验收就用新题目，不再先重跑旧方向报告。

- 真实检验自主拆解、同行/产业链/时效资料补查、SQL/原文计算、并行多轮研究、反证与责任修订、报告和人工交互。记录实际角色和任务数，不要求为了好看人为制造回派。
- 研究正文应回答增长质量、盈利/现金兑现、经营目标执行压力；包含支持依据、机制、最强反证与改变判断的信号。对已存在且重要的数据做有意义核算；资料不足时明确哪项推断受影响，而非让整篇只剩边界。
- 重点人工检查约 15–20 条重要主张并检查全部影响核心判断的关键计算：来源是否真支持、公司/期间/单位/口径是否正确、事实/计算/推断/情景是否区分、同行是否可比。该抽查是开发验收，不代表随机统计样本或普遍准确率。
- 模型审查只是一个环节；最终交用户看正文和最重要未决项。重大未解释错误不能称通过，小建议不必全部消灭，也不以 100% recall/零 advisory 为交付条件。
- 首次完整运行与必要责任修订合并在一个明确任务下统计。先完成一轮合并反馈和局部修订；如仍有重大错误，定位责任层并说明证据，不自动整篇重写/全案重跑直到 PASS。若确需再一次全链，须有影响完整接线/研究目标的明确原因并单列费用。
- 产出同一次任务的报告、运行图/事件、LangSmith、底稿与来源、每角色/阶段用量费用耗时、失败/修订及人工验收结论。端到端未完成就明确 failed/needs-review；已有分段成功不能相加冒充本次完成。

**工作包 6：增加少量新场景，并完成可展示、可讲解的交付。**

Dell 内容与产品链达到上述开发验收后才开始，先新短任务，再有界长任务；不同时开多个研究面。

- 新短任务选非 Dell 的现有公司数据，覆盖直接事实、计算、跨来源问答及不可回答场景；先核可用数据/工具再定题。组合约 8–12 个不同问题，报告每题实际质量与成本，失败保留，不重复同一 SQL 问法刷数量。
- 新长任务优先从已有 HPE/Micron 等资料中选一个问题范围明确、只需少量补源的研究题；选择依据是数据覆盖和产品价值，不是为了多写一个公司名。仍走同一父图和方法接口，不写第二套 case runtime。
- 复用约 4–6 类代表性故障/安全检查：错公司/期间、伪造引用、工具拒绝、来源失败、主动停止/人工点恢复、越权请求。已有无关通过项不为凑指标全部重跑；仅标明当前版本真正验证的范围。
- 汇总长研究、短问答、续问/修订的任务级结果、费用、调用数、耗时和人工介入。样本少时列原始值/范围，n=1 不报 P95，不把模型请求成功率说成用户任务成功率。
- 输出实际架构图、可复现启动/操作说明、一个短演示和两个真实失败—定位—修复案例，突出动态分工、MCP、SQL/RAG、上下文管理、HITL、LangSmith及成本路由，而不是堆关键词。
- 秋招任务的只读咨询已完成（`秋招投递计划`，`01a003d0-b798-7b52-b27b-a9b3b062d058`）：建议优先完整交付、可信小规模指标和本人讲解；当前 MCP/LangGraph 已实用，旧“待接入”记录已过时。交付实际证据后再协调简历描述，不提前代写成功指标/改投递。本人须能讲清一次任务流转、一次故障及至少一处 adapter/测试，不能只有工具生成的演示。
- 不为 JD 额外加消息中间件、Kubernetes、模型训练或宣称生产大规模；未经对照不声称多 Agent 必然优于单 Agent，也不为了这一说法再开庞大消融项目。

#### D. 费用与上下文：这次具体怎么控制

截至 09:26Z 审计，截图北京时间 9 月 6 日账单 26.54 元；本地 243 请求/240 有 usage、11,332,214 tokens，按已核官网价格估 26.0417 元，差额约 0.4983 元待逐笔对齐。它含多次失败、研究、写作修订和 UI 测试，不是一次完整任务。审查/作者修订/写作改稿约 11.59 元，但不能把这部分全称浪费。

历史可比较的工作量锚点：一次研究 A2 约 6.63 元（父图失败、保留七份新稿与 Q1 seed，不是完整案例成功）；Q8 定向补研究约 1.98 元；全案跨稿审查约 1.92 元；一次 P07/Writer/Verifier 约 1.98 元；真实短问答约 0.0198 元是单题样本。不同输入/缓存/修订轮数不能直接外推保证。

- 下一次完整新 Dell 暂按 **约 10–18 元量级** 作运行前费用预估，不是承诺价格、已批准的新无限预算或删减研究的硬上限。新题目更深入、不能复用旧结论、缓存及定价时段会改变成本；启动前根据实际配置/输入重算，明显偏离此量级先说明。接近预估上沿仍有大量核心工作未完成时，在安全点报告原因和剩余工作，不暗中无限扩大。
- 初始用途路由：Lead 调度/简单短问答沿用已实证 Flash disabled；需要复杂分析的研究者、综合写作和跨稿审查先沿用现有 Pro/low 等任务配置。Flash 大上下文审查曾截断，不能全改 Flash；thinking 能否关闭按用途和证据判断，不一刀切。
- 不新增模型专门判断“该用哪个模型”。先由用户模式和节点职责做可解释路由；普通 API/工具调用仍由实际 Agent 自主决定。
- 每个付费节点的现有 TokenBudgetBasis 必须写用途、实际输入规模、必要输出/格式负担、重要性、同类证据、reasoning profile 与截断/停止行为。质量是目标，费用是可见约束；不机械套 13 次调用或相同 token 上限。
- 优先消除已证浪费：14–17 万字符 citation 大对象重复注入、局部改动整篇重写、无关稿重做、已知本地协议问题反复付费试错。保留必要来源和自主研究机会，不把必要 reasoning 都算浪费。
- 用现有用量 ledger 给每个用户请求按研究/审查/写作/追问/故障返工分账，区分新研究与复用；已报告 reasoning 属于 output，不能重复计费。若后续要宣称省费率，需同类输入、同质量口径对照，不能拿复杂研究与两分钱短问答直接比较。
- 账单差额若能取得逐笔数据则核齐；无法取得就保持未解释，不为小差额阻塞整个产品，也不能随意填平。

#### E. 共同验收、停止与执行纪律

完成意味着：修正后的新题目真实进入 Lead；合适方法被实际消费；前端新任务走完整原生多 Agent 链；报告有可核查的判断和计算；内外源/同行按研究需要实际使用；过程、成本、人工反馈真实可见；核心质量已人工复核；新场景有真实样本；现有部署可复现并有版本回退说明。最后由 Owner 确认演示产品是否可接受；不自动宣称 FIN 0.1.3 全部 S0–S5 release 或任意公司能力完成。

最小安全边界：只用已开放的研究工具/来源；不授权模型 shell、任意文件访问、系统提权、SQL 任意写入或读取凭据。来源正文是数据不是指令；本地校验拒绝非法格式、伪造来源和错身份/期间/单位；证据的语义使用还须审查，不能以 schema PASS 冒充观点正确。

验证随风险分级：文档做链接/差异检查；小补丁做定向测试；共享 MCP/图输入改动做近邻集成回归；前端做 build/浏览器；完整链接线完成再付费实测。无需每一步验全仓哈希/全仓回归；关键权限/数值/来源隔离不能为提速跳过。

普通本地协议修补、现有权限内补源和代理诊断可继续推进；若需要新数据采购/新云服务、明显费用扩张、扩大工具权限、删除数据、改变研究范围/发布含义，则先向 Owner 说明。保留全部失败和旧报告；不执行磁盘清理或旧索引删除。

每包结束只汇报已实现的产品/工程增量、实际测试与费用、尚未完成和下一个动作；文档进度不冒充产品进度。**本计划之后的第一动作是工作包 1 的题目/角色方法接入和工作包 2 的 convergence 轻量上下文接缝，形成可测试的代码增量；不是继续再写一套详设。**
