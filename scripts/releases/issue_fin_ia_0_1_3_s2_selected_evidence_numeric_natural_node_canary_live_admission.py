...
def _require_clean_synced() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("live_canary_issuance_requires_clean_worktree")
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise RuntimeError("live_canary_issuance_requires_synced_head")
    return head
...
def _load(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise RuntimeError("live_canary_issuance_json_invalid")
    return value


def main() -> int:
    if OUTPUT_PATH.exists():
        raise RuntimeError("live_canary_issuance_already_exists")
    ...
    if preflight.get("status") != "pass":
        raise RuntimeError(
            "live_canary_issuance_project_os_blocked:"
    ...
    if credential["credential_present"] is not True:
        raise RuntimeError("live_canary_issuance_credential_missing")
    source_bindings = [
        {
            "ref": ref,
    ...
    issuance = issue_live_canary_admission(
        ...
        user_authority=(
            "User instructed continue after being told that the next bounded step "
            "implements the isolated live path and issues one unconsumed admission. "
            "This issuance does not authorize or start the Provider call."
        ),
    )
    validate_live_canary_issuance(
        issuance,
