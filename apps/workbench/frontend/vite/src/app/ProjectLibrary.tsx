...
export function ProjectLibrary({project, sessions, navigate}:{project:{id:string;name:string};sessions:Session[];navigate:(view:string,id?:string)=>void}) {
  ...
  return <section className="fs-page fs-project-library"><span className="fs-kicker">项目资料</span><h1>{project.name}</h1>
    ...
    <div className="fs-project-documents">{items.map(item=><article key={item.document_id}><h2>{item.name}</h2><small>{item.text_status==='searchable'?'正文可查找':'含需识别的图片或扫描页'} · 用户提供，待核验</small><p>{item.excerpt}</p>
    ...
    <h2>项目内的研究与成果</h2><p>研究任务及其中的报告保持原有权限和版本。项目资料尚未自动接入 Agent 取证。</p>
