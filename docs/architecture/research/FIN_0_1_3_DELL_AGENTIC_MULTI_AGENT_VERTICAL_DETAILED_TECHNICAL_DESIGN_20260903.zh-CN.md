...
文档状态：`IMPLEMENTED_REVIEW_CANDIDATE / FIN_0_1_3 / DELL_REPORT_V4 / OWNER_REVIEW_PENDING / HERMES_NOT_EVALUATED`

当前执行入口（2026-09-08，Owner 审阅前收口）：原五项与新增能力已进入可操作交付候选，Dell v4、真实上传/问答、四格式、少量新场景和正式双语文档已有证据。逐项完成/限制以 [Owner 审阅清单](../../product/FIN_0_1_3_OWNER_REVIEW_20260908.zh-CN.md) 为准。PR #3 完成最终 CI、main 合并与远端分支保全清理后停止，不评估 Hermes。下面按时间保存的失败和旧计划不覆盖本条。Owner 内容/产品接受仍待完成。

历史执行入口（2026-09-07 历史引用接缝修复）：本文件 [原五项＋新增需求剩余工作](#remaining-work-20260907) 的步骤一与 S3/190 文末。代码 `c8da5a2c` 复用现有 LangChain ToolMessage.artifact/LangGraph checkpoint，让完整历史引用的读取与提交共享绑定；旧完整 JSON 引用窗口和内含 NUMFACT 仅恢复引用能力，不伪造可执行 CALC 或晋升来源权限。明确 false＝未验证、缺字段＝未记录，引用复用不要求新 ID/强制重算。112 项定向检查通过；保存的首次被拒真实答案原文不改，在旧版复现 9 个引用拒绝，修复版四回读＋一次提交绑定 4 CALC/6 NUMFACT，0 新模型调用；证据 `Z:/FIN_Insight_Agent_qualification/dell_reference_vertical/20260907_citation_binding_offline_a3/receipt.json`。上轮真实 4 调用/263,171 tokens/估 1.710333 元的内容失败仍保留，修后模型自主表达尚未实测，不能称同质量通过。摘要仍 disabled、v3 needs_revision，不进入步骤二/P02/前端；原费用/历史 LangSmith 190,815 缺额不变。Owner 最新要求“出现大量手写规则先找成熟栈”已记录，本轮采用已安装组件，不新增 harness、记忆服务或自然语言判断器。以下逐时点状态仅供追溯。
...
<a id="remaining-work-20260907"></a>
...
#### 2026-09-08 Owner 顺序调整：原五项＋新增需求 → GitHub 展示与投递 → Hermes

Owner 明确担心 Hermes 接入分散交付，决定先完成既有功能和 GitHub 展示，再投简历。采纳此顺序；原要求映射与下方七段编号保留，Hermes 不加入本轮产品依赖，不重开摘要资格或整体 runtime 选型。

Owner 随后明确批准连续实施，并指定 **Hermes 评估前停下供其逐项审阅**。GitHub 最终收口同时统一正式 README、活动文档、产品版本号及迭代详情；保全有效提交后清理远端 `codex/*`，仅保留 `main` 与有实际版本意义的历史分支。公开仓库不能单独隐藏某个分支，故通过合并、归档与安全清理引用实现展示要求，不提前删除当前交付分支，不因文档或报告修订自动升级产品版本。

| 执行顺序 | 复用原清单 | 完成时可检查的结果 |
| --- | --- | --- |
| 报告与研究正确性收口 | 二，及五的来源/计算问题 | P02责任底稿、综合与正文一致；重要主张/全部关键计算核查；公式、操作数和出处可读 |
| 完成交互与新增输入能力 | 三、四 | 问答/修订分流、真实版本差异/原因、意见状态与费用；真实文件/图片经过上传、检索、模型消费和引用 |
| 统一产品候选与展示证据 | 五、六、七的既定小范围案例/讲解 | 同一版本的报告/图表/四格式、可操作演示、独立启动说明、真实成本/质量记录和少量新场景；原新增需求不删除 |
| 投递可用的 GitHub 展示收齐 | 六的默认首页/双语导航/公开内容核查，可提前做 | 默认入口指向实际版本，产品介绍、架构、操作、验证、局限和演示可找到；代码/资料条件清楚 |
| 本轮交付后再评估 Hermes | 既有外部候选记录 | 仅在有具体质量/费用需求时验证一个节点；不承诺一定迁移、不阻塞投递 |

“完成”仍按原清单的功能、内容和产品验收判断。投递准备不要求大规模架构迁移、企业HA、无限新案例或翻译全部历史工作日志；不得借招聘展示降低引用、计算、重大判断或真实上传消费要求。普通实现/本地测试连续推进，具体 paid 切片保留任务 TokenBudgetBasis 与有界费用依据，不复用已经结束的付费批次。

2026-09-08 GitHub 实查更正：仓库已经 public，默认 main 仍展示历史固定 Pack，并把当前开发分支已发生的动态研究写为未来能力。展示收齐必须处理默认入口和分支含义，不能只修改开发分支 README；当前公开事实也不等于已审核完整 Git 历史/所有资料或已发布最终报告。先做有界首页说明，不把数百提交整体合入 main 当作文档清理。

首页先行切片已完成：PR #2经修正两处旧路由测试兼容后，CI编译/测试/浏览器步骤通过并合入main（114a935f）；默认中英文入口已读回确认。当前候选源码独立检查17项及合成四格式生成通过，复用本机依赖、0模型；完整独立启动和原产品验收仍未完成，不能把这次展示切片称为整包交付。

#### 1. 规划起点对照（历史；当前完成情况见 Owner 审阅清单）

| 原要求 | 已成立的证据 | 尚需补齐 | 下文执行步骤 |
| --- | --- | --- | --- |
| 原1：新研究题目与六组角色方法 | 新题目实际进入真实研究；六组无答案方法可经MCP读取，部分角色实读 | 方法实际消费与核算/判断的对应；旧新retain/adapt/hold简表；不能仅以目录读取判通过 | 二 |
| 原2：研究/工具接缝与token整改 | SQL/原文/来源计算与直接引用已用；初始大引用对象已削减；现有费用审计 | 局部编辑、旧CALC观察登记、chart提交形状、可操作错误、LangSmith用量；上下文清理/摘要和用途路由的效果验证 | 一、二 |
| 原3：动态完整多Agent | 九研究面/九份底稿、十专家任务含Q4失败替代；并发二非总数二；真实审查/责任修订/综合/Writer/终审 | 跨底稿—综合—正文的修订同步；不把开发接续称一次成功 | 一、二、五 |
| 原4：真实前端与人工交互 | 新研究入口、过程、来源、追问/修订/意见/停止已有真实操作 | 版本差异、修改原因、对话与成果联动、费用清晰度和完整交互验收 | 三、五 |
| 原5：新题目完整Dell验收 | 从UI发起真实新题目，v3与四格式已经产出 | P02重大矛盾；15–20重要主张和全部关键计算的完整开发核查；Owner内容/产品验收 | 二、五 |
| 新增：按需本地/外源补充 | 真实使用同业/产业链；SQL当前DELL/MU/NVDA，HPE等走原文 | 随关键问题修具体来源/期间/解析失败，先宿主用实际工具核验再交模型；不另重建整库 | 二、四 |
| 新增：上传文件进入任务RAG | PDF/DOCX/MD/TXT/CSV/HTML解析、结构分块/BM25、任务副本与读取已实现/定向测 | 从真实上传到模型使用及引用的产品证据；检索定位/表格失败就地修 | 四、五 |
| 新增：图片作为Pro的视觉工具 | 独立Flash vision真实合成探针1请求423tokens，缓存复读 | 真实财务图片/扫描页经MCP被研究/问答消费；不能从两数识别外推OCR准确率 | 四 |
| 新增：sandbox与只读用户文件边界 | 受控对象ID、无shell/任意路径/提权，副本隔离及部分负例 | 结合新入口/压缩/上传重验相关权限与来源绑定；当前可信本地，不承诺公网恶意文件强沙盒 | 一、四、五 |
| 新增：图表审美 | v3三图，来源可打开，显示精度已修 | 图表是否帮助判断、期间/单位/轴是否正确、正文与图表联动，Owner可读性检查 | 三、五 |
| 新增：MD/PDF/Word/PPT | 四格式已实际下载与渲染，导出不调用模型 | 最终候选版本一致和受影响版式复查；不重做导出引擎、不冒称演讲稿 | 五 |
| 新增：通用仓库与中英文公开文档 | 通用会话/上传/交付模块及顶层EN/CN说明已有 | 分离新研究启动与私有旧bundle，公开层级文档、许可/敏感项/数据范围及用户公开决定 | 六 |
| 早前后续：1–2新案例与秋招证据 | 已有一个旧短问答n=1证据、已咨询JD | Dell后少量新短问答与有界长任务，真实成本/质量、演示及本人讲解；未开始，不能消失 | 七 |
...
- 规划起点未接工具清理/摘要；现已将LangChain1.4.0原生 `ClearToolUsesEdit` 接到共用SDK请求投影，post-research原生节点也已接请求副本摘要。完整messages及artifact留给宿主核验，已补原观察按ID回读；不能删证据后放宽validator。50k近似触发/保留最近6次工具清理已真实出站，摘要80k/24k试验的模型接续未过，默认disabled；未部署，不以成熟组件已安装冒充产品资格。
...
以下“一至七”仅是此次剩余工作的顺序，不是新产品阶段或新的审批状态机。一次主要切片在做即可；等待外部咨询/测试时可以做不冲突的前端或文档整理，不能同时启动多个付费研究。

**一、成本、局部编辑与上下文整改——下一次付费前的首要工作。**

**2026-09-07收口：本步骤已完成本地工程整改与有界资格，结果包含明确的HOLD。** 选择原生工具清理/完整证据保存/按需回读；激进早摘要不采用，继续disabled；局部编辑保留审阅职责。未要求所有实验变绿、未声称模型自主语义保障或整体报告发布。Owner明确要求连续做完整步，不逐补丁审批；本轮20次已知调用/估4.0968309元，117项测试通过。完整结果见 [S3/190收口](../../worklog/fin_0_1_3_s3/190_dell_cost_external_and_interactive_delivery.md#step1-closeout-20260907)。

- [x] 沿用现有audit完成每请求/阶段/角色归账、价格时段与未知项界定。旧1连接失败用量不可恢复；3个云端零usage依据本地真实消息单独记190815，云端根因未证实、不手改旧trace。新native/legacy统一fin_call_id，5次新调用云端/local用量已精确对应；不新造计费后台，不把历史缺额伪装修复。
...
- [x] 非图表CALC接缝的有界离线资格：原始CALC ID和归档别名可指向同一个宿主记录，同会话计算结果可继续运算；新角色明确收到同案专家归档后可按原始ID读/引/算，普通追问可直接保留已保存报告的计算引用。错误数值/身份/权威和跨案未观察来源仍拒绝。
- [x] 一组真实局部编辑/整稿开发比较已执行并宿主复核：5调用估0.5116455元，接口/保全/费用有证据；语义未过，不选择winner或宣称同质量节省。下列上下文/接缝完整效果项不因此完成。
- [x] 历史引用接缝工程修复：原生 content_and_artifact 保存完整宿主绑定，读/引共用投影，旧完整 JSON 与内含 S2 引用保留身份/期间/权限；13 项新边界/原生流程测试纳入 112 项检查通过。真实首次被拒答复离线原文重放，4 CALC/6 NUMFACT 全部绑定；不把摘要重建为可执行 CALC，不新增正文关键词判断器。
- [x] 修后自主长接续的有界记录保留资格完成：新tool_edit A3使用原40消息及明确四个原有CALC ID的任务，2次Pro/325423tokens/估1.1224047元，公式、十操作数、期间、12引用和false/缺字段状态保留，旧提交错误明确未保存且不猜确切ID。原金融口径与所有未来回答的语义正确性并未因此通过；任务范围收窄，不与摘要臂做同题因果比较。
- [x] 局部编辑与审阅流程完成真实资格：整稿控制臂4次/估0.5795022元；修后局部A2为3次/0.2501985元，宿主源文审阅后的A3为2次/0.2611134元；再经3处宿主精确替换、0模型完成目标措辞。42引用/3图/标题保留，原v3未晋升。自主候选仍有来源期间混写，保留负例与审阅职责；不宣称无人审阅通过或普遍省费。
- [x] 原生摘要资格以HOLD收口：新A1暴露摘要配额错误中止，已修为维持当前投影与全部新消息；新A2一摘要＋三接续成功执行，但遗漏操作数并猜测旧错误。原失败不改判，`context_summarization.enabled=false`。采用已资格的工具清理路径，待出现实际容量压力再单独资格晚摘要；不把摘要执行成功当内容通过，也不持续付费追绿。
- [x] 在既有用途路由验证一个真实短数字记录任务：Flash2调用13837tokens/估0.0304886元，合法引用经反馈修正，公式/四操作数/期间/单位/非S2身份正确。保持既有quick_writer→Flash、复杂研究/审查→Pro；不新增router模型，不把短数值样本外推材料选择/写作/语义/故障诊断全部适合Flash。
...
2026-09-07 第二切片：`578a94cb`仅扩展现有case来源解析与MCP观察表，不增加计算结果库。新计算引用保留完整宿主CALC，派生运算链接父CALC ID/公式/单位而不递归复制父操作数树；结果继续标识非S2、仅算术验证、金融语义未验证。新增`allow_report_edits=False`仅用于同一revision角色的整稿对照，默认True，不切成“新建报告”角色制造假对照。已验证三类先失败后修复的接缝、40项来源/计算及28项收敛近邻；后者新增两项fixture最初错把chart当report提交，被原validator拒绝，修正fixture后通过，没有放宽validator。未重启服务，未修改任何原研究产物。

**已执行小批：一组局部编辑真实开发对照（原预算依据保留，非整案重跑）。**

执行更新：Owner在明示预算与确认问题后回复“继续”，批准本节这一组、不超过3元；现a3已结束，两组均生成隔离候选，5调用/131,509 tokens/估0.5116455元，无重试、超时或未知usage。A/B根时长130.735/86.867秒；B总tokens少37.108%、实价估费少25.604%，但n=1且语义未通过，不作为泛化同质量降本率。无缓存同谷价重计费A/B为0.472671/0.2895345元，只是情景归一化。原申请文字为历史预算依据，不是下一组许可。完整结果、真实工具动作、差异和负结果在 `Z:/FIN_Insight_Agent_qualification/dell_reference_vertical/20260907_report_local_edit_comparison_a3/review-notes.md` 及S3/190。

语义复核更正了本题起点：开头已有“非已实现”歧义，不能先认定正确后要求别段向其看齐。P01:C14和宿主实际读取的FY27 Q1电话会页4解析节点保留管理层称“符合目标”；P07:C3是三月第三方分析，不能未经核对与五月电话会合为同口径。A/B均未修开头，B还传播歧义；原稿不改，候选不晋升。两组都只读P01/P07/P09 claims，没有模型方法或原文阅读调用。2699字符来源探针是保存绑定envelope，不是完整原文；宿主另只读2080字符实际源节点核实。后续在同有界脚本移除了“开头正确”的暗示，既有方法提示/finance/writer/verifier补一般性的披露与实现、说话者/期间区分，50项零模型近邻证明输入接线，不算本批已验证提示效果。

题目取新研究v3的实际问题：开头/第二节已区分AI服务器“中个位数营业利润率目标”，第七节反证却省略“目标”限定。只核查并修正这一表述的不一致，保留其余数字、观点、引用和三图；若原文与现有说法不符，说明源与写作责任，不预填研究结论。P02矛盾、利润排他因果、DFS归因及触发线仍是步骤二独立待办，不能因这一小改写通过而全部关闭。

| TokenBudgetBasis 项 | 本小批安排 |
| --- | --- |
| 目的/风险 | 比较同一真实局部修订采用完整提交与局部edits的正确性、无关改动、输入/输出及费用；不测完整研究质量、模型优劣或盲测准确率 |
| 实际输入 | 新题目v3正文7,281字符、42个引用、3图的同一宿主快照；当前相关paper/source按需读取，不预塞九份完整底稿/巨大citation对象。模型原始提交见末run `01a0787d-961c-7f21-a590-53887cfe2c75` 的Writer call `5e8fc59f-9374-4832-8264-3fab924c69dd`；显示快照在 `D:/temp/finsight-dell-final-20260907-a1`。同目录旧 `report-v3.agent-original.md` 实际是旧审计题目，不可混用。发送前本地组装并记录实际消息/schema大小，原始私有推理不注入新的Writer |
| 必要输出/schema | A用原`submit_case_report`完整提交；B可用`submit_report_edits`只提交实际替换，原校验与宿主合并得到同样可检查的完整候选。正文自由写，不新增格式模板；两份候选仅进隔离测试目录，不覆盖v3 |
| 模型/变量 | 两臂均现有`deepseek-v4-pro`、thinking enabled/low；相同revision角色、问题、材料、读取工具和预算。唯一处理差异是是否提供/推荐局部提交接口。不同时换Flash、开摘要或改上下文清理；清理本组均关闭，原配置默认值不改 |
| 轮数/输入依据 | 最近一次真实Writer修订4调用，输入21,726→69,779 tokens、输出合计27,523，估费0.6420216元；这是Writer子环节而非整案。两臂各最多4模型调用/16工具，整批最多8模型调用；首请求预计约20–30k tokens量级，依据现有输入，不冒称精确tokenizer测量。每请求180,000字符输入上限含schema，超出传输前停；不得为了便宜删掉必要证据 |
| 输出/延迟 | 每请求最多16,000输出tokens（含thinking，旧末两次完整提交输出11,246/9,372），超时480秒，无transport重试、无resume/fallback；额度不足/截断只记未完成，不给成功标签 |
| 费用申请 | 官方当次已核Pro峰值每百万tokens缓存输入0.30元、未缓存9元、输出27元；谷值一半。最近Writer四调用若按峰值约1.284元，两份同规模约2.568元，故本组预计约1–2.6元、**申请批次预算不超过3元**，不是已批准或账单保证。首请求前重核价格/输入，逐请求用现有audit合计本组费用并留下一请求余量；余量不足/预计越界就停，不自动加钱。超时未知用量不计零、不盲重发 |
| 检查/停止 | 宿主逐句核“目标/实现值”语义、依据和上下文；比较完整候选、引用/图表保留、非目标段落diff、回读/失败/耗时和实际provider usage。只读/来源绑定不得放宽；任何一臂未完成/重写无关分析/引用失效都保留负结果。n=1开发对照，不报百分比泛化节省或P95；无付费评审节点，不据此称独立投研验收 |

沿用原生create_agent、现有模型SDK、CaseModelAudit、LangSmith和费用脚本；不新建执行协议/计费服务。审批后先零provider组装/工具检查再运行本组；如果首请求所需资料/费用实质扩大，先报告，不从“继续”或旧“额度管够”推定追加预算。本组之外的长历史清理对照/摘要/用途路由要依据结果确定，不捆绑新付费；其余可离线推进的原计划不取消。价格依据：[DeepSeek官方价格](https://api-docs.deepseek.com/zh-cn/quick_start/pricing/)。

2026-09-07 第一项集中收口的新批次：Owner明确批准“本批不超过5元”，至多8次模型调用，含摘要，不沿用上批3元余额。本批沿用已有40消息Synthesis会话（原call `2a142e7d-fc42-41c6-b1be-6f8302dc97a3`，provider已报input200851）做同一隔离接续；检查已有CALC公式/结果/操作数/期间/权威、未解决提交错误和补读，不续跑整案或改报告。三种上下文为原样、工具清理、原生摘要＋清理；另将同一实际计算与错误作为短事实任务交Flash，不把它当同输入的模型优劣比较。各主臂最多2调用/8工具，摘要Flash独立计费，输出主臂4500/摘要6000上限，480秒、transport与摘要均无重试，截断/未知usage/下一请求预算余量不足即停。费用守门用当前峰值无缓存价；原上下文可使用其完整已报200851输入tokens，加全部新增/改变消息和schema的UTF-8字节上界作保守余量，不能拿字符减少比例估算降费。真实价格/缓存影响分别报告；n=1开发证据，不得称整份报告达到同质量低价。

原生摘要只接入post-research create_agent节点，LangGraph在原messages旁保存摘要投影；不删除宿主证据，不增加Redis记忆或自研摘要算法。LangChain1.4原生工具配对/切点保留，4k前缀裁剪关闭；默认隐式retry替换为现有审计过的单次SDK runnable。原用户任务逐字保留，摘要含目标/进展/数字公式期间/来源/冲突/下一步但非Evidence，完整原消息与artifact继续供validator使用。同一前缀不重复收费，完成提交后不再摘要。80000近似触发、24000近似保留，明确不是精确provider token窗口。未完成真实质量检查前，不开启正式配置；旧Lead/Specialist继续共用SDK工具清理，不在本轮重写其运行结构。

本批最终事实：A1原样1调用204772tokens/估1.923948元，4500输出全部reasoning并截断；A3摘要＋2Pro共205315tokens/估1.4707206元，4个旧CALC回读失败、2轮额度耗尽；A4 Flash短任务2调用13837tokens/估0.0304886元成功保存，但“未验证/未通过”和source-error归因措辞仍有限。**累计6调用423924tokens/估3.4251572元，无未知usage，不追加paid。**具体记录见S3/190最新节及`Z:/FIN_Insight_Agent_qualification/dell_reference_vertical/20260907_context_continuation_a4/review-notes.md`。A5_offline验证修后四来源回读/摘要仍保留宿主证据/合法提交，无provider。步骤一工程与费用归账已落地，但未全部验收；只剩修后长接续资格，不把hold项说完成、不自动进入步骤二。

**二、关闭报告责任问题，并验证研究方法实际起作用。**

2026-09-08结果：P02责任修订已同步；20重要判断维度与31算术检查完成，六组方法按实际消费保留retain/adapt/hold；形成原生v4，Owner待审。以下勾选为开发工作完成，不代表六组方法全部有效或产品已接受。

- [x] 先离线核P02:C12/thesis/narrative、正文及相关来源；修订要同步相关研究产物，不手改旧底稿/报告、不预设研究观点。原有9稿/失败/所有版本保留。
- [x] 在第一步验证后的局部路径完成必要作者修订与独立定向核验；其余八面不重研究。若关键问题超出当前P02，按来源/计算/研究/写作归责，不把所有残差塞给Writer。
- [x] 从实际trace/底稿做六组方法的retain/adapt/hold及消费记录：未读、只读目录、读方法、分析中有应用分别写；不要求每角色机械读所有方法，不把答案写入Skill。
- [x] 人工重点核查约15–20重要主张和全部影响核心判断的计算，覆盖增长、利润、现金、执行要求、同业、反证和改变判断条件。记录正确/错误/不足/未评估，开发抽查不是随机准确率、盲测或专业投研认证。
- [x] 将秋招只读复核指出的具体措辞纳入同一核查：利润“组合/杠杆而非AI自身”的排他性是否有充分拆解；公司现金/DFS变化能否归因AI；“中个位数利润率目标”既不冒充独立披露的精确实现值，也不得反向否认来源称符合目标的实际表现陈述；分别核五月FY27 Q1管理层与三月第三方的时间和口径。CFO接近净利润、ISG约15%等触发线说明是基准/目标/研究假设，不包装成验证过的预测模型。有证据则保留有分量判断，依据不够则补核或针对性限定，不将正文改回通篇边界说明。一般性方法提示已修/零模型消费已证，真实输出效果与原稿修订尚未完成。
- [x] 需要补源时，复用现有SQL、结构检索、原文窗口与Exa；先宿主用同一工具读到，再交模型。SQL覆盖范围如实列，非SQL披露可计算但不升为NumericFact。只修发现的资料/解析问题，不重选整套RAG。

交付：一份值得阅读且关键判断有依据的报告候选、逐项核查和版本差异。若仍有重大错误如实待修订，不用反复全稿生成或清零全部advisory替代产品判断。

**三、完成对话式前端与真实人工交互。**

- [x] 基于现有React、BFF、原生stream实现左历史/中对话/右成果；阅读宽版、窄窗口标签；当前case名称来自真实配置，不把支持范围伪装成任意公司。
- [x] 新研究、快问、深问、局部修订清晰分流；来源卡片/原文定位与正文联动，版本选择和真实文字差异采用成熟diff能力，保留修改原因/责任来源。
- [x] 实际任务依赖、等待、工具动作、错误、回派原因可展开；意见保存/送达/应用有区分；停止后可保留产物并按原生状态继续。显示公开简明依据，不复制私有CoT或另造LangSmith。
- [x] 本次操作与会话累计分别显示input/output/cache/估费/调用/耗时/失败及未知用量；超预期在安全点提示实际剩余工作，不将取消说成零收费。角色/工具明细用于解释成本，而不是堆满首页。

接入点：`apps/workbench/frontend/vite/src/app/ResearchSession.tsx`、`apps/workbench/backend/api/v1/report_sessions.py` 与现有来源/交付组件。CSS/差异组件先本地build/BFF/浏览器，不为排版跑模型。交付不是截图：从提问、查看研究、补充意见、阅读来源到追问/修订/版本比较的一段真实可操作流程。

**四、上传、图片与任务RAG的真实消费验收。**

- [x] 沿用当前pdfplumber/python-docx/HTML解析、LangChain分块/BM25和任务存储，选一个权利清楚的真实财务PDF或表格文件、一个相关图像/扫描页，走UI上传→解析状态→MCP检索/原文→模型使用→来源可点，不再只做合成两数探针。
- [x] 图片由 `deepseek-v4-flash-vision-exp` 作为只读工具按需解释；普通Pro不直接冒充视觉模型。图片页码/原图和识别不确定性保留，视觉数值不自动升为SQL权威。明确样本n与非全面OCR资格。
- [x] 无关格式/过大文件/跨任务读取/路径穿越/来源指令执行要拒绝；模型不能修改用户原文件、拿凭据或shell。当前仍可信本地使用，不额外建设公网恶意文件处理平台。
- [x] 抽看实际chunk、标题、表格和引用错在哪里；BM25可定位就先用，不为了完成表格引入全部dense/graph/reranker。新增解析依赖只因实测缺口且按成熟栈优先。

这一切片可用独立受控小任务验证，不把上传样本悄悄塞入历史Dell结果，也不为证明视觉重跑全案。

**五、同一Dell产品链与交付文件的最终收口。**

- [x] 将第二步的内容核查、第三步的交互、第四步的新增能力合并验收；区分新研究原始执行、明确复用底稿的修订、独立上传探针，不能相加冒充一轮无辅助成功。
- [x] 检查图表所用数值/期间/单位/轴/来源与正文一致、是否帮助判断；保留已完成MD/PDF/Word/PPT导出，只对最终版本及受影响布局渲染核查，导出0模型调用。
- [x] 修实际Markdown可见的交付引用缺口：[11]/[35]等CALC不能仅展示“本地来源绑定计算”，须能读到公式、输入及来源；[38]空条目和[P05]内部底稿标识须追溯到真实来源或明确可读说明。检查同一引用投影是否影响其余格式/UI，不能未查就宣称全坏；关键来源、计算和当前版本可定位属于正确性交付，不归为可延期美化。
- [x] 固定服务配置、PG/Redis和报告版本保全；交付当前可启动状态及回退说明。检查代理/本机服务不付费，历史PID不当当前状态，不清卷/删历史。
- [ ] Owner阅读最终报告并操作工作台，记录接受或具体剩余问题。四格式可读、模型无重大意见都不自动代表Owner验收。

原新题目fresh整链已经执行；只有实际改动影响端到端调度且局部/接缝验证不足时，才提出有理由、单列成本的再一次完整运行。默认不重跑九面。不能把修改后的所有运行时能力标为旧run已验证。

**六、公开仓库、可复现启动与中英文展示面。**

- [x] 将新研究初始化与旧Dell报告bundle的兼容依赖分离；保留明确legacy入口。新模块按职责命名，触及处整理重复active实现，不全仓无差别改名/搬目录。
- [x] 形成明确的公开路径清单：代码/锁文件/合成fixture/脱敏demo与私有语料、SQL/index、上传、凭据、原始模型记录/CoT分离。完成当前/历史文本敏感模式扫描与CI依赖许可清单；8个binary/large不在文本扫描范围，资料重分发权另行判断，不因已有50路径检查就称全仓安全。
- [x] README与公开的架构、数据/MCP、Agent/context、前后端交互、运行/回滚、评测成本/局限各层级有中英文入口；复用已有文档而非翻译全部历史worklog。独立启动给可操作的数据准备说明，不预填旧答案，不要求仓库公开原始语料。
- [x] 在不改当前工作树的独立目录验证公开模式源码/合成检查和新研究初始化；真实provider依赖/私有数据准备如实注明。独立源码23项检查通过。代码公开已获授权，报告选段/完整视频外部分享仍由Owner选择。

**七、少量新场景与秋招交付证据。**

2026-09-08结果：NVIDIA短问、Micron有界深问、八不同短问题均已执行；费用/失败和语义意见见Owner清单。真实只读演示已录制；本人讲解练习仍由Owner完成。它们不是两家公司独立全案或研究准确率benchmark。

- [x] Dell候选开发核查后已完成非Dell短任务/问答，再一个已有资料支持的有界长任务；沿同一runtime，不写第二套case引擎。原先1–2新case要求保留，不能因本轮仅计划而删除，付费规模需另核。
- [x] 约8–12不同短问题覆盖查数、计算、跨源和合理不可回答；4–6类已有故障/权限测试复用，长任务逐例列结果/成本。样本少不报P95或泛化成功率，不重复同题刷指标。
- [ ] 一段短演示、真实架构/工具和数据边界说明、两个失败—定位—修复故事、请求级质量/成本/时延表；本人讲解练习由Owner审阅，材料已经准备。可先整理已有证据，不等所有工作完成才学习/准备讲解。
- [x] 招聘建议作为优先级参考，和JD明确要求分开。不给简历添加未验证省费率、生产HA/多租户/全公司或“多Agent优于单Agent”；不为关键词加K8s、消息平台或训练栈。

#### 4. 费用、验证与停止安排

Owner确认后的局部修订对照已按3元内预算结束，实估0.5116455元，剩余额度不解释成自动追加调用；当前不再启动本批或整案paid。后续必要真实验证按具体目的、输入和费用单列，普通离线核查/小修继续不逐项求批。仍沿用现有TokenBudgetBasis，不以旧“额度管够”解释无限扩张。

预算目标是必要研究可见、小问不重研究、小修不全稿、长历史可控；不以压掉证据/关闭全部思考/砍九研究面达到便宜。真实接口结果未知不盲重发；同类已定位故障重复出现先本地修，不用paid继续试错。新增外部采购/云服务、权限扩大、删除/覆盖、发布含义改变需另确认。

本轮文档仅做链接、diff与JSONL检查；后续局部补丁近邻测试、共享工具/消息接口做集成、前端build和浏览器；不例行全仓回归/哈希。冻结报告/失败不改。回滚以已提交代码/配置、原生checkpoint和已保存版本为依据，不建设新恢复系统。

#### 5. 秋招任务咨询

咨询目标：请“秋招投递计划”（`01a003d0-b798-7b52-b27b-a9b3b062d058`）亲读当前报告/交付状态和本节，对照已有JD给出必须补/可延期、可信指标/演示和本人讲解建议；尤其评估成本整改、报告质量与新增技术栈的边际价值。仅只读咨询，不投递、不改简历/项目、不启动模型；不能从旧记录继续说新前端整链尚未发生。

2026-09-07咨询已返回（turn `01a07997-cc97-7342-af8d-49fcd33917aa`）。对方实际读完v3正文、交付状态和本节；这是招聘侧与成果可读性点评，不是全体财务数据/一手来源的独立审计。主任务另只读确认Markdown中空[38]、内部[P05]、CALC仅有标签，以及“目标”限定跨段丢失的展示事实；本轮不修改原稿、不新增付费。

采纳并融入上面步骤，不另建计划：

| 返回意见 | 合并处理与理由 |
| --- | --- |
| 已有真实前端→动态研究→报告案例，不应仍称只有RAG原型 | 展示如实提升到端到端研究交付候选；保留开发修复/接续/人工反馈、未Owner验收，不冒称一次无辅助或稳定生产 |
| 最值得补的是局部修订、质量受约束的成本对照、可理解交互 | 步骤一先控制无效工作/长上下文，步骤二关闭责任同步，步骤三显示真实diff/原因/本次费用；不新建调度、追踪或审批平台 |
| 现金流桥接、H2指引倒算、利润/现金分层已有研究价值，部分因果和限定仍不足 | 并入步骤二15–20条主张检查；步骤五补CALC/空来源/内部标识投影，保留论证优点而非重写成合规说明 |
| 成本对照同题/同资料/同起点，主要改变一个因素，允许负结果 | 1–2组开发对照连摘要/回读/纠错/失败一并计费，同时列质量/延迟；不把84.92%缓存当压缩，不把少量样本报成P95或泛化降本率 |
| 真实上传视觉、独立启动、新长短场景值得补 | 保留步骤四/六/七；合成探针不等于真实上传、四格式不等于演讲稿、旧私有bundle不能暗作公开启动前提 |
| 不等全部打磨完才整理投递证据 | 现有成果的短讲解和演示脚本可穿插准备；不等于这次代为投递、修改简历或公开仓库，不删除用户尚未交付需求 |

可在投递准备之后继续打磨的建议，只作为顺序弹性而非取消：全量视觉细节、完整各层级双语文档、更广扫描格式覆盖、第二新案例及持续评测、专门演讲型PPT。引用可用、当前版本清楚和重大判断一致不属于这一类。公开发布和“完整产品验收”仍需各自相应条件。

本人讲解清单沿用步骤七：九研究面/十任务/并发二及回派；局部修改影响哪些产物与费用；PG/Redis/provider缓存/活动上下文的区别；SQL、外源、视觉、计算、推断的来源链；失败接续与未知usage边界；能解释并修改一处实际adapter和测试。招聘任务结合已核去哪儿AI应用与百度AI产品JD提出此建议；这是能力匹配与个人学习建议，不是岗位强制指定某个Agent数量、指标阈值或框架，也不预测过筛概率。当前没有证据要求新增K8s、训练框架或自研治理栈。
