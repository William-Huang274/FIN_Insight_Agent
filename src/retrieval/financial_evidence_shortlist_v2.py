...
from .evidence_role_v2 import evaluate_evidence_role
...
FINANCIAL_EVIDENCE_SHORTLIST_SCHEMA_VERSION = (
    "fin_ia_financial_evidence_shortlist_v1_3"
)
...
_FACET_ROLE_PRIORITY: Mapping[str, tuple[str, ...]] = {
    "reported_results": (
        ...
        "mechanism_or_definition_context",
        "management_guidance",
    ),
    "cash_generation": (
        "financial_statement_or_reconciliation",
    ...
    *,
    ...
    role = evaluate_evidence_role(
        ...
        request_intent_terms=(
            tuple(request.get("metric_intents") or ())
            + tuple(request.get("product_intents") or ())
    ...
    return {
        ...
        "fiscal_year": (
            int(base["fiscal_year"])
            if base.get("fiscal_year") not in (None, "")
            else None
        ),
...
def rank_financial_evidence_shortlist(
    *,
    ...
    ranked = tuple(sorted(rows, key=_ranking_key))
    return _prioritize_same_basis_temporal_pairs(ranked, request=request)


def _prioritize_same_basis_temporal_pairs(
    rows: Sequence[Mapping[str, Any]],
    *,
    request: Mapping[str, Any] | None,
) -> tuple[dict[str, Any], ...]:
    """Reserve bounded same-metric period pairs before general relevance.

    This is candidate diversification, not numeric adjudication.  It uses only
    the typed request, already-bound fiscal year, and each candidate's selected
    RetrievalNeed.  No qrel, value, formula or score threshold participates.
    """

    typed_rows = tuple(dict(value) for value in rows)
    if request is None:
        return typed_rows
    ...
    metrics = tuple(
        dict.fromkeys(
            str(value).strip()
            for value in request.get("metric_intents") or ()
            if str(value).strip()
        )
    )
    if len(years) < 2 or not metrics:
        return typed_rows
    ...
    for metric in metrics:
        ...
        for year in years:
            for row in typed_rows:
                object_id = str(row["compiled_object_id"])
                need = row.get("best_retrieval_need") or {}
                terms = tuple(str(value) for value in need.get("intent_terms") or ())
                if (
                    object_id not in selected_ids
                    and row.get("fiscal_year") == year
                    and terms
                    and terms[0].casefold() == metric_key
                    and row.get("composite_compatibility") != "incompatible"
                ):
                    selected.append(row)
    ...
    return tuple(
        ...
        + [
            row
            for row in typed_rows
            if str(row["compiled_object_id"]) not in selected_ids
        ]
    )
