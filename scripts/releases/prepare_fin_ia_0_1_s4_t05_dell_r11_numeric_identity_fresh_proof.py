...
R10_ISSUANCE = ROOT / (
    "configs/releases/fin_ia_0_1_s4_t05_dell_r10_profile_aware_"
    "artifact_lineage_fresh_exact_admission_issuance_v1_0.json"
)
R10_RESULT = ROOT / (
    "configs/releases/fin_ia_0_1_s4_t05_dell_r10_profile_aware_"
...
DECISION = ROOT / (
    "configs/releases/fin_ia_0_1_s4_t05_dell_r11_numeric_identity_"
    "fresh_agent_proof_decision_v1_0.json"
)
PROSPECTIVE_ADMISSION = ROOT / (
    "configs/releases/fin_ia_0_1_s4_t05_dell_r11_numeric_identity_"
    "fresh_exact_admission_r11.json"
)
EXECUTION_IDENTITY = (
    "fin01-s4-t05-dell-r11-numeric-identity-exact-live-r11"
)
ADMISSION_ID = (
    "fin01-s4-t05-dell-r11-numeric-identity-fresh-exact-admission-r11"
)
EXECUTION_MODE = "exact_live_s4_dell_r11_numeric_identity_r11"
...
def prepare(runtime_root: Path = RUNTIME_ROOT) -> dict[str, Any]:
    ...
    if implementation["status"] != (
        ...
        raise RuntimeError("R11_zero_call_implementation_not_proven")
    for relative_path, expected in implementation[
        "exact_code_bindings"
    ].items():
        if _sha256(ROOT / relative_path) != expected:
            raise RuntimeError("R11_implementation_code_binding_drift")
    r10 = S3ThreeCellBoundedAgentAdmission.model_validate(
        _load(R10_ADMISSION)
    )
    r10_result = _load(R10_RESULT)
    if (
        r10_result["canonical_terminal_truth"]["artifact_count"] != 9
        or r10_result["admission"]["consumed"] is not True
    ):
        raise RuntimeError("immutable_R10_success_truth_mismatch")
    target = load_execution_target(R10_ISSUANCE)
    provisional = r10.model_copy(
        update={
    ...
    if overlay is None:
        raise RuntimeError("R11_profile_overlay_missing")
    source_pack = load_s4_source_grounded_input_pack(ROOT, "DELL")
    case_id = str(r10.case_id)
    ...
    def forbidden_provider(**_: Any) -> dict[str, Any]:
        ...
        provider_calls += 1
        raise AssertionError("provider_forbidden_in_R11_fresh_proof")

    with tempfile.TemporaryDirectory(
        prefix="fin01-s4-t05-dell-r11-numeric-identity-proof-"
    ) as temp:
        ...
        if first.model_dump(mode="json") != second.model_dump(
            mode="json"
        ):
            raise RuntimeError("R11_double_prepare_parity_failed")
    ...
    if (
        ...
        or first.research_run_id in before_snapshot[
            "research_run_ids"
        ]
        or PROSPECTIVE_ADMISSION.exists()
    ):
        raise RuntimeError("R11_freshness_or_zero_call_boundary_failed")
    payload = prospective.digest_payload()
    return {
        "schema_version": (
            "fin_ia_0_1_s4_t05_dell_r11_numeric_identity_"
            "fresh_agent_proof_decision_v1_0"
        ),
        "decision_id": (
            "S4-T05-DELL-R11-NUMERIC-IDENTITY-FRESH-AGENT-PROOF-R1"
        ),
        "recorded_at": "2026-07-28T17:00:00+08:00",
        "status": (
            "pass_zero_call_fresh_agent_proof_"
            "R11_admission_issuance_pending"
        ),
        "source_refs": {
            "implementation": _display(IMPLEMENTATION),
            "implementation_sha256": _sha256(IMPLEMENTATION),
            "immutable_R10_admission": _display(R10_ADMISSION),
            "immutable_R10_result": _display(R10_RESULT),
        },
        "fresh_identity": {
            "execution_identity": EXECUTION_IDENTITY,
            "case_id": case_id,
        ...
        "future_exact_live": {
            ...
            "new_L1_failure_opens_R12": False,
            "paired_assessment_authorized_after_success": True,
            "S4_T06_authorized_after_owner_acceptance": True,
        },
        "stage_acceptance": {
            "RC_P36_067": (
                "implementation_fixture_proven_fresh_proof_pass"
            ),
            "RC_P36_068": (
                "implementation_fixture_proven_fresh_proof_pass"
            ),
        ...
        "next_action": (
            "S4-T05-DELL-R11-NUMERIC-IDENTITY-"
...
def main() -> int:
    ...
    if first != second:
        raise RuntimeError("R11_independent_fresh_proof_mismatch")
