...
_DATA_BUILD_STEPS = {
    ...
    "retrieval_build_current_compiled_object_views": _step(
        ...
        [
            ...
            _param(
                ...
                "--documents",
                "父文档 JSONL",
                default="data/workbench_private/fin_0_1_3_s1b_current_financial_object_store/v2/documents.jsonl",
            ),
            _param(
                "records",
                "--records",
                "source-bound child JSONL",
                default="data/workbench_private/fin_0_1_3_s1b_current_financial_object_store/v2/records.jsonl",
            ),
            _param(
                "output_dir",
                "--output-dir",
                "私有编译对象目录",
                default="data/workbench_private/fin_0_1_3_s1c_compiled_financial_object_views/v3",
            ),
            _param(
                "result_output",
                "--result-output",
                "零调用证明摘要",
                default="configs/retrieval/fin_ia_0_1_3_s1c_query_object_fact_route_zero_call_result_v1_2.json",
            ),
        ],
    ...
    "retrieval_materialize_s1c_qrels": _step(
        ...
        "Retrieval Eval",
        "重资格 S1-C relevance labels",
        "把 Owner 已接受的 relevance labels 通过 lineage 绑定到当前金融 child；只生成评测身份，不晋升 Evidence。",
        ...
        [
            _param(
                ...
                "--policy",
                "排名策略",
                default="configs/retrieval/fin_ia_0_1_3_s1c_ranking_comparison_policy_v1_0.json",
            ),
            _param(
                "output",
                "--output",
            ...
            _param(
                ...
                "--successor-decision",
                "Owner successor 决策",
                default="configs/retrieval/fin_ia_0_1_3_s1c_owner_qrel_successor_decision_v1_0.json",
            ),
        ],
        output_parameters=["output"],
    ),
    "retrieval_run_s1c_ranking_comparison": _step(
        "retrieval_run_s1c_ranking_comparison",
        "Retrieval Eval",
        "执行 S1-C 同对象排名对照",
        "在同一当前对象和 qrels 上比较 BM25、BGE-M3、固定 RRF 与确定性金融规则重排；本地模型路径必填，禁止下载。",
        ...
        [
            ...
            _param("model", "--model", "本地 BGE-M3 目录", required=True),
            _param(
                "cache_dir",
                "--cache-dir",
                "私有向量缓存",
            ...
            _param(
                ...
                "--projection-output",
                "Workbench 安全投影",
                default="configs/retrieval/fin_ia_0_1_3_s1c_ranking_workbench_projection_candidate_v1_1.json",
            ),
        ],
        output_parameters=["output", "projection_output"],
        timeout_hint_s=1800,
    ),
    "retrieval_materialize_s1c_financial_role_eval": _step(
        "retrieval_materialize_s1c_financial_role_eval",
        "Retrieval Eval",
        "生成 S1-C 金融角色评测集",
        "把三案 Owner qrels、现有业务 hard negatives 与逐条复核的 ORCL/ASML/ANET 留出标签冻结为 shadow-only 评测集。",
    ...
    "retrieval_run_s1c_cross_encoder_role_shadow": _step(
        "retrieval_run_s1c_cross_encoder_role_shadow",
        "Retrieval Eval",
        "运行 S1-C Cross-Encoder 与角色 shadow",
        "离线比较 BGE reranker 与 abstaining Evidence Role；分数和标签均不得晋升 Evidence。",
        ...
        [
            ...
            _param(
                "bge_model", "--bge-model", "本地 BGE-M3 目录", required=True
            ),
            _param(
                "cross_encoder_model",
                "--cross-encoder-model",
    ...
    "retrieval_materialize_s1c_runtime_query_atoms": _step(
        "retrieval_materialize_s1c_runtime_query_atoms",
        "Retrieval Eval",
        "编译 S1-C Runtime Query Atom 评测集",
        "把类型化 EvidenceRequest 编译成单 facet、单披露主体的查询原子；标签只在查询编译完成后连接。",
        "scripts/data_retrieval/materialize_s1c_runtime_query_atoms.py",
        [
            _param(
                "manifest",
                "--manifest",
    ...
    "retrieval_run_s1c_runtime_query_atom_model_shadow": _step(
        "retrieval_run_s1c_runtime_query_atom_model_shadow",
        "Retrieval Eval",
        "运行 S1-C Query Atom 模型 shadow",
        "在同一当前对象上离线比较 BM25、BGE、Qwen Embedding 与两个 reranker；受控标签池不得进入 Runtime、Evidence 或 NumericFact。",
        "scripts/data_retrieval/run_s1c_runtime_query_atom_model_shadow.py",
        [
            _param(
                "policy",
                "--policy",
