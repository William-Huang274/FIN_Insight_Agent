...
2026-09-04 r8 fresh attempt1=`20260904T045906+0800-zero-model-r8` 已在 clean pushed `2b88bbf106d7976b3bb0c2e804188289213e6af5` 上真实执行并保留为 immutable failed，不是 PASS。Compose build/up 成功，Agent Server/PostgreSQL/Redis healthy，API 仅绑定 `127.0.0.1:18128`；固定 Q1 zero-model 图真实到达 native interrupt，remote Run=`success`、Thread=`interrupted`、current state=`zero_model_mcp_qualified`、interrupt=1、`next=["qualification_interrupt"]`。失败码 `r8_remote_terminal_status_mismatch` 的最早责任层是 harness 把 Thread 的 `interrupted` 错当成 Run 终态；固定 Agent Server 0.13.3 的正确分层是 dynamic interrupt 后 Run 成功、Thread 等待恢复。故这不是图、数据、Docker 或代理失败：Docker 仍经 `http.docker.internal:3128` 完成 164.016 秒 build/up，图执行 8.295 秒，LangSmith phase 尚未开始。attempt1 manifest SHA=`727cd3fb8803bf2711bbb46ca0d5def39b793834160c7d7edbcd3f72a0b50b5e`，failure receipt SHA=`84046a2696e301200cf535cd4222202fa64a4c866fd4225704f1d4e96e36667b`，旧 project/container/volume 均未清理。当前同一 R8 correction candidate 已改为分层验收：START/RESUME Run 都须 `success`；START/readback Thread=`interrupted` 且 next/interrupt 精确存在；RESUME/final Thread=`idle` 且 next/interrupt 为空；作者分离复核未发现新 P0/P1，相邻回归=`215 passed in 21.74s`。为保留 attempt1，successor 固定 fresh project=`finsight-dell-qualification-20260904-r8a2`、loopback `18129` 与新 volume；须 clean commit/push 后才运行 attempt2。Project OS 回归仍为既存 sealed SHA drift 的 `81 passed / 1 failed`，故 full repository green=false。R7 bounded baseline 仍是 current；r8 完整 restart/SSE/LangSmith 尚未通过，durable orphan lifecycle、unknown-outcome 自动重试、distributed exactly-once、profile 入库、guard/binder 分离、完整 multi-agent、模型/DeepSeek/实时外源/付费、Evidence admission、S2 write、HITL、报告和 production 仍为 false。
