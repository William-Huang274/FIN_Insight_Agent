...
from retrieval.dell_report_internal_chain_ceiling_r6 import (
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    AUTHORITY,
    BRANCH,
...
from scripts.data_retrieval import (
    run_dell_report_internal_chain_ceiling_r6 as r6_runner,
)
...
def test_R6_freezes_every_R4_semantic_attack(
    ...
    result = classify_dell_report_internal_chain_r6_package(
        target_id=target_id,
        text=text,
        metadata=_metadata("DELL"),
    )
...
def test_R6_freezes_fresh_R5_clause_and_polarity_attacks(
    target_id: str, text: str
) -> None:
...
def test_R6_rejects_speculative_or_absent_typed_propositions(
    target_id: str, text: str
) -> None:
    result = classify_dell_report_internal_chain_r6_package(
        target_id=target_id,
...
def test_R6_clause_scope_preserves_valid_positive_before_unrelated_negative(
    target_id: str, text: str
) -> None:
    result = classify_dell_report_internal_chain_r6_package(
        target_id=target_id,
...
def test_R6_positive_roles_remain_complete(target_id: str, text: str) -> None:
    result = classify_dell_report_internal_chain_r6_package(
        target_id=target_id,
        text=text,
        metadata=_metadata("DELL"),
...
def test_R6_guard_limitation_is_not_added_when_guard_itself_passes() -> None:
    result = classify_dell_report_internal_chain_r6_package(
...
def test_R6_assigns_positions_before_identical_sentence_deduplication() -> None:
    ...
    result = assess_dell_report_internal_chain_r6_packages(
        target_id="DELL-RSQ-03A-TARGET-ASP",
        source_rows=source_rows,
        object_rows=[],
    )
...
def test_R6_preserves_true_bounded_raw_occurrence_adjacency() -> None:
...
def test_R6_typed_anchors_exclude_product_code_digits() -> None:
...
def test_R6_product_code_separator_variants_share_one_canonical_anchor(
...
def test_R6_two_and_four_digit_fiscal_years_share_one_anchor() -> None:
...
def test_R6_natural_article_before_count_is_not_A100_product_code() -> None:
...
def test_R6_numeric_anchor_15_is_not_covered_by_150() -> None:
...
def test_R6_exact_typed_anchor_is_covered() -> None:
...
def test_R6_product_and_fiscal_year_variants_are_semantically_covered() -> None:
...
def test_R6_material_anchors_exclude_unrelated_next_process_target() -> None:
...
def _R6_projection_fixture() -> dict:
    path = (
        ROOT
        / "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
        "candidate_ceiling/dell-rsq-03b-internal-chain-r5/full_result.json"
    )
    ...
    authority["03B_R6_execution_consumed"] = authority.pop(
        "03B_R5_execution_consumed"
    )
...
def _R6_policy_inputs() -> tuple[dict, dict[str, dict]]:
    refs = {
    ...
    policy = {
        ...
        "TokenBudgetBasis": {
            "node_purpose": "one exact R6 local candidate-chain audit",
            "input_scale": "five requests, 1,888 sources, 34,199 objects",
            "required_outputs": "raw positions, typed coverage and routes",
            "schema_burden": "R5 failure, R39 runtime and zero authority",
            "materiality_quality_risk": "false adjacency, anchors or roles",
            "comparable_run_evidence": "immutable R5 plus failed fresh audit",
            "reasoning_profile": "one local 0.6B batch and deterministic R6",
...
def _validate_policy(policy: dict, values: dict[str, dict]) -> dict:
    return validate_dell_report_internal_chain_ceiling_r6_policy(
        policy,
...
def test_R6_policy_rejects_missing_R5_root_cause() -> None:
    policy, values = _R6_policy_inputs()
    drift = deepcopy(values)
    audit = deepcopy(values["R5_fresh_audit"])
    audit["material_findings"] = [
        row
        for row in audit["material_findings"]
        if row.get("root_cause_id")
...
def test_R6_disk_capacity_gate_runs_before_attempt(
    ...
    with pytest.raises(RuntimeError, match="minimum_free_disk_capacity"):
        r6_runner._require_output_disk_capacity()
    ...
    receipt = r6_runner._require_output_disk_capacity()
