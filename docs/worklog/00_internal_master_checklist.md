...
- [ ] D9.1: Move Gate Registry from persist-only projection into pre-Memo hard gate checkpoints, add SQL-backed append-only gate history, and connect D5 stale/time mismatch, D6 unresolved conflict, D8 source boundary, and D2 commercial gap to replayable eval cases.
...
- [ ] D10.1: Move Derived Metric Layer into Memo Writer / ClaimCard fact selection, add SQL-backed formula registry and derived output store, and add formula-level eval fixtures for capex sign, negative prior period, quarter length, product/unit mismatch, and ASP boundary.
...
- [ ] D12.1d: Move entity/source/vintage/reconciliation/ontology/source-policy/derived/memory cross-run reads into DB-default readers where runtime nodes actually consume prior-run state, with stale/supersession rules.
