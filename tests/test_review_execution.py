...
def test_native_review_truncation_drains_paid_sibling_and_blocks_new_dispatch():
    async def exercise():
        ...
        for role in ('counter','verifier'):
            ...
            reviewers[role]=create_agent(model=AsyncReviewModel(actor=role,signal=signal,control=control,calls=calls),
                tools=[read_probe],state_schema=CaseReviewerState,middleware=audit.middlewares())
        ...
        assert len(settled)==2 and sorted(calls)==['counter','verifier']
