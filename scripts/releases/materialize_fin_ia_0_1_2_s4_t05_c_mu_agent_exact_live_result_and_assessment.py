...
RUNTIME_ROOT = ROOT / ".codex_runtime/fin012-s4-t05c-mu-agent-exact-live-r1"
EXACT_RESULT = RUNTIME_ROOT / "execution-result.json"
EVIDENCE_PACK = ROOT / "configs/releases/fin_ia_0_1_2_s4_t05_c_mu_current_evidence_pack_v1_0.json"
AGENT_INPUT = ROOT / "configs/releases/fin_ia_0_1_2_s4_t05_c_mu_agent_exact_input_v1_0.json"
EXPECTED_RESULT_SHA256 = "c7bdf239e3f6fe1e980be856448ae7549e3401276c91805444b41e39cd1b3602"
EXPECTED_TERMINAL_DIGEST = "0eaeb000e819de99418fd2bb0b2080c638dd4b7fade25b4552461c153e44fcd4"
OUTPUT = ROOT / "configs/releases/fin_ia_0_1_2_s4_t05_c_mu_agent_exact_live_result_and_independent_assessment_v1_0.json"


class T05CMUExactAssessmentError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise T05CMUExactAssessmentError(code)


def _load(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    _require(isinstance(value, dict), "s4_t05_c_mu_exact_json_object_required")
    return value
...
def materialize(*, recorded_at: str) -> dict[str, Any]:
    _require(_sha256(EXACT_RESULT) == EXPECTED_RESULT_SHA256, "s4_t05_c_mu_exact_result_drift")
    ...
    _require(
        ...
        "s4_t05_c_mu_exact_terminal_invalid",
    )
    captures = []
    for ref in result["capture_objects"]:
        path = RUNTIME_ROOT / "restricted-audit-objects" / ref["object_key"]
        _require(_sha256(path) == ref["digest"], "s4_t05_c_mu_capture_digest_mismatch")
        capture = _load(path)
        _require(
            ...
            "s4_t05_c_mu_capture_completion_or_secret_boundary_invalid",
        )
    ...
    _require(set(artifacts) == set(BOUNDED_AGENT_ARTIFACT_TYPES), "s4_t05_c_mu_artifact_topology_invalid")
    ...
    _require(
        manifest["case_ticker"] == verifier["entity_label"] == "MU"
        and manifest["input_digest"] == agent_input["input_digest"]
        and manifest["s4_case_runtime"]["source_grounded_input_digest"] == evidence["evidence_pack_digest"]
        ...
        == [9, 3, 9, 9],
        "s4_t05_c_mu_manifest_identity_or_topology_invalid",
    )
    projected = {
    ...
    expected = {(row["metric_family"], row["numeric_ref"]): row["value"] for row in evidence["numeric_rows"]}
    _require(projected == expected, "s4_t05_c_mu_numeric_authority_drift")
    findings = verifier["verification"]["findings"]
    _require(len(findings) == 4 and all(row["status"] == "pass" for row in findings), "s4_t05_c_mu_machine_verifier_invalid")
    ...
    body = {
        "schema_version": "fin_ia_0_1_2_s4_t05_c_mu_agent_exact_live_result_and_independent_assessment_v1_0",
        "result_id": "FIN-0.1.2-S4-T05-C-MU-AGENT-EXACT-LIVE-R1",
        ...
        "operational_finding": {
            "code": "windows_gbk_stdout_unicode_encode_after_result_materialization",
            "business_chain_impact": "none_result_and_terminal_already_immutable_success",
            "rerun_required": False,
            "read_only_inspection_workaround": "PYTHONIOENCODING=utf-8",
            "disposition": "controlled_successor_CLI_output_hardening_without_mutating_consumed_runner_binding",
        },
        "next_action": "FIN-0.1.2-S4-T05-C-MU-VERIFIED-PRODUCT-SURFACE-AND-PAIRED-READINESS-ZERO-CALL",
    }
...
def main() -> int:
    ...
    if path.exists() and path.read_text(encoding="utf-8") != rendered:
        raise T05CMUExactAssessmentError("s4_t05_c_mu_assessment_existing_output_mismatch")
