...
ISSUANCE = (
    RELEASES
    / "fin_ia_0_1_s3_t09_owner_grade_v3_fresh_exact_admission_issuance_v1_0.json"
)
DECISION = (
    RELEASES
...
def test_issued_admission_matches_frozen_v3_decision_and_factory_contract() -> None:
...
def test_issuance_is_unconsumed_and_only_new_admission_count_is_one() -> None:
    issuance = _load(ISSUANCE)
    ...
    assert authority["fresh_v3_exact_admission_issuance_authorized"] is True
    assert authority["fresh_v3_exact_admission_consumption_or_execution_authorized"] is False
...
def test_exact_binding_budget_blinding_and_execution_preconditions_are_frozen() -> None:
    ...
    assert binding["input_digest"] == decision["fresh_identity"]["input_digest"]
    assert binding["preparation_digest"] == decision["fresh_identity"]["preparation_digest"]
    assert binding["predicted_work_unit_id"] == decision["fresh_identity"]["work_unit_id"]
    assert binding["predicted_attempt_id"] == decision["fresh_identity"]["attempt_id"]
    assert binding["predicted_research_run_id"] == decision["fresh_identity"]["research_run_id"]
    assert envelope["maximum_output_tokens_total"] == 10200
    assert envelope["retry_budget"] == 0
    assert envelope["automatic_retry_repair_fallback_or_broad_rerun_allowed"] is False
...
def test_existing_runner_can_load_new_issuance_without_consuming_it() -> None:
    ...
    assert target.research_run_id == "research_run_fin01_b939a453b921cb5bcf3c2edf"
    assert target.maximum_output_tokens == 10200


def test_backlog_records_later_exact_execution_and_stops_at_root_cause_decision() -> None:
