...
- [ ] Owner 已授权的 bounded engineering 已形成 implementation candidate，但最终签发待完成：`pyproject.toml + uv.lock`单一依赖源（157 lock records，含setuptools artifact hashes）；core/control-plane/qualification与locked supply tooling分层；PostgreSQL 16.15 transaction/UNIQUE/advisory lock/restart/host-roundtrip dump-restore；Dagster PostgreSQL run/event storage；真实DELL/MU/NVDA S2 fact-mart 1,319 observations、24/24 qrels legacy/Dagster shadow exact。两个镜像已候选化为non-root、OCI revision-bound、Compose secret file与只读data合同；网络按真实client位置拆分为Windows host-runner专用loopback bridge与CI container-to-container internal network。前端nanoid high已在lock层修复。旧34/76、pre-supply 33/86/88 manifests、034026/040515和失败的094310均非final；网络合同修正后必须从新clean commit和全新combined locked env重跑。
- [x] thin Dagster adapter只编排现有 CLI，legacy入口与 rollback保留、删除量0；LangGraph因本纵切面无 checkpoint/HITL/Agent graph需求而继续 HOLD。
- [ ] `4817a556...` candidate已提交并推送，但首次exact-clean attempt因Windows host-runner/internal-network拓扑失败；先提交当前同阶段loopback bridge修正，再从新commit创建fresh locked env和新attempt。通过后才继续最终Docker只读真实数据job、两个final image SBOM/CVE、全仓与frontend回归、independent read-only review和第二次evidence/Project OS commit/push。已暴露EIA key的外部轮换仍由Owner完成；之后由Owner决定下一条有用户价值的产品vertical，禁止自动全仓Dagster化、PostgreSQL cutover或LangGraph引入。
