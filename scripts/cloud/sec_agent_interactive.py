...
PLANNER_RETRIEVAL_ROUTES = {"ledger_first", "filing_text", "8k_commentary", "milvus_semantic", "market_snapshot", "industry_snapshot", "risk_text"}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Optional DuckDB market snapshot catalog generated by scripts/market/20_build_market_snapshot_catalog.py.",
    )
    parser.add_argument(
        "--market-snapshot-id",
    ...
    parser.add_argument(
        "--industry-snapshot-db-path",
        default=os.environ.get("INDUSTRY_SNAPSHOT_DB_PATH", ""),
        help="Optional DuckDB industry snapshot catalog generated by scripts/industry/20_merge_industry_source_snapshots.py.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--market-industry-manifest-summary-path",
...
def _write_run_data_fingerprint(
    ...
    payload = {
        ...
        "inputs": {
            ...
            "market_catalog": _path_fingerprint(_repo_path(getattr(args, "market_catalog_path", "")), kind="large_file")
            if getattr(args, "market_catalog_path", "")
            else None,
            ...
            "industry_snapshot_db": _path_fingerprint(_repo_path(getattr(args, "industry_snapshot_db_path", "")), kind="large_file")
            if getattr(args, "industry_snapshot_db_path", "")
            else None,
        },
        "runtime_knobs": {
    ...
    *,
    ...
    return SecAgentState.create(
        ...
        metadata={
            ...
            "effective_context_runner": _context_runner_mode(args),
            "market_evidence_path": args.market_evidence_path or "",
            "market_catalog_path": getattr(args, "market_catalog_path", "") or "",
            "industry_evidence_path": getattr(args, "industry_evidence_path", "") or "",
            "industry_snapshot_db_path": getattr(args, "industry_snapshot_db_path", "") or "",
            "max_context_rows": args.max_context_rows,
            "reranker_top_k": getattr(args, "reranker_top_k", None),
            "reranker_candidate_limit": getattr(args, "reranker_candidate_limit", None),
            "reranker_batch_size": getattr(args, "reranker_batch_size", None),
...
def _load_manifest_index_cached(path: Path) -> dict[tuple[str, int, str], dict[str, Any]]:
    ...
    for row in rows:
        ...
        if ticker and year is not None and form:
            manifest_index[(ticker, int(year), form)] = row
...
def _form_type_from_source_id(value: Any) -> str:
    match = re.search(r"(?:^|[^A-Z0-9])(?P<form>10-?K|10-?Q|8-?K|20-?F|40-?F|6-?K)(?:[^A-Z0-9]|$)", str(value or "").upper())
    if not match:
        return ""
    form = _normalize_form_type(match.group("form"))
    return form if form in {"10-K", "10-Q", "8-K", "20-F", "40-F", "6-K"} else ""
...
def _project_inventory(args: argparse.Namespace, manifest_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return build_project_inventory(
        ...
        market_catalog_path=getattr(args, "market_catalog_path", ""),
        market_snapshot_id=args.market_snapshot_id,
        market_as_of_date=args.market_as_of_date,
        industry_evidence_path=args.industry_evidence_path,
        industry_snapshot_db_path=getattr(args, "industry_snapshot_db_path", ""),
...
def _query_planner_system_prompt(project_inventory: dict[str, Any], tickers: list[str], years: list[int]) -> str:
    ...
    return (
        ...
        "- evidence_routes 只能来自 ledger_first、filing_text、8k_commentary、milvus_semantic、market_snapshot、industry_snapshot、risk_text。财务数值、capex、cash flow、margin、RPO、银行指标等走 ledger_first；10-K/10-Q 管理层讨论和业务解释走 filing_text；8-K 业绩新闻稿和 Exhibit 99 管理层口径走 8k_commentary；Milvus typed semantic 只能作为 SEC filing 语义补召回 route 走 milvus_semantic，不得替代 ledger_first；市场/估值/相对收益走 market_snapshot；宏观/行业背景/监管/利率/大宗商品/需求环境走 industry_snapshot；风险和反证走 risk_text。\n"
...
def _config_summary(args: argparse.Namespace) -> dict[str, Any]:
    return {
        ...
        "market_catalog_path": getattr(args, "market_catalog_path", "") or None,
        "industry_evidence_path": args.industry_evidence_path or None,
        "industry_snapshot_db_path": getattr(args, "industry_snapshot_db_path", "") or None,
        "bm25_index_dir": args.bm25_index_dir,
        "object_bm25_index_dir": args.object_bm25_index_dir,
        "bge_model": args.bge_model,
        "bge_device": args.bge_device,
