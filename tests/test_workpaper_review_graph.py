...
def test_real_a1_feedback_identifies_only_invalid_claim_quotes_and_preserves_accepted_span():
    from sec_agent.agent_runtime.specialist_graph import SubmitWorkpaperAction, _submission_errors
    ...
    with open_specialist_scripted_qualification_composition(run_id="test:a1-successor", run_invocation_id="test:a1-successor:1",
...
def test_actual_sdk_reviewer_tool_schema_and_actor_history_isolation(defect):
    ...
    assert len(wires) == 4
    # This fixture binds only evidence and finance ports. The frozen adapter
    # advertises allowed actions, so unavailable readers must not reach the SDK.
    assert all({t["function"]["name"] for t in w["tools"]} == {
        "RequestEvidenceAction", "RequestFinanceAction", "RequestHumanReviewAction", "SubmitReviewAction"} for w in wires), [
            sorted(t["function"]["name"] for t in wire["tools"]) for wire in wires
        ]
...
def test_real_r11_seed_handoff_approved_composition_no_model_no_rewrite():
    ...
    from sec_agent.agent_runtime.specialist_composition import (
        open_specialist_receipted_composition, open_specialist_scripted_qualification_composition)
    ...
    def turn(request):
        ...
        return _review(request)
    with open_specialist_receipted_composition(run_id="review-test", run_invocation_id="review-test-invocation",
    ...
    def real_mcp_driver(request):
        ...
        return _review(request)
    with open_specialist_scripted_qualification_composition(run_id="review-mcp-test", run_invocation_id="review-mcp-invocation",
