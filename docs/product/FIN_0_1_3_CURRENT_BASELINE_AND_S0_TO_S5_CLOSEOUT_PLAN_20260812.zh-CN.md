...
25. **DELL 五单元动态完整案例（R4 保留三份有效 Judgment，submission authority 待结构关闭）**：有限 S2 回归、五个 RoleMethodPack、本案即时 GraphContextPack 和稳定 runner 已资格化。R1 的容量矛盾、R2 三个分析节点预算耗尽、R3 交卷表面失败均按原 attempt 不可变保留；紧凑分析视图已让三个失败分析全部自然收敛，现金转换 Judgment 通过。R4 又精确复用三份有效 Judgment 和两份分析草稿，只做 value/counter 两次 strict Beta 交卷；远端均返回 Tool Call，但真实参数分别违反真实 wire Schema 的 `pattern`，本地正确拒绝。更重要的是，value 仍把 AI 服务器组合因果归为公司毛利率下降，而 typed product-profit bridge 不存在；counter 则只有 `limit` Evidence 却选择正向 `mixed` 权威。当前最早责任层是 S3 submission-authority，而非网络、S1/S2、Skill 或 Graph。下一步先零调用把远端 strict 降为形状辅助、隔离分析草稿中的数字／来源权威表面、由 Evidence role／gap 确定性编译状态，并增加产品／分部到利润率的双向 causal bridge gate；通过 replay、mutation、全仓、clean push 和新范围门后，才允许只修受影响节点的新 attempt。即使完成五单元，也仍须依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容；不得自动等同 S3 acceptance。
