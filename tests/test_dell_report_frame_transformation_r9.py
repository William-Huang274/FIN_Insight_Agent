...
from retrieval.dell_report_frame_transformation_r9 import (
    FrameTransformationBindingR9Error,
    build_frame_transformation_binding_r9,
    transformation_binding_digest_r9,
    validate_frame_transformation_binding_r9,
    validate_predicate_frame_r9_integrity,
)
...
def test_r9_source_compiled_binding_accepts_semantic_equivalence_with_different_representation(
    target_id: str,
    source_text: str,
    compiled_text: str,
) -> None:
    ...
    binding = build_frame_transformation_binding_r9(
        canonical_source_family_id="FAMILY::DELL::R9::001",
        source_record_id="SOURCE::DELL::R9::001",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R9::001"],
        compiled_window_ids=["WINDOW::DELL::R9::001"],
    ...
    assert len(binding.role_mappings) == (
        len(source.role_bindings) + len(source.scope_bindings)
    )
    validate_frame_transformation_binding_r9(binding.as_dict())
...
def test_r9_transformation_diagnostics_and_gate_reject_semantic_role_loss() -> None:
    source = _frame(
        ASP_TARGET,
        "Dell offered PowerEdge hardware for USD 15 in FY2026.",
    )
    ...
    diagnostic = build_frame_transformation_binding_r9(
        canonical_source_family_id="FAMILY::DELL::R9::002",
        source_record_id="SOURCE::DELL::R9::002",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R9::002"],
        compiled_window_ids=["WINDOW::DELL::R9::002"],
    ...
    assert diagnostic.loss_flags == (
        "unmapped_source_role:price:15:1",
    )
    assert diagnostic.addition_flags == (
        "unmapped_compiled_role:price:16:1",
    )
    assert "semantic_signature_mismatch" in diagnostic.ambiguity_flags
    with pytest.raises(
        FrameTransformationBindingR9Error,
        match="R9_transformation_not_lossless",
    ):
        build_frame_transformation_binding_r9(
            canonical_source_family_id="FAMILY::DELL::R9::002",
            source_record_id="SOURCE::DELL::R9::002",
...
def test_r9_transformation_recomputes_frame_integrity_before_trusting_digest() -> None:
    frame = _frame(
        SUPPLIER_TARGET,
        "NVIDIA provides GPUs to Dell.",
    )
    validate_predicate_frame_r9_integrity(frame)
    tampered = replace(frame, semantic_signature_digest="0" * 64)
    with pytest.raises(
        FrameTransformationBindingR9Error,
        match="semantic_digest_invalid",
    ):
        validate_predicate_frame_r9_integrity(tampered)


def test_r9_transformation_binding_set_digest_is_order_independent() -> None:
    source = _frame(
        SUPPLIER_TARGET,
        "Context. NVIDIA provides GPUs to Dell.",
    )
    compiled = _frame(SUPPLIER_TARGET, "NVIDIA provides GPUs to Dell.")
    first = build_frame_transformation_binding_r9(
        canonical_source_family_id="FAMILY::DELL::R9::003",
        source_record_id="SOURCE::DELL::R9::003",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R9::003"],
        compiled_window_ids=["WINDOW::DELL::R9::003"],
        compiled_frame=compiled,
        transformation_type="bounded_window",
    )
    second = build_frame_transformation_binding_r9(
        canonical_source_family_id="FAMILY::DELL::R9::004",
        source_record_id="SOURCE::DELL::R9::004",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R9::004"],
        compiled_window_ids=["WINDOW::DELL::R9::004"],
        compiled_frame=compiled,
        transformation_type="many_object_same_source",
    )
    assert transformation_binding_digest_r9([first, second]) == (
        transformation_binding_digest_r9([second, first])
    )
