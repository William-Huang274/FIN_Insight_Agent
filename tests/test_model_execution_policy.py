...
from sec_agent.agent_runtime.model_execution_policy import (
...
def _authority(tmp_path: Path) -> ModelExecutionAuthority:
    ...
    return ModelExecutionAuthority.model_validate(
...
def test_authority_rejects_artifact_root_not_bound_to_execution(
    ...
    with pytest.raises(ValidationError, match="artifact_execution_binding"):
        ModelExecutionAuthority.model_validate_json(
            json.dumps(
                {**body, "decision_digest": canonical_sha256(body)},
                ensure_ascii=False,
            )
...
def _lead_authority(tmp_path):
    ...
    return ModelExecutionAuthority.model_validate_json(json.dumps({**body, "decision_digest": canonical_sha256(body)}))
...
def test_lead_scope_is_explicit_and_keeps_old_authorities_readable(tmp_path, defect):
    ...
    if defect:
        with pytest.raises(ValidationError): ModelExecutionAuthority.model_validate_json(encoded)
    else:
        assert ModelExecutionAuthority.model_validate_json(encoded).lead_scope.max_parallel_tasks == 2
        assert _authority(tmp_path).lead_scope is None


@pytest.mark.parametrize("defect", [None, "missing", "duplicate", "capacity"])
def test_full_scope_is_explicit_and_old_two_topic_files_do_not_expand(tmp_path, defect):
    from sec_agent.agent_runtime.model_execution_policy import FULL_RESEARCH_BRANCHES
    ...
    body["lead_scope"].update(allowed_branch_ids=list(FULL_RESEARCH_BRANCHES), max_tasks=12)
    body["live_external_calls_authorized"] = True
    if defect == "missing": body["lead_scope"]["allowed_branch_ids"].pop()
    if defect == "duplicate": body["lead_scope"]["allowed_branch_ids"].append(FULL_RESEARCH_BRANCHES[0])
    if defect == "capacity": body["lead_scope"]["max_tasks"] = 4
    encoded = json.dumps({**body, "decision_digest": canonical_sha256(body)})
    if defect:
        with pytest.raises(ValidationError): ModelExecutionAuthority.model_validate_json(encoded)
    else:
        assert len(ModelExecutionAuthority.model_validate_json(encoded).lead_scope.allowed_branch_ids) == 9
...
def test_lead_can_use_non_thinking_native_history_without_changing_worker(tmp_path):
    ...
    authority = ModelExecutionAuthority.model_validate_json(json.dumps({**body, "decision_digest": canonical_sha256(body)}))
...
def test_targeted_completion_is_explicit_bounded_and_not_full_case(tmp_path, branches, valid):
    ...
    if valid:
        parsed = ModelExecutionAuthority.model_validate_json(json.dumps(body))
        assert parsed.lead_scope.allowed_branch_ids == ("Q8_COMPETITION_VALUE_POOL",)
    else:
        with pytest.raises(ValueError):
            ModelExecutionAuthority.model_validate_json(json.dumps(body))
