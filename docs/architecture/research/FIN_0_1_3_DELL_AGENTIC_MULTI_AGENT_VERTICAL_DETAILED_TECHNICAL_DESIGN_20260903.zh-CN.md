...
当前执行入口（2026-09-07 修后真实接续收口）：本文件 [原五项＋新增需求剩余工作](#remaining-work-20260907) 的步骤一与 S3/190 文末。Owner 明确允许真实调用后，单分支 `20260907_context_postfix_summary_a1` 在 `55619b57` 完成：1 Flash 摘要＋3 Pro 接续，4 调用、263,171 tokens、估 1.710333 元，0 重试/未知/截断。模型自主四次旧 CALC 回读全部成功，保留数字/期间相符；最终答复可保存，但只保留 1 个正式 CALC 引用，其他被拒的 3 CALC/6 NUMFACT 变成裸 ID，数字断言仍在，且把“未验证”说成“未通过”，错误要求新 ID。因此只确认回读整改和数字保留，内容验收 HOLD；摘要默认 disabled、原 v3 needs_revision，不进入步骤二/P02/前端。4 个 LangSmith LLM span 与本地输入/输出/总量一致。新批已结束、不追加 paid；下一用保存反例核对历史窗口、完整计算对象、引用登记和反馈语义，不能为让测试通过而把说明文字造为新 CALC 或扩写 NLP 规则。旧批 6 调用/3.4251572 元、局部编辑对照 0.5116455 元及原研究 265/264 已知/28.0927149 元均单列不改，历史 LangSmith 缺额 190,815 根因不因此关闭。以下逐时点状态仅供追溯。
...
- [ ] 真实小批已完成修后接续：模型自主四次历史 CALC 回读成功，数字/期间保留；旧文字 CALC 仍只提供历史窗口，未捏造成可执行对象。最终答复撤掉 3 CALC/6 NUMFACT 的正式引用后保留数字断言，当前引用合同/模型反馈还需核对；局部编辑语义问题亦未关闭，不宣称同质量通过。
