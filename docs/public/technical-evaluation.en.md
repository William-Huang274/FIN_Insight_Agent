...
Retrieval combines BM25, Qwen text-embedding-v4 and qwen3-rerank, with completed-request caching. The existing SEC companyfacts parser and SQLite fact mart supply observations and derived metrics with issuer, period, unit, formula and source identity. OIDC/PKCE and server-side ownership checks protect product resources. Docker and native approval recovery are qualified separately from the currently blocked product sandbox deployment.
...
Docker isolation passed three real-container scenarios. Four approval scenarios exercised checkpoint reopening, HTTP MCP and Docker execution, including denial with zero execution and explicit approval before execution. **The workspace sandbox service on port 18796 has not been deployed: host automatic approval review blocked its startup/integration.** Isolated execution qualification is not reported as a deployed product security feature.
