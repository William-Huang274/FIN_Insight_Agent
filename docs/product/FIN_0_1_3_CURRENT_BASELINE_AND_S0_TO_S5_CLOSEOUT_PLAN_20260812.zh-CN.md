...
25. **DELL 五单元动态完整案例（R7 完整物化，Case Truth claim-polarity successor 待正式复证）**：R7 复用 R6 上游，只执行 Value repair 和两次 synthesis，共 3 次调用、0 retry，首次形成五个 Judgment、综合和完整内部报告。合同、身份、期间、数值、引用与产品利润桥边界通过，但 Operating／Counterevidence 错误否认当前 Case 已有的 AI revenue、orders、backlog，综合又制造 false conflict，故内容 L1/L2 失败。Case Truth split 零调用 R3 已证明五个三-surface slice 与本地聚合；natural R1 的 max-thinking 零输出已由 R2 non-thinking 可见输出关闭，但 R2 又证明旧 `asserted_state` 混淆 claim 与 authoritative truth、缺少跨公司 context，并因 supporting-fact 枚举造成 Operating 截断和 Counter 14 条 mixed finding。current successor 改为 claim polarity、分层 alias view 和每 surface 最多 12 个直接命题。下一步先做 R2 capture replay、三案／留出、context／cross-cell／容量 formal zero-call；通过后最多一次 Operating／Counterevidence 四调用 natural successor。通过后才考虑受影响 Judgment 与 Synthesis；即使修复报告形成，仍须完整执行 L1/L2、八维、paired 和 qualified-human。
...
4. 15-surface semantic R1 与两单元 R1 的 reasoning exhaustion 均保持不可变；专用 non-thinking R2 已让两份 analysis 正常可见，但 Operating submission 在 2k 截断，Counter submission 的 14 条 finding 混合了 polarity 误判、alias 误选、缺少 context 状态和真实 cross-cell leakage。当前先完成 claim-polarity＋tiered alias view＋direct-proposition capacity 的正式零调用复证；通过后只允许同两个 cell 各一条 analysis＋submission，共最多四次调用、0 retry。通过前不运行剩余三 cell 或修复 Judgment／Synthesis；Demand、Value、Cash、Planner、S1/S2 与五个原研究 analysis 保持不可变。
