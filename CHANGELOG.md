...
- 长会话通过原生 GET 游标接口读取更早报告版本，修复 API 0.13.3 POST 分页接缝；实际 v3/v4 差异已读回。成功 SQL 的 typed_gap 可作为非事实查询回执引用，仍不能用作计算输入或证明发行人未披露。
- 修复正文裸写 CALC/来源 ID 漏绑定：复用 MarkdownIt/remark，将正文ID交给同一来源校验，只链接服务端已绑定引用，保留代码和原链接；原答案与金融语义意见不自动改写或追认。
...
本轮逐项结果：[原五项与新增需求审阅清单](docs/product/FIN_0_1_3_OWNER_REVIEW_20260908.zh-CN.md)。八个不同短问题已保存答案，另两次失败保留，合计49调用/1,106,250 tokens/0.405444元；来源可回读与研究语义通过分别记录。视频为本地Owner审阅材料，不自动公开完整报告。
