...
DECISION_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_nvda_raw_replacement_r2_authority_v1_0.json"
)
R1_RESULT_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_nvda_raw_r1_terminal_and_numeric_scale_disposition_v1_0.json"
)
...
DEFAULT_AUTHORITY_ROOT = (
    ...
    / "fin013_s2_05"
    / "authorities"
    / "NVDA_RAW_REPLACEMENT_R2"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Issue one NVDA raw replacement R2 same-evidence admission"
    )
    ...
    r1_result = _read_json(ROOT / R1_RESULT_REF)
    _validate_decision(decision, r1_result)
    _validate_repository(decision)
    _validate_frozen_bindings(decision)

    policy = load_runtime_policy(ROOT)
    blind = load_frozen_blind_inputs(ROOT, policy)
    case_input = next(row for row in blind["cases"] if row["case_key"] == "NVDA")
    fairness = decision["fairness_guard"]
    if (
        case_input["model_visible_digest"] != fairness["NVDA_model_visible_digest"]
        or blind["blind_input_digest"] != fairness["frozen_blind_input_digest"]
    ):
        raise RuntimeError("experiment_a_nvda_r2_frozen_visible_input_mismatch")
    ...
    output = {
        ...
        "prior_case_raw_correction_or_hidden_gold_read": False,
    }
    if not args.dry_run:
        authority_root = args.authority_root.resolve()
        if authority_root != DEFAULT_AUTHORITY_ROOT.resolve():
            raise RuntimeError("experiment_a_nvda_r2_authority_root_invalid")
        authority_root.mkdir(parents=True, exist_ok=True)
        if list(authority_root.glob("*.json")):
            raise RuntimeError("experiment_a_nvda_r2_admission_already_exists")
...
def _validate_decision(decision: dict[str, Any], r1_result: dict[str, Any]) -> None:
    ...
    r1_root = r1_result.get("root_cause") or {}
    r1_campaign = r1_result.get("campaign_disposition") or {}
    if (
        decision.get("decision_digest") != canonical_digest(body)
        or decision.get("status")
        != "NVDA_raw_replacement_R2_admission_and_execution_authorized_not_issued"
        ...
        or authority.get("maximum_execution_attempts") != 1
        or authority.get("authorized_case") != "NVDA"
        or authority.get("replacement_ordinal") != "R2"
        or authority.get("maximum_provider_calls") != 12
        or authority.get("retry_count") != 0
        or authority.get("fallback_count") != 0
        or authority.get("automatic_replacement_or_rerun") is not False
        ...
        or fairness.get("model_visible_contract_changed_after_R1") is not False
        or fairness.get("prior_case_raw_or_correction_visible") is not False
        or fairness.get("hidden_gold_visible") is not False
        or r1_result.get("status")
        != "NVDA_raw_r1_terminal_failed_project_numeric_scale_false_positive_repaired_zero_call_replacement_authority_pending"
        or r1_root.get("runtime_false_positive") is not True
        or r1_root.get("model_authored_unbound_financial_value") is not False
        or r1_campaign.get("automatic_replacement_or_rerun") is not False
        or r1_campaign.get("replacement_authority_may_be_considered_separately") is not True
        or storage.get("required_root")
        != ".codex_runtime/fin013_s2_05/authorities/NVDA_RAW_REPLACEMENT_R2"
    ):
        raise RuntimeError("experiment_a_nvda_r2_replacement_authority_invalid")


def _validate_repository(decision: dict[str, Any]) -> None:
    if _git("status", "--porcelain"):
        raise RuntimeError("experiment_a_nvda_r2_repository_not_clean")
    head = _git("rev-parse", "HEAD")
    audited = str(decision["audited_repository_state"]["head"])
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", audited, head], cwd=ROOT, check=False
    ).returncode != 0:
        raise RuntimeError("experiment_a_nvda_r2_head_not_descendant")
    upstream = _git("rev-parse", "--abbrev-ref", "@{upstream}")
    if _git("rev-list", "--left-right", "--count", f"{upstream}...HEAD").split() != ["0", "0"]:
        raise RuntimeError("experiment_a_nvda_r2_repository_not_synced")
...
def _validate_frozen_bindings(decision: dict[str, Any]) -> None:
    for binding in decision["frozen_bindings"].values():
        ...
        if not ref or not expected or _sha256(ROOT / ref) != expected:
            raise RuntimeError("experiment_a_nvda_r2_frozen_binding_mismatch")


def _read_json(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("experiment_a_nvda_r2_decision_json_invalid")
    return value
