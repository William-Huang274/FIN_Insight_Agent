...
const fixture=process.env.FIN_PROJECT_REPORT_SOURCE?'project_report_web':'project_library_web';
if(!state) throw new Error('Use a new isolated FINSIGHT_LOCAL_STATE_ROOT for each attempt.');
export default defineConfig({
  testDir:'./e2e',testMatch:['project-library.spec.ts','project-sec.spec.ts','project-report.spec.ts'],workers:1,retries:0,reporter:'list',
  ...
  webServer:[
    {command:`"${python}" -m uvicorn tests.qualification.fixtures.${fixture}:app --host 127.0.0.1 --port 8767`,cwd:repo,url:'http://127.0.0.1:8767/auth/status',reuseExistingServer:false,timeout:60000},
