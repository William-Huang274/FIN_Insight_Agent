...
- [ ] 修后模型自主内容验收仍未完成：上轮真实小批虽四次旧 CALC 回读成功，最终撤掉 3 CALC/6 NUMFACT 方括号并保留数字断言，未验证/未通过及新 ID 表述亦错。当前接口和状态反馈已修、原样重放通过，但这不能证明模型实际采纳；局部编辑语义问题也未关闭，不宣称同质量通过。
- [ ] 原生摘要已经真实生成并完成后续模型接续，4 调用/263,171 tokens/估 1.710333 元；原 messages/证据保留和回读有实测，引用完整性及“未验证”状态表述未过，故 `context_summarization.enabled=false`。不凭首轮输入降至 75,126 tokens 宣称整轮同质量省费。
