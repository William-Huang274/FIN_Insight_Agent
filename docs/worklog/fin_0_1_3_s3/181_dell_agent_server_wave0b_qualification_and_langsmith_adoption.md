...
状态：`ADOPT_DIRECTION_OWNER_APPROVED / DEV_ONLINE_QUALIFIED / NO_RUNTIME_FALLBACK / LANGGRAPH_UP_KEY_AND_RESTART_PARITY_PENDING / RC-S3-105_OPEN / A03_ABSENT`
...
“不商用”不等于免除 key/license。官方当前路径是：本地 `langgraph up` 测试需要可访问 LangSmith 的 `LANGSMITH_API_KEY`；生产 self-host 需要 `LANGGRAPH_CLOUD_LICENSE_KEY`。免费 LangSmith 账户可以创建 API key，官方也允许本地 Agent Server 免费用于 testing/development。因此合法的 Developer PAT 是下一次本地 `up` 资格测试的配置前置，而不是改造或绕开许可证的理由。
...
Developer 套餐当前为零席位费并提供基础 trace 配额，但没有 LangSmith 托管 Deployment；这不妨碍本机 `langgraph up` development/testing。Plus 的托管 serverless entitlement 与本机 `up` 不是同一能力。若未来把演示变为长期公网或真实用户服务，即使免费，也必须重新核 production 许可，不能沿用本记录的非生产结论。
...
解释：`langgraph dev` 的 in-memory 开发 runtime 足以证明在线 API、并发、interrupt/resume、cancel 和同进程断线续传，但不能证明 completed stream 在进程重启后的历史 replay。官方文档保证 `stream_resumable=true` 与 `Last-Event-ID` 的断线续传，但没有把完整 Compose 停止/重启、Redis 重启、retention TTL 之后的 replay 写成稳定保证；因此这些项目必须在固定版本的 `langgraph up` 上实测。
...
本机 Process/User/Machine 三个 scope 均未配置 `LANGSMITH_API_KEY`、`LANGCHAIN_API_KEY` 或 `LANGGRAPH_CLOUD_LICENSE_KEY`。因此本记录没有运行、也没有声称运行 `langgraph up`。

下一次部署资格测试必须：

1. 由用户在免费 LangSmith 账户创建合法 PAT，并通过本机环境或 ignored secret file 配置；密钥不得粘贴到聊天、Git、receipt 或日志；
2. 保持 `LANGSMITH_TRACING=false`，仅把 PAT 用于本地开发资格验证；
...
Wave 0B 已经完成“选哪条路”的裁决和 dev 在线语义证明；`langgraph up` deployment parity 仍是并行阻断门。为避免等待 key 时再次停在文档上，下一项零模型工程工作立即回到原最早责任层 `RC-S3-105`：
...
`RC-S3-105` 仍阻断 A03 和任何新模型/provider/paid successor。当前没有 Agent Server mainline adapter、真实 S1/S2 tool loop、multi-agent、HITL 前端或 Dell 最终报告，不能把本轮 runtime qualification 写成产品纵切完成。
