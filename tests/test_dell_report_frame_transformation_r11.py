...
from retrieval.dell_report_frame_transformation_r11 import (
    FrameTransformationBindingR11Error,
    build_frame_transformation_binding_r11,
    transformation_binding_digest_r11,
    validate_frame_transformation_binding_r11,
    validate_predicate_frame_r11_integrity,
)
...
def test_r11_source_compiled_binding_accepts_semantic_equivalence_with_different_representation(
    target_id: str,
    source_text: str,
    compiled_text: str,
) -> None:
    ...
    binding = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::001",
        source_record_id="SOURCE::DELL::R11::001",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::001"],
        compiled_window_ids=["WINDOW::DELL::R11::001"],
    ...
    if target_id == ASP_TARGET:
        ...
        } == {"object", "product", "connector", "price"}
        assert {
            row[0] for row in relation_mapping.compiled_proof_spans
        } == {"object", "product", "connector", "price"}
    validate_frame_transformation_binding_r11(binding.as_dict())
...
def test_r11_transformation_diagnostics_and_gate_reject_semantic_role_loss() -> None:
    source = _frame(
        ASP_TARGET,
        "Dell offered PowerEdge hardware for USD 15 in FY2026.",
    )
    ...
    diagnostic = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::002",
        source_record_id="SOURCE::DELL::R11::002",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::002"],
        compiled_window_ids=["WINDOW::DELL::R11::002"],
    ...
    with pytest.raises(
        FrameTransformationBindingR11Error,
        match="R11_transformation_not_lossless",
    ):
        build_frame_transformation_binding_r11(
            canonical_source_family_id="FAMILY::DELL::R11::002",
            source_record_id="SOURCE::DELL::R11::002",
...
def test_r11_relation_loss_rejects_cross_group_product_price_compilation() -> None:
    source = _frame(
        ASP_TARGET,
    ...
    diagnostic = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::RELATION",
        source_record_id="SOURCE::DELL::R11::RELATION",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::RELATION"],
        compiled_window_ids=["WINDOW::DELL::R11::RELATION"],
        compiled_frame=compiled,
        transformation_type="normalized_slice",
        require_lossless=False,
    )
...
def test_r11_price_attachment_proof_type_change_is_semantic_drift() -> None:
    source = _frame(
        ASP_TARGET,
        "Dell offered PowerEdge XE9680 hardware for USD 15.",
    )
    ...
    assert source.semantic_signature_digest != compiled.semantic_signature_digest
    diagnostic = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::PROOF-TYPE",
        source_record_id="SOURCE::DELL::R11::PROOF-TYPE",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::PROOF-TYPE"],
        compiled_window_ids=["WINDOW::DELL::R11::PROOF-TYPE"],
        compiled_frame=compiled,
        transformation_type="normalized_slice",
        require_lossless=False,
    )
...
def test_r11_clause_ownership_proof_loss_rejects_transformation() -> None:
    source = _frame(
        SUPPLIER_TARGET,
    ...
    assert source.semantic_signature_digest != compiled.semantic_signature_digest
    diagnostic = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::OWNER-PROOF",
        source_record_id="SOURCE::DELL::R11::OWNER-PROOF",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::OWNER-PROOF"],
        compiled_window_ids=["WINDOW::DELL::R11::OWNER-PROOF"],
        compiled_frame=compiled,
        transformation_type="normalized_slice",
        require_lossless=False,
    )
...
def test_r11_equal_proofs_map_provenance_even_when_absolute_spans_shift() -> None:
    source = _frame(
        SUPPLIER_TARGET,
    ...
    assert source.semantic_signature_digest == compiled.semantic_signature_digest
    binding = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::OWNER-MAP",
        source_record_id="SOURCE::DELL::R11::OWNER-MAP",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::OWNER-MAP"],
        compiled_window_ids=["WINDOW::DELL::R11::OWNER-MAP"],
        compiled_frame=compiled,
        transformation_type="bounded_window",
    )
    proof_mapping = next(
...
def test_r11_transformation_recomputes_frame_integrity_before_trusting_digest() -> None:
    frame = _frame(
        SUPPLIER_TARGET,
        "NVIDIA provides GPUs to Dell.",
    )
    validate_predicate_frame_r11_integrity(frame)
    tampered = replace(frame, semantic_signature_digest="0" * 64)
    with pytest.raises(
        FrameTransformationBindingR11Error,
        match="semantic_digest_invalid",
    ):
        validate_predicate_frame_r11_integrity(tampered)


def test_r11_transformation_recomputes_price_attachment_proof_digest() -> None:
    frame = _frame(
        ASP_TARGET,
        "Dell offered PowerEdge XE9680 hardware for USD 15.",
    )
...
def test_r11_transformation_recomputes_clause_ownership_proof_digest() -> None:
    frame = _frame(
        SUPPLIER_TARGET,
        "NVIDIA provides GPUs and in Q2 ships them to Dell.",
    )
...
def test_r11_transformation_binding_set_digest_is_order_independent() -> None:
    source = _frame(
        SUPPLIER_TARGET,
        "Context. NVIDIA provides GPUs to Dell.",
    )
    compiled = _frame(SUPPLIER_TARGET, "NVIDIA provides GPUs to Dell.")
    first = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::003",
        source_record_id="SOURCE::DELL::R11::003",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::003"],
        compiled_window_ids=["WINDOW::DELL::R11::003"],
        compiled_frame=compiled,
        transformation_type="bounded_window",
    )
    second = build_frame_transformation_binding_r11(
        canonical_source_family_id="FAMILY::DELL::R11::004",
        source_record_id="SOURCE::DELL::R11::004",
        source_frame=source,
        compiled_object_ids=["OBJECT::DELL::R11::004"],
        compiled_window_ids=["WINDOW::DELL::R11::004"],
        compiled_frame=compiled,
        transformation_type="many_object_same_source",
    )
    assert transformation_binding_digest_r11([first, second]) == (
        transformation_binding_digest_r11([second, first])
    )
