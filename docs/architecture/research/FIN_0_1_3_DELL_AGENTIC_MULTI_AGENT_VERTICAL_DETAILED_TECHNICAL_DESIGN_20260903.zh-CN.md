...
当前执行入口（2026-09-08，投递优先）：Owner 已明确先完成原五项及新增需求、收齐 GitHub 展示面，再考虑 Hermes 接入。步骤一已收口；下一推进报告责任/关键判断与计算的修订，随后交互、上传视觉和统一交付，公开展示准备可穿插。唯一剩余需求清单仍是下方 §0，不新开产品版本或架构规划。Hermes 为投递交付后的候选，不阻塞当前工作。
...
- [ ] 先离线核P02:C12/thesis/narrative、正文及相关来源；修订要同步相关研究产物，不手改旧底稿/报告、不预设研究观点。原有9稿/失败/所有版本保留。
- [ ] 在第一步验证后的局部路径完成必要作者修订与独立定向核验；其余八面不重研究。若关键问题超出当前P02，按来源/计算/研究/写作归责，不把所有残差塞给Writer。
- [ ] 从实际trace/底稿做六组方法的retain/adapt/hold及消费记录：未读、只读目录、读方法、分析中有应用分别写；不要求每角色机械读所有方法，不把答案写入Skill。
- [ ] 人工重点核查约15–20重要主张和全部影响核心判断的计算，覆盖增长、利润、现金、执行要求、同业、反证和改变判断条件。记录正确/错误/不足/未评估，开发抽查不是随机准确率、盲测或专业投研认证。
- [ ] 将秋招只读复核指出的具体措辞纳入同一核查：利润“组合/杠杆而非AI自身”的排他性是否有充分拆解；公司现金/DFS变化能否归因AI；“中个位数利润率目标”既不冒充独立披露的精确实现值，也不得反向否认来源称符合目标的实际表现陈述；分别核五月FY27 Q1管理层与三月第三方的时间和口径。CFO接近净利润、ISG约15%等触发线说明是基准/目标/研究假设，不包装成验证过的预测模型。有证据则保留有分量判断，依据不够则补核或针对性限定，不将正文改回通篇边界说明。一般性方法提示已修/零模型消费已证，真实输出效果与原稿修订尚未完成。
- [ ] 需要补源时，复用现有SQL、结构检索、原文窗口与Exa；先宿主用同一工具读到，再交模型。SQL覆盖范围如实列，非SQL披露可计算但不升为NumericFact。只修发现的资料/解析问题，不重选整套RAG。

交付：一份值得阅读且关键判断有依据的报告候选、逐项核查和版本差异。若仍有重大错误如实待修订，不用反复全稿生成或清零全部advisory替代产品判断。

**三、完成对话式前端与真实人工交互。**

- [ ] 基于现有React、BFF、原生stream实现左历史/中对话/右成果；阅读宽版、窄窗口标签；当前case名称来自真实配置，不把支持范围伪装成任意公司。
- [ ] 新研究、快问、深问、局部修订清晰分流；来源卡片/原文定位与正文联动，版本选择和真实文字差异采用成熟diff能力，保留修改原因/责任来源。
- [ ] 实际任务依赖、等待、工具动作、错误、回派原因可展开；意见保存/送达/应用有区分；停止后可保留产物并按原生状态继续。显示公开简明依据，不复制私有CoT或另造LangSmith。
- [ ] 本次操作与会话累计分别显示input/output/cache/估费/调用/耗时/失败及未知用量；超预期在安全点提示实际剩余工作，不将取消说成零收费。角色/工具明细用于解释成本，而不是堆满首页。
...
- [ ] 沿用当前pdfplumber/python-docx/HTML解析、LangChain分块/BM25和任务存储，选一个权利清楚的真实财务PDF或表格文件、一个相关图像/扫描页，走UI上传→解析状态→MCP检索/原文→模型使用→来源可点，不再只做合成两数探针。
- [ ] 图片由 `deepseek-v4-flash-vision-exp` 作为只读工具按需解释；普通Pro不直接冒充视觉模型。图片页码/原图和识别不确定性保留，视觉数值不自动升为SQL权威。明确样本n与非全面OCR资格。
- [ ] 无关格式/过大文件/跨任务读取/路径穿越/来源指令执行要拒绝；模型不能修改用户原文件、拿凭据或shell。当前仍可信本地使用，不额外建设公网恶意文件处理平台。
- [ ] 抽看实际chunk、标题、表格和引用错在哪里；BM25可定位就先用，不为了完成表格引入全部dense/graph/reranker。新增解析依赖只因实测缺口且按成熟栈优先。

这一切片可用独立受控小任务验证，不把上传样本悄悄塞入历史Dell结果，也不为证明视觉重跑全案。

**五、同一Dell产品链与交付文件的最终收口。**

- [ ] 将第二步的内容核查、第三步的交互、第四步的新增能力合并验收；区分新研究原始执行、明确复用底稿的修订、独立上传探针，不能相加冒充一轮无辅助成功。
- [ ] 检查图表所用数值/期间/单位/轴/来源与正文一致、是否帮助判断；保留已完成MD/PDF/Word/PPT导出，只对最终版本及受影响布局渲染核查，导出0模型调用。
- [ ] 修实际Markdown可见的交付引用缺口：[11]/[35]等CALC不能仅展示“本地来源绑定计算”，须能读到公式、输入及来源；[38]空条目和[P05]内部底稿标识须追溯到真实来源或明确可读说明。检查同一引用投影是否影响其余格式/UI，不能未查就宣称全坏；关键来源、计算和当前版本可定位属于正确性交付，不归为可延期美化。
- [ ] 固定服务配置、PG/Redis和报告版本保全；交付当前可启动状态及回退说明。检查代理/本机服务不付费，历史PID不当当前状态，不清卷/删历史。
...
- [ ] 将新研究初始化与旧Dell报告bundle的兼容依赖分离；保留明确legacy入口。新模块按职责命名，触及处整理重复active实现，不全仓无差别改名/搬目录。
- [ ] 形成明确的公开路径清单：代码/锁文件/合成fixture/脱敏demo与私有语料、SQL/index、上传、凭据、原始模型记录/CoT分离。扫描当前及拟公开历史、依赖/第三方资料许可，不因已有50路径检查就称全仓安全。
- [ ] README与公开的架构、数据/MCP、Agent/context、前后端交互、运行/回滚、评测成本/局限各层级有中英文入口；复用已有文档而非翻译全部历史worklog。独立启动给可操作的数据准备说明，不预填旧答案，不要求仓库公开原始语料。
- [ ] 在不改当前工作树的独立目录验证公开模式源码/合成检查和新研究初始化；真实provider依赖/私有数据准备如实注明。Owner确认报告选段/公开文件与仓库可见性后才发布，本次仅准备。

**七、少量新场景与秋招交付证据。**

- [ ] Dell验收后优先非Dell短任务/问答，再一个已有资料支持的有界长任务；沿同一runtime，不写第二套case引擎。原先1–2新case要求保留，不能因本轮仅计划而删除，付费规模需另核。
- [ ] 约8–12不同短问题覆盖查数、计算、跨源和合理不可回答；4–6类已有故障/权限测试复用，长任务逐例列结果/成本。样本少不报P95或泛化成功率，不重复同题刷指标。
- [ ] 一段短演示、真实架构/工具和数据边界说明、两个失败—定位—修复故事、请求级质量/成本/时延表；本人能解释一次任务及一处adapter/测试。可先整理已有证据，不等所有工作完成才学习/准备讲解。
- [ ] 招聘建议作为优先级参考，和JD明确要求分开。不给简历添加未验证省费率、生产HA/多租户/全公司或“多Agent优于单Agent”；不为关键词加K8s、消息平台或训练栈。
