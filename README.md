...
公司资料库将披露文件、检索节点和阅读入口放在同一页。研究员可以按公司、年份、文件类型和研究截止日筛选，阅读原文，再把来源用于研究。冻结基线包含 **70 份文档、66 个独立 URL、9,253 个节点和 12,994 个检索块**；可标准化的公开资料进入资料库，外部搜索与抓取补充已有库的覆盖。

*实际部署界面，2026-09-11；截图中的 60 份文件属于扩充前快照，以上 70 份为冻结时点计数。*

### 财务数字同时保留期间、单位和出处

![财务数据：公司指标、期间和来源查询](docs/public/images/financial-data.png)

财务查询支持按公司和指标查看数据，区分报告期间、申报版本、单位和原始来源。冻结范围为 **5 家公司、2,274 条财务观测**，另有 **37 项来源绑定派生指标**。计算回读展示表达式和操作数，帮助检查分母、单位与期间；算术校验通过后，金融含义仍需审阅。
...
[交互与模式测试题](eval_sets/workbench_execution_modes.json)覆盖简短追问、指定专家、跨公司单 Agent 研究、节点微修订和自由分派。用量按任务和调用记录；失败与未知费用不会被当成零成本。历史多层审查存在明显开销，冻结版本没有宣称普遍节费或无人审阅金融正确性。

### 审阅、修改，再交付同一份报告

![HPE 报告：人工确认版本、修订记录与导出入口](docs/public/images/hpe-reviewed-report.png)

研究员可以阅读专家底稿、检查结论依据、直接修订报告并保留版本差异。上图为实际 HPE 报告 v2，记录两次人工修改；右侧提供 Markdown、PDF、Word、PowerPoint 导出。内容版本与产品版本独立，人工确认也不会把此前失败的模型执行改写为成功。
...
FIN 0.1.3 已按本地研究工作台范围冻结。下面的结果分别对应有限样本，不合并成“金融正确率”：
...
| 报告交付 | 人工修订、版本差异、来源与计算回读、四格式导出。AI/内存案例形成 7 页分析师审阅报告，原生 Writer 未完成，不能称为自动全链路通过。 |

模型仍可能误读财务语义或遗漏反证。自动摘要默认关闭；动态金融判断方法库、规则更新、专家有限委派与研究期限收敛属于 **0.1.4 规划**。详细样本、失败和费用范围见[评测报告](docs/public/technical-evaluation.zh-CN.md)与[证据说明](docs/public/sharing-scope.md)。
...
当前代码树保留产品运行、数据构建、部署和回归检查所用代码。一次性实验及原 `archive/versions/` 内容已从当前树清出，使用[冻结标签](https://github.com/William-Huang274/FIN_Insight_Agent/tree/archive/fin-0.1.3-before-cleanup-20260913)恢复；[归档说明](archive/README.md)提供精确提交和操作方法。内部工作日志保留原时点决策与失败，当前对外说明以本页及 `docs/public/` 为准。仓库公开供代码与工程审阅，尚未选定统一开源许可证；第三方组件遵循各自许可证。
