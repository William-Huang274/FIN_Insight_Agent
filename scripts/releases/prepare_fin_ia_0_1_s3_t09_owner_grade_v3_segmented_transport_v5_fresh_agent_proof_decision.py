...
DECISION_STATUS = (
    "pass_fresh_segmented_transport_v5_exact_proof_contract_decided_"
    "admission_issuance_ready"
)
DECISION_CONTRACT_REF = (
    "fin01.s3.owner_grade_v3.segmented_transport_v5_fresh_agent_"
    "proof_decision:v1"
)
...
def prepare(
    *,
    ...
    result["provider_route_review"].update(
        {
            "decision": "retain_deepseek_for_one_exact_transport_v5_proof_only",
            "transport_ref": S3_OWNER_GRADE_SEGMENTED_SPECIALIST_TRANSPORT_V5_REF,
            "epistemic_status_state_machine_fixture_proven": True,
            "bounded_assembly_fixture_proven": True,
            "provider_output_capture_policy_ref": (
                S3_PROVIDER_OUTPUT_CAPTURE_POLICY_REF
            ),
            "exact_assistant_final_text_capture_required": True,
            "real_transport_v5_provider_conformance_proven": False,
    ...
    result["experiment_governance"] = {
        "hypothesis": (
            "The versioned 8192-byte assembly envelope should admit a valid segmented "
            "union while every v4 semantic, authority and field gate stays fail-closed."
        ),
        "decision_target": (
        ...
        "stop_condition": (
            ...
            "retry, fallback, patch or hidden rerun."
        ),
...
def main() -> int:
    ...
    parser.add_argument(
        "--transport-v4-live-result",
        type=Path,
        default=releases
        / "fin_ia_0_1_s3_t09_owner_grade_v3_segmented_transport_v4_fresh_live_execution_result_v1_0.json",
    )
    parser.add_argument(
        "--transport-v5-repair-result",
        type=Path,
        default=releases
        / "fin_ia_0_1_s3_t09_owner_grade_v3_segmented_transport_v5_bounded_assembly_repair_v1_0.json",
    )
    ...
    print(
        json.dumps(
            prepare(
                ...
                transport_v4_live_result_path=args.transport_v4_live_result.resolve(),
                transport_v5_repair_result_path=args.transport_v5_repair_result.resolve(),
            ),
            ensure_ascii=False,
            indent=2,
            sort_keys=True,
