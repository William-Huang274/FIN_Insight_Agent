...
SOURCE_REFS = (
    ...
    "configs/releases/fin_ia_0_1_3_s3_small_judgment_atom_projection_minimum_zero_call_implementation_v1_0.json",
    "configs/releases/fin_ia_0_1_3_s3_small_judgment_atom_projection_clean_independent_proof_v1_0.json",
    "configs/releases/fin_ia_0_1_3_s3_small_judgment_atom_successor_natural_canary_value_cost_risk_decision_v1_0.json",
    "src/sec_agent/s3_small_judgment_atom_projection.py",
    "src/sec_agent/s3_small_judgment_atom_projection_live.py",
    "scripts/releases/issue_fin_ia_0_1_3_s3_small_judgment_atom_live_admission.py",
    "scripts/releases/run_fin_ia_0_1_3_s3_small_judgment_atom_live.py",
)
...
def _require_clean_synced() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("s3_small_atom_live_issuance_requires_clean_worktree")
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise RuntimeError("s3_small_atom_live_issuance_requires_synced_head")
    return head
...
def _load(path: Path) -> dict:
    ...
    if not isinstance(value, dict):
        raise RuntimeError("s3_small_atom_live_issuance_json_invalid")
    return value


def main() -> int:
    if OUTPUT_PATH.exists():
        raise RuntimeError("s3_small_atom_live_issuance_already_exists")
    ...
    if preflight.get("status") != "pass":
        raise RuntimeError(
            "s3_small_atom_live_issuance_project_os_blocked:"
    ...
    if credential["credential_present"] is not True:
        raise RuntimeError("s3_small_atom_live_issuance_credential_missing")
    source_bindings = [
        {
            "ref": ref,
    ...
    issuance = issue_successor_live_admission(
        decision=decision,
        clean_proof=proof,
        material=material,
        ...
        user_authority=(
            "User authorized the approved six-step sequence and bounded blocker "
            "repairs. This record issues one fresh successor admission but does not "
            "authorize or start the Provider call."
        ),
    )
    validate_successor_live_issuance(
        issuance,
        ...
        json.dumps(
            {
                ...
                "attempt_id": issuance["admission"]["attempt_id"],
