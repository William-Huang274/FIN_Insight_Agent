...
from retrieval.dell_report_internal_chain_ceiling_r12 import (  # noqa: E402
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    BRANCH,
    ...
    compile_dell_report_internal_chain_ceiling_r12_result,
...
IMMUTABLE_R11_ATTEMPT_ID = "dell-rsq-03b-internal-chain-r11"
IMMUTABLE_R11_RAW_EXECUTION_SHA256 = (
    "0e9e4456ba75ecd07bc2e3bd6d5deddafc1972ba19700b029b2e6793e99f7458"
)
R12_RAW_REUSE_CAPTURE_SCHEMA_VERSION = (
    "fin_ia_dell_report_internal_chain_raw_reuse_capture_v1_0"
)
R12_RAW_REUSE_REASON = (
    "R12_changes_only_post_candidate_route_projection_semantic_compilation_"
    "and_provenance_R11_query_vectors_union_and_raw_ranks_are_immutable"
)
R12_NEW_CALL_COUNTERS = {
...
def _candidate_generation_equivalence_proof(
    ...
    body = {
        "frozen_raw_execution_sha256": IMMUTABLE_R11_RAW_EXECUTION_SHA256,
        ...
        "upstream_local_embedding_inference_batches": summary.get(
            "local_embedding_inference_batches"
        ),
        "R12_new_local_embedding_inference_batches": 0,
    ...
    if (
        ...
        or body["upstream_local_embedding_inference_batches"] != 1
    ):
        raise ValueError("dell_03B_R12_candidate_generation_equivalence_invalid")
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"dell_03B_R12_json_not_mapping:{path.name}")
    return value
...
def _bound_path(policy: Mapping[str, Any], binding_id: str) -> Path:
    ...
    if not path.is_file():
        raise ValueError(f"dell_03B_R12_bound_input_missing:{binding_id}")
    if base._sha256(path) != str(row.get("sha256") or ""):  # noqa: SLF001
        raise ValueError(f"dell_03B_R12_bound_input_sha_drift:{binding_id}")
    return path
...
def _expected_request_ids(
    r11_private: Mapping[str, Any],
) -> set[str]:
    return {
        str(request_id)
        for target in r11_private.get("target_results") or ()
        for request_id in target.get("request_ids") or ()
    }


def _validated_immutable_r11_execution(
    *,
    r11_private: Mapping[str, Any],
    r11_raw_capture: Mapping[str, Any],
) -> tuple[dict[str, Any], str]:
    """Return the one frozen R11 candidate surface or fail closed."""

    raw = dict(r11_raw_capture)
    ...
    if (
        ...
        or raw.get("status") != "raw_execution_captured_before_R11_compilation"
        or raw.get("attempt_id") != IMMUTABLE_R11_ATTEMPT_ID
        or not _self_digest(raw)
        or r11_private.get("attempt_id") != IMMUTABLE_R11_ATTEMPT_ID
        or execution_sha256 != IMMUTABLE_R11_RAW_EXECUTION_SHA256
        or execution_sha256 != raw.get("raw_execution_sha256")
        or execution_sha256 != r11_private.get("raw_execution_sha256")
        or execution != dict(r11_private.get("raw_execution_receipt") or {})
    ):
        raise ValueError("dell_03B_R12_R11_saved_raw_identity_mismatch")
    validate_dell_report_internal_chain_r12_saved_raw_execution(
        execution,
        expected_request_ids=_expected_request_ids(r11_private),
    )
    return execution, execution_sha256


def _validated_r12_raw_reuse_capture(
    *,
    ...
    """Validate R12's receipt for reusing, rather than rerunning, R11."""

    row = dict(capture)
    source = dict(row.get("source_R11_raw_execution_capture") or {})
    if (
        row.get("schema_version") != R12_RAW_REUSE_CAPTURE_SCHEMA_VERSION
        or row.get("status")
        ...
        != {
            "ref": r11_raw_ref,
            "sha256": r11_raw_sha256,
            "result_digest": r11_raw_capture.get("result_digest"),
            "attempt_id": IMMUTABLE_R11_ATTEMPT_ID,
            "raw_execution_sha256": IMMUTABLE_R11_RAW_EXECUTION_SHA256,
        }
        or row.get("reuse_reason") != R12_RAW_REUSE_REASON
        or row.get("candidate_generation_equivalence_proof")
        != _candidate_generation_equivalence_proof(
            dict(row.get("raw_execution") or {})
        )
        or row.get("R12_new_call_counters") != R12_NEW_CALL_COUNTERS
    ):
        raise ValueError("dell_03B_R12_raw_reuse_capture_identity_invalid")
    execution, execution_sha256 = _validated_immutable_r11_execution(
        r11_private=r11_private,
        r11_raw_capture=r11_raw_capture,
    )
    if (
        dict(row.get("raw_execution") or {}) != execution
        or row.get("raw_execution_sha256") != execution_sha256
    ):
        raise ValueError("dell_03B_R12_raw_reuse_capture_payload_mismatch")
    return execution, execution_sha256
...
def _require_clean() -> None:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("dell_03B_R12_clean_worktree_required")
...
def _clean_exact_git_receipt(policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("dell_03B_R12_exact_clean_synced_git_identity_required")
    return {
        "branch": branch,
        "head": head,
...
def _validate_implementation_bindings(policy: Mapping[str, Any]) -> None:
    ...
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        raise ValueError("dell_03B_R12_implementation_bindings_invalid")
    seen: set[str] = set()
    for row in rows:
...
def _validate_r17_bundle(manifest: Mapping[str, Any]) -> None:
    ...
    for binding_id, raw in rows.items():
        ...
        if not path.is_file() or base._sha256(path) != str(  # noqa: SLF001
            ...
            raise ValueError(
                f"dell_03B_R12_R17_bundle_sha_drift:{binding_id}"
            )
...
def _validate_canonical_output_paths() -> None:
    ...
    if len(paths) != 5:
        raise ValueError("dell_03B_R12_output_paths_must_be_distinct")
    if any(
        ...
        raise ValueError("dell_03B_R12_attempt_receipt_private_parent_mismatch")


def _require_output_disk_capacity() -> dict[str, int]:
    usage = shutil.disk_usage(ROOT)
    if usage.free < MIN_FREE_BYTES_BEFORE_ATTEMPT:
        raise RuntimeError("dell_03B_R12_minimum_free_disk_capacity_required")
    return {
...
def _write_attempt_consumption_receipt(
    *,
    ...
    if ATTEMPT_RECEIPT.exists() or ATTEMPT_RECEIPT.parent.exists():
        raise FileExistsError("dell_03B_R12_attempt_already_consumed")
    body = {
        "schema_version": (
...
def _write_raw_execution_capture(
    *,
    ...
    if RAW_EXECUTION_CAPTURE.exists():
        raise FileExistsError("dell_03B_R12_raw_execution_capture_exists")
    actual_execution_sha256 = hashlib.sha256(
        base._canonical_json_bytes(execution)  # noqa: SLF001
    ).hexdigest()
    if (
        execution_sha256 != IMMUTABLE_R11_RAW_EXECUTION_SHA256
        or execution_sha256 != actual_execution_sha256
        or not source_capture_ref.endswith(
            "/dell-rsq-03b-internal-chain-r11/raw_execution_capture.json"
        )
        or len(source_capture_sha256) != 64
        or len(source_capture_result_digest) != 64
    ):
        raise ValueError("dell_03B_R12_raw_reuse_capture_input_invalid")
    body = {
        "schema_version": R12_RAW_REUSE_CAPTURE_SCHEMA_VERSION,
        "status": (
            "immutable_R11_raw_reused_before_R12_compilation_zero_new_calls"
        ),
        "attempt_id": ATTEMPT_ID,
        "recorded_at": recorded_at,
        "policy_digest": policy.get("result_digest"),
        "source_R11_raw_execution_capture": {
...
def _write_terminal_failure_receipt(
    *,
    ...
    if TERMINAL_FAILURE_RECEIPT.exists():
        raise FileExistsError("dell_03B_R12_terminal_failure_receipt_exists")
...
def _publish_atomic_pair(*, private_bytes: bytes, public_bytes: bytes) -> None:
    if DEFAULT_PRIVATE.exists() or DEFAULT_PUBLIC.exists():
        raise FileExistsError("dell_03B_R12_output_collision")
    ...
    if private_tmp.exists() or public_tmp.exists():
        raise FileExistsError("dell_03B_R12_temporary_output_collision")
...
def _runtime_rows(
    ...
    if (
        ...
        raise ValueError("dell_03B_R12_runtime_source_or_object_sha_drift")
...
def _request_payloads(
    *,
    ...
    if len(payloads) != 5 or {
        ...
        raise ValueError("dell_03B_R12_request_payload_population_invalid")
    return payloads, request_ids


def run_authorized_formal() -> dict[str, Any]:
    """Consume the unique R12 authority after every fail-closed gate."""

    _validate_canonical_output_paths()
    disk_receipt = _require_output_disk_capacity()
    if not POLICY.is_file():
        raise FileNotFoundError("dell_03B_R12_canonical_policy_missing")
    policy = _read_json(POLICY)
    if policy.get("schema_version") != POLICY_SCHEMA_VERSION:
        raise ValueError("dell_03B_R12_policy_schema_invalid")
    ...
    if (
        ...
        raise FileExistsError("dell_03B_R12_attempt_or_output_exists")
    ...
    r11_private = validate_dell_report_internal_chain_ceiling_r12_policy(
        policy,
        **values,
    )
    ...
    if (
        ...
        raise ValueError("dell_03B_R12_runtime_and_policy_data_path_mismatch")
    _, request_ids = _request_payloads(
        execution_program=values["execution_program"],
        r11_private=r11_private,
    )
    if request_ids != _expected_request_ids(r11_private):
        raise ValueError("dell_03B_R12_request_identity_mismatch")
    r11_raw_path, r11_raw = bound["R11_raw_execution_capture"]
    r11_raw_ref = base._relative(r11_raw_path)  # noqa: SLF001
    r11_raw_sha256 = base._sha256(r11_raw_path)  # noqa: SLF001
    execution, execution_sha256 = _validated_immutable_r11_execution(
        r11_private=r11_private,
        r11_raw_capture=r11_raw,
    )
    ...
    failure_stage = "immutable_R11_raw_successor_capture"
    try:
        _write_raw_execution_capture(
            policy=policy,
            execution=execution,
            execution_sha256=execution_sha256,
            source_capture_ref=r11_raw_ref,
            source_capture_sha256=r11_raw_sha256,
            source_capture_result_digest=str(
                r11_raw.get("result_digest") or ""
            ),
            recorded_at=recorded_at,
        )
        failure_stage = "persisted_raw_reuse_capture_revalidation"
        execution, execution_sha256 = _validated_r12_raw_reuse_capture(
            capture=_read_json(RAW_EXECUTION_CAPTURE),
            policy=policy,
            r11_private=r11_private,
            r11_raw_capture=r11_raw,
            r11_raw_ref=r11_raw_ref,
            r11_raw_sha256=r11_raw_sha256,
        )
        ...
        private_result = compile_dell_report_internal_chain_ceiling_r12_result(
            r11_private_result=r11_private,
            r12_policy=policy,
            execution=execution,
            execution_sha256=execution_sha256,
...
def preview_from_r11_saved_raw() -> dict[str, Any]:
    """Recompile immutable R11 raw bytes without calls or output writes."""

    r11_private_path = ROOT / (
        "data/workbench_private/"
        "fin_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling/"
        "dell-rsq-03b-internal-chain-r11/full_result.json"
    )
    r11_raw_path = r11_private_path.with_name("raw_execution_capture.json")
    if not r11_private_path.is_file() or not r11_raw_path.is_file():
        raise FileNotFoundError("dell_03B_R12_R11_saved_raw_predecessor_missing")
    r11_private = _read_json(r11_private_path)
    r11_raw = _read_json(r11_raw_path)
    ...
    bindings = dict(r11_private.get("input_bindings") or {})
    source_path = base._resolve(  # noqa: SLF001
        str((bindings.get("source_records") or {}).get("ref") or "")
    )
    object_path = base._resolve(  # noqa: SLF001
    ...
    if (
        ...
        raise ValueError("dell_03B_R12_preview_source_or_object_sha_drift")
    source_rows = base._read_jsonl(source_path)  # noqa: SLF001
    object_rows = base._read_jsonl(object_path)  # noqa: SLF001
    execution, execution_sha256 = _validated_immutable_r11_execution(
        r11_private=r11_private,
    ...
    compiled = compile_dell_report_internal_chain_ceiling_r12_result(
        r11_private_result=r11_private,
        r12_policy={"result_digest": "0" * 64},
        execution=execution,
        execution_sha256=execution_sha256,
        source_rows=source_rows,
        object_rows=object_rows,
    ...
    predecessor_targets = {
        ...
        for row in r11_private.get("target_results") or ()
    }
    package_stages = {
    ...
    for row in compiled["target_results"]:
        ...
        invariants = {
            ...
            == 0,
            "R12_structural_diagnostics_materialized": all(
        ...
        if partial_family_delta == {"added": [], "removed": []}:
            ...
            partial_delta_explanation = (
                "R12_proof_schema_reclassifies_partial_only_transformation_"
        ...
        targets.append(
            {
                ...
                "R11_source_compiled_union_final": before_counts,
                "R12_source_compiled_union_final": after_counts,
                "complete_family_delta": complete_family_delta,
                "R11_best_final_rank": before_rank,
                "R12_best_final_rank": after_rank,
                "R11_partial_source_compiled": [
                ...
                "R11_transformation": before_transform,
                "R12_transformation": after_transform,
                "R11_downstream": before_downstream,
                "R12_downstream": after_downstream,
                "R11_active_mandatory_external_route_ids": before_route_ids,
                "R12_active_mandatory_external_route_ids": after_route_ids,
                "expected_03A_R2_mandatory_external_route_ids": (
                    expected_route_ids
                ),
                "R12_route_identity_digest": route_identity_digest,
                "R12_active_route_id_set_digest": route_id_set_digest,
                "route_id_delta": route_id_delta,
                "route_delta_explanation": (
                    "R12_restores_constant_frozen_03A_R2_route_identity_"
    ...
    if any(
        ...
        raise ValueError("dell_03B_R12_preview_unexplained_material_delta")
    return {
        "mode": "preview_from_immutable_R11_saved_raw_zero_call",
        "elapsed_seconds": (
...
def replay_saved_formal() -> dict[str, Any]:
    """Recompile captured R12 raw bytes and prove canonical private equality."""
    ...
    if not all(
        ...
        raise FileNotFoundError("dell_03B_R12_formal_replay_inputs_missing")
    policy = _read_json(POLICY)
    ...
    replay = compile_dell_report_internal_chain_ceiling_r12_result(
        r11_private_result=r11_private,
        r12_policy=policy,
        execution=execution,
        execution_sha256=execution_sha256,
    ...
    if not equal:
        raise ValueError("dell_03B_R12_exact_private_replay_mismatch")
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.mode == "preview":
        result = preview_from_r11_saved_raw()
