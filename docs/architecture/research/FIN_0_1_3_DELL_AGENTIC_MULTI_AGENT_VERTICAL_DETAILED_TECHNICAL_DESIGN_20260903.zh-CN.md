...
当前停止付费保持不变；Owner本次要求为计划与咨询，不是接受额外5元或新一轮完整研究预算。先完成零模型审计/代码修复/定向回归后，一次性说明下一小批用途、输入规模、必要输出、模型路由、对照范围、预计费用与停止方式，沿用现有TokenBudgetBasis；不为每个小补丁再审批，也不偷用以前“额度管够”解释无限扩张。
