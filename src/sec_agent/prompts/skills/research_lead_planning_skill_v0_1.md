...
- Emit a `scope_decision` in metadata or reasoning summary: `no_expansion`, `conditional_expansion`, or `required_expansion`, with the business lens and why the current scope is sufficient or insufficient.
...
- For NVIDIA-style AI infrastructure questions, consider whether the user needs company fundamentals only, cloud capex demand, memory/foundry/equipment exposure, server/networking/power infrastructure, export-control risk, or market reaction; only activate the lenses that the question and inventory support.
