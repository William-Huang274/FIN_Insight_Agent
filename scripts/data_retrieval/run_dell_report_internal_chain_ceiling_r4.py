...
from retrieval.dell_report_internal_chain_ceiling_r4 import (  # noqa: E402
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    BRANCH,
    POLICY_REF,
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"dell_03B_R4_json_not_mapping:{path.name}")
    return value
...
def _read_jsonl(path: Path) -> list[dict[str, Any]]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if not isinstance(value, dict):
                raise ValueError(
                    f"dell_03B_R4_jsonl_row_not_mapping:{path.name}:{line_number}"
                )
            rows.append(value)
    return rows
...
def _require_clean() -> None:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("dell_03B_R4_clean_worktree_required")
...
def _clean_exact_git_receipt(policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("dell_03B_R4_exact_clean_synced_git_identity_required")
    return {
        "branch": branch,
        "head": head,
...
def _bound_json(
    ...
    path = _resolve(str(row.get("ref") or ""))
    if not path.is_file():
        raise ValueError(f"dell_03B_R4_bound_input_missing:{binding_id}")
    if _sha256(path) != str(row.get("sha256") or ""):
        raise ValueError(f"dell_03B_R4_bound_input_sha_drift:{binding_id}")
...
def _validate_implementation_bindings(policy: Mapping[str, Any]) -> None:
    ...
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        raise ValueError("dell_03B_R4_implementation_bindings_invalid")
    seen: set[str] = set()
    for row in rows:
        ...
        if ref in seen:
            raise ValueError("dell_03B_R4_implementation_binding_duplicate")
        seen.add(ref)
        path = _resolve(ref)
        if not path.is_file() or _sha256(path) != str(row.get("sha256") or ""):
            raise ValueError(f"dell_03B_R4_implementation_binding_drift:{ref}")
...
def _source_record_ids(rows: Sequence[Mapping[str, Any]]) -> list[str]:
    ...
    for row_number, row in enumerate(rows, start=1):
        if "source_record_id" in row:
            raise ValueError(
                f"dell_03B_R4_source_record_id_alias_forbidden:{row_number}"
            )
        ...
        if (
            ...
            raise ValueError(f"dell_03B_R4_source_evidence_id_missing:{row_number}")
        if source_id in seen:
            raise ValueError(f"dell_03B_R4_source_evidence_id_duplicate:{source_id}")
        seen.add(source_id)
...
def _validate_canonical_output_paths() -> None:
    ...
    if private_resolved == public_resolved or len(
        ...
        raise ValueError("dell_03B_R4_output_paths_must_be_distinct")
    if private_resolved.parent != receipt_resolved.parent:
        raise ValueError("dell_03B_R4_attempt_receipt_private_parent_mismatch")
...
def _write_attempt_consumption_receipt(
    *,
    ...
    if ATTEMPT_RECEIPT.exists() or ATTEMPT_RECEIPT.parent.exists():
        raise FileExistsError("dell_03B_R4_attempt_already_consumed")
    body = {
        "schema_version": (
...
def _publish_atomic_pair(
    *, private_bytes: bytes, public_bytes: bytes
) -> None:
    if DEFAULT_PRIVATE.exists() or DEFAULT_PUBLIC.exists():
        raise FileExistsError("dell_03B_R4_output_collision")
    ...
    if private_tmp.exists() or public_tmp.exists():
        raise FileExistsError("dell_03B_R4_temporary_output_collision")
...
def _material_blueprints(
    ...
    if not isinstance(contract, Mapping) or contract.get("mode") != (
        ...
        raise ValueError("dell_03B_R4_material_scope_blueprint_missing")
    roles_by_request = contract.get("required_roles_by_request")
    if not isinstance(roles_by_request, Mapping):
        raise ValueError("dell_03B_R4_material_scope_roles_missing")
    ...
    if not request_ids.issubset(requests) or not request_ids.issubset(
        ...
        raise ValueError("dell_03B_R4_material_scope_request_set_invalid")
    ...
    for request_id in sorted(request_ids):
        ...
        if len(facets) != 1 or not products or not entities or not roles:
            raise ValueError(f"dell_03B_R4_material_scope_invalid:{request_id}")
        result[request_id] = {
...
def run() -> dict[str, Any]:
    ...
    if not POLICY.is_file():
        raise FileNotFoundError("dell_03B_R4_canonical_policy_missing")
    policy = _read_json(POLICY)
    if policy.get("schema_version") != POLICY_SCHEMA_VERSION:
        raise ValueError("dell_03B_R4_policy_schema_invalid")
    ...
    if DEFAULT_PRIVATE.exists() or DEFAULT_PUBLIC.exists() or ATTEMPT_RECEIPT.parent.exists():
        raise FileExistsError("dell_03B_R4_attempt_or_output_exists")
    ...
    legacy_policy = validate_dell_report_internal_chain_ceiling_r4_policy(
        policy,
    ...
    if len(request_payloads) != 5 or {
        ...
        raise ValueError("dell_03B_R4_request_payload_population_invalid")
    blueprints = _material_blueprints(
        execution_program,
    ...
    objects_path = _resolve(str(bindings.get("compiled_objects", {}).get("ref") or ""))
    sources_path = _resolve(str(bindings.get("source_records", {}).get("ref") or ""))
    if _sha256(objects_path) != str(
        bindings.get("compiled_objects", {}).get("sha256") or ""
    ):
        raise ValueError("dell_03B_R4_compiled_object_sha_drift")
    if _sha256(sources_path) != str(
        bindings.get("source_records", {}).get("sha256") or ""
    ):
        raise ValueError("dell_03B_R4_source_record_sha_drift")
    ...
    execution_sha256 = hashlib.sha256(
        _canonical_json_bytes(execution)
    ).hexdigest()
    input_bindings = {
        name: {
            "ref": _relative(path),
    ...
    private_result = compile_dell_report_internal_chain_ceiling_r4_result(
    ...
    public_result = build_dell_report_internal_chain_ceiling_r4_public_projection(
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the exact-once canonical DELL 03B R4 semantic and material "
            "coverage audit."
        )
    )
