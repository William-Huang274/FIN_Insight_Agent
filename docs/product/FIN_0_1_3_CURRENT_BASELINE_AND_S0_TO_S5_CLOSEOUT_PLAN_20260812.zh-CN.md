...
24. **三层验收与第一层（旧 live 失败不可变、Claim Surface successor 已实现、formal clean proof 待签）**：fixed Pack 只测试“给定合格资料时能否可靠分析”，不计作 Agentic Research；第二层才是 DELL 单单元动态 S1/S2/EvidenceResponse 纵切；第三层才是五单元动态完整案例。唯一自然 Chat 选择了产品级 management assertion，不再产生旧 R2 的公司利润归因和半固定成本机制，但 reviewed source 的“中个位数”目标没有 typed alias，故 terminal failed。Owner 随后只授权零调用收口：当前 successor 已把该目标编译为 source-bound QF，禁止单点化，并要求 thesis／mechanism／counterargument 分别提交结构化 subject、outcome、relation、attribution 和 scope。定向测试与失败 payload 三阶段回放已通过 working-tree 门；还需干净提交、上游一致和 exact-once formal proof。未证明新的自然输出，第一层 acceptance 与第二层继续 blocked。
