...
from sec_agent.agent_runtime import agent_server_identity as identity
...
from sec_agent.agent_runtime.agent_server_recovery import (
...
def _repository(
    ...
    return PostgresAgentServerIdentityRepository(_ScriptedPool(connection))
...
def _session_row(
    ...
    *,
    ...
    return (
        ...
        session.thread_id,
        server_thread_id,
        identity.AGENT_SERVER_ASSISTANT_ID,
        stable_digest,
        NOW,
    )
...
def _lifecycle_row(
    *,
    ...
    event_digest = identity._run_create_event_digest(
        ...
        assistant_id=identity.AGENT_SERVER_ASSISTANT_ID,
...
def test_repository_requires_pool_checkout_and_rejects_active_connection() -> None:
    ...
    with pytest.raises(
        ...
        PostgresAgentServerIdentityRepository(bare_connection)  # type: ignore[arg-type]

    active_connection = _ScriptedConnection()
    active_connection.info.transaction_status = 2
    pool = _ScriptedPool(active_connection)
    repository = PostgresAgentServerIdentityRepository(pool)
    with pytest.raises(
        AgentServerIdentityStoreError,
    ...
    idle_repository = PostgresAgentServerIdentityRepository(idle_pool)
    for _ in range(2):
