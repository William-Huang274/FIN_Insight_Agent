...
def import_rows(path, manifest):
    ...
    with closing(sqlite3.connect(path)) as db, db:
        ...
        for item in manifest['rows']:
            ...
            page=item['pdf_page']; marker=f'[PDF page {page}]'
            if marker not in body:raise ValueError('pdf_page_not_found')
            section=body.split(marker,1)[1].split('[PDF page ',1)[0]
            ...
            value=Decimal(token.replace('(','-').replace(')',''))*Decimal(str(item['scale']))
