...
from .deepseek_structured_agents import TokenBudgetBasis, load_deepseek_structured_agent_config
...
def build_report_session_graph(*, writer, verifier, artifacts, initial, audits=None):
    ...
    def human_review(state):
        ...
        return Command(update={"request_action": action.action, "message": action.message,
            "phase": "working", "conversation": [{"role": "user", "content": action.message, "action": action.action}]}, goto="writer")
    ...
    def seed(state, role):
        ...
        if role == "writer":
            ...
            body.update(request_action=state["request_action"], user_message=state["message"],
                public_conversation=state.get("conversation", [])[:-1],
                revision_request={"prior_report": {k: state["report"][k] for k in ("title", "narrative_markdown")},
    ...
    graph.add_node("human_review", human_review, destinations=("writer", END))
    for role, agent in (("writer", writer), ("verifier", verifier)):
...
async def dell_report_session_graph(config: RunnableConfig, runtime: ServerRuntime):
    ...
    with open_dell_approved_data_composition(run_invocation_id=invocation, source_read_enabled=True,
        ...
        async with Client(data.mcp_server, raise_exceptions=False, read_timeout_seconds=120) as client:
            ...
            agents, audits = {}, {}
            for role in ("writer", "verifier"):
                basis = TokenBudgetBasis.model_validate_json(json.dumps(settings["node_budgets"][role]))
                profile = model_config.profile_for("specialist" if role == "writer" else "verifier")
                ...
                agents[role] = build_case_output_agent(role=role, model=case_chat_model(profile, basis, model_config, SecretStr(os.environ["DEEPSEEK_API_KEY"])),
                    tools=tools, artifacts=artifacts, limits=settings["node_limits"][role], audit=audits[role],
                    report_revision=True, allow_answers=role == "writer")
