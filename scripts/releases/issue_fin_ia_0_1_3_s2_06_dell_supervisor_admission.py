...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Issue one FIN 0.1.3 S2-06 DELL Supervisor admission"
    )
    ...
    corrected_run_id = "fin013_s2_06_supervised_dell_" + nonce
    ...
    admission = compile_governed_admission(
        ...
        admission_id="fin013-s2-06-dell-supervisor-" + secrets.token_hex(16),
    ...
    if not args.dry_run:
        ...
        if list(AUTHORITY_ROOT.glob("*.json")):
            raise RuntimeError("s2_06_dell_supervisor_admission_already_exists")
