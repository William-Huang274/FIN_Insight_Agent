...
WORKER_REF = (
    "scripts/releases/"
    "prove_fin_ia_0_1_3_s1_dell_bounded_semantic_anchor_replay_worker.py"
)


class DellBoundedSemanticAnchorCleanProofError(RuntimeError):
    pass
...
def _require_clean_synced() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise DellBoundedSemanticAnchorCleanProofError(
            "bounded_semantic_anchor_clean_proof_requires_clean_worktree"
        )
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise DellBoundedSemanticAnchorCleanProofError(
            "bounded_semantic_anchor_clean_proof_requires_synced_head"
        )
    return head
...
def _hydrate_private_inputs(
    ...
    refs.extend(str(row["ref"]) for row in bindings["response_captures"])
    hydrated: list[dict[str, str]] = []
    for ref in refs:
        source = ROOT / ref
        target = worker_root / ref
        _link_file(source, target)
        hydrated.append({"ref": ref, "sha256": _sha256(source)})
    return hydrated
...
def _run_worker(
    *,
    ...
    if completed.returncode != 0:
        raise DellBoundedSemanticAnchorCleanProofError(
            f"bounded_semantic_anchor_clean_worker_failed:{worker_index}:"
...
def main() -> int:
    ...
    if args.output.exists():
        raise DellBoundedSemanticAnchorCleanProofError(
            "bounded_semantic_anchor_clean_proof_output_already_exists"
        )
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin013-dell-bounded-anchor-proof-",
        dir=temp_parent,
    ) as directory:
    ...
    workers = [pair[0] for pair in pairs]
    if workers[0] != workers[1]:
        raise DellBoundedSemanticAnchorCleanProofError(
            "bounded_semantic_anchor_clean_workers_not_byte_equivalent"
        )
    counts = dict(workers[0]["observed_counts"])
    body = {
        "schema_version": PROOF_SCHEMA,
        "contract_ref": CONTRACT_REF,
        "status": "clean_independent_bounded_semantic_anchor_replay_passed",
        ...
        "corrected_pack_payload_digest": workers[0][
            "corrected_pack_payload_digest"
        ],
        "observed_counts": counts,
        "gate_status": workers[0]["gate_status"],
        "route_results": workers[0]["route_results"],
        ...
        "private_input_hydration": {
            ...
            "input_count": len(pairs[0][1]),
            "worker_manifests_equal": pairs[0][1] == pairs[1][1],
            "input_digests": pairs[0][1],
        },
        "known_boundary": (
            "Two clean Git archive workers replayed the same immutable long-document "
            "captures and produced the same core-ready corrected Pack. No source, "
            "network or model call occurred; report quality remains unproven."
        ),
    }
