...
DECISION_REF = (
    "configs/releases/"
    "fin_ia_0_1_3_s2_05_dell_layered_replacement_exact_live_authority_v1_0.json"
)
...
DEFAULT_AUTHORITY_ROOT = (
    ...
    / "fin013_s2_05"
    / "authorities"
    / "DELL_REPLACEMENT_LAYERED"
)


def main() -> int:
    parser = argparse.ArgumentParser(
        description="Issue one DELL layered replacement exact-live admission"
    )
    ...
    if not args.dry_run:
        ...
        if authority_root != DEFAULT_AUTHORITY_ROOT.resolve():
            raise RuntimeError("experiment_a_layered_authority_root_invalid")
        authority_root.mkdir(parents=True, exist_ok=True)
        if list(authority_root.glob("*.json")):
            raise RuntimeError("experiment_a_layered_replacement_admission_already_exists")
...
def _validate_decision(decision: dict[str, Any]) -> None:
    ...
    if (
        ...
        != "DELL_layered_replacement_admission_and_execution_authorized_not_issued"
        ...
        != ".codex_runtime/fin013_s2_05/authorities/DELL_REPLACEMENT_LAYERED"
    ):
        raise RuntimeError("experiment_a_layered_replacement_authority_invalid")


def _validate_repository(decision: dict[str, Any]) -> None:
    if _git("status", "--porcelain"):
        raise RuntimeError("experiment_a_layered_repository_not_clean")
    head = _git("rev-parse", "HEAD")
    audited = str(decision["audited_repository_state"]["head"])
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", audited, head], cwd=ROOT, check=False
    ).returncode != 0:
        raise RuntimeError("experiment_a_layered_head_not_descendant")
    upstream = _git("rev-parse", "--abbrev-ref", "@{upstream}")
    if _git("rev-list", "--left-right", "--count", f"{upstream}...HEAD").split() != ["0", "0"]:
        raise RuntimeError("experiment_a_layered_repository_not_synced")
...
def _validate_frozen_bindings(decision: dict[str, Any]) -> None:
    for binding in decision["frozen_bindings"].values():
        ...
        if not ref or not expected or _sha256(ROOT / ref) != expected:
            raise RuntimeError("experiment_a_layered_frozen_binding_mismatch")


def _read_json(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(value, dict):
        raise RuntimeError("experiment_a_layered_decision_json_invalid")
    return value
