...
from retrieval.dell_report_internal_chain_ceiling_r11 import (  # noqa: E402
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    BRANCH,
    ...
    build_dell_report_internal_chain_ceiling_r11_public_projection,
    compile_dell_report_internal_chain_ceiling_r11_result,
    validate_dell_report_internal_chain_ceiling_r11_policy,
)
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"dell_03B_R11_json_not_mapping:{path.name}")
    return value
...
def _bound_path(policy: Mapping[str, Any], binding_id: str) -> Path:
    ...
    if not path.is_file():
        raise ValueError(f"dell_03B_R11_bound_input_missing:{binding_id}")
    if base._sha256(path) != str(row.get("sha256") or ""):  # noqa: SLF001
        raise ValueError(f"dell_03B_R11_bound_input_sha_drift:{binding_id}")
    return path
...
def _require_clean() -> None:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise RuntimeError("dell_03B_R11_clean_worktree_required")
...
def _clean_exact_git_receipt(policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("dell_03B_R11_exact_clean_synced_git_identity_required")
    return {
        "branch": branch,
        "head": head,
...
def _validate_implementation_bindings(policy: Mapping[str, Any]) -> None:
    ...
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        raise ValueError("dell_03B_R11_implementation_bindings_invalid")
    seen: set[str] = set()
    for row in rows:
...
def _validate_r17_bundle(manifest: Mapping[str, Any]) -> None:
    ...
    for binding_id, raw in rows.items():
        ...
        if not path.is_file() or base._sha256(path) != str(  # noqa: SLF001
            ...
            raise ValueError(
                f"dell_03B_R11_R17_bundle_sha_drift:{binding_id}"
            )
...
def _validate_canonical_output_paths() -> None:
    ...
    if len(paths) != 5:
        raise ValueError("dell_03B_R11_output_paths_must_be_distinct")
    if any(
        ...
        raise ValueError("dell_03B_R11_attempt_receipt_private_parent_mismatch")


def _require_output_disk_capacity() -> dict[str, int]:
    usage = shutil.disk_usage(ROOT)
    if usage.free < MIN_FREE_BYTES_BEFORE_ATTEMPT:
        raise RuntimeError("dell_03B_R11_minimum_free_disk_capacity_required")
    return {
...
def _write_attempt_consumption_receipt(
    *,
    ...
    if ATTEMPT_RECEIPT.exists() or ATTEMPT_RECEIPT.parent.exists():
        raise FileExistsError("dell_03B_R11_attempt_already_consumed")
    body = {
        "schema_version": (
...
def _write_raw_execution_capture(
    *,
    ...
    if RAW_EXECUTION_CAPTURE.exists():
        raise FileExistsError("dell_03B_R11_raw_execution_capture_exists")
    body = {
        "schema_version": (
            "fin_ia_dell_report_internal_chain_raw_execution_capture_v1_0"
        ),
        "status": "raw_execution_captured_before_R11_compilation",
        "attempt_id": ATTEMPT_ID,
        "recorded_at": recorded_at,
        "policy_digest": policy.get("result_digest"),
...
def _write_terminal_failure_receipt(
    *,
    ...
    if TERMINAL_FAILURE_RECEIPT.exists():
        raise FileExistsError("dell_03B_R11_terminal_failure_receipt_exists")
...
def _publish_atomic_pair(*, private_bytes: bytes, public_bytes: bytes) -> None:
    if DEFAULT_PRIVATE.exists() or DEFAULT_PUBLIC.exists():
        raise FileExistsError("dell_03B_R11_output_collision")
    ...
    if private_tmp.exists() or public_tmp.exists():
        raise FileExistsError("dell_03B_R11_temporary_output_collision")
...
def _runtime_rows(
    ...
    if (
        ...
        raise ValueError("dell_03B_R11_runtime_source_or_object_sha_drift")
...
def _request_payloads(
    *,
    ...
    request_ids = {
        str(request_id)
        for target in r10_private.get("target_results") or ()
    ...
    if len(payloads) != 5 or {
        ...
        raise ValueError("dell_03B_R11_request_payload_population_invalid")
    return payloads, request_ids


def run_authorized_formal() -> dict[str, Any]:
    """Consume the unique R11 authority after every fail-closed gate."""

    _validate_canonical_output_paths()
    disk_receipt = _require_output_disk_capacity()
    if not POLICY.is_file():
        raise FileNotFoundError("dell_03B_R11_canonical_policy_missing")
    policy = _read_json(POLICY)
    if policy.get("schema_version") != POLICY_SCHEMA_VERSION:
        raise ValueError("dell_03B_R11_policy_schema_invalid")
    ...
    if (
        ...
        raise FileExistsError("dell_03B_R11_attempt_or_output_exists")
    ...
    r10_private = validate_dell_report_internal_chain_ceiling_r11_policy(
        policy,
        **values,
    )
    ...
    if (
        ...
        raise ValueError("dell_03B_R11_runtime_and_policy_data_path_mismatch")
    ...
    try:
        ...
        if not isinstance(execution, Mapping):
            raise TypeError("dell_03B_R11_execution_not_mapping")
        ...
        private_result = compile_dell_report_internal_chain_ceiling_r11_result(
            r10_private_result=r10_private,
            r11_policy=policy,
            execution=execution,
            execution_sha256=execution_sha256,
...
def preview_from_r10_saved_raw() -> dict[str, Any]:
    """Recompile immutable R10 raw bytes without calls or output writes."""

    r10_private_path = ROOT / (
        "data/workbench_private/"
        "fin_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling/"
        "dell-rsq-03b-internal-chain-r10/full_result.json"
    )
    r10_raw_path = r10_private_path.with_name("raw_execution_capture.json")
    if not r10_private_path.is_file() or not r10_raw_path.is_file():
        raise FileNotFoundError("dell_03B_R11_R10_saved_raw_predecessor_missing")
    r10_private = _read_json(r10_private_path)
    r10_raw = _read_json(r10_raw_path)
    bindings = dict(r10_private.get("input_bindings") or {})
    source_path = base._resolve(  # noqa: SLF001
        str((bindings.get("source_records") or {}).get("ref") or "")
    )
    object_path = base._resolve(  # noqa: SLF001
    ...
    if (
        ...
        raise ValueError("dell_03B_R11_preview_source_or_object_sha_drift")
    source_rows = base._read_jsonl(source_path)  # noqa: SLF001
    object_rows = base._read_jsonl(object_path)  # noqa: SLF001
    execution = dict(r10_raw.get("raw_execution") or {})
    execution_sha256 = hashlib.sha256(
        base._canonical_json_bytes(execution)  # noqa: SLF001
    ).hexdigest()
    if (
        execution_sha256 != r10_raw.get("raw_execution_sha256")
        or execution_sha256 != r10_private.get("raw_execution_sha256")
        or execution != dict(r10_private.get("raw_execution_receipt") or {})
    ):
        raise ValueError("dell_03B_R11_R10_saved_raw_identity_mismatch")
    started_at = datetime.now(timezone.utc)
    compiled = compile_dell_report_internal_chain_ceiling_r11_result(
        r10_private_result=r10_private,
        r11_policy={"result_digest": "0" * 64},
        execution=execution,
        execution_sha256=execution_sha256,
        source_rows=source_rows,
        object_rows=object_rows,
    ...
    predecessor_targets = {
        ...
        for row in r10_private.get("target_results") or ()
    }
    package_stages = {
    ...
    for row in compiled["target_results"]:
        ...
        invariants = {
            ...
            "downstream_disposition_equal": before_downstream == after_downstream,
        }
        if partial_family_delta == {"added": [], "removed": []}:
            partial_delta_explanation = "no_partial_family_delta"
        else:
            partial_delta_explanation = (
                "R11_proof_schema_reclassifies_partial_only_transformation_"
        ...
        targets.append(
            {
                ...
                "R10_source_compiled_union_final": before_counts,
                "R11_source_compiled_union_final": after_counts,
                "complete_family_delta": complete_family_delta,
                "R10_best_final_rank": before_rank,
                "R11_best_final_rank": after_rank,
                "R10_partial_source_compiled": [
                ...
                "R10_transformation": before_transform,
                "R11_transformation": after_transform,
                "R10_downstream": before_downstream,
                "R11_downstream": after_downstream,
    ...
    if any(
        ...
        raise ValueError("dell_03B_R11_preview_unexplained_material_delta")
    return {
        "mode": "preview_from_immutable_R10_saved_raw_zero_call",
        "elapsed_seconds": (
...
def replay_saved_formal() -> dict[str, Any]:
    """Recompile captured R11 raw bytes and prove canonical private equality."""
    ...
    if not all(
        ...
        raise FileNotFoundError("dell_03B_R11_formal_replay_inputs_missing")
    policy = _read_json(POLICY)
    ...
    replay = compile_dell_report_internal_chain_ceiling_r11_result(
        r10_private_result=r10_private,
        r11_policy=policy,
        execution=execution,
    ...
    if not equal:
        raise ValueError("dell_03B_R11_exact_private_replay_mismatch")
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.mode == "preview":
        result = preview_from_r10_saved_raw()
