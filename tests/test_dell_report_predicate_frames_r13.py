...
from retrieval.dell_report_predicate_frames_r13 import (
    ASP_TARGET,
    CAPACITY_TARGET,
    HBM_TARGET,
    SUPPLIER_TARGET,
...
@pytest.mark.parametrize(
    ...
    (
        (
            ASP_TARGET,
            "Dell offered PowerEdge XE9680 server for USD 15 in FY2026.",
        ),
        (
            SUPPLIER_TARGET,
            "Nvidia supplied GPUs to Dell in FY2026.",
        ),
        (
            CAPACITY_TARGET,
            "Nvidia allocated GPU capacity to Dell in Q1 2026.",
        ),
        (
            YIELD_TARGET,
            "HBM production yield reached 80% in Q1 2026.",
        ),
        (
            HBM_TARGET,
            "Nvidia allocated HBM to Dell PowerEdge in Q1 2026.",
        ),
        (
            UNITS_TARGET,
            "Dell shipped 10 PowerEdge servers in FY2026.",
        ),
    ),
)
def test_r13_candidate_surface_is_necessary_for_positive_frame(
    target_id: str,
    text: str,
) -> None:
    assert predicate_frame_candidate_surface(target_id=target_id, text=text)
    ...
    assert frames
    public = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
    )
    fast, runtime_frames, deferred = classify_package_with_frames(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
    ...
    assert fast == public
    assert runtime_frames == tuple(frames)
...
def test_r13_no_comma_independent_events_split_and_cannot_union_roles(
    target_id: str,
    text: str,
) -> None:
...
def test_r13_compound_company_subject_keeps_one_shared_predicate_frame() -> None:
...
def test_r13_open_vocabulary_right_owner_splits_without_entity_allowlist(
    right_owner: str,
) -> None:
    text = (
...
def test_r13_fronted_functional_adjunct_proves_shared_subject(
    adjunct: str,
) -> None:
...
def test_r13_case_preserved_predicate_token_collision_is_owner_not_verb(
    collision_owner: str,
) -> None:
    text = (
        "Dell quoted support for USD 150 and "
...
def test_r13_unseen_material_clause_head_creates_ambiguity_barrier(
    unknown_head: str,
) -> None:
    text = (
...
def test_r13_casefold_length_alignment_failure_is_a_typed_role_barrier() -> None:
    text = (
...
def test_r13_shared_object_or_auxiliary_continuation_does_not_false_split(
    text: str,
) -> None:
    decisions = frame_boundary_decisions(text)
...
def test_r13_predicate_at_right_offset_zero_is_shared_subject_continuation() -> None:
    text = (
...
def test_r13_semantic_state_is_typed_and_bound_by_exact_scope_edge(
    text: str,
    ...
    assert edge.source_modifier_frame_id.startswith("MODIFIER::R13::")
...
def test_r13_unrelated_state_change_does_not_veto_independent_positive_frame() -> None:
    text = (
...
def test_r13_price_is_bound_to_typed_argument_group_not_record_global_value(
    text: str,
...
def test_r13_non_hardware_price_near_neighbors_fail_closed(text: str) -> None:
...
def test_r13_price_copresence_without_direct_attachment_never_admits_relation(
    text: str,
) -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
...
def test_r13_affirmative_price_attachment_has_traversable_span_proof(
    text: str,
    proof_type: str,
) -> None:
    frame = extract_predicate_frames(
...
def test_r13_cross_group_product_and_hardware_price_cannot_complete_asp() -> None:
    text = (
...
def test_r13_generic_hardware_price_has_no_specific_product_relation() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
...
def test_r13_multiple_product_price_relations_fail_closed() -> None:
    text = (
...
def test_r13_multiple_products_inside_one_price_group_fail_closed() -> None:
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
        text=(
...
def test_r13_inherits_r8_fresh_negative_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
...
def test_r13_inherits_frozen_positive_surface(target_id: str, text: str) -> None:
    assessment = classify_package(
        target_id=target_id,
        text=text,
        metadata=_metadata(),
    ...
    assert assessment["classification"] == "complete_bounded_target_package"
    assert assessment["accepted_frame_id"].startswith("FRAME::R13::")
...
def test_r13_inherits_all_63_r7_negative_semantic_attacks() -> None:
...
def test_r13_frame_records_and_bindings_are_deterministic_and_immutable() -> None:
    kwargs = {
...
@pytest.mark.parametrize(
    "text",
    [
        "Dell quoted support for USD 150 and eBay Systems wugged PowerEdge "
        "XE9680 hardware for USD 15.",
        "Dell quoted support for USD 150 and vanadium labs zorps PowerEdge "
        "XE9680 hardware for USD 15.",
        "Dell quoted support for USD 150 and rose systems offered PowerEdge "
        "XE9680 hardware for USD 15.",
    ],
)
def test_r13_open_vocabulary_and_lowercase_right_owner_isolates_roles(
    text: str,
) -> None:
    decisions = frame_boundary_decisions(text)
    assert len(decisions) == 1
    assert decisions[0].decision == "split"
    assert decisions[0].ownership_state in {
...
def test_r13_unseen_structural_fronted_adjunct_preserves_shared_subject(
    adjunct: str,
) -> None:
    text = f"NVIDIA provides GPUs and {adjunct} ships them to Dell."
    decision = frame_boundary_decisions(text)[0]
...
@pytest.mark.parametrize(
    ...
    [
        ...
        "quasar coverage",
        "nebula escrow",
        "vanadium arrangement",
    ],
)
def test_r13_intervening_governing_nominal_head_cannot_be_hardware_price(
    governing_head: str,
) -> None:
    text = (
        f"Dell quoted {governing_head} for PowerEdge XE9680 hardware at USD 15."
    )
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
        text=text,
    ...
    assert frame.accepted is False
    assert frame.argument_groups[0].proof_state == "ambiguous"
    assert frame.argument_groups[0].ambiguity == (
        "intervening_governing_nominal_head"
    )
    assert frame.argument_groups[0].governing_nominal_head_span is not None


def test_r13_direct_hardware_price_and_explicit_price_head_remain_positive() -> None:
    for text in (
        "Dell offered PowerEdge XE9680 hardware at USD 15.",
        "Dell quoted a purchase price for PowerEdge XE9680 hardware at USD 15.",
        "Dell quoted a configuration price for PowerEdge XE9680 hardware at USD 15.",
        "Dell quoted a PowerEdge XE9680 hardware bundle at USD 400000, "
        "including NVIDIA H100 GPUs.",
    ):
        frame = extract_predicate_frames(
            target_id=ASP_TARGET,
            text=text,
        ...
        assert frame.accepted is True
        assert frame.argument_groups[0].proof_state == "affirmative"
        assert frame.argument_groups[0].governing_nominal_head_class == (
            "direct_typed_object"
        )
...
def test_r13_ownerless_unseen_event_head_cannot_borrow_left_quote(
    event_head: str,
) -> None:
    text = (
        "Dell quoted support for USD 150 and "
        f"{event_head} PowerEdge XE9680 hardware for USD 15."
    )
    decisions = frame_boundary_decisions(text)
    assert len(decisions) == 1
    assert decisions[0].decision == "split"
    assert decisions[0].ownership_state == "ambiguous_material_boundary"
    assert decisions[0].reason == "ownerless_unrecognized_material_event_head"
    assessment = classify_package(
        target_id=ASP_TARGET,
        text=text,
        metadata=_metadata(),
...
def test_r13_ownerless_event_with_adverb_or_auxiliary_still_splits(
    event_surface: str,
) -> None:
    text = (
        "Dell quoted support for USD 150, and "
        f"{event_surface} PowerEdge XE9680 hardware for USD 15."
    )
    decisions = frame_boundary_decisions(text)
    assert decisions
    assert decisions[-1].decision == "split"
    assessment = classify_package(
        target_id=ASP_TARGET,
        text=text,
        metadata=_metadata(),
...
@pytest.mark.parametrize(
    "text",
    [
        "Dell quoted PowerEdge XE9680 hardware and XE9712 hardware for USD 15.",
        "Dell quoted one PowerEdge XE9680 hardware system for USD 15.",
        "Dell quoted two PowerEdge XE9680 hardware systems for USD 15.",
    ],
)
def test_r13_ownerless_event_barrier_preserves_product_list_controls(
    text: str,
) -> None:
    assessment = classify_package(
        target_id=ASP_TARGET,
...
@pytest.mark.parametrize(
    ("head", "link"),
    [
        ("a maintenance service", "covering"),
        ("a delivery arrangement", "including"),
        ("lease financing", "bundling"),
        ("quasar coverage", "containing"),
        ("nebula escrow", "comprising"),
        ("vanadium arrangement", "featuring"),
        ("a support plan", "that includes"),
        ("a service contract", "which covers"),
    ],
)
def test_r13_participial_or_relative_governing_head_cannot_be_hardware_price(
    head: str,
    link: str,
) -> None:
    text = f"Dell quoted {head} {link} PowerEdge XE9680 hardware at USD 15."
    frame = extract_predicate_frames(
        target_id=ASP_TARGET,
        text=text,
        metadata=_metadata(),
