...
def test_workbench_backend_starts_expanded_a6_eval_with_transient_secret(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    assert catalog_response.status_code == 200
    assert "expanded_a6_full_chain_smoke" in {item["eval_id"] for item in catalog_response.json()["evals"]}
    assert start_response.status_code == 200
    payload = start_response.json()
