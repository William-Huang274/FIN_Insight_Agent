...
OUTPUT_PATH = ROOT / (
    "configs/releases/fin_ia_0_1_3_s1_internal_qrels_review_packet_v1_1.json"
)
...
def main() -> int:
    ...
    if preflight.get("status") != "pass":
        raise RuntimeError("internal_qrels_successor_project_os_preflight_blocked")
    ...
    result = build_internal_qrels_successor_packet(policy=policy, inputs=inputs)
    body = dict(result)
    ...
    body.update(
        {
            ...
            "implementation": {
                "module_ref": "src/sec_agent/s1_internal_qrels_successor.py",
                "materializer_ref": (
                    "scripts/releases/materialize_fin_ia_0_1_3_s1_internal_"
                    "qrels_review_packet_v1_1.py"
                ),
    ...
    if OUTPUT_PATH.exists():
        ...
        if existing.get("review_digest") != output["review_digest"]:
            raise RuntimeError("internal_qrels_successor_result_path_already_occupied")
        output = existing
    else:
