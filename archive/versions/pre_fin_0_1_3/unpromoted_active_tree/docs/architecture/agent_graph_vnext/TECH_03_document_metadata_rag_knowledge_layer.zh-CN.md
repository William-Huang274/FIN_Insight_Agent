...
fresh R2 已在唯一 authority 下 terminal succeeded：同一 93-spec population 生成 93 条 ObjectBM25 record 与 93 个 BGE-M3 vector，Milvus close/reopen 后 `current_seq=count=93`，目录 manifest／schema／Parquet data／FLAT index 均被 tree receipt 绑定；artifact=`aebe67d0...3f35` 在整个 root rename 前后相同，result=`6b239d7b...1ce7`。这关闭当前 corpus 的物理发布 owner，但不准入 dense 或 fusion；下一项必须先跑六案 Query Facet 对照并把错公司、错期间、错关系、错章节和内容过泛逐条归因。

六案 retrieval evaluator v1 的固定执行形状为 `72 queries = 18 Owner-qrel bundles + 54 canonical-slot bundles`。查询从冻结 Query Facet／通用金融研究合同／行业 Pack 编译，候选生成完成后才允许加载 qrels/slot labels；ObjectBM25 与 BGE-M3 只按 research case 隔离，均取 top-10，fusion 使用 `RRF(k=60, weights=1:1)`，不执行 reranker 或结果后调权。BGE 在一次 encode 中产生 72 query vectors，Milvus 按六个 case 分组执行六次 search；所有 network/provider/LLM/document fetch/Evidence call 为 0。`typed_exact_ceiling` 只是候选上限/oracle，不是生产检索方法。
