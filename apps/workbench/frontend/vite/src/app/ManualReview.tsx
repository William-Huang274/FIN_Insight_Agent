...
type Draft={base_version:number;report_markdown:string;papers:Paper[];review:Session['report_review']};
...
export function ManualReview({session,onSaved}:{session:Session;onSaved:()=>Promise<void>}){
  ...
  async function save(){if(!draft)return;setBusy(true);setError('');
    try{const r=await fetch(`/api/v1/research-sessions/${session.thread_id}/actions`,{method:'POST',headers:{'Content-Type':'application/json','X-Workbench-Request':'1'},body:JSON.stringify({action:'manual_complete',manual_review:{base_version:draft.base_version,report_markdown:draft.report_markdown,reason,confirmed,paper_edits:draft.papers.filter((p,i)=>p.body!==original?.papers[i].body).map(p=>({paper_id:p.paper_id,body:p.body}))}})});const data=await r.json();if(!r.ok)throw Error(typeof data.detail==='string'?data.detail:'保存未完成');setOriginal(draft);setReason('');dialog.current?.close();await onSaved();}catch(e){setError((e as Error).message);}finally{setBusy(false);}}
  return <>{session.can_manual_complete&&<button className="fs-context-menu-button" onClick={()=>void open()}><FilePenLine size={16}/>人工修改与确认</button>}
    ...
    {!!session.human_edits?.length&&<details className="fs-human-history"><summary>人工修改 {session.human_edits.length} 次 · 查看角色与底稿</summary>{session.human_edits.map(h=><section key={h.number}><h3>第 {h.number} 次 · {h.reason}</h3><p>{new Date(h.recorded_at).toLocaleString()} · {h.owner}</p>{[...h.papers,...(h.report_before||h.report_after?[{paper_id:'report',actor:'writer',title:'最终报告',before:h.report_before,after:h.report_after}]:[])].map(p=><details key={p.paper_id}><summary>{roleName(p.actor)} — {p.title}</summary><ReportDiffView proseOnly value={{before_version:h.base_version,after_version:h.base_version+1,reason:h.reason,diff:createPatch('正文',p.before,p.after),citations_changed:false,charts_changed:false}}/><details><summary>查看修改后完整正文</summary><ReactMarkdown remarkPlugins={[remarkGfm]}>{p.after}</ReactMarkdown></details></details>)}</section>)}</details>}
