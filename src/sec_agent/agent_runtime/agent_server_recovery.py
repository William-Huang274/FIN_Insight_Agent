"""Represent uncertain run creation and explicit recovery decisions."""
...
class AgentServerRecoveryError(RuntimeError):
...
class AgentServerRecoveryCase:
    ...
    def __post_init__(self) -> None:
        try:
            ...
            raise AgentServerRecoveryError(
        ...
        if (
            ...
            raise AgentServerRecoveryError("recovery_case_id_invalid")
        if self.research_run.status != "RECOVERY_REQUIRED":
            raise AgentServerRecoveryError("recovery_case_run_state_invalid")
        if (
            ...
            raise AgentServerRecoveryError("recovery_case_identity_invalid")
        if self.source_invocation.finished_at < self.ambiguous_action.terminal_at:
            raise AgentServerRecoveryError("recovery_case_time_invalid")
        ...
        if self.recovery_case_digest != recovery_case_digest(self):
            raise AgentServerRecoveryError("recovery_case_digest_invalid")


def run_create_action_attempt_id(invocation_id: str) -> str:
    if not isinstance(invocation_id, str) or not invocation_id.strip():
        raise AgentServerRecoveryError("run_create_invocation_id_invalid")
...
def create_run_create_action_dispatched(intent: ActionAttempt) -> ActionAttempt:
    ...
    if intent.state != "INTENT_COMMITTED" or intent.was_dispatched:
        raise AgentServerRecoveryError("run_create_action_intent_invalid")
    return create_action_attempt(
        **{
            **intent.model_dump(
...
def create_recovery_case(
    *,
    ...
    return AgentServerRecoveryCase(
...
def recovery_case_digest(value: AgentServerRecoveryCase) -> str:
