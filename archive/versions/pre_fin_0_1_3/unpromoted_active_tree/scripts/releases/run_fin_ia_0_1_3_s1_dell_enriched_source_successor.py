...
AUTHORITY_PATH = ROOT / (
    "configs/releases/"
    "fin_ia_0_1_3_s1_dell_enriched_source_successor_authority_v1_0.json"
)
DEFAULT_OUTPUT = ROOT / (
    "configs/releases/"
    "fin_ia_0_1_3_s1_dell_enriched_source_successor_result_v1_0.json"
)
PRIVATE_ROOT = ROOT / "data/workbench_private/fin_0_1_3_s1_dell_enriched_source_successor/live"
LEDGER_PATH = PRIVATE_ROOT / "shared/admission_consumption.sqlite3"


class DellEnrichedSourceRunnerError(RuntimeError):
    pass
...
def _validate_repository() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_requires_clean_worktree")
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_requires_synced_branch")
    return head
...
def _load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_json_invalid")
    return value
...
def preflight() -> dict[str, Any]:
    ...
    validate_dell_enriched_source_authority(
        ...
        execution_commit=str(authority.get("implementation_commit") or ""),
    )
    if authority.get("clean_proof_digest") != proof.get("proof_digest"):
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_proof_binding_invalid")
    ...
    if ancestor.returncode != 0:
        raise DellEnrichedSourceRunnerError(
            "dell_enriched_runner_implementation_not_ancestor"
        )
    project_os = run_project_os_preflight(ROOT, run_scope=RUN_SCOPE)
    if project_os.get("status") != "pass":
        raise DellEnrichedSourceRunnerError(
            "dell_enriched_runner_project_os_blocked:"
            + json.dumps(project_os.get("errors") or [], ensure_ascii=False)
        )
    credential = str(os.environ.get("ALPHAVANTAGE_API_KEY") or "").strip()
    if not credential:
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_market_credential_missing")
    if importlib.util.find_spec("akshare") is None:
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_akshare_shadow_unavailable")
    import akshare  # type: ignore[import-not-found]

    if akshare.__version__ != policy["market_request"]["shadow_dependency_version"]:
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_akshare_shadow_version_drift")
    runtime = PRIVATE_ROOT / "attempts" / str(authority["run_id"])
    if runtime.exists():
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_attempt_root_already_exists")
    return {
        "head": head,
        "policy": policy,
        "proof": proof,
...
def execute(*, output_path: Path) -> dict[str, Any]:
    if output_path.exists():
        raise DellEnrichedSourceRunnerError("dell_enriched_runner_output_already_exists")
    state = preflight()
    ...
    try:
        ...
        result = execute_dell_enriched_source_successor(
            ...
            primary_market_adapter=AlphaVantageDailyAdapter(),
            primary_market_credential=state["credential"],
            shadow_market_adapter=AkshareDailyShadowAdapter(),
            observed_at=state["observed_at"],
            execution_commit=state["authority"]["implementation_commit"],
...
def main() -> int:
            ...
            json.dumps(
                {
                    ...
                    "maximum_live_network_invocations": state["policy"]["budget"][
                        "maximum_live_network_invocations"
                    ],
                    "alpha_vantage_credential_configured": True,
                    "akshare_shadow_available": True,
                    "model_calls": 0,
                    "retries": 0,
        ...
        json.dumps(
            {
                ...
                "routes": [
                    {
                        "route_or_provider": row.get("route_id") or row.get("provider_id"),
                        "status": row.get("status"),
                        "failure_code": row.get("failure_code"),
                        "fragments_materialized": row.get("fragments_materialized"),
                        "authoritative_for_pack": row.get("authoritative_for_pack"),
                    }
    ...
    return 0 if result["gate_status"]["core_research_ready"] else 2
