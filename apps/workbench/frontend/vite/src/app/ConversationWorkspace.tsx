...
import { submissionFetch } from '../api/submissionFetch';
...
type Conversation = { asset_input?:{finsight_asset_revision:number;finsight_asset_memory:{version:number;body:string}}; asset_context?:import('../api/assetWorkspace').AssetContext; harness?:string; thread_id: string; title: string; status: string; messages: {id: string; role: string; content: string; final_answer?: boolean}[]; events: Event[]; runs: NonNullable<Session["runs"]>; permissions_notice: string; attachments?: {document_id:string;name:string}[];handoff?:Handoff;checkpoint_id?:string;approvals?:Approval[];approval_sources?:Record<string,{title:string;preview:string;source_url?:string;source_role?:string;unit?:string;period_start?:string;period_end?:string;accession_numbers?:string[]}> };
async function request<T>(path: string, body?: unknown): Promise<T> {
  const response = await submissionFetch(`/api/v1/conversations${path}`, { method: body === undefined ? "GET" : "POST", headers: {"Content-Type":"application/json","X-Workbench-Request":"1"}, ...(body === undefined ? {} : {body:JSON.stringify(body)}) });
  const result = await response.json();
  if (!response.ok) throw new Error(typeof result.detail === "string" ? result.detail : `请求未完成 (${response.status})`);
  return result;
}

type EmbeddedConversation={thread:string;onThread:(id:string)=>void;onResearch:(checkpoint:string,question:string)=>Promise<void>;onRemember:(text:string)=>void};
export default function ConversationWorkspace({embedded}:{embedded?:EmbeddedConversation}={}) {
  const [params, setUrlParams] = useSearchParams(); const id = embedded?.thread ?? params.get("thread") ?? "";
  const setParams=(value:{thread?:string})=>{if(embedded)embedded.onThread(value.thread||'');else setUrlParams(value);};
  ...
  useEffect(()=>{if(session?.asset_context && !session.messages.length && prefilled.current!==id){setText(session.asset_context.question);prefilled.current=id;}},[session,id]);
  ...
  return <div data-theme={embedded?(localStorage.getItem('finsight.theme')||'system'):undefined} className={`rs-shell fs-workspace fs-assistant-shell ${navOpen ? "nav-open" : ""}`}><aside className="fs-assistant-nav"><a className="fs-brand" href="/workspace"><Layers size={24}/><strong>FinSight</strong></a><button className="fs-primary" disabled={sending} onClick={() => { setParams({}); setText(""); setFiles([]); setNavOpen(false); }}><Plus size={17}/>新对话</button><a href="/workspace">研究工作台 →</a><a href="/workspace/assets">资产工作区 →</a><nav aria-label="已保存对话">{threads.map(t => <button key={t.thread_id} aria-current={id===t.thread_id ? "page":undefined} disabled={sending} onClick={() => {setParams({thread:t.thread_id}); setFiles([]); setNavOpen(false);}}><MessageSquare size={15}/><span>{t.title || "未命名对话"}</span></button>)}</nav></aside>
    <main className="fs-assistant-main"><header><button className="fs-assistant-menu" aria-expanded={navOpen} onClick={()=>setNavOpen(!navOpen)}>对话列表</button><div><small>FINSIGHT · ASSISTANT</small><h1>{session?.title || "从一个问题开始"}</h1></div><a href={embedded?`/workspace/assistant?thread=${id}`:'/workspace'}>{embedded?'完整对话':'返回研究'}</a></header>
      <div className="fs-assistant-messages" aria-live="polite" ref={reading} onScroll={e=>{const el=e.currentTarget;followBottom.current=el.scrollHeight-el.scrollTop-el.clientHeight<120;}}>{!id && <p>日常问答、资料阅读与数据核对，可以在同一对话中继续。复杂投研仍可从研究工作台启动。</p>}
        <AssetContextBanner context={session?.asset_context}/>{session?.asset_input&&<p className="aw-muted">最近一轮固定资料 r{session.asset_input.finsight_asset_revision} · 个人记忆 v{session.asset_input.finsight_asset_memory.version}。后续发送采用最新已保存记忆。</p>}{session?.handoff && <aside className="fs-assistant-handoff"><strong>从已保存的对话接续</strong><p>{session.handoff.note}</p><a href={`/workspace/assistant?thread=${session.handoff.source_thread}`}>查看原对话与依据 →</a><small>按需回读交接时的固定版本；旧回答不自动作为已核实事实。</small></aside>}
        ...
        {active && Object.entries(draftMessages).filter(([messageId]) => !session?.messages.some(m=>m.id===messageId)).map(([messageId,content])=><article className="fs-assistant-message assistant" key={messageId}><small>FinSight · 正在输出</small><ReactMarkdown remarkPlugins={[remarkGfm]}>{content.trimStart().startsWith('<｜｜DSML') ? '模型正在返回协议文本；这不是已执行的工具结果或完整回答。' : content}</ReactMarkdown></article>)}
      ...
      <footer className="fs-assistant-compose">{awaitingApproval && <button className="fs-approval-jump" onClick={()=>reading.current?.querySelector(".fs-assistant-approval")?.scrollIntoView({block:"start",behavior:"smooth"})}>有待批准操作 · 查看内容</button>}
        {embedded&&session&&<div className="aw-discussion-actions"><button disabled={sending||!!active||awaitingApproval||!session.checkpoint_id} onClick={async()=>{const question=text.trim()||[...session.messages].reverse().find(m=>m.role==='user')?.content||'';if(question.length<10){setError('请在输入框写下至少10字的研究问题，再交给研究团队。');return;}setSending(true);try{await embedded.onResearch(session.checkpoint_id!,question);}catch(e){setError((e as Error).message);}finally{setSending(false);}}}>交给研究团队</button><button onClick={()=>embedded.onRemember(text.trim())}>维护我的记忆</button><small>研究输入框或最近的提问，保留当前讨论与依据。</small></div>}
        {!embedded && id && <UserContextMenu key={`settings:${id}`} endpoint={`/api/v1/conversations/${id}/user-context`}>{session?.harness!=='hermes' && session?.runs[0] && <ContextUsage usage={session.runs[0].context_usage} nodeName={() => "当前助理"}/>}</UserContextMenu>}
        {!embedded && id && <ConversationMemory key={id} thread={id} memory={(session as (Conversation & {context_memory?:ContextMemory})|null)?.context_memory}/>}{!embedded && id && <div className="fs-assistant-memory-actions"><button disabled={sending || !!active} onClick={async()=>{try {const value=await request<HandoffPreview>(`/${id}/handoff-preview`);if(current.current!==id)return;setHandoff(value);setHandoffNote(value.latest_user_request);handoffDialog.current?.showModal();}catch(e){setError((e as Error).message);}}}><GitBranch size={15}/>保留进度并开新对话</button><button onClick={async()=>{try{await navigator.clipboard.writeText(`${location.origin}/workspace/assistant?thread=${id}`);setCopied(true);window.setTimeout(()=>setCopied(false),2000);}catch{setError("无法访问剪贴板，请复制浏览器地址栏中的对话链接。");}}}><Link size={15}/>{copied?"链接已复制":"复制对话链接"}</button></div>}
        {!embedded && id && <WorkingNotes key={`notes:${id}`} endpoint={`/api/v1/conversations/${id}/working-notes`}/>}
        ...
        <label className="fs-harness-selector">执行方式 <select aria-label="执行方式" value={id?(session?.harness||'native'):harness} disabled={!!id||sending} onChange={e=>{setHarness(e.target.value);if(e.target.value==='hermes')setFiles([]);}}><option value="native">当前 Agent</option><option value="hermes">Hermes · 工作底稿试用</option></select></label>{(session?.harness||harness)==='hermes'&&<small>本试用支持普通问答和工作底稿读写检索；执行方式在本窗口固定，其他工具尚未接入。</small>}
        <div className="fs-assistant-controls"><label>模型 <select value={model} onChange={e=>setModel(e.target.value)} disabled={sending || !!active || awaitingApproval}><option value="deepseek-v4-flash">DeepSeek V4 Flash</option><option value="deepseek-v4-pro">DeepSeek V4 Pro</option></select></label><label className="fs-permission-selector">权限 <select value={permission} onChange={e=>setPermission(e.target.value)} disabled={sending || !!active || awaitingApproval}><option value="request_standard">请求标准</option><option value="approve_for_me">代我批准</option><option value="full_access">完全访问权限</option></select></label>{active ? <button onClick={async()=>{try {await request(`/${id}/stop`,{}); await refresh();} catch(e){setError((e as Error).message);}}}><Square size={16}/>停止</button> : <button className="fs-primary" disabled={!text.trim() || sending || awaitingApproval} onClick={send}><ArrowUp size={17}/>{sending ? "发送中" : "发送"}</button>}</div>
