...
type SourceRef={read_passage_id?:string;read_offset?:number;id:string;title:string;url:string;category:string;published_at:string|null;captured_at?:string;publisher_names?:string[];metadata:{original_filing_url?:string;[key:string]:unknown}};
...
export default function MaterialReader({source,onClose,label}:{source:SourceRef;onClose:()=>void;label:(x:string)=>string}){
  const [offset,setOffset]=useState(source.read_offset||0),[view,setView]=useState<Presentation|null>(null),[parts,setParts]=useState<{body:string;id:string;locator:string;context?:string}[]>([]),[total,setTotal]=useState(0),[error,setError]=useState('');
  const reader=useRef<HTMLElement>(null);
  useEffect(()=>{const previous=document.activeElement as HTMLElement|null;reader.current?.focus();const listener=(e:KeyboardEvent)=>{if(e.key==='Escape'){e.stopImmediatePropagation();onClose();}if(e.key==='Tab'){const items=reader.current?.querySelectorAll<HTMLElement>('button:not(:disabled),a[href]');if(!items?.length)return;const first=items[0],last=items[items.length-1];if(e.shiftKey&&(document.activeElement===first||document.activeElement===reader.current)){e.preventDefault();last.focus();}else if(!e.shiftKey&&document.activeElement===last){e.preventDefault();first.focus();}}};window.addEventListener('keydown',listener,true);return()=>{window.removeEventListener('keydown',listener,true);previous?.focus();};},[onClose]);
  useEffect(()=>{const controller=new AbortController();setError('');setView(null);setParts([]);
    ...
    const read=async()=>{
      ...
      if(presentation.kind==='directory'&&!source.read_passage_id){setView(presentation);setTotal(presentation.total||0);return;}
      const anchor=source.read_passage_id&&offset===(source.read_offset||0)?`&passage_id=${encodeURIComponent(source.read_passage_id)}`:"";
      const r=await fetch(`${base}?offset=${offset}&limit=3${anchor}`,{signal:controller.signal});if(!r.ok)throw new Error('材料读取失败');const result=await r.json();setParts(result.items||[]);setTotal(result.total||0);setView({...presentation,kind:'body'});
    };void read().catch(e=>{if(e.name!=='AbortError')setError(e.message);});return()=>controller.abort();
  },[source.id,source.read_passage_id,source.read_offset,offset]);
  const directory=view?.kind==='directory',size=directory?20:3;
  return createPortal(<div className="cw-reader-backdrop" role="presentation" onClick={onClose}><section ref={reader} tabIndex={-1} className="cw-reader" role="dialog" aria-modal="true" aria-label="材料原文" onClick={e=>e.stopPropagation()}>
    ...
    <div className="cw-reader-body">{error&&<p role="alert">{error}</p>}{!view&&!error&&<p>正在读取…</p>}
      ...
      {!directory&&parts.map(part=><article className="cw-reader-fragment" key={part.id}><small>{part.locator}</small>{part.context&&<details className="cw-fragment-context" open><summary>章节与表头上下文</summary><TextBody text={part.context}/></details>}<TextBody text={part.body}/></article>)}
  ...
  </section></div>,document.querySelector('.fa-workspace')||document.body);
}
