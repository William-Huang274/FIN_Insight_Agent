...
25. **DELL 五单元动态完整案例（R7 完整物化，Case Truth split 工程门已闭合）**：R7 复用 R6 上游，只执行 Value repair 和两次 synthesis，共 3 次调用、0 retry，首次形成五个 Judgment、综合和完整内部报告。合同、身份、期间、数值、引用与产品利润桥边界通过，但 Operating／Counterevidence 错误否认当前 Case 已有的 AI revenue、orders、backlog，综合又制造 false conflict，故内容 L1/L2 失败。provider-neutral Case Truth v1 已用 reviewed Evidence facet、NumericFact、NumericRelation、source-bound QF、typed gap 和 bridge boundary 编译全案权威，并把十五个 atom surface 与 synthesis premise 纳入穷尽语义 reconciliation。15-surface natural R1 因分析与 strict submission 共置而耗尽 8k reasoning；正式零调用 R3 已证明按 cell 三 surface 分片、可见分析、non-thinking strict 交卷与本地 parent-bound 聚合，且精确拦住三条错误、保留真实利润桥 gap、拒绝漏片与重片。下一步只允许 Operating／Counterevidence 两个受影响 cell 的四调用 semantic successor；通过后才考虑其余三 cell、受影响 Judgment 与 Synthesis。即使修复报告形成，仍须完整执行 L1/L2、八维、paired 和 qualified-human。
...
4. 15-surface 低思考 semantic R1 已执行，但在完整 HTTP 200 响应中把 8k completion 全部耗作 reasoning，零可见 Tool Call；它保持不可变且不算内容失败。claim-document slice＋语义 analysis／non-thinking submission＋本地 receipt 聚合已由 formal R3 零调用通过；R2 的 canonical／wire proof 断言错误保持失败。下一门只验证 Operating／Counterevidence 两个受影响 cell，各一条 analysis＋submission，共最多四次调用、0 retry。通过后才允许新 authority 下处理其余三 cell 或修复 Operating、Counterevidence 和 Synthesis；Demand、Value、Cash、Planner、S1/S2 与五个研究 analysis 默认复用。
