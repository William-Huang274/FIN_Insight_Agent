...
POLICY_REF = Path(
    "configs/runtime/fin_ia_0_1_3_s1_candidate_bundle_"
    "sparse_dense_manifest_r4_policy_v1_0.json"
)
SOURCE_RESULT_REF = Path(
    "configs/releases/fin_ia_0_1_3_s1_candidate_bundle_"
    "sparse_dense_manifest_r4_zero_call_proof_v1_0.json"
)
PROOF_REF = Path(
    "configs/releases/fin_ia_0_1_3_s1_candidate_bundle_"
    "sparse_dense_manifest_r4_clean_independent_proof_v1_0.json"
)
REPARSE_POLICY_REF = Path(
    "configs/runtime/fin_ia_0_1_3_s1_three_held_out_"
    "current_source_reparse_successor_r9_policy_v1_0.json"
)
REPARSE_RESULT_REF = Path(
    "configs/releases/fin_ia_0_1_3_s1_three_held_out_"
    "current_source_reparse_successor_r9_result_v1_0.json"
)
REPARSE_PROOF_REF = Path(
    "configs/releases/fin_ia_0_1_3_s1_three_held_out_"
...
CURRENT_SCOPE = "S1_IMMUTABLE_SUPPLEMENTAL_DENSE_INDEX_REPLACEMENT_BUILD"
EXPECTED_RESULT_DIGEST = (
    "d84b7ef21bac98d90567c70300fd45b55daac74089ae45b813d70c994c98e7a1"
)
EXPECTED_REPARSE_RESULT_DIGEST = (
    "caee03a519403b3dbcf6b15bd3cf9969482596771a09e8277bc094cb016c7f3e"
)
ATTEMPT_ID = "S1-CANDIDATE-BUNDLE-SPARSE-DENSE-MANIFEST-R4-CLEAN-PROOF-A1"
PREDECESSOR_FAILURE_REFS = (
    Path(
        "configs/releases/fin_ia_0_1_3_s1_candidate_bundle_sparse_dense_"
        "manifest_zero_call_r1_failure_v1_0.json"
    ),
    Path(
        "configs/releases/fin_ia_0_1_3_s1_candidate_bundle_sparse_dense_"
        "manifest_zero_call_r2_failure_v1_0.json"
    ),
    Path(
        "configs/releases/fin_ia_0_1_3_s1_candidate_bundle_sparse_dense_"
        "manifest_zero_call_r3_business_audit_failure_v1_0.json"
    ),
)
...
class CleanManifestR4ProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise CleanManifestR4ProofError(code)
...
def _assert_committed_source_synced() -> dict[str, Any]:
    ...
    _require(not _git("diff", "--cached", "--name-only"), "source_index_not_clean")
    ...
    with tarfile.open(archive_path, "r") as archive:
        for member in archive.getmembers():
            ...
            try:
                ...
                raise CleanManifestR4ProofError(
                    "git_archive_member_outside_root"
                ) from exc
...
def _selected_capture_descriptor(
    *,
    ...
    if selector == "held_out_case_source":
        ...
        raise CleanManifestR4ProofError(
            f"capture_selector_unsupported:{selector}"
        )
...
def _copy_bound_captures(archive_root: Path) -> list[dict[str, Any]]:
    policy = _load(ROOT / REPARSE_POLICY_REF)
    artifacts = {
    ...
    for binding in policy.get("source_bindings") or []:
        ...
        try:
            ...
            raise CleanManifestR4ProofError(
                "capture_copy_path_outside_root"
            ) from exc
        expected_digest = str(descriptor["response_capture_digest"])
        _require(source.is_file(), f"capture_source_missing:{case_key}")
        _require(
            _sha256(source) == expected_digest,
            f"capture_source_digest_drift:{case_key}",
        )
        destination.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(source, destination)
        _require(
            _sha256(destination) == expected_digest,
            f"capture_copy_digest_drift:{case_key}",
        )
        copied.append(
            {
                "case_key": case_key,
    ...
    _require(
        [row["case_key"] for row in copied] == ["ORCL", "ASML", "ANET"],
        "capture_case_order_drift",
    )
    _require(
        len({row["response_capture_digest"] for row in copied}) == 3,
        "capture_digest_not_unique",
    )
    return copied


def _blocked_network(*args: Any, **kwargs: Any) -> Any:
    raise CleanManifestR4ProofError("clean_manifest_r4_proof_network_forbidden")
...
def _worker_payload(runtime_root: Path) -> dict[str, Any]:
    ...
    from sec_agent.s1_candidate_bundle_index_manifest import (
    ...
    reparse_preflight = run_project_os_preflight(
        runtime_root,
        run_scope=REPARSE_SCOPE,
    )
    manifest_preflight = run_project_os_preflight(
        runtime_root,
        run_scope=CURRENT_SCOPE,
    )
    _require(
        reparse_preflight.get("status") == "pass"
        and manifest_preflight.get("status") == "pass",
        "clean_scope_preflight_failed",
    )
    _require(
        not reparse_preflight.get("contract_errors")
        and not manifest_preflight.get("contract_errors"),
        "clean_scope_contract_error",
    )
    committed_reparse = _load(runtime_root / REPARSE_RESULT_REF)
    _require(
        str(committed_reparse.get("result_digest") or "")
        == EXPECTED_REPARSE_RESULT_DIGEST,
        "committed_reparse_result_digest_unexpected",
    )
    reparse_policy = load_current_source_reparse_policy(
        runtime_root / REPARSE_POLICY_REF,
        repo_root=runtime_root,
    )
    reparse_root = runtime_root / (
        "data/workbench_private/"
        "fin_0_1_3_s1_three_held_out_current_source_reparse/zero-call-r9"
    )
    _require(not reparse_root.exists(), "clean_worker_reparse_root_exists")
    reproduced_reparse = execute_current_source_reparse(
        policy=reparse_policy,
        repo_root=runtime_root,
        runtime_root=reparse_root,
    )
    validate_current_source_reparse_result(reproduced_reparse)
    _require(
        _canonical_bytes(reproduced_reparse) == _canonical_bytes(committed_reparse),
        "clean_worker_reparse_result_not_exact_match",
    )
    committed_manifest = _load(runtime_root / SOURCE_RESULT_REF)
    _require(
        str(committed_manifest.get("proof_digest") or "") == EXPECTED_RESULT_DIGEST,
        "committed_manifest_result_digest_unexpected",
    )
    manifest_policy = load_candidate_bundle_index_policy(
        runtime_root / POLICY_REF,
        repo_root=runtime_root,
    )
    manifest_root = runtime_root / (
        "data/workbench_private/"
        "fin_0_1_3_s1_candidate_bundle_sparse_dense_manifest/clean-proof-r4"
    )
    _require(not manifest_root.exists(), "clean_worker_manifest_root_exists")
    reproduced_manifest = materialize_candidate_bundle_index_zero_call_proof(
        policy=manifest_policy,
        repo_root=runtime_root,
        output_runtime_root=manifest_root,
        source_runtime_root=reparse_root,
    )
    validate_candidate_bundle_index_zero_call_proof(reproduced_manifest)
    _require(
        _canonical_bytes(reproduced_manifest) == _canonical_bytes(committed_manifest),
        "clean_worker_manifest_result_not_exact_match",
    )
    reparse_calls = reproduced_reparse["observed_calls"]
    manifest_calls = reproduced_manifest["observed_calls"]
    _require(
        set(reparse_calls.values()) == {0}
        and set(manifest_calls.values()) == {0},
        "clean_worker_nonzero_call",
    )
    return {
        "schema_version": "fin_ia_0_1_3_s1_candidate_bundle_manifest_r4_clean_worker_v1_0",
        "status": "pass",
        "preflight_scopes": [REPARSE_SCOPE, CURRENT_SCOPE],
        "preflights": ["pass", "pass"],
        "matches_committed_result": True,
        "result_digest": reproduced_manifest["proof_digest"],
        "reparse_result_digest": reproduced_reparse["result_digest"],
        "case_bundle_counts": [
            row["observed_counts"]["bundle_projected"]
            for row in reproduced_reparse["case_results"]
        ],
        "case_slot_counts": [
            len(row["projected_slot_ids"])
            for row in reproduced_reparse["case_results"]
        ],
        "selection_summary": reproduced_manifest["selection_summary"],
        "fake_build": reproduced_manifest["fake_build"],
        "manifest_mutations_passed": [
            sum(
                row["observed_code"] != "did_not_fail_closed"
                for row in reproduced_manifest["mutation_proof"]["rows"]
            ),
            reproduced_manifest["mutation_proof"]["scenario_count"],
        ],
        "observed_calls": {
            "reparse": reparse_calls,
...
def build_result() -> dict[str, Any]:
    ...
    _require(
        str(committed.get("proof_digest") or "") == EXPECTED_RESULT_DIGEST,
    ...
    source_bindings = {
        ...
        for ref in (
            ...
            Path("src/sec_agent/s1_candidate_bundle_index_manifest.py"),
        )
    }
    workers: list[dict[str, Any]] = []
    capture_manifests: list[list[dict[str, Any]]] = []
    with tempfile.TemporaryDirectory(
        prefix="fin013-s1-manifest-r4-clean-proof-"
    ) as parent:
        parent_path = Path(parent).resolve()
        _require(
            parent_path.name.startswith(
                "fin013-s1-manifest-r4-clean-proof-"
            ),
    ...
    body = {
        "schema_version": "fin_ia_0_1_3_s1_candidate_bundle_sparse_dense_manifest_r4_clean_independent_proof_v1_0",
        "contract_ref": "fin_0_1_3.S1.candidate_bundle_sparse_dense_manifest_r4_clean_independent_proof:v1",
        ...
        "upstream_reparse_result_ref": REPARSE_RESULT_REF.as_posix(),
        "upstream_reparse_result_digest": EXPECTED_REPARSE_RESULT_DIGEST,
        ...
        "proof_method": {
            ...
            "capture_copy_mode": (
                "three_exact_publicly_referenced_response_capture_objects_only"
            ),
            "capture_digest_verification": True,
            "project_os_preflight_scopes_each_archive": [
                REPARSE_SCOPE,
                CURRENT_SCOPE,
            ],
            "upstream_reparse_reexecuted_each_archive": True,
            "manifest_materialized_each_archive": True,
        ...
        "stage_acceptance": {
            ...
            "full_business_period_role_audit": True,
            "ubuntu_real_build_authority_decision_admitted": True,
        ...
        "decision_zh": (
            "两个独立 Git archive 在两个 fresh process 中仅注入三份 digest-bound "
            "原始响应，均先逐字节重现 R9，再逐字节重现 manifest R4 的 93 specs、"
            "19 条叙事隔离、15 mutations 与 fake sparse/dense。R1-R3 失败继续保留；"
            "下一步只准入 Ubuntu 真实 build 的独立 authority decision，不自动执行 BGE 或 Milvus。"
        ),
        "known_boundary": (
            "This proof establishes clean reproducibility of the R9-to-manifest-R4 zero-call "
            "chain only. It does not create a physical sparse/dense index, prove ranking, "
            "promote Evidence, close external coverage, call DeepSeek, establish report "
            "quality or accept release."
        ),
    }
