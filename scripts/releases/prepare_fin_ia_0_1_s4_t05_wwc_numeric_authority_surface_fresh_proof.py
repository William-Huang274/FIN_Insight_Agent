...
EXECUTION_IDENTITY = (
    "fin01-s4-t05-dell-wwc-numeric-authority-exact-live-r4"
)
PROSPECTIVE_ADMISSION_ID = (
    "fin01-s4-t05-dell-wwc-numeric-authority-"
    "fresh-exact-admission-r4"
)
EXECUTION_MODE = "exact_live_s4_dell_wwc_numeric_authority_r4"
...
class S4T05WhatWouldChangeAuthorityFreshProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T05WhatWouldChangeAuthorityFreshProofError(code)
...
def prepare(
    *,
    ...
    _require(
        source_admission.task_claim_link_policy_ref
        == S3_TASK_CLAIM_LINK_POLICY_REF,
        "source_task_claim_policy_binding_missing",
    )
    _require(
        source_admission.transport_ref
        == S3_OWNER_GRADE_SEGMENTED_SPECIALIST_TRANSPORT_V7_REF,
        "source_transport_is_not_v7",
    )
    ...
    _require(
        ...
        "v7_WWC_authority_capability_missing",
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin01-s4-t05-WWC-authority-fresh-proof-"
    ) as temp_dir:
    ...
    cell_inputs = first.input_pack.model_dump(mode="json")["cell_inputs"]
    demand_cell = next(
        row
        for row in cell_inputs
    ...
    authority_contract = authority_policy.prompt_contract()
    _require(
        authority_contract["contract_ref"]
        == S3_WHAT_WOULD_CHANGE_AUTHORITY_POLICY_REF,
        "WWC_authority_contract_ref_mismatch",
    )
    _require(
        len(authority_policy.numeric_refs) == 6,
        "DELL_demand_numeric_authority_count_mismatch",
    )
    _require(
        set(authority_policy.allowed_refs)
        == {
            ref
            for values in authority_contract[
                "allowed_refs_by_authority_class"
            ].values()
            for ref in values
        },
        "WWC_prompt_validator_surface_mismatch",
    )
    ...
    _require(
        ...
        == S3_TASK_CLAIM_LINK_POLICY_REF,
        "prospective_task_claim_policy_binding_missing",
    )
    _require(
        prospective_admission.transport_ref
        == S3_OWNER_GRADE_SEGMENTED_SPECIALIST_TRANSPORT_V7_REF,
        "prospective_v7_transport_binding_missing",
    )
    callback_calls = 0
    ...
    return {
        ...
        "recorded_at": "2026-07-27T05:00:00+08:00",
        "status": (
        ...
        "implementation_reaudit": {
            ...
            "single_membership_owner": "cell_input.authority_refs",
            "v7_runtime_injected": True,
            "node_level_fixture_proven": True,
            "historical_admission_capture_or_Run_rewritten": False,
        },
        "fresh_identity": {
            "execution_identity": EXECUTION_IDENTITY,
            "case_id": case_id,
        ...
        "WWC_authority_reproof": {
            "contract_ref": S3_WHAT_WOULD_CHANGE_AUTHORITY_POLICY_REF,
            ...
            "DELL_demand_numeric_ref_count": len(
                authority_policy.numeric_refs
            ),
            "provider_prompt_and_local_validator_surface_equal": True,
            "legacy_numeric_input_membership_owner": False,
            "cross_cell_normalization_fuzzy_remap_drop_or_relink_allowed": False,
            "complete_WWC_failure_taxonomy_claimed": False,
        },
        "prospective_admission": {
            ...
            "v7_WWC_authority_capability_bound_by_transport_and_code": True,
            "task_claim_link_policy_preserved": True,
        ...
        "future_success_contract": {
            ...
            "all_three_WWC_segments_consume_shared_authority_policy": True,
            "all_three_WWC_segments_consume_task_claim_policy": True,
            "outside_or_cross_Cell_WWC_authority_count": 0,
        ...
        "root_cause_disposition": {
            ...
            "prior_status": (
                "implementation_full_S4_regression_proven_"
                "fresh_agent_proof_pending"
            ),
            "new_status": (
