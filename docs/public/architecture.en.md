...
v0.1.3 · Local financial research workspace · [中文](architecture.zh-CN.md)

The current architecture supports source queries, multi-agent research, review and report delivery. Method libraries, rule updates and research time budgets are planned for v0.1.4.
...
Old tool text is cleared only from the outgoing request projection; messages and artifacts remain in native checkpoints. Complete CALCs from saved answers are exposed through a task-scoped read-only tool view without reinserting entire citation trees into model messages. Missing, conflicting or cross-task records are rejected. Automatic summaries remain disabled pending further evaluation. The BFF paginates all native runs and uses run ID plus call ID for identity; unknown usage/cache/time remains explicit, with external imported-revision costs separate.
...
v0.1.3 includes OIDC/PKCE with two users, 39 resource-ownership checks, 8 concurrent reads and 4 actual frontend approval scenarios. Approved operations resume natively through authenticated MCP into a Docker sandbox; rejection prevents execution. Tools follow their configured capabilities. These are local evaluations, not production multitenancy, comprehensive malicious-document security or HA certification. Empty retrieval does not prove non-disclosure.

Verified workflows include reports and human review, failure recovery, targeted revisions, restart handoff and submission deduplication. The 19 real user turns include recovery and do not establish financial quality across arbitrary companies or production P95. Runtime modules use functional names while existing graph, database and source identifiers remain compatible.
...
A September 8 live short question verified that an edited method entered two model requests and changed answer organization without changing the report. Native synthetic graph tests cover specialist request validation and review order. This is not live evaluation of every role or company.
