...
from sec_agent.agent_runtime.execution_profile_checks import (
...
class _ResearchGraphTestGraph:
    ...
    def _thread_id(config: Mapping[str, Any] | None) -> str:
        ...
        if not isinstance(thread_id, str) or not thread_id.strip():
            raise ResearchGraphError("thread_id_required")
    ...
    def _safe_input(
        ...
        if isinstance(value, Command):
            if value.update is not None:
                raise ResearchGraphError("command_update_not_allowed")
            if value.goto:
                raise ResearchGraphError("command_goto_not_allowed")
            if value.graph is not None:
                raise ResearchGraphError("command_graph_override_not_allowed")
            if value.resume is None:
                raise ResearchGraphError("command_resume_value_required")
            return value
        if not isinstance(value, Mapping):
            raise ResearchGraphError("initial_graph_input_must_be_mapping")
        initial = dict(value)
        if initial.get("run_id") != thread_id:
            raise ResearchGraphError("thread_id_run_id_mismatch")
        return initial
...
class FakeRuntime:
    ...
    def dependencies(self) -> ResearchGraphDependencies:
...
def test_model_output_with_failure_receipt_is_rejected() -> None:
    ...
    graph = _build_research_graph_test_graph(
...
def _build(runtime: FakeRuntime, *, execution_profile: str = "product"):
    return _build_research_graph_test_graph(
        dependencies=runtime.dependencies(),
        checkpointer=InMemorySaver(),
        execution_profile=execution_profile,
    )
...
def test_join_rejects_duplicate_missing_and_cross_branch_lane_results() -> None:
    ...
    with pytest.raises(ResearchGraphError, match="duplicate"):
        _join_initial_lane_results(
            tasks=tasks,
            evidence_values=[evidence[0], evidence[0], evidence[1]],
            finance_values=finance,
        )
    with pytest.raises(ResearchGraphError, match="set_mismatch"):
        _join_initial_lane_results(
            tasks=tasks,
    ...
    with pytest.raises(ResearchGraphError, match="binding_mismatch"):
        _join_initial_lane_results(
            tasks=tasks,
...
def test_local_test_wrapper_requires_checkpointer_and_exact_thread_binding() -> None:
    ...
    with pytest.raises(
        ...
        _build_research_graph_test_graph(
            dependencies=runtime.dependencies(),
            checkpointer=None,
        )

    graph = _build(runtime)
    with pytest.raises(ResearchGraphError, match="thread_id_run_id_mismatch"):
        graph.invoke(_start_input(), _config("another-thread"))
    with pytest.raises(ResearchGraphError, match="command_update_not_allowed"):
