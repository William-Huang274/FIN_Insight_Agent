...
export const pageTitles: Record<string, string> = { completed: "已完成研究", home: "开始研究", studio: "研究配置", all: "全部研究", inbox: "待审阅", new: "新建研究", preferences: "外观与偏好", review: "审查意见", ...Object.fromEntries(taskViews.map(v => [v.id, v.title])) };
export const isResearchComplete = (s: Session) => !["busy","error"].includes(s.status) && ["human_completed","human_reviewed_not_released"].includes(s.phase || "");
export const sessionStatus = (s: Session) => s.is_draft || s.phase === "draft" ? "资料准备中" : s.status === "busy" ? "运行中" : s.status === "error" ? "执行失败" : s.phase === "research_orientation_submitted" ? "预研究完成" : s.phase === "research_needs_attention" ? "研究受阻 · 待处理" : s.phase === "single_agent_unreviewed" ? "单 Agent 结果 · 未复核" : s.phase === "human_completed" ? `人工修改 ${s.human_edit_count || s.human_edits?.length || 0} 次 · 已完成` : s.phase === "human_reviewed_not_released" ? "已人工审阅" : s.status === "interrupted" ? "等待审阅" : "已保存";

export function WorkspaceNavigation({ sessions, id, page, collapsed, onCollapse, navigate, projects, onProjects, editingDisabled, onOpenProject }: { sessions: Session[]; id: string; page: string; collapsed: boolean; onCollapse: () => void; navigate: (page: string, id?: string) => void; projects: ProjectIndex; onProjects: (index: ProjectIndex) => Promise<boolean>; editingDisabled: boolean; onOpenProject: (id:string)=>void }) {
  ...
  const content = (mobile = false) => <>
    ...
    <a className="fs-assistant-entry" href="/workspace/projects"><FolderOpen size={17}/><span>所有项目</span></a>
    <a className="fs-assistant-entry" href="/workspace/assets?view=files"><Database size={17}/><span>资料库</span></a>
    ...
    <div className="fs-recent"><div className="fs-side-caption">项目与研究 <button title="管理项目" aria-label="管理项目" onClick={() => { drawer.current?.close(); organize.current?.showModal(); }}><FolderPlus size={16} /></button></div>
      <nav aria-label="最近研究">
        ...
        {[{id:"__completed",name:"已完成研究"}, ...projects.projects, {id:"",name:"进行中与待处理"}].map(project => {
          const items = sessions.filter(s => project.id === "__completed" ? isResearchComplete(s) : !isResearchComplete(s) && (projects.assignments[s.thread_id] || "") === project.id);
          if (!project.id && !items.length) return null;
          return <details className="fs-project" key={project.id} open><summary><ChevronRight size={13} /><FolderOpen size={16} /><span>{project.name}</span><small>{items.length}</small>{project.id && project.id !== '__completed' && <button aria-label={`打开项目 ${project.name}`} onClick={e=>{e.preventDefault();e.stopPropagation();drawer.current?.close();onOpenProject(project.id);}}>资料</button>}</summary>
            {items.map(s => <div key={s.thread_id} className={s.thread_id === id ? "fs-task selected" : "fs-task"}>
              ...
              {s.thread_id === id && !["home","all","completed","inbox","preferences","studio","new"].includes(page) && <div className="fs-subnav">{taskViews.map(v => <button key={v.id} aria-current={page === v.id ? "page" : undefined} onClick={() => go(v.id, id)}><v.icon size={15} />{v.title}</button>)}</div>}
  ...
  return <><aside className={`fs-sidebar ${collapsed ? "is-collapsed" : ""}`}>{content()}</aside>
    ...
    <dialog ref={organize} className="fs-organize-dialog" aria-label="管理项目">
      <header><div><h2>整理你的研究</h2><p>项目 → 研究任务 → 报告、追问与修订。项目、归类与置顶保存在当前工作台服务。</p></div><button aria-label="关闭项目管理" onClick={() => organize.current?.close()}><X size={19} /></button></header>
      <fieldset disabled={editingDisabled}><form onSubmit={e => { e.preventDefault(); const name = projectName.trim(); if (!name || projects.projects.some(p => p.name === name)) return; void onProjects({...projects, projects:[...projects.projects,{id:crypto.randomUUID(),name}]}).then(saved=>{if(saved)setProjectName("");}); }}>
