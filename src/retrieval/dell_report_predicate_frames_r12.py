...
def _frame_records_and_boundaries(
    ...
    for sentence_index, raw_sentence in enumerate(raw_sentences):
        ...
        if sentence_start < 0:
            raise ValueError("R12_normalized_sentence_provenance_lost")
...
def _modifier_frame_id(
    *,
    ...
    return f"MODIFIER::R12::{digest[:24].upper()}"
...
_PRODUCT_COMPLEMENT_LINK = re.compile(r"\b(?:for|of|on)\b", re.IGNORECASE)
    ...
    for index, price in enumerate(prices):
        ...
        output.append(
            ArgumentGroupBinding(
                group_id=f"ARG::R12::{index:02d}::{digest[:20].upper()}",
                span_start=body["span_start"],
                span_end=body["span_end"],
                raw_text=text,
...
def _finalize(
    *,
    ...
    body = {
        ...
        # Compatibility aliases are intentionally explicit in the R12 public IR.
        "modality": actuality,
    ...
    return PredicateFrame(
        frame_id=f"FRAME::R12::{digest[:24].upper()}",
        frame_digest=digest,
        assertion_frame_id=assertion_frame_id,
        target_id=target_id,
        sentence_index=record.sentence_index,
...
def predicate_frame_candidate_surface(*, target_id: str, text: str) -> bool:
    """Cheap necessary-condition guard; false proves no R12 target frame."""

    if target_id not in TARGET_IDS:
        raise ValueError(f"unsupported_R12_target:{target_id}")
...
def _r12_frame_base_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
) -> dict[str, Any]:
    """Build only inherited fields that survive when an R12 frame exists."""
...
def classify_package_with_frames(
    *,
    ...
    assessment["semantic_guard_revision"] = (
        "R12_CLAUSE_OWNERSHIP_PRICE_ATTACHMENT_PROOF_FRAME"
    )
    ...
    if accepted and assessment.get("in_period") is True:
        ...
        limitations.update(
            f"missing_R12_role:{role}" for role in selected.missing_required_roles
        )
        limitations.update(f"R12_ambiguity:{row}" for row in selected.ambiguities)
        ...
        assessment["limitations"] = sorted(
            ...
            | {"no_single_R12_relational_target_frame"}
        )
...
def classify_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
    ...
    if boundary_deferred:
        raise AssertionError("R12_public_classification_cannot_defer_boundaries")
    return assessment
