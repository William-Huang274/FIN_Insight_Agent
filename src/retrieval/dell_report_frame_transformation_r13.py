...
class FrameTransformationBindingR13Error(ValueError):
    pass
...
def _require_identifier(value: str, *, field: str) -> str:
    if not value or value.strip() != value or re.search(r"[\x00-\x1f]", value):
        raise FrameTransformationBindingR13Error(
            f"R13_transformation_invalid_identifier:{field}"
        )
    return value
...
def validate_predicate_frame_r13_integrity(frame: PredicateFrame) -> None:
    for proof in frame.clause_ownership_proofs:
        ...
        if observed_proof_digest != canonical_digest(proof_body):
            raise FrameTransformationBindingR13Error(
                "R13_transformation_clause_ownership_proof_digest_invalid"
            )
    for group in frame.argument_groups:
        ...
        if group.normalized_proof_identity_digest != expected_proof_identity:
            raise FrameTransformationBindingR13Error(
                "R13_transformation_normalized_proof_identity_invalid"
            )
        proof_body = {
            "proof_state": group.proof_state,
            "proof_type": group.proof_type,
        ...
        if group.proof_digest != canonical_digest(proof_body):
            raise FrameTransformationBindingR13Error(
                "R13_transformation_price_attachment_proof_digest_invalid"
            )
        ...
        if observed_group_digest != canonical_digest(group_body):
            raise FrameTransformationBindingR13Error(
                "R13_transformation_argument_group_digest_invalid"
            )
        if (
            ...
            raise FrameTransformationBindingR13Error(
                "R13_transformation_price_attachment_proof_structure_invalid"
            )
    ...
    if frame.semantic_signature_digest != expected_semantic:
        raise FrameTransformationBindingR13Error(
            "R13_transformation_source_or_compiled_semantic_digest_invalid"
        )
    if (
        frame.frame_digest != expected_representation
        or frame.representation_frame_digest != expected_representation
        or frame.frame_id
        != f"FRAME::R13::{expected_representation[:24].upper()}"
    ):
        raise FrameTransformationBindingR13Error(
            "R13_transformation_source_or_compiled_representation_digest_invalid"
        )
...
def _mapping(
    ...
    mapping_id = f"ROLEMAP::R13::{canonical_digest(core)[:24].upper()}"
...
def build_frame_transformation_binding_r13(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    validate_predicate_frame_r13_integrity(source_frame)
    validate_predicate_frame_r13_integrity(compiled_frame)
    ...
    binding_id = f"FRAMEBIND::R13::{canonical_digest(core)[:24].upper()}"
    ...
    if require_lossless and not binding.binding_accepted:
        ...
        raise FrameTransformationBindingR13Error(
            "R13_transformation_not_lossless:" + "|".join(findings)
        )
    return binding


def build_missing_compiled_frame_binding_r13(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    core = {
        ...
        "compiled_object_ids": (),
        "compiled_window_ids": (),
        "compiled_frame_id": "MISSING::R13::COMPILED_FRAME",
...
def transformation_binding_digest_r13(
...
def validate_frame_transformation_binding_record_r13(
    ...
    if set(row) != _BINDING_RECORD_KEYS:
        raise FrameTransformationBindingR13Error(
            "R13_transformation_binding_unknown_or_missing_key"
        )
    ...
    if not isinstance(raw_mappings, Sequence) or isinstance(
        ...
        raise FrameTransformationBindingR13Error(
            "R13_transformation_role_mappings_not_sequence"
        )
    mapping_ids: set[str] = set()
    for raw_mapping in raw_mappings:
        if not isinstance(raw_mapping, Mapping):
            raise FrameTransformationBindingR13Error(
                "R13_transformation_role_mapping_not_mapping"
            )
        mapping = dict(raw_mapping)
        if set(mapping) != _ROLE_MAPPING_RECORD_KEYS:
            raise FrameTransformationBindingR13Error(
                "R13_transformation_role_mapping_unknown_or_missing_key"
            )
        ...
        if (
            ...
            raise FrameTransformationBindingR13Error(
                "R13_transformation_role_mapping_identity_invalid"
            )
    ...
    if (
        ...
        raise FrameTransformationBindingR13Error(
            "R13_transformation_binding_self_digest_invalid"
        )
    ...
    if accepted is True:
        if row.get("semantic_signature_equal") is not True or material_flags:
            raise FrameTransformationBindingR13Error(
                "R13_transformation_binding_contains_material_flags"
            )
    elif accepted is False:
        if not material_flags:
            raise FrameTransformationBindingR13Error(
                "R13_transformation_failed_binding_without_material_flag"
            )
    else:
        raise FrameTransformationBindingR13Error(
            "R13_transformation_binding_acceptance_not_boolean"
        )
    return dict(value)


def validate_frame_transformation_binding_r13(
    value: Mapping[str, Any],
) -> None:
    row = validate_frame_transformation_binding_record_r13(value)
    if row.get("binding_accepted") is not True:
        raise FrameTransformationBindingR13Error(
            "R13_transformation_binding_not_accepted"
        )
