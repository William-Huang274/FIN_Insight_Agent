...
from sec_agent.agent_runtime import agent_server_entry as entry
from sec_agent.agent_runtime.agent_server_identity import (
...
def _persisted_binding(**overrides: Any) -> PersistedRunInvocationBinding:
    fields = {
        ...
        "assistant_id": entry.AGENT_SERVER_ASSISTANT_ID,
...
def test_root_langgraph_config_exposes_only_the_server_factory() -> None:
    ...
    assert value["graphs"] == {
        "dell_reference_vertical": {
            "path": (
                "sec_agent.agent_runtime.agent_server_entry:"
                'research_graph'
            ),
            "description": "Research and report-review workflow",
        }
    }
...
def test_read_and_schema_access_open_no_execution_resources(
    ...
    monkeypatch.setenv(entry.EXECUTION_PROFILE_ENV, "not-a-valid-profile")

    async def exercise() -> None:
        async with entry.research_graph({}, _ReadRuntime()) as graph:
...
def test_paid_shadow_schema_uses_same_server_factory_without_credentials(
    ...
    monkeypatch.setenv(
        entry.SERVING_MODE_ENV,
        entry.Q1_PAID_SHADOW_SERVING_MODE,
    )
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
    monkeypatch.delenv(entry.Q1_PAID_SHADOW_AUTHORITY_ENV, raising=False)
...
def test_execution_profile_is_deployment_owned_and_checked_before_resources(
    ...
    monkeypatch.setattr(entry, "_open_execution_dependencies", forbidden)
    monkeypatch.setenv(entry.EXECUTION_PROFILE_ENV, "qualification")

    async def exercise() -> None:
        async with entry.research_graph(
            _config(), _ExecutionRuntime(_context())
        ):
            pass

    with pytest.raises(entry.AgentServerEntryError) as failure:
        asyncio.run(exercise())
    assert failure.value.code == "dell_execution_profile_invalid"
    assert opened == 0


def test_lead_schema_is_resource_free_and_not_an_execution_fallback(monkeypatch):
    from test_specialist_graph import _input
    from sec_agent.agent_runtime.lead_research_graph import LeadResearchError
    monkeypatch.setenv(entry.SERVING_MODE_ENV, entry.LEAD_RESEARCH_SERVING_MODE)
    monkeypatch.delenv("DEEPSEEK_API_KEY", raising=False)
    async def exercise():
        async with entry.research_graph({}, _ReadRuntime()) as graph:
            assert set(graph.get_graph().nodes) >= {"lead", "lead_tools", "specialist", "collect_task_artifacts"}
            with pytest.raises(LeadResearchError, match="schema_introspection_graph_not_executable"):
                graph.invoke(_input())
    asyncio.run(exercise())
...
def test_invalid_run_context_traceback_does_not_chain_rejected_input() -> None:
    ...
    except entry.AgentServerEntryError as exc:
        rendered = "".join(
...
def test_execution_profile_accepts_only_exact_supported_values(
    ...
    monkeypatch.setenv(entry.EXECUTION_PROFILE_ENV, value)
...
def test_execution_requires_langsmith_tracing_and_pat(
    ...
    async def missing_tracing_call() -> None:
        async with entry.research_graph(_config(), runtime):
            pass

    with pytest.raises(entry.AgentServerEntryError) as missing_tracing:
    ...
    with pytest.raises(entry.AgentServerEntryError) as tracing_alias:
        asyncio.run(missing_tracing_call())
    assert tracing_alias.value.code == "langsmith_tracing_required"

    monkeypatch.setenv("LANGSMITH_TRACING", "true")
    with pytest.raises(entry.AgentServerEntryError) as missing_pat:
        asyncio.run(missing_tracing_call())
    assert missing_pat.value.code == "langsmith_api_key_required"

    monkeypatch.setenv("LANGSMITH_API_KEY", "test-only-not-a-real-key")
    with pytest.raises(entry.AgentServerEntryError) as missing_project:
        asyncio.run(missing_tracing_call())
...
def test_execution_requires_one_deployment_owned_langsmith_project(
    ...
    async def call(config: dict[str, Any]) -> None:
        async with entry.research_graph(config, runtime):
            pass

    with pytest.raises(entry.AgentServerEntryError) as wrong_project:
        asyncio.run(call(_config()))
    assert wrong_project.value.code == "langsmith_project_mismatch"

    monkeypatch.setenv("LANGSMITH_PROJECT", entry.LANGSMITH_PROJECT)
    config: dict[str, Any] = _config()
    config["configurable"]["__langsmith_project__"] = "per-run-replica"
    with pytest.raises(entry.AgentServerEntryError) as run_override:
...
def test_execution_requires_langsmith_input_and_output_redaction(
    ...
    with pytest.raises(entry.AgentServerEntryError) as missing_inputs:
        asyncio.run(call())
    assert missing_inputs.value.code == "langsmith_inputs_must_be_hidden"

    monkeypatch.setenv("LANGSMITH_HIDE_INPUTS", "true")
    with pytest.raises(entry.AgentServerEntryError) as missing_outputs:
        asyncio.run(call())
    ...
    with pytest.raises(entry.AgentServerEntryError) as truthy_alias:
        asyncio.run(call())
    assert truthy_alias.value.code == "langsmith_inputs_must_be_hidden"

    monkeypatch.setenv("LANGSMITH_HIDE_INPUTS", "true")
    monkeypatch.setenv("LANGSMITH_HIDE_OUTPUTS", "1")
    with pytest.raises(entry.AgentServerEntryError) as output_alias:
        asyncio.run(call())
...
def test_durable_identity_guard_requires_uri_and_exact_persisted_binding(
    ...
    with pytest.raises(entry.AgentServerEntryError) as missing_uri:
    ...
    with pytest.raises(entry.AgentServerEntryError) as profile_conflict:
        entry._require_durable_execution_binding(
            identity,
    ...
    with pytest.raises(entry.AgentServerEntryError) as conflict:
        entry._require_durable_execution_binding(
            identity,
            execution_profile="zero_model_control_plane_v1",
        )
...
def test_execution_rejects_pending_or_corrupt_reconciled_lifecycle() -> None:
    ...
    try:
        ...
        with pytest.raises(entry.AgentServerEntryError) as pending:
            entry._require_durable_execution_binding(
                identity,
                execution_profile="zero_model_control_plane_v1",
            )
        ...
        with pytest.raises(entry.AgentServerEntryError) as corrupt:
            entry._require_durable_execution_binding(
                identity,
                execution_profile="zero_model_control_plane_v1",
            )
...
def test_missing_durable_binding_opens_no_data_or_mcp_resources(
    ...
    async def exercise() -> None:
        async with entry._open_execution_dependencies(
            identity,
            AgentServerRunContext.model_validate(_context()),
            execution_profile="zero_model_control_plane_v1",
        ):
            pass
...
def test_context_and_server_identity_are_both_required() -> None:
    with pytest.raises(entry.AgentServerEntryError) as missing_context:
        entry.bind_agent_server_identity(config=_config(), run_context={})
    assert missing_context.value.code == "fin_run_context_invalid"

    with pytest.raises(entry.AgentServerEntryError) as missing_server_run:
        entry.bind_agent_server_identity(
...
def test_authorized_resources_are_scoped_to_one_factory_lifecycle(
    ...
    @asynccontextmanager
    async def opened(
        identity: entry.AgentServerIdentityBinding,
