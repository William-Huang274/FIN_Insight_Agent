"""Bind specialist tasks to data tools, models and execution budgets."""
...
from .research_contracts import ResearchTaskSpec, canonical_digest
...
class SpecialistAgenticCompositionError(RuntimeError):
...
class SpecialistScriptedQualificationComposition:
...
@dataclass(frozen=True)
class SpecialistReceiptedComposition:
...
def _model_json(model: type[Any], value: Any, *, code: str) -> Any:
    try:
        ...
        raise SpecialistAgenticCompositionError(code) from None
    ...
    *,
    ...
    if research_question is not None:
        ...
        if not research_question:
            raise SpecialistAgenticCompositionError("research_question_empty")
...
def _bind_research_task(
    ...
    for dependency_id in task.dependency_ids:
        ...
        if prior["task"]["task_id"] != dependency_id:
            raise SpecialistAgenticCompositionError("delegated_dependency_identity_mismatch")
        for field in ("case_id", "snapshot_id", "research_as_of", "foundation_digest"):
            if prior["task"][field] != getattr(graph_input.task, field):
                raise SpecialistAgenticCompositionError("delegated_dependency_case_scope_mismatch")
...
def _reference_from_item(item: Mapping[str, Any]) -> SpecialistObservedReference | None:
    ...
    if state == "source_bound_passage":
        ...
        if (not passage or not item.get("source_url") or not item.get("source_locator")
            ...
            raise SpecialistAgenticCompositionError("source_passage_binding_invalid")
        ...
        if state == "non_authoritative_metric" and (item.get("arithmetic_verified") is not True
            ...
            raise SpecialistAgenticCompositionError("specialist_calculation_result_invalid")
        ref_id = str(
...
@contextmanager
def _open_specialist_composition(
    *,
    run_id: str,
    run_invocation_id: str,
    branch_id: str,
        ...
        with open_approved_data_composition(
            ...
            if source_read_enabled and environment and environment.get('FINSIGHT_RESEARCH_LIBRARY_PATH'):
                ...
                for capability in body['l0_context']['capability_summaries']:
                    if capability.get('capability_ref') == 'capability:dell:source-document-read':
                        ...
                        capability['library_usage'] = 'Public immutable library: catalog(optional query) lists exact entity IDs before documents; company(entity_id) returns company card, classified source menu and coverage gaps; data(entity_id,data_kind=financial|prices|positions|filings,optional query) reads SQL observations preserving units, periods and sources; related(entity_id,graph_depth=1|2) finds source-bound relationships and separately labelled unverified mentions; search(query,optional entity_id) combines FTS, optional Qwen dense/rerank and graph candidates; observations(entity_id) reads legacy data. read(document_id,node_id) returns originals. Copy IDs from catalog. None are S2 admission; unavailable/unsearched is not undisclosed.'
            ...
            if research_task is not None:
                if collaboration_context is not None:
                    raise SpecialistAgenticCompositionError("delegation_and_author_revision_are_separate_operations")
    ...
    except SpecialistAgenticCompositionError:
        raise
    except ApprovedDataCompositionError as exc:
        raise SpecialistAgenticCompositionError(exc.code) from None


@contextmanager
def open_specialist_scripted_qualification_composition(
    *,
    run_id: str,
    run_invocation_id: str,
    branch_id: str,
    ...
    dependency_workpapers: Mapping[str, Mapping[str, Any]] | None = None,
    research_question: str | None = None,
) -> Iterator[SpecialistScriptedQualificationComposition]:
    """Open a scripted, zero-call qualification over the approved local MCP."""

    with _open_specialist_composition(
        run_id=run_id,
        run_invocation_id=run_invocation_id,
        branch_id=branch_id,
        max_model_turns=max_model_turns,
...
@contextmanager
def open_specialist_receipted_composition(
    *,
    run_id: str,
    run_invocation_id: str,
    branch_id: str,
    ...
    if live_web_read_enabled and turn_source != "provider_model":
        raise SpecialistAgenticCompositionError("saved_replay_cannot_enable_live_web")
    with _open_specialist_composition(
        run_id=run_id,
        run_invocation_id=run_invocation_id,
        branch_id=branch_id,
...
__all__ = [
    ...
    "SpecialistReceiptedComposition",
    "SpecialistScriptedQualificationComposition",
