...
from sec_agent.s3_formal_anchor_runtime import (  # noqa: E402
...
def _assert_clean_synced() -> str:
    ...
    if status or head != upstream:
        raise RuntimeError("s3_formal_anchor_head_not_clean_and_synced")
    return head


def _surface() -> tuple[dict[str, dict], dict[str, dict], list[dict[str, str]], dict]:
    decision = _load(S2_DECISION)
    context_program = _load(CONTEXT_PROGRAM)
    policy = _load(POLICY)
    request_map = {row["request_id"]: row for row in decision["research_question_method_program"]["representative_requests"]}
    context_map = {row["request_id"]: row for row in context_program["role_scoped_contexts"]}
    order = policy["request_order"]
    if order != list(request_map) or set(context_map) != set(request_map):
        raise RuntimeError("s3_formal_anchor_surface_order_or_coverage_drift")
    requests = {request_id: request_map[request_id] for request_id in order}
    contexts = {request_id: context_map[request_id] for request_id in order}
    bindings = [
        {
...
def prepare(admission_path: Path) -> dict:
    ...
    if admission_path.exists():
        raise RuntimeError("s3_formal_anchor_admission_path_already_exists")
    ...
    admission = issue_formal_anchor_admission(
        ...
        context_program_sha256=_sha(CONTEXT_PROGRAM),
    ...
    return {
        ...
        "aggregate_compact_characters": sum(int(row["capacity"]["compact_characters"]) for row in contexts.values()),
        "maximum_single_request_compact_characters": max(int(row["capacity"]["compact_characters"]) for row in contexts.values()),
...
def execute(*, admission_path: Path, runtime_root: Path, shared_ledger_path: Path) -> dict:
    ...
    return {
        ...
        "usage": {
            "input_tokens": sum(row["usage"]["input_tokens"] for row in result["family_results"]),
            "output_tokens": sum(row["usage"]["output_tokens"] for row in result["family_results"]),
            "total_tokens": sum(row["usage"]["total_tokens"] for row in result["family_results"]),
        },
