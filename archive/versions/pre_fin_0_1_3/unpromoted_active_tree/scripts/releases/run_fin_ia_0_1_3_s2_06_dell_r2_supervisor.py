...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one FIN 0.1.3 S2-06 DELL R2 Supervisor candidate"
    )
    ...
    if args.preflight_only:
        print(
            json.dumps(
                {
                    "status": "zero_call_DELL_R2_supervisor_preflight_pass_admission_not_required",
                    "case_key": args.case,
                    ...
                    "provider_call_ceiling": material["capacity"][
                        "provider_call_ceiling"
                    ],
                    "supervisor_request_characters": material["observed_capacity"][
                        "supervisor_request_characters"
                    ],
                    "retry_count": 0,
                    "fallback_count": 0,
                    "provider_calls": 0,
    ...
    if admission_path.parent != AUTHORITY_ROOT.resolve():
        raise RuntimeError("s2_06_dell_r2_supervisor_admission_path_invalid")
    admission = load_json(admission_path)
    if (
        admission.get("case_key") != args.case
        or not str(admission.get("corrected_run_id") or "").startswith(
            "fin013_s2_06_supervised_dell_r2_"
        )
    ):
        raise RuntimeError("s2_06_dell_r2_supervisor_case_or_identity_invalid")
    validate_admission_governance(
        admission,
        material=material,
        execution_git_commit=head,
    ...
    if runtime_root.exists():
        raise RuntimeError("s2_06_dell_r2_supervisor_runtime_root_already_exists")
