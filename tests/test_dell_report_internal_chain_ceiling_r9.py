...
from retrieval.dell_report_internal_chain_ceiling_r9 import (
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    AUTHORITY,
    EXECUTION_CONTRACT,
    ...
    DellReportInternalChainCeilingR9Error,
    assess_dell_report_internal_chain_r9_packages,
    build_dell_report_internal_chain_ceiling_r9_public_projection,
    validate_dell_report_internal_chain_ceiling_r9_policy,
)
from retrieval.dell_report_predicate_frames_r9 import ASP_TARGET
from retrieval.query_plan import canonical_digest
from scripts.data_retrieval import (
    run_dell_report_internal_chain_ceiling_r9 as r9_runner,
)
...
def test_r9_assessment_emits_lossless_complete_frame_transformation() -> None:
    source_id = "SOURCE::DELL::ASP::R9"
    source_rows = [
        _source(
            source_id,
    ...
    result = assess_dell_report_internal_chain_r9_packages(
...
def _r9_projection_fixture() -> dict:
    path = ROOT / (
        "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
        "candidate_ceiling/dell-rsq-03b-internal-chain-r8/full_result.json"
    )
    ...
    bindings = {
        ...
        for binding_id in EXPECTED_BOUND_INPUT_IDS | {"R9_policy"}
    }
...
def _r9_policy_fixture() -> tuple[dict, dict[str, dict]]:
    paths = {
        "R8_policy": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_policy_v1_7.json"
        ),
        "R8_public": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_result_v1_7.json"
        ),
        "R8_private": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r8/full_result.json"
        ),
        "R8_attempt_receipt": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r8/"
            "attempt_consumption_receipt.json"
        ),
        "R8_fresh_audit": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_aa58a503_dell_03b_"
            "r8_fresh_split_dual_audit_fail_v1_0.json"
        ),
        "R17_report_audit": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_aa58a503_dell_03b_"
            "r8_fresh_split_dual_audit_fail_v1_0.json"
        ),
        "R17_report_bundle_carry_forward": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_aa58a503_dell_03b_"
            "r8_fixed_dual_audit_manifest_v1_0.json"
        ),
    }
    r8_private = json.loads(paths["R8_private"].read_text(encoding="utf-8"))
    inherited = r8_private["input_bindings"]
    for binding_id in (
        "execution_program",
        "runtime_registry",
    ...
    policy = {
        ...
        "decision_target": "One bounded R9 exact attempt.",
        "owner_basis": "Owner-approved same-stage R9 implementation.",
        ...
        "TokenBudgetBasis": {
            "node_purpose": "Bounded R9 qualification.",
            ...
            "comparable_run_evidence": "R8 and zero-call R9 preview.",
...
def test_r9_runner_requires_explicit_mode() -> None:
    with pytest.raises(SystemExit) as exc_info:
        r9_runner.main([])
    assert exc_info.value.code == 2


def test_r9_formal_fails_before_receipt_when_policy_is_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    monkeypatch.setattr(r9_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r9_runner, "DEFAULT_PUBLIC", tmp_path / "public.json")
    monkeypatch.setattr(
        r9_runner,
        "ATTEMPT_RECEIPT",
    ...
    monkeypatch.setattr(r9_runner, "POLICY", tmp_path / "missing.json")
    monkeypatch.setattr(
        r9_runner,
    ...
    with pytest.raises(FileNotFoundError, match="canonical_policy_missing"):
        r9_runner.run_authorized_formal()
    assert not r9_runner.ATTEMPT_RECEIPT.exists()


def test_r9_raw_capture_precedes_redacted_terminal_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr(r9_runner, "RAW_EXECUTION_CAPTURE", raw_path)
    monkeypatch.setattr(r9_runner, "TERMINAL_FAILURE_RECEIPT", terminal_path)
    execution = {"request_results": [{"request_id": "REQ::DELL::ASP::V1"}]}
    sha = hashlib.sha256(
        r9_runner.base._canonical_json_bytes(execution)
    ).hexdigest()
    r9_runner._write_raw_execution_capture(
        policy={"result_digest": "a" * 64},
        execution=execution,
        execution_sha256=sha,
        recorded_at="2026-08-27T00:00:00+00:00",
    )
    receipt = r9_runner._write_terminal_failure_receipt(
        policy={"result_digest": "a" * 64},
...
def test_r9_attempt_receipt_is_exclusive_exact_once(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    receipt = tmp_path / "attempt" / "attempt_consumption_receipt.json"
    monkeypatch.setattr(r9_runner, "ATTEMPT_RECEIPT", receipt)
    kwargs = {
        "policy": {"result_digest": "a" * 64},
        "git_receipt": {
            "head": "b" * 40,
    ...
    first = r9_runner._write_attempt_consumption_receipt(**kwargs)
    ...
    with pytest.raises(FileExistsError, match="attempt_already_consumed"):
        r9_runner._write_attempt_consumption_receipt(**kwargs)


def test_r9_atomic_pair_rolls_back_second_publish_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    ...
    monkeypatch.setattr(r9_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r9_runner, "DEFAULT_PUBLIC", public)
    real_link = r9_runner.os.link
    calls = 0
    ...
    monkeypatch.setattr(r9_runner.os, "link", fail_second)
    with pytest.raises(OSError, match="synthetic_second_publish_failure"):
        r9_runner._publish_atomic_pair(
