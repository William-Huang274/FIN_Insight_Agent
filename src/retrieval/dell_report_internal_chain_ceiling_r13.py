...
from . import dell_report_internal_chain_ceiling_r12 as r12
from .dell_report_frame_transformation_r13 import (
    FrameTransformationBinding,
    FrameTransformationBindingR13Error,
    build_frame_transformation_binding_r13,
    build_missing_compiled_frame_binding_r13,
    transformation_binding_digest_r13,
    validate_frame_transformation_binding_record_r13,
)
from .dell_report_predicate_frames_r13 import (
    TARGET_IDS,
    PredicateFrame,
    classify_package as _classify_predicate_frame_r13,
    classify_package_with_frames as _classify_with_frames_r13,
...
POLICY_SCHEMA_VERSION = "fin_ia_dell_report_internal_chain_ceiling_policy_v2_2"
PRIVATE_RESULT_SCHEMA_VERSION = (
    "fin_ia_dell_report_internal_chain_candidate_ceiling_private_result_v2_2"
)
PUBLIC_RESULT_SCHEMA_VERSION = (
    "fin_ia_dell_report_internal_chain_candidate_ceiling_result_v2_2"
)
ATTEMPT_ID = "dell-rsq-03b-internal-chain-r13"
PROGRAM_ID = "FIN-0.1.3-S1-DELL-RSQ-03B-R13"
BRANCH = r12.BRANCH
POLICY_REF = (
    "configs/retrieval/"
    "fin_ia_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling_policy_v2_2.json"
)
PUBLIC_REF = (
    "configs/retrieval/"
    "fin_ia_0_1_3_s1_dell_03b_internal_chain_candidate_ceiling_result_v2_2.json"
)
PRIVATE_REF = (
...
MIN_FREE_BYTES_BEFORE_ATTEMPT = r12.MIN_FREE_BYTES_BEFORE_ATTEMPT
ZERO_EXECUTION_FIELDS = r12.ZERO_EXECUTION_FIELDS
EXECUTION_CONTRACT = {
    **dict(r3.EXECUTION_CONTRACT),
    "local_embedding_inference_batches": 0,
    "canonical_R11_raw_local_embedding_inference_batches": 1,
    "upstream_R12_new_local_embedding_inference_batches": 0,
    "saved_R12_raw_reuse_count": 1,
}
AUTHORITY = {
    **dict(r3.AUTHORITY),
    "current_0_6B_query_embedding_authorized": False,
    "saved_R12_raw_reuse_authorized": True,
}
SEMANTIC_CONTRACT = {
    **dict(r12.SEMANTIC_CONTRACT),
    "predicate_frame_mode": (
        "event_local_structural_clause_ownership_decision_v4_typed_scope_"
        "argument_group_governing_price_head_proof_v3_frame_v9"
    ),
    "scope_mode": (
        "frame_local_assertion_owner_actuality_lifecycle_and_scope_edge_v1"
    ),
    "material_coverage_mode": (
        "persisted_authoritative_reconciliation_v1_connector_proof_identity_"
        "relational_semantic_signature_and_lossless_frame_transformation_"
        "binding_v5"
    ),
}
EXPECTED_IMPLEMENTATION_PATHS = frozenset(
    set(r12.EXPECTED_IMPLEMENTATION_PATHS)
    | {
...
EXPECTED_BOUND_INPUT_IDS = frozenset(
    {
        ...
        "R12_private",
        "R12_attempt_receipt",
        "R12_raw_execution_capture",
        "R12_fresh_audit",
        "R12_fixed_audit_manifest",
        "R17_report_audit",
...
R12_RAW_CANDIDATE_GENERATION_BINDING_IDS = frozenset(
    {
        "source_records",
        "compiled_objects",
        "execution_program",
...
class DellReportInternalChainCeilingR13Error(ValueError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise DellReportInternalChainCeilingR13Error(code)
...
def build_route_contract_identity_registry_r13(
    ...
    _require(
        ...
        "dell_03B_R13_residual_route_program_identity_invalid",
    )
    targets = {
    ...
    _require(
        ...
        "dell_03B_R13_residual_route_target_population_invalid",
    )
    ...
    for target_id in sorted(TARGET_IDS):
        ...
        _require(
            ...
            f"dell_03B_R13_route_target_digest_invalid:{target_id}",
        )
        contracts: dict[str, dict[str, Any]] = {}
        for raw_contract in target.get("route_contracts") or ():
            _require(
                isinstance(raw_contract, Mapping),
                f"dell_03B_R13_route_contract_not_mapping:{target_id}",
            )
            ...
            _require(
                ...
                f"dell_03B_R13_route_contract_identity_invalid:{target_id}",
            )
        ...
        _require(
            ...
            f"dell_03B_R13_external_route_contract_population_invalid:{target_id}",
        )
        body = {
            "target_id": target_id,
        ...
        _require(
            ...
            f"dell_03B_R13_frozen_route_identity_drift:{target_id}",
        )
    return registry
...
def _validate_private_route_identity_r13(
    value: Mapping[str, Any],
    *,
    target_id: str,
    ...
    _require(
        ...
        f"dell_03B_R13_private_route_identity_invalid:{target_id}",
    )
...
def validate_dell_report_internal_chain_ceiling_r13_policy(
    ...
    **bound_values: Mapping[str, Any],
    ...
    _require(
        ...
        "dell_03B_R13_policy_output_contract_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R13_policy_bound_input_population_invalid",
    )
    for binding_id, raw in raw_bound.items():
        ...
        _require(
            ...
            f"dell_03B_R13_policy_bound_input_invalid:{binding_id}",
        )
        supplied = dict(bound_values[binding_id])
        if "result_digest" in row:
            _require(
                supplied.get("result_digest") == row.get("result_digest"),
                f"dell_03B_R13_policy_bound_result_digest_drift:{binding_id}",
            )

    r12_private = _validate_r12_predecessor_result(bound_values["R12_private"])
    r12_policy = dict(bound_values["R12_policy"])
    r12_public = dict(bound_values["R12_public"])
    r12_attempt = dict(bound_values["R12_attempt_receipt"])
    r12_raw = dict(bound_values["R12_raw_execution_capture"])
    _require(
        _self_digest(r12_policy)
        and _self_digest(r12_public)
        and _self_digest(r12_attempt)
        and _self_digest(r12_raw)
        and r12_policy.get("result_digest") == r12_private.get("policy_digest")
        and r12_public.get("private_result_digest")
        == r12_private.get("result_digest")
        and r12_attempt.get("attempt_id") == r12.ATTEMPT_ID
        and r12_attempt.get("policy_digest") == r12_private.get("policy_digest")
        and r12_raw.get("attempt_id") == r12.ATTEMPT_ID
        and r12_raw.get("policy_digest") == r12_private.get("policy_digest")
        and r12_raw.get("raw_execution_sha256")
        == r12_private.get("raw_execution_sha256")
        and dict(r12_raw.get("raw_execution") or {})
        == dict(r12_private.get("raw_execution_receipt") or {}),
        "dell_03B_R13_R12_predecessor_chain_invalid",
    )
    predecessor_bindings = dict(r12_private.get("input_bindings") or {})
    for binding_id in sorted(R12_RAW_CANDIDATE_GENERATION_BINDING_IDS):
        _require(
            dict(raw_bound.get(binding_id) or {})
            == dict(predecessor_bindings.get(binding_id) or {}),
            f"dell_03B_R13_candidate_generation_binding_drift:{binding_id}",
        )
    ...
    _require(
        ...
        "dell_03B_R13_route_registry_population_invalid",
    )
    r12_audit = dict(bound_values["R12_fresh_audit"])
    r12_manifest = dict(bound_values["R12_fixed_audit_manifest"])
    r17_audit = dict(bound_values["R17_report_audit"])
    engineering_verdict = dict(r12_audit.get("R12_engineering_verdict") or {})
    report_verdict = dict(r12_audit.get("R17_report_quality") or {})
    _require(
        _self_digest(r12_audit)
        and _self_digest(r12_manifest)
        and r12_audit.get("audit_manifest", {}).get("result_digest")
        == r12_manifest.get("result_digest")
        and r12_audit.get("audit_id") == r12_manifest.get("audit_id")
        and r12_audit.get("audit_manifest", {}).get("verified_hashes") == 40
        and r12_audit.get("audit_manifest", {}).get(
            "exact_changed_path_contract_pass"
        )
        is False
        and r12_audit.get("audit_manifest", {}).get(
            "case_sensitive_path_string_mismatches"
        )
        == 7
        and {
            (
            ...
            for row in r12_audit.get("audit_envelope_findings") or ()
            if isinstance(row, Mapping)
        }
        == {("R12-ENV-P2-CASE-FOLDED-CHANGED-PATHS", "P2", 7)}
        and engineering_verdict.get("verdict")
        == "FAIL_SAME_STAGE_03B_MATERIAL_FINDINGS"
        and engineering_verdict.get("new_implementation_severity_counts")
        == {"P0": 0, "P1": 1, "P2": 2, "P3": 0}
        and engineering_verdict.get("separate_audit_envelope_severity_counts")
        == {"P0": 0, "P1": 0, "P2": 1, "P3": 0}
        and engineering_verdict.get("total_observed_R12_audit_counts")
        == {"P0": 0, "P1": 1, "P2": 3, "P3": 0}
        and {
            str(row.get("finding_id") or "")
            for row in r12_audit.get("R12_material_findings") or ()
            if isinstance(row, Mapping)
        }
        == {
    ...
    _require(
        ...
        "dell_03B_R13_policy_execution_identity_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R13_policy_implementation_binding_population_invalid",
    )
    ...
    _require(
        ...
        "dell_03B_R13_policy_TokenBudgetBasis_invalid",
    )
    _require(
        value.get("authority") == _POLICY_AUTHORITY,
        "dell_03B_R13_policy_authority_invalid",
    )
    return r12_private


def classify_dell_report_internal_chain_r13_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
) -> dict[str, Any]:
    return _classify_predicate_frame_r13(
        target_id=target_id,
        text=text,
        metadata=metadata,
    )
...
def build_dell_report_internal_chain_r13_corpus_index(
    *,
    source_rows: Sequence[Mapping[str, Any]],
    object_rows: Sequence[Mapping[str, Any]],
) -> dict[str, Any]:
    return r12.build_dell_report_internal_chain_r12_corpus_index(
        source_rows=source_rows,
        object_rows=object_rows,
    )


def _package_windows_r13(
    *,
    target_id: str,
    ...
    for unit in units:
        ...
        assessment["_R13_runtime_frame_objects"] = runtime_frames
        assessment["_R13_boundary_decisions_deferred"] = boundary_deferred
...
def _best_package_r13(
    ...
    *,
    ...
    if windows:
        ...
        value = dict(selected)
        if value.pop("_R13_boundary_decisions_deferred", False):
        ...
        value = classify_dell_report_internal_chain_r13_package(
...
def _selected_frame_r13(
    *,
    ...
    if not isinstance(selected_id, str):
        return None
    if "_R13_runtime_frame_objects" in package:
        runtime_frames = package["_R13_runtime_frame_objects"]
        return next(
...
def _family_transformation_binding_r13(
    *,
    target_id: str,
    family_id: str,
    metadata: Mapping[str, Any],
    ...
    if (
        ...
        return None
    source_frame = source_frame or _selected_frame_r13(
        target_id=target_id,
        package=source_package,
        metadata=metadata,
    )
    compiled_frame = compiled_frame or _selected_frame_r13(
        target_id=target_id,
    ...
    return build_frame_transformation_binding_r13(
        ...
        compiled_window_ids=[f"WINDOW::R13::{value}" for value in object_ids],
...
def _matching_compiled_frame_r13(
    *,
    target_id: str,
    ...
    for raw_window in object_windows:
        ...
        frames = (
            list(window["_R13_runtime_frame_objects"])
            if "_R13_runtime_frame_objects" in window
...
def assess_dell_report_internal_chain_r13_packages(
    *,
    target_id: str,
    source_rows: Sequence[Mapping[str, Any]],
    object_rows: Sequence[Mapping[str, Any]],
    ...
    index = (
        ...
        else build_dell_report_internal_chain_r13_corpus_index(
            source_rows=source_rows,
            object_rows=object_rows,
        )
    )
    _require(
        ...
        "dell_03B_R13_corpus_index_population_or_position_drift",
    )
    ...
    for family_id in family_ids:
        ...
        if selected is None:
            source_windows = _package_windows_r13(
                target_id=target_id,
            ...
            source_package = _best_package_r13(
                source_windows,
                family_id=family_id,
                metadata=metadata,
        ...
        object_windows = _package_windows_r13(
            target_id=target_id,
        ...
        compiled_package = _best_package_r13(
            object_windows,
            family_id=family_id,
            metadata=metadata,
            object_package=True,
        ...
        if source_package is not None:
            source_frame = _selected_frame_r13(
                target_id=target_id,
                package=source_package,
                metadata=metadata,
            )
            matching_compiled = (
                _matching_compiled_frame_r13(
                    target_id=target_id,
                    source_frame=source_frame,
            ...
            binding = _family_transformation_binding_r13(
                target_id=target_id,
                family_id=family_id,
                metadata=metadata,
            ...
            if binding is None and source_frame is not None:
                binding = build_missing_compiled_frame_binding_r13(
                    canonical_source_family_id=family_id,
    ...
    def without_runtime_frames(row: Mapping[str, Any]) -> dict[str, Any]:
        value = dict(row)
        value.pop("_R13_runtime_frame_objects", None)
        value.pop("_R13_boundary_decisions_deferred", None)
        return value
    ...
    return {
        ...
        "frame_transformation_binding_digest": (
            transformation_binding_digest_r13(bindings)
        ),
...
def _clause_decision_counts_r13(
...
def _governing_head_partial_count_r13(
    packages: Sequence[Mapping[str, Any]],
) -> int:
    return sum(
        any(
...
_R13_RECONCILIATION_SCOPE = (
    "persisted_target_semantic_source_and_compiled_packages_plus_"
    "coverage_gaps_and_validated_transformation_bindings"
)


def _validated_mapping_rows_r13(
    value: Any,
    *,
    code: str,
) -> list[dict[str, Any]]:
    _require(
        ...
        f"{code}_not_sequence",
    )
    rows: list[dict[str, Any]] = []
    for raw in value:
        _require(isinstance(raw, Mapping), f"{code}_row_not_mapping")
        rows.append(dict(raw))
    return rows


def _canonical_mapping_row_set_digest_r13(
    rows: Sequence[Mapping[str, Any]],
) -> str:
    normalized = [dict(row) for row in rows]
    return canonical_digest(
        sorted(normalized, key=lambda row: canonical_digest(row))
    )


def derive_private_semantic_reconciliation_summary_r13(
    *,
    bindings: Any,
    source_packages: Any,
    compiled_packages: Any,
    coverage_gaps: Any,
    target_id: str,
) -> dict[str, Any]:
    """Derive every material private summary from persisted authority rows.

    R12 mixed full-corpus diagnostic aggregates with a filtered persisted
    package population.  R13 gives the summary one explicit population and
    reconstructs every field from the rows that survive in the immutable
    private artifact.  The public projector calls this function again before
    discarding any of those rows.
    """

    _require(target_id in TARGET_IDS, f"dell_03B_R13_target_invalid:{target_id}")
    source_rows = _validated_mapping_rows_r13(
        source_packages,
        code=f"dell_03B_R13_source_packages:{target_id}",
    )
    compiled_rows = _validated_mapping_rows_r13(
        compiled_packages,
        code=f"dell_03B_R13_compiled_packages:{target_id}",
    )
    coverage_rows = _validated_mapping_rows_r13(
        coverage_gaps,
        code=f"dell_03B_R13_coverage_gaps:{target_id}",
    )
    raw_binding_rows = _validated_mapping_rows_r13(
        bindings,
        code=f"dell_03B_R13_transformation_bindings:{target_id}",
    )
    binding_rows = [
        validate_frame_transformation_binding_record_r13(row)
        for row in raw_binding_rows
    ]
    binding_ids = [str(row["binding_id"]) for row in binding_rows]
    _require(
        len(binding_ids) == len(set(binding_ids)),
        f"dell_03B_R13_transformation_binding_duplicate:{target_id}",
    )

    source_complete = r4._complete_ids(source_rows)  # noqa: SLF001
    source_partial = r4._partial_ids(source_rows)  # noqa: SLF001
    compiled_complete = r4._complete_ids(compiled_rows)  # noqa: SLF001
    ...
    failed_rows = [
        row for row in binding_rows if row.get("binding_accepted") is not True
    ]
    ...
    failed_complete_rows = [
        row
        for row in failed_rows
        if row.get("canonical_source_family_id") in source_complete
    ]
    body = {
        "summary_population_scope": _R13_RECONCILIATION_SCOPE,
        "source_package_count": len(source_rows),
        "source_package_set_digest": _canonical_mapping_row_set_digest_r13(
            source_rows
        ),
        "compiled_package_count": len(compiled_rows),
        "compiled_package_set_digest": _canonical_mapping_row_set_digest_r13(
            compiled_rows
        ),
        "coverage_gap_count": len(coverage_rows),
        "coverage_gap_set_digest": _canonical_mapping_row_set_digest_r13(
            coverage_rows
        ),
        "binding_count": len(binding_rows),
        "accepted_binding_count": len(binding_rows) - len(failed_rows),
        "binding_set_digest": canonical_digest(
            sorted(binding_rows, key=lambda row: str(row["binding_id"]))
        ),
        ...
        "failed_binding_count": len(failed_rows),
        "failed_complete_binding_count": len(failed_complete_rows),
        "proof_rebind_failure_count": sum(
            bool(row.get("proof_rebind_flags")) for row in failed_rows
        ),
        "source_governing_nominal_head_partial_count": (
            _governing_head_partial_count_r13(source_rows)
        ),
        "compiled_governing_nominal_head_partial_count": (
            _governing_head_partial_count_r13(compiled_rows)
        ),
        "source_clause_ownership_decision_counts": (
            _clause_decision_counts_r13(source_rows)
        ),
        "compiled_clause_ownership_decision_counts": (
            _clause_decision_counts_r13(compiled_rows)
        ),
        "complete_transformation_coverage_pass": (
            ...
            and not failed_complete_rows
        ),
        "complete_transformation_coverage_non_vacuous": bool(source_complete),
    }
    return {**body, "reconciliation_digest": canonical_digest(body)}


def _validate_r12_predecessor_result(
    value: Mapping[str, Any],
) -> dict[str, Any]:
    result = dict(value)
    _require(
        result.get("schema_version") == r12.PRIVATE_RESULT_SCHEMA_VERSION
        and result.get("attempt_id") == r12.ATTEMPT_ID
        ...
        "dell_03B_R13_R12_predecessor_private_identity_invalid",
    )
    return result


def validate_dell_report_internal_chain_r13_saved_raw_execution(
    execution: Mapping[str, Any],
    *,
    expected_request_ids: Iterable[str],
) -> dict[str, Any]:
    """Validate the canonical raw identity reused by R12 and R13."""
    ...
    _require(
        ...
        "dell_03B_R13_canonical_raw_execution_counter_invalid",
    )
    return {
        **validated,
        "summary": {
...
def compile_dell_report_internal_chain_ceiling_r13_result(
    *,
    r12_private_result: Mapping[str, Any],
    r13_policy: Mapping[str, Any],
    execution: Mapping[str, Any],
    ...
    """Compile one fresh or explicitly saved execution through the R13 IR."""

    predecessor = _validate_r12_predecessor_result(r12_private_result)
    route_registry = build_route_contract_identity_registry_r13(
        residual_route_program
    )
    predecessor_targets = {
    ...
    _require(
        ...
        "dell_03B_R13_R12_predecessor_target_population_invalid",
    )
    ...
    validated = validate_dell_report_internal_chain_r13_saved_raw_execution(
        execution,
        expected_request_ids=expected_request_ids,
    )
    ...
    _require(
        ...
        "dell_03B_R13_raw_execution_sha_mismatch",
    )
    request_by_id = validated["request_results_by_id"]
    corpus_index = build_dell_report_internal_chain_r13_corpus_index(
        source_rows=source_rows,
        object_rows=object_rows,
    )
    object_ids = {
    ...
    for target_id in sorted(TARGET_IDS):
        ...
        _require(
            ...
            f"dell_03B_R13_candidate_object_binding_invalid:{target_id}",
        )
        total_union_occurrences += len(union_ids)
        corpus = assess_dell_report_internal_chain_r13_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        union = assess_dell_report_internal_chain_r13_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        final = assess_dell_report_internal_chain_r13_packages(
            target_id=target_id,
            source_rows=source_rows,
            object_rows=object_rows,
        ...
        _require(
            ...
            f"dell_03B_R13_external_route_identity_unresolved:{target_id}",
        )
        ...
        private_transformation_summary = (
            derive_private_semantic_reconciliation_summary_r13(
                bindings=bindings,
                source_packages=private_source_packages,
                compiled_packages=private_compiled_packages,
        ...
        _require(
            ...
            == complete_transformation_coverage_pass,
            f"dell_03B_R13_runtime_persisted_coverage_drift:{target_id}",
        )
        target_results.append(
            {
                ...
                "semantic_evidence_unit": (
                    "one_typed_R13_assertion_frame_with_structural_clause_"
                ...
                "private_source_packages": private_source_packages,
                "private_compiled_packages": private_compiled_packages,
                "private_union_packages": private_union_packages,
                "private_final_packages": private_final_packages,
    ...
    body = {
        ...
        "status": (
            "dell_03B_R13_route_clause_governing_price_connector_proof_ceiling_executed"
        ),
        ...
        "authority": {
            "03B_R13_execution_consumed": True,
        ...
        "known_boundary": (
            "R13 exact authority permits one digest-bound reuse of immutable "
            "R12 raw-reuse capture and zero new embedding/model/network calls; "
            "the transitive canonical candidate bytes remain the frozen R11 "
            "execution. R13 adds event-local ownerless boundary proof, "
            "participial and relative governing-head proof, and authoritative "
            "persisted transformation-summary reconciliation. "
            "It grants no external capture, 4B embedding, reranker, Evidence, "
            "report or product authority."
        ),
        "policy_digest": r13_policy.get("result_digest"),
    }
    return {**body, "result_digest": canonical_digest(body)}


_R13_AUTHORITY_KEYS = frozenset(
    {
        "03B_R13_execution_consumed",
...
_R13_PUBLIC_BINDING_IDS = frozenset(
...
_R13_TRANSFORMATION_SUMMARY_KEYS = frozenset(
    {
...
def _validate_private_transformation_surface_r13(
    *,
    bindings: Any,
    summary: Any,
    source_packages: Any,
    ...
    summary_row = _exact_mapping(
        summary,
        _R13_TRANSFORMATION_SUMMARY_KEYS,
        f"dell_03B_R13_private_transformation_summary:{target_id}",
    )
    expected = derive_private_semantic_reconciliation_summary_r13(
        bindings=bindings,
    ...
    _require(
        ...
        f"dell_03B_R13_private_transformation_summary_invalid:{target_id}",
    )
...
def _public_input_bindings_r13(value: Any) -> dict[str, Any]:
    bindings = _exact_mapping(
        value,
        _R13_PUBLIC_BINDING_IDS,
        "dell_03B_R13_public_input_bindings",
    )
    output: dict[str, Any] = {}
    for binding_id, raw in bindings.items():
        if binding_id == "git_identity":
            row = _exact_mapping(
                ...
                "dell_03B_R13_public_git_identity",
            )
            _require(
                ...
                "dell_03B_R13_public_git_identity_value_invalid",
            )
            ...
            row = _exact_mapping(
                ...
                "dell_03B_R13_public_disk_preflight",
            )
            _require(
                ...
                == MIN_FREE_BYTES_BEFORE_ATTEMPT,
                "dell_03B_R13_public_disk_preflight_value_invalid",
            )
            output[binding_id] = row
        else:
            _require(
                isinstance(raw, Mapping),
                f"dell_03B_R13_public_binding_not_mapping:{binding_id}",
            )
            row = dict(raw)
            _require(
                {"ref", "sha256"}.issubset(row)
                and set(row).issubset(r7._STANDARD_BINDING_KEYS),  # noqa: SLF001
                f"dell_03B_R13_public_binding_unknown_or_missing_key:{binding_id}",
            )
            _require(
                ...
                f"dell_03B_R13_public_binding_value_invalid:{binding_id}",
            )
            output[binding_id] = row
    return output


def build_dell_report_internal_chain_ceiling_r13_public_projection(
    *,
    ...
    _require(
        ...
        "dell_03B_R13_private_projection_identity_invalid",
    )
    _require(
        private_ref == PRIVATE_REF
        and bool(re.fullmatch(r"[0-9a-f]{64}", private_sha256)),
        "dell_03B_R13_private_projection_binding_invalid",
    )
    try:
        target_results = []
        for raw_row in private.get("target_results") or ():
            _require(
                ...
                "dell_03B_R13_public_target_not_mapping",
            )
            row = dict(raw_row)
            ...
            _require(
                ...
                "dell_03B_R13_private_transformation_surface_invalid",
            )
            _validate_private_transformation_surface_r13(
                bindings=transformation_bindings,
                summary=transformation_summary,
                source_packages=row.get("private_source_packages"),
                compiled_packages=row.get("private_compiled_packages"),
                coverage_gaps=row.get("private_source_to_object_coverage_gaps"),
                target_id=str(row.get("target_id") or ""),
            )
            _require(
                isinstance(route_identity, Mapping),
                "dell_03B_R13_private_route_identity_not_mapping",
            )
            _validate_private_route_identity_r13(
                route_identity,
                target_id=str(row.get("target_id") or ""),
        ...
        raise DellReportInternalChainCeilingR13Error(
            str(exc).replace("dell_03B_R7_", "dell_03B_R13_", 1)
        ) from exc
    except FrameTransformationBindingR13Error as exc:
        raise DellReportInternalChainCeilingR13Error(str(exc)) from exc
    _require(
        len(target_results) == len(TARGET_IDS)
        and {str(row["target_id"]) for row in target_results} == set(TARGET_IDS),
        "dell_03B_R13_public_target_population_invalid",
    )
    authority = _exact_mapping(
        private["authority"],
        _R13_AUTHORITY_KEYS,
        "dell_03B_R13_public_authority",
    )
    _require(
        authority.get("03B_R13_execution_consumed") is True
        and all(
            authority.get(key) is False
            for key in _R13_AUTHORITY_KEYS - {"03B_R13_execution_consumed"}
        ),
        "dell_03B_R13_public_authority_value_invalid",
    )
    body = {
        ...
        "input_bindings": _public_input_bindings_r13(private["input_bindings"]),
        "runtime_registry": _exact_mapping(
            private["runtime_registry"],
            r7._RUNTIME_REGISTRY_KEYS,  # noqa: SLF001
            "dell_03B_R13_public_runtime_registry",
        ),
        ...
        "execution_summary": _exact_mapping(
            ...
            "dell_03B_R13_public_execution_summary",
        ),
        "target_results": target_results,
        "summary": _exact_mapping(
            private["summary"],
            r7._SUMMARY_KEYS,  # noqa: SLF001
            "dell_03B_R13_public_summary",
        ),
    ...
    validate_r13_public_tree(body)
    return {**body, "result_digest": canonical_digest(body)}


def validate_r13_public_tree(value: Any) -> None:
