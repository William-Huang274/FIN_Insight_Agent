...
BASELINE_RUNTIME = ROOT / (
    ".codex_runtime/"
    "fin012-s4-t05c-mu-current-evidence-deterministic-baseline-r1"
)
...
DEFAULT_OUTPUT = ROOT / (
    "configs/releases/fin_ia_0_1_2_s4_t05_c_mu_verified_product_surface_"
    "and_paired_readiness_v1_0.json"
)


class T05CMUProductSurfaceMaterializationError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise T05CMUProductSurfaceMaterializationError(code)


def _load(path: Path) -> dict[str, Any]:
    value = json.loads(path.read_text(encoding="utf-8"))
    _require(isinstance(value, dict), "s4_t05_c_mu_surface_json_object_required")
    return value
...
def materialize() -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    _require(
        ...
        "s4_t05_c_mu_surface_exact_result_drift",
    )
    ...
    _require(
        ...
        "s4_t05_c_mu_surface_exact_result_was_modified",
    )
    ...
    body = {
        "schema_version": (
            "fin_ia_0_1_2_s4_t05_c_mu_verified_product_surface_and_"
            "paired_readiness_v1_0"
        ),
        "recorded_at": "2026-08-05T15:00:00+08:00",
        "status": (
            "MU_zero_call_product_surface_L4_pass_paired_readiness_pass_"
        ...
        "acceptance_boundary": {
            ...
            "S4_T05_D_entry": "blocked_pending_MU_owner_acceptance",
            "RC_P36_119": "deferred_nonblocking_T08_T10_S5",
            "RC_P36_121": "closed_current_result_controlled_successor",
        },
        "recommended_next": (
            "FIN-0.1.2-S4-T05-C-MU-FORMAL-PAIRED-L1-L4-ASSESSMENT-"
            "AND-OWNER-DECISION"
        ),
    }
