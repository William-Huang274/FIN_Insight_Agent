...
type SourceRef={read_offset?:number;id:string;title:string;url:string;category:string;published_at:string|null;captured_at?:string;publisher_names?:string[];metadata:{original_filing_url?:string;[key:string]:unknown}};
...
export default function MaterialReader({source,onClose,label}:{source:SourceRef;onClose:()=>void;label:(x:string)=>string}){
  const [offset,setOffset]=useState(source.read_offset||0),[view,setView]=useState<Presentation|null>(null),[parts,setParts]=useState<{body:string;id:string;locator:string}[]>([]),[total,setTotal]=useState(0),[error,setError]=useState('');
