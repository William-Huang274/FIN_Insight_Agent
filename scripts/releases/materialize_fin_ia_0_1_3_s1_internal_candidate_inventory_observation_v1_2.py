...
POLICY_PATH = ROOT / (
    "configs/runtime/fin_ia_0_1_3_s1_internal_candidate_ceiling_policy_v1_2.json"
)
OUTPUT_PATH = ROOT / (
    "configs/releases/fin_ia_0_1_3_s1_internal_candidate_"
    "inventory_observation_v1_2_attempt_r3.json"
)
ATTEMPT_ID = "S1-INTERNAL-ASSET-PORTFOLIO-COMPARATOR-R3"
SUPERSEDES = (
    "configs/releases/fin_ia_0_1_3_s1_internal_candidate_"
    "inventory_observation_v1_1_attempt_r2.json"
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Materialize the bounded read-only FIN 0.1.3 internal asset-portfolio "
...
def main(argv: list[str] | None = None) -> int:
    ...
    preflight = run_project_os_preflight(ROOT, run_scope=CORPUS_REFRESH_SCOPE)
    if preflight.get("status") != "pass":
        raise RuntimeError("internal_asset_portfolio_project_os_preflight_blocked")
    ...
    body.update(
        {
            ...
            "implementation": {
                ...
                "materializer_ref": (
                    ...
                    "candidate_inventory_observation_v1_2.py"
                ),
    ...
    if target.exists():
        ...
        if existing.get("result_digest") != output["result_digest"]:
            raise RuntimeError("internal_asset_portfolio_result_path_already_occupied")
        output = existing
    else:
    ...
    summary = {
        ...
        "milvus_status": output["resource_qualification"]["milvus_dense"]["status"],
