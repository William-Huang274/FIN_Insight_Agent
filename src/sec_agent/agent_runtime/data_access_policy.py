"""Validate access decisions for configured source and financial datasets."""
...
from sec_agent.agent_runtime.research_contracts import canonical_digest
...
class DataAccessPolicyError(ValueError):
...
class DataAccessPolicyDecision(StrictFrozenModel):
    ...
    def validate_decision(self) -> "DataAccessPolicyDecision":
...
def load_owner_data_gate_decision(
    ...
    *,
    ...
    if _file_sha256(decision_path) != expected_file_sha256:
        raise DataAccessPolicyError("owner_data_gate_decision_file_sha256_mismatch")
    try:
        decision = DataAccessPolicyDecision.model_validate_json(
            decision_path.read_text(encoding="utf-8")
        )
    except (OSError, UnicodeError, ValueError) as exc:
        raise DataAccessPolicyError("owner_data_gate_decision_invalid") from exc
    if decision.decision_digest != expected_decision_digest:
        raise DataAccessPolicyError("owner_data_gate_decision_untrusted_digest")
    return decision


def validate_trusted_owner_data_gate_decision(
    decision: DataAccessPolicyDecision,
) -> DataAccessPolicyDecision:
    ...
    validated = DataAccessPolicyDecision.model_validate(
...
__all__ = [
    ...
    "DEFAULT_OWNER_DATA_GATE_DECISION_PATH",
    "DataAccessPolicyDecision",
    "DataAccessPolicyError",
    'load_owner_data_gate_decision',
    'validate_trusted_owner_data_gate_decision',
]
