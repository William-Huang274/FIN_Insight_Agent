"""Specialist tool loop for source reading, calculation and working papers."""
...
class SpecialistAgenticGraphError(RuntimeError):
...
class SpecialistAgenticDependencies:
...
def _validate_model_json(model: type[BaseModel], value: Any, *, code: str) -> Any:
    try:
        ...
        raise SpecialistAgenticGraphError(code) from None
...
def _validate_decision(value: Any) -> SpecialistDecision:
    try:
        ...
        raise SpecialistAgenticGraphError("specialist_model_action_invalid") from None
...
def build_specialist_agentic_state_graph(
    *,
    ...
    def initialize(
        ...
        if validated.collaboration_context is not None:
            ...
            if (prior.branch_id != validated.task.branch_id
                ...
                raise SpecialistAgenticGraphError("collaboration_data_scope_mismatch")
            if (collaboration.mode == "repair") != (validated.agent_id == collaboration.target_agent_id):
                raise SpecialistAgenticGraphError("collaboration_role_identity_mismatch")
    ...
    def route_after_submission(state: SpecialistAgenticState) -> str:
        return "end" if state.get("final_submission") is not None else "decide"

    def route_after_tool(state: SpecialistAgenticState) -> str:
