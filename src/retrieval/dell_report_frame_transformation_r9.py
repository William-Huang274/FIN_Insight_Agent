...
class FrameTransformationBindingR9Error(ValueError):
    pass
...
def _require_identifier(value: str, *, field: str) -> str:
    if not value or value.strip() != value or re.search(r"[\x00-\x1f]", value):
        raise FrameTransformationBindingR9Error(
            f"R9_transformation_invalid_identifier:{field}"
        )
    return value
...
def validate_predicate_frame_r9_integrity(frame: PredicateFrame) -> None:
    ...
    if frame.semantic_signature_digest != expected_semantic:
        raise FrameTransformationBindingR9Error(
            "R9_transformation_source_or_compiled_semantic_digest_invalid"
        )
    if (
        frame.frame_digest != expected_representation
        or frame.representation_frame_digest != expected_representation
        or frame.frame_id
        != f"FRAME::R9::{expected_representation[:24].upper()}"
    ):
        raise FrameTransformationBindingR9Error(
            "R9_transformation_source_or_compiled_representation_digest_invalid"
        )
...
def _semantic_role_rows(frame: PredicateFrame) -> tuple[RoleBinding, ...]:
    return tuple(
        sorted(
            (*frame.role_bindings, *frame.scope_bindings),
            key=lambda row: (
                row.role,
...
def _mapping(
    ...
    mapping_id = f"ROLEMAP::R9::{canonical_digest(core)[:24].upper()}"
...
def build_frame_transformation_binding_r9(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    validate_predicate_frame_r9_integrity(source_frame)
    validate_predicate_frame_r9_integrity(compiled_frame)
    ...
    binding_id = f"FRAMEBIND::R9::{canonical_digest(core)[:24].upper()}"
    ...
    if require_lossless and not binding.binding_accepted:
        ...
        raise FrameTransformationBindingR9Error(
            "R9_transformation_not_lossless:" + "|".join(findings)
        )
    return binding


def build_missing_compiled_frame_binding_r9(
    *,
    canonical_source_family_id: str,
    source_record_id: str,
    source_frame: PredicateFrame,
    ...
    core = {
        ...
        "compiled_object_ids": (),
        "compiled_window_ids": (),
        "compiled_frame_id": "MISSING::R9::COMPILED_FRAME",
...
def transformation_binding_digest_r9(
...
def validate_frame_transformation_binding_r9(
    ...
    if observed != canonical_digest(body):
        raise FrameTransformationBindingR9Error(
            "R9_transformation_binding_self_digest_invalid"
        )
    if value.get("binding_accepted") is not True:
        raise FrameTransformationBindingR9Error(
            "R9_transformation_binding_not_accepted"
        )
    if value.get("semantic_signature_equal") is not True:
        raise FrameTransformationBindingR9Error(
            "R9_transformation_binding_semantic_signature_mismatch"
        )
    if any(value.get(field) for field in (
        ...
        raise FrameTransformationBindingR9Error(
            "R9_transformation_binding_contains_material_flags"
        )
