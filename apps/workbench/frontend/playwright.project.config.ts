...
const fixture=process.env.FIN_PROJECT_REPORT_SOURCE?'project_report_web':process.env.FIN_PROJECT_VERSIONS_FIXTURE?'project_versions_web':'project_library_web';
if(!state) throw new Error('Use a new isolated FINSIGHT_LOCAL_STATE_ROOT for each attempt.');
export default defineConfig({
  testDir:'./e2e',testMatch:['project-library.spec.ts','project-sec.spec.ts','project-report.spec.ts','project-versions.spec.ts'],workers:1,retries:0,reporter:'list',
