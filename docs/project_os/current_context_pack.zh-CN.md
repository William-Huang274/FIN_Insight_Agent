...
零模型结构包已由 formal R3 关闭结构门。当前只允许签发一次 Chat `CELL::value_capture` 自然复验：复用当前 DELL Evidence Pack／NumericFact，要求模型真实读取 typed relation、RoleMethodPack、GraphContextPack 和可执行来源路线；0 retry、0 fallback，不运行 Responses、不访问外源、不发布产品。运行后先做 L1，再评估归因、机制、反方和 WWC 的内容增益，并与 paired R1 Chat 同输入基线比较。无论成功或失败，本轮都先返回 Owner；五单元、其他 RoleMethodPack 和完整图 handler 不自动开始。
