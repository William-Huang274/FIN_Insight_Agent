...
export default function ProjectResearchWorkspace({embedded=false}:{embedded?:boolean}){
  ...
  return <div className={`pr-workspace ${embedded?'pw-business':''}`}>{!embedded&&<aside className="pr-sidebar"><a className="pr-brand" href="/workspace/session"><Layers size={25}/><span>FinSight<small>研究工作区</small></span></a>
    ...
    <main><header className="pr-heading"><div><p className="pr-eyebrow">PROJECT RESEARCH</p><h1>{selected?.name||'项目研究任务'}</h1><p>从问题到研究，保留每一次提交与结果的关联。</p></div><button disabled={busy} onClick={()=>{void projects.reload();refresh(n=>n+1);}}><RefreshCw size={16}/>刷新记录</button></header>
      ...
      <section className="pr-card pr-detail">{taskId?(detail?<><p className="pr-eyebrow">研究准备</p><h2>{detail.title}</h2><p className="pr-question">{detail.question}</p>
        ...
        <p className="pr-footnote">研究截止时点：{detail.binding.research_as_of}</p><details><summary>资料版本与运行关联</summary><dl><dt>资料发布版本</dt><dd className="pr-hash">{detail.binding.snapshot_ref}</dd><dt>业务任务 ID</dt><dd className="pr-hash">{detail.id}</dd></dl></details>
        ...
        <div className="pr-actions">{detail.prepare.status==='ready'&&<button disabled={busy} onClick={()=>void action('prepare')}>完成草稿准备</button>}{detail.prepare.status==='received'&&detail.start.status==='ready'&&<button className="pr-primary" disabled={busy} onClick={()=>void action('start')}>确认并启动研究</button>}{(detail.prepare.status==='unknown'||detail.start.status==='unknown')&&<button disabled={busy} onClick={()=>void action('reconcile')}>核对原提交</button>}
          {detail.thread_id&&<a href={`/workspace/session?thread=${detail.thread_id}&view=activity&project=${project}`}>打开研究 <ArrowUpRight size={15}/></a>}</div>
