...
25. **DELL 五单元动态完整案例（R7 完整物化，Case Truth claim-polarity formal R4 工程门通过）**：R7 复用 R6 上游，只执行 Value repair 和两次 synthesis，共 3 次调用、0 retry，首次形成五个 Judgment、综合和完整内部报告。合同、身份、期间、数值、引用与产品利润桥边界通过，但 Operating／Counterevidence 错误否认当前 Case 已有的 AI revenue、orders、backlog，综合又制造 false conflict，故内容 L1/L2 失败。Case Truth split 零调用 R3 已证明五个三-surface slice 与本地聚合；natural R1 的 max-thinking 零输出已由 R2 non-thinking 可见输出关闭。R2 暴露的 `asserted_state` 歧义、跨公司 context 缺失与 supporting-fact 枚举已由 claim polarity、分层 alias view 和每 surface 最多 12 个直接命题的 formal R4 结构性关闭；R2 capture replay、三案／留出、context／cross-cell／容量 mutation 全部通过。下一步只允许一次 Operating／Counterevidence 四调用 natural successor。自然语义通过后才考虑受影响 Judgment 与 Synthesis；即使修复报告形成，仍须完整执行 L1/L2、八维、paired 和 qualified-human。
