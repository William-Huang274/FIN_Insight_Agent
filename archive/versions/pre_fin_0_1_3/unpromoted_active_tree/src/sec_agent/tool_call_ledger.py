...
class ToolCallLedger:
    ...
    def executed_tool_call_count(self, *, agent_id: str | None = None) -> int:
        return self._executed_tool_call_count(agent_id=agent_id)

    def executed_tool_call_counts_by_agent(self) -> dict[str, int]:
        counts: dict[str, int] = {}
        for record in self.records:
            if record.status in {"blocked", "cached"}:
                continue
