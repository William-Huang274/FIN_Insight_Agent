...
2026-09-08 实测：从提交 `9e363302` 的独立干净源码工作树，确认导出模块来自该工作树；仅取Git跟踪文件，未复制工作区 `.env` 或额外挂载私有研究资料。上述两个测试文件共17项通过，四格式合成导出成功，0模型调用。检查复用了本机已安装依赖，未覆盖全新依赖安装、前端构建或完整新研究独立启动，也未重做视觉验收。
