...
R1 已保存并停止。v1.1 全仓回归、active-baseline、secret scan、干净远端提交和独立 zero-call proof 均已通过，确保合同更合理但不会把 R1 越界判断自动追认为事实。GA profiles、四工具循环和 capture-first tool-step 已在活动树实现；当前先执行全仓回归、secret scan、干净提交和独立零调用复证。复证通过后才可为 DELL 单研究单元 JSON／strict paired canary 签发新的 exact-once authority；strict final-tool 只作 Beta 资格实验，不自动进入五单元运行或 R2。
