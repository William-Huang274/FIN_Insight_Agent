...
from scripts.releases.assess_fin_ia_0_1_2_s4_t05_d_nvda_current_evidence_product_pair import (  # noqa: E402
    DEFAULT_OUTPUT,
    T05DNVDAFormalPairedAssessmentError,
    assess,
...
def test_nvda_formal_pair_records_limited_gain_and_honest_owner_boundary() -> None:
    assessment = assess()
    ...
    assert assessment["L2_evidence_reliability_and_coverage"]["authority_cells"] == 3
    assert assessment["L3_agent_gain"]["agent_claim_dependency_conflict_gap_WWC"] == [6, 1, 2, 4, 9]
    assert [
        row["issue"] for row in assessment["L3_agent_gain"]["quality_findings"]
    ] == ["RC-P36-119", "RC-P36-125"]
    assert assessment["L4_final_delivery"]["status"] == "pass"
    assert assessment["owner_decision_request"]["material_gain_accepted"] is None
    assert assessment["acceptance_boundary"]["post_transfer_NVDA_R2"] is False
    assert assessment["observed_counts"]["new_model_calls"] == 0
    assert assessment["observed_counts"]["formal_paired_assessments"] == 1


def test_nvda_owner_acceptance_cannot_be_inferred_from_continue() -> None:
    changed = deepcopy(_load(DEFAULT_OUTPUT))
    changed["owner_decision_request"]["material_gain_accepted"] = True
    changed["acceptance_boundary"]["post_transfer_NVDA_R2"] = True
    changed["observed_counts"]["owner_decisions"] = 1
    _redigest(changed)
    with pytest.raises(
        T05DNVDAFormalPairedAssessmentError,
        match="s4_t05_d_nvda_paired_owner_boundary_invalid",
    ):
        validate_formal_paired_assessment(changed)


def test_nvda_agent_gain_counts_or_quality_findings_cannot_drift() -> None:
    changed = deepcopy(_load(DEFAULT_OUTPUT))
    changed["L3_agent_gain"]["agent_claim_dependency_conflict_gap_WWC"][1] = 2
    _redigest(changed)
    with pytest.raises(
        T05DNVDAFormalPairedAssessmentError,
        match="s4_t05_d_nvda_paired_agent_gain_or_finding_invalid",
    ):
        validate_formal_paired_assessment(changed)


def test_nvda_pair_binding_rejects_baseline_input_mutation() -> None:
...
def test_nvda_paired_assessment_digest_mutation_fails_closed() -> None:
    changed = deepcopy(_load(DEFAULT_OUTPUT))
    changed["L4_final_delivery"]["preview_digest"] = "mutated"
    with pytest.raises(
        T05DNVDAFormalPairedAssessmentError,
        match="s4_t05_d_nvda_paired_assessment_digest_mismatch",
    ):
        validate_formal_paired_assessment(changed)
