...
from retrieval.dell_report_internal_chain_ceiling_r13 import (
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    AUTHORITY,
    EXECUTION_CONTRACT,
    ...
    DellReportInternalChainCeilingR13Error,
    _validate_private_transformation_surface_r13,
    assess_dell_report_internal_chain_r13_packages,
...
from retrieval.dell_report_predicate_frames_r13 import ASP_TARGET, classify_package
from retrieval.query_plan import canonical_digest
from scripts.data_retrieval import (
    run_dell_report_internal_chain_ceiling_r13 as r13_runner,
)
...
def test_r13_assessment_emits_lossless_complete_frame_transformation() -> None:
    source_id = "SOURCE::DELL::ASP::R13"
    source_rows = [
        _source(
            source_id,
    ...
    result = assess_dell_report_internal_chain_r13_packages(
...
def test_r13_deferred_irrelevant_boundaries_are_realized_for_selected_package() -> None:
    source_id = "SOURCE::DELL::ASP::NO_FRAME::R13"
    text = (
    ...
    result = assess_dell_report_internal_chain_r13_packages(
        ...
        object_rows=[_object("OBJECT::DELL::ASP::NO_FRAME::R13", source_id, text)],
    )
...
def _r13_projection_fixture() -> dict:
    path = ROOT / (
        "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
        "candidate_ceiling/dell-rsq-03b-internal-chain-r12/full_result.json"
    )
    ...
    bindings = {
        ...
        for binding_id in EXPECTED_BOUND_INPUT_IDS | {"R13_policy"}
    }
    ...
    for target in private["target_results"]:
        ...
        target["private_frame_transformation_summary"] = (
            derive_private_semantic_reconciliation_summary_r13(
                bindings=target["private_frame_transformation_bindings"],
                source_packages=target["private_source_packages"],
                compiled_packages=target["private_compiled_packages"],
                coverage_gaps=target["private_source_to_object_coverage_gaps"],
                target_id=target_id,
            )
        )
...
def test_r13_public_projection_recomputes_clause_decision_maps(field: str) -> None:
    ...
    summary[field] = {**dict(summary[field]), "forged": 1}
    _redigest(private)
    with pytest.raises(
        DellReportInternalChainCeilingR13Error,
        match="private_transformation_summary_invalid",
...
def test_r13_public_projection_recomputes_coverage_boolean_and_digest() -> None:
    for field in (
        ...
        summary[field] = not bool(summary[field])
        _redigest(private)
        with pytest.raises(
            DellReportInternalChainCeilingR13Error,
            match="private_transformation_summary_invalid",
...
def test_r13_public_projection_binds_persisted_package_surface() -> None:
    ...
    target["private_source_packages"][0]["limitations"] = [
        *target["private_source_packages"][0].get("limitations", []),
        "forged_after_summary",
    ]
    _redigest(private)
    with pytest.raises(
        DellReportInternalChainCeilingR13Error,
...
def test_r13_reconciliation_rejects_mutation_of_every_summary_field() -> None:
    summary = derive_private_semantic_reconciliation_summary_r13(
        bindings=[],
        source_packages=[],
        compiled_packages=[],
        coverage_gaps=[],
    ...
    for field, original in summary.items():
        mutated = deepcopy(summary)
        if isinstance(original, bool):
            mutated[field] = not original
        elif isinstance(original, int):
            mutated[field] = original + 1
        elif isinstance(original, str):
            mutated[field] = f"{original}::forged"
        elif isinstance(original, list):
            mutated[field] = [*original, "forged"]
        elif isinstance(original, dict):
            mutated[field] = {**original, "forged": 1}
        else:  # pragma: no cover - schema expansion must add an explicit mutator
            raise AssertionError(f"unsupported summary field type:{field}")
        with pytest.raises(
            DellReportInternalChainCeilingR13Error,
            match="private_transformation_summary_invalid",
        ):
...
def test_r13_route_registry_preserves_exact_mandatory_external_identity() -> None:
    registry = build_route_contract_identity_registry_r13(
...
def test_r13_route_registry_rejects_rehashed_semantic_route_drift() -> None:
...
def test_r13_route_identity_survives_false_then_true_active_state() -> None:
    ...
    _redigest(private)
    first = build_dell_report_internal_chain_ceiling_r13_public_projection(
        private_result=private,
        private_ref=PRIVATE_REF,
        private_sha256="c" * 64,
    )
    ...
    _redigest(private)
    second = build_dell_report_internal_chain_ceiling_r13_public_projection(
        private_result=private,
        private_ref=PRIVATE_REF,
        private_sha256="d" * 64,
    )
...
def _r13_policy_fixture() -> tuple[dict, dict[str, dict]]:
    paths = {
        "R12_policy": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_policy_v2_1.json"
        ),
        "R12_public": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_result_v2_1.json"
        ),
        "R12_private": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r12/full_result.json"
        ),
        "R12_attempt_receipt": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r12/"
            "attempt_consumption_receipt.json"
        ),
        "R12_raw_execution_capture": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r12/"
            "raw_execution_capture.json"
        ),
        "R12_fresh_audit": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_057eb98e_dell_03b_"
            "r12_fresh_dual_audit_fail_v1_0.json"
        ),
        "R12_fixed_audit_manifest": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_057eb98e_dell_03b_"
            "r12_fixed_dual_audit_manifest_v1_0.json"
        ),
    ...
    r12_private = json.loads(paths["R12_private"].read_text(encoding="utf-8"))
    inherited = r12_private["input_bindings"]
    for binding_id in (
        "R17_report_audit",
    ...
    policy = {
        ...
        "decision_target": "One bounded R13 exact attempt.",
        "owner_basis": "Owner-approved same-stage R13 implementation.",
        ...
        "TokenBudgetBasis": {
            "node_purpose": "Bounded R13 qualification.",
            ...
            "comparable_run_evidence": "R12 and zero-call R13 preview.",
...
def test_r13_policy_rejects_resigned_R12_case_audit_boundary_drift() -> None:
    policy, values = _r13_policy_fixture()
    drift = dict(values)
    drift["R12_fresh_audit"] = deepcopy(values["R12_fresh_audit"])
    drift["R12_fresh_audit"]["audit_envelope_findings"][0][
        "mismatch_count"
    ] = 6
    _redigest(drift["R12_fresh_audit"])
    policy["bound_inputs"]["R12_fresh_audit"]["result_digest"] = drift[
        "R12_fresh_audit"
    ]["result_digest"]
    _redigest(policy)
    with pytest.raises(
        DellReportInternalChainCeilingR13Error,
        match="R12_or_R17_audit_failure_boundary_invalid",
    ):
        validate_dell_report_internal_chain_ceiling_r13_policy(
            policy,
            **drift,
...
def test_r13_saved_raw_validation_projects_zero_new_embedding_batch() -> None:
    ...
    raw = values["R12_raw_execution_capture"]
    ...
    assert all(
        ...
        for field in r13_runner.ZERO_EXECUTION_FIELDS
    )


def test_r13_formal_runner_has_no_fresh_retrieval_or_model_execution_path() -> None:
    source = inspect.getsource(r13_runner.run_authorized_formal)
    assert "ResearchRetrievalService" not in source
    assert "execute_current_runtime_requests" not in source
    assert "_validated_immutable_r12_execution" in source
    assert "_validated_r13_raw_reuse_capture" in source
...
def test_r13_runner_requires_explicit_mode() -> None:
    with pytest.raises(SystemExit) as exc_info:
        r13_runner.main([])
    assert exc_info.value.code == 2


def test_r13_formal_fails_before_receipt_when_policy_is_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    monkeypatch.setattr(r13_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r13_runner, "DEFAULT_PUBLIC", tmp_path / "public.json")
    monkeypatch.setattr(
        r13_runner,
        "ATTEMPT_RECEIPT",
    ...
    monkeypatch.setattr(r13_runner, "POLICY", tmp_path / "missing.json")
    monkeypatch.setattr(
        r13_runner,
    ...
    with pytest.raises(FileNotFoundError, match="canonical_policy_missing"):
        r13_runner.run_authorized_formal()
...
def test_r13_raw_capture_precedes_redacted_terminal_failure(
    ...
    monkeypatch.setattr(r13_runner, "RAW_EXECUTION_CAPTURE", raw_path)
    monkeypatch.setattr(r13_runner, "TERMINAL_FAILURE_RECEIPT", terminal_path)
    _, values = _r13_policy_fixture()
    source_capture = values["R12_raw_execution_capture"]
    r12_private = values["R12_private"]
    ...
    sha = hashlib.sha256(
        r13_runner.base._canonical_json_bytes(execution)
    ).hexdigest()
    r13_runner._write_raw_execution_capture(
    ...
    assert captured["R13_new_call_counters"] == r13_runner.R13_NEW_CALL_COUNTERS
    assert captured["reuse_reason"] == r13_runner.R13_RAW_REUSE_REASON
    ...
    assert proof["canonical_R11_local_embedding_inference_batches"] == 1
    assert proof["predecessor_R12_new_local_embedding_inference_batches"] == 0
    assert proof["R13_new_local_embedding_inference_batches"] == 0
    assert captured["source_R12_raw_reuse_capture"]["attempt_id"] == (
        "dell-rsq-03b-internal-chain-r12"
    )
    validated_execution, validated_sha = (
        r13_runner._validated_r13_raw_reuse_capture(
            capture=captured,
            policy={"result_digest": "a" * 64},
            r12_private=r12_private,
            r12_raw_capture=source_capture,
            r12_raw_ref=source_capture_ref,
            r12_raw_sha256=source_capture_sha256,
        )
    )
    assert validated_execution == execution
    assert validated_sha == sha
    receipt = r13_runner._write_terminal_failure_receipt(
        policy={"result_digest": "a" * 64},
...
def test_r13_attempt_receipt_is_exclusive_exact_once(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    receipt = tmp_path / "attempt" / "attempt_consumption_receipt.json"
    monkeypatch.setattr(r13_runner, "ATTEMPT_RECEIPT", receipt)
    kwargs = {
        "policy": {"result_digest": "a" * 64},
        "git_receipt": {
            "head": "b" * 40,
    ...
    first = r13_runner._write_attempt_consumption_receipt(**kwargs)
    ...
    with pytest.raises(FileExistsError, match="attempt_already_consumed"):
        r13_runner._write_attempt_consumption_receipt(**kwargs)


def test_r13_atomic_pair_rolls_back_second_publish_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    ...
    monkeypatch.setattr(r13_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r13_runner, "DEFAULT_PUBLIC", public)
    real_link = r13_runner.os.link
    calls = 0
    ...
    monkeypatch.setattr(r13_runner.os, "link", fail_second)
    with pytest.raises(OSError, match="synthetic_second_publish_failure"):
        r13_runner._publish_atomic_pair(
