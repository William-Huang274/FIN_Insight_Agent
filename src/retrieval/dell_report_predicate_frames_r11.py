...
_TEMPORAL_ADJUNCT = re.compile(
    ...
    r"(?:the\s+)?(?:q[1-4](?:\s+(?:fy)?\d{2,4})?|fy\s*\d{2,4}|"
    r"20\d{2}|quarter|period|year|month|meantime|time)\b(?:\s+|,\s*)",
    re.IGNORECASE,
)
_AGREEMENT_ADJUNCT = re.compile(
    r"(?:under|pursuant\s+to|subject\s+to)\s+(?:the\s+|an?\s+)?"
    r"(?:agreement|contract|arrangement|plan|terms?)\b(?:\s+|,\s*)",
    re.IGNORECASE,
)
...
def _case_token_is_predicate_collision(
    right_case: str | None,
    match: re.Match[str],
) -> bool:
    if right_case is None:
        return False
    surface = right_case[match.start() : match.end()]
...
def _predicate_candidates(
    ...
    usable = [
        ...
        if not _case_token_is_predicate_collision(right_case, row)
    ]
...
def _owner_span_before_predicate(
    ...
    *,
    ...
    if right_case is None:
        return None
    owner_case = right_case[start:end]
    tokens = re.findall(r"[A-Za-z][A-Za-z0-9&.'-]*|\d+", owner_case)
    if not tokens:
        return None
    if not tokens[0][0].isupper():
        return None
    if all(token[0].isupper() or token[0].isdigit() for token in tokens):
        return owner_span
    return None
...
def _unknown_owner_clause_candidate(
    ...
    *,
    ...
    words = list(re.finditer(r"[A-Za-z][A-Za-z0-9&.'-]*|\d+", right_case[start:]))
    if len(words) < 3:
        return None
    for owner_count in range(1, min(5, len(words) - 1)):
        owner_words = words[:owner_count]
        verb = words[owner_count]
        owner_tokens = [row.group(0) for row in owner_words]
        if not owner_tokens[0][0].isupper():
            continue
        if not all(token[0].isupper() or token[0].isdigit() for token in owner_tokens):
            continue
        verb_surface = verb.group(0)
        if not verb_surface[0].islower():
            continue
        if verb_surface.casefold() in _NON_VERB_AFTER_OWNER:
            continue
        verb_start = start + verb.start()
        verb_end = start + verb.end()
        if not _MATERIAL_OBJECT_SURFACE.search(right[verb_end:]):
            continue
...
def frame_boundary_decisions(text: str) -> list[FrameBoundaryDecision]:
    ...
    for sentence_index, raw_sentence in enumerate(raw_sentences):
        ...
        if sentence_start < 0:
            raise ValueError("R11_normalized_sentence_provenance_lost")
...
def _modifier_frame_id(
    *,
    ...
    return f"MODIFIER::R11::{digest[:24].upper()}"
    ...
    for index, price in enumerate(prices):
        ...
        output.append(
            ArgumentGroupBinding(
                group_id=f"ARG::R11::{index:02d}::{digest[:20].upper()}",
                span_start=body["span_start"],
                span_end=body["span_end"],
                raw_text=text,
...
def _finalize(
    *,
    ...
    body = {
        ...
        # Compatibility aliases are intentionally explicit in the R11 public IR.
        "modality": actuality,
    ...
    return PredicateFrame(
        frame_id=f"FRAME::R11::{digest[:24].upper()}",
        frame_digest=digest,
        assertion_frame_id=assertion_frame_id,
        target_id=target_id,
        sentence_index=record.sentence_index,
...
def extract_predicate_frames(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
    ...
    if target_id not in TARGET_IDS:
        raise ValueError(f"unsupported_R11_target:{target_id}")
...
def classify_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
    ...
    assessment["semantic_guard_revision"] = (
        "R11_CLAUSE_OWNERSHIP_PRICE_ATTACHMENT_PROOF_FRAME"
    )
    ...
    if accepted and assessment.get("in_period") is True:
        ...
        limitations.update(
            f"missing_R11_role:{role}" for role in selected.missing_required_roles
        )
        limitations.update(f"R11_ambiguity:{row}" for row in selected.ambiguities)
        ...
        assessment["limitations"] = sorted(
            ...
            | {"no_single_R11_relational_target_frame"}
        )
    return assessment
