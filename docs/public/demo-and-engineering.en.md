...
Use a configured workspace and saved results; there is no need to rerun full research for a demonstration. Each report shows its own review state. Public screenshots show the actual application without distributing the full report or source corpus.
...
| 1:30–2:00 | Inspect a saved report diff, then collapse it | Targeted feedback carries a target and baseline. Revisions preserve the old report; completed execution is not human acceptance. |
