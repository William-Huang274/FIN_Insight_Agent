...
> 状态：`AUTHORIZED / IN_PROGRESS / MIXED EVIDENCE: DELL DOCLING SHADOW PASS; TENCENT COMPONENT PASS BUT FIN ADAPTER FAIL_CLOSED; DOCLING HOLD; MINERU EXACT ENVIRONMENT AND VLM SNAPSHOT PASS BUT FIRST INFERENCE PREFLIGHT HOLD_RESOURCE; PRODUCT DELTA NONE`
> 日期：2026-09-01
...
- 可继续本地技术资格的是VLM `opendatalab/MinerU2.5-Pro-2605-1.2B`。HF exact revision固定为`bff20d4ae2bf202df9f45284b4d43681555a97ed`，本地已完整落盘并复算为13 files／`2,328,028,720` bytes；主权重`model.safetensors`为`2,312,126,640` bytes、SHA-256=`abf8681ca63b8dec7b67de257af47b821f179442f72998d0696ae2ed9232a5f0`，`tokenizer.json`为`11,423,550` bytes、SHA-256=`dceac5fc54a795ee7570d17902b47bd05412dc2afa62bdf325c3f97fcb5b87fe`。13项本地SHA和总字节数均由manifest重放验证，未依赖unpinned `main`；顶层model card声明Apache-2.0，但没有独立LICENSE文件，生产、SaaS和再分发仍不因此放行。
- 当前host为RTX 4060 Laptop 8,188 MiB、driver 555.97／compute 8.9；物理内存总量15.628 GiB。官方VLM最低画像为8GB显存，Docker/vLLM路径还要求更高CUDA driver、8GB available VRAM与`--shm-size 32g`，当前明确不满足。首选qualification profile已落为native Windows Python 3.11.14独立`mineru[vlm]` transformers环境，不安装lmdeploy/vllm；环境、uv/HF cache、模型与输出全部位于Z盘。推理仍必须先通过冻结的一页资源门，资源不足即`HOLD_RESOURCE`，不降低门、不杀Owner进程、不直接跑282页。
- exact环境、dependency lock、model snapshot和local/offline配置均已落盘；首次推理preflight因RAM和VRAM双门失败，模型未加载、PDF子集未生成、文档推理未启动。下一合法动作只是在资源自然满足后用新immutable attempt运行同一物理页130单页smoke；不能在当前失败attempt内重试、换模型、换页或换backend。`D:\FIN_Insight_Agent\data\indexes`仍不删除；R14、R15/R16、formal、Evidence、S2/S3、report/product/release权限不变。
...
### 8.18 MinerU exact环境、模型落盘与首次推理preflight

- qualification根继续使用`Z:\FIN_Insight_Agent_qualification\20260901_s1_mature_data_plane_v1`。exact wheel实际文件仍为`1,541,914` bytes、SHA-256=`4a73b865920bb9109c1b8b1bc46567e296bf0133a67106a04effd219536ae72d`。依赖输入只有MinerU、Torch、torchvision和Transformers四个顶层约束；resolved lock为`122,947` bytes、SHA-256=`75c099d2af356655092342b54258f1789fc24abdd0cdac09d2e486f383b98e66`，解析为83个唯一distribution和1,371个唯一SHA-256。独立只读复核确认80个registry pin都有Windows CPython 3.11 wheel，禁入的lmdeploy/vllm/gradio/ray/xgrammar均为0；lock同时收录部分sdist hash，因此实际安装和复核分别以`--no-build`和互斥的`--only-binary :all:`禁止源码构建，未把两参数错误地同时传给uv。
- Z盘隔离环境为`envs\mineru-3.4.5-vlm-transformers-torch2.6.0-cu124-py311`。dry-run为83包；实际安装为83包；binary-only后审计为`Audited 83 packages`，`uv pip check`为83/83 compatible，实际normalized inventory与lock精确相等，missing/extra/version mismatch均为0。关键运行时为MinerU 3.4.5、Torch 2.6.0+cu124、torchvision 0.21.0+cu124、Transformers 4.57.3、Accelerate 1.14.0；46组广义imports、15个关键版本、torchvision native ops、CUDA 12.4、RTX 4060 compute 8.9和微型CUDA张量均通过，实际engine日志明确为`Using transformers as the inference engine for VLM.`。环境receipt SHA-256=`75acb0debd94561c3714dd9030a26068d2c0ff5c14cbc7e4e2297dad1a952ed5`；它只签`PASS_COMPONENT_RUNTIME_ONLY`，全部产品/Evidence/生产authority仍false。
- exact HF snapshot使用revision=`bff20d4ae2bf202df9f45284b4d43681555a97ed`。首次hf-xet传输完成12项后在主权重约0.64 GiB处停止增长，明确中断；后继仍是同一revision、同一目录和同一partial file，设置`HF_HUB_DISABLE_XET=1`与单worker后由官方HTTP/LFS续传完成，没有换源或重下12个已完成文件。最终13个顶层payload逐项复算，总计`2,328,028,720` bytes；manifest SHA-256=`84ac49fd8501e003c01557a58a0f8750d3e57608b61ae2d897981360f429141d`。下载成功只关闭artifact acquisition，不是模型质量或产品通过。
- `mineru.json`固定config version 1.3.2和上述绝对VLM根，SHA-256=`12c2703789d8a62f7dd40d2f8a658a5e5c11da284caf896515fb0f3954a1b23d`。实际只读解析在`MINERU_MODEL_SOURCE=local`、HF/Transformers offline下返回source=`local`、engine=`transformers`。MinerU公开CLI没有显式`transformers`开关，当前确定性来自隔离环境中lmdeploy/vllm不可导入、显式本地`--model-path`和启动日志校验；未来若环境多出lmdeploy必须fail closed。源码审计还确认`MINERU_TABLE_MERGE_ENABLE=false`只关闭最终middle-json merge，VLM client内另一路跨页判断硬编码为true；本单页无page pair，未来逐页qualification还必须同时保持`MINERU_PROCESSING_WINDOW_SIZE=1`，不能只靠一个开关声称完全禁用跨页改写。
- 首个immutable推理preflight为`attempts\mineru_vlm\tencent_2025_annual_report\20260901T102240Z-preflight1`。绑定同一Tencent原PDF、物理页130（0-based `[129,129]`、未来subset `page_idx=0`）、上述环境/config/model manifest和TokenBudgetBasis。三次资源样本的free RAM为`4.224/4.205/4.201 GiB`，free VRAM均为`6,964 MiB`；冻结门为RAM `>=10 GiB`且VRAM `>=7,680 MiB`，两项均失败。因此receipt正确返回`HOLD_RESOURCE / DO_NOT_LOAD_MODEL_OR_RUN_DOCUMENT_INFERENCE`：model loaded=false、PDFium subset=false、document inference=false、outputs=0、OOM=false；没有降门、杀Owner进程、删除indexes或在同一attempt重试。receipt为`7,059` bytes、SHA-256=`f5a75cb5693d796834c5d8e0d029b286d8d58bea9be0d25871e9ba465cbbe257`。
- 当前精确结论是：MinerU exact native-Windows Transformers环境和Apache-stated VLM snapshot已经真实落地并通过组件/runtime/supply校验；pipeline许可仍`HOLD_LICENSE`，VLM文档推理仍`HOLD_RESOURCE / NOT_STARTED`，parser质量、FIN adapter、cell provenance、adoption、mainline、NumericFact、Evidence、production/SaaS/redistribution和legacy retirement全部没有通过。资源自然恢复到双门后，才以新attempt运行同一页单并发smoke；下一次receipt应补全字面环境变量、绝对`mineru.exe`路径和engine import guard，当前immutable失败receipt不为这个P3回写。
- 两路作者分离、严格只读复核均确认上述停止判断与磁盘绑定成立，且均没有P0/P1/P2。runtime/source复核为`0/0/0/1`；environment/model/supply复核为`0/0/0/3`并给出`PASS_BOUNDED / HOLD_RESOURCE / HOLD_LICENSE`。P3仅涉及未来receipt的自摘要、安装/runtime transcript、完整forbidden-policy与URL normalization，以及绝对可执行文件、字面环境变量和import guard；这些不能追改当前immutable失败receipt，也不能把组件级通过升级成文档推理、parser质量或生产供应链通过。
