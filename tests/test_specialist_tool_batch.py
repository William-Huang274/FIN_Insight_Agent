...
from sec_agent.agent_runtime.specialist_graph import (
    ...
    SpecialistNotebook, build_specialist_agentic_state_graph,
)
...
def test_invalid_call_ids_reject_before_dispatch(defect):
    ...
    with pytest.raises(SpecialistAgenticGraphError, match="specialist_model_action_invalid"):
        _exercise(mutate)
...
def test_immutable_r6_response_real_mcp_and_sdk_four_result_continuation(monkeypatch):
    ...
    import sec_agent.agent_runtime.specialist_composition as module
    from sec_agent.agent_runtime.specialist_graph import SpecialistL0Context
    ...
    with open_specialist_receipted_composition(run_id=old["run_id"], run_invocation_id=old["run_invocation_id"],
