...
- [ ] 当前下一活动是MinerU 3.4.5成熟challenger：exact wheel/tag/commit已闭合；默认pipeline因`PDF-Extract-Kit-1.0`官方许可冲突为`HOLD_LICENSE`；先在Z盘建立exact native-Windows `mineru[vlm]` transformers环境并固定Apache-2.0 VLM revision/file manifest，再做一页resource smoke。当前约6.7 GiB可用显存与3.6 GiB空闲RAM不得直接运行282页；资源不足必须`HOLD_RESOURCE`，不得降门或删除`data/indexes`。
