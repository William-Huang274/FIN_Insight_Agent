...
PRIVATE_ROOT = ROOT / (
    ...
    "fin_0_1_3_s3_dell_value_profit_repair_canary/live"
)
LEDGER_PATH = PRIVATE_ROOT / "shared/admission_consumption.sqlite"


class S3LiveCanaryRunnerError(RuntimeError):
    pass
...
def _load(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise S3LiveCanaryRunnerError("s3_live_canary_runner_json_invalid")
    return value


def _validate_repository() -> str:
    if _git("status", "--porcelain", "--untracked-files=all"):
        raise S3LiveCanaryRunnerError("s3_live_canary_runner_requires_clean_worktree")
    head = _git("rev-parse", "HEAD")
    if head != _git("rev-parse", "@{upstream}"):
        raise S3LiveCanaryRunnerError("s3_live_canary_runner_requires_synced_head")
    return head
...
def preflight() -> dict[str, Any]:
    ...
    if ancestor.returncode != 0:
        raise S3LiveCanaryRunnerError("s3_live_canary_implementation_not_ancestor")
    project_os = run_project_os_preflight(ROOT, run_scope=LIVE_SCOPE)
    if project_os.get("status") != "pass":
        raise S3LiveCanaryRunnerError(
            "s3_live_canary_runner_project_os_blocked:"
    ...
    if credential["credential_present"] is not True:
        raise S3LiveCanaryRunnerError("s3_live_canary_runner_credential_missing")
    ...
    if runtime_root.exists():
        raise S3LiveCanaryRunnerError("s3_live_canary_attempt_root_already_exists")
    return {
        "head": head,
        "material": material,
        "decision": decision,
...
def execute(*, execution_authority_path: Path) -> dict[str, Any]:
    if not execution_authority_path.is_file():
        raise S3LiveCanaryRunnerError(
            "s3_live_canary_separate_execution_authority_missing"
        )
    state = preflight()
    ...
    return execute_live_canary(
        ...
        repo_root=ROOT,
        provider_call=build_no_retry_provider_call(profile=state["material"]["profile"]),
...
def main() -> int:
    ...
    if args.execution_authority is None:
        raise S3LiveCanaryRunnerError(
            "s3_live_canary_separate_execution_authority_required"
        )
