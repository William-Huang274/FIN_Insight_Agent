...
from retrieval.dell_report_internal_chain_ceiling_r7 import (  # noqa: E402
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    BRANCH,
...
def _read_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError(f"dell_03B_R7_json_not_mapping:{path.name}")
    return value
...
def _bound_json(
    ...
    if not path.is_file():
        raise ValueError(f"dell_03B_R7_bound_input_missing:{binding_id}")
    if base._sha256(path) != str(row.get("sha256") or ""):  # noqa: SLF001
        raise ValueError(f"dell_03B_R7_bound_input_sha_drift:{binding_id}")
    return path, _read_json(path)


def _require_clean() -> None:
    if base._git("status", "--porcelain", "--untracked-files=all"):  # noqa: SLF001
        raise RuntimeError("dell_03B_R7_clean_worktree_required")
...
def _clean_exact_git_receipt(policy: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("dell_03B_R7_exact_clean_synced_git_identity_required")
    return {
        "branch": branch,
        "head": head,
...
def _validate_implementation_bindings(policy: Mapping[str, Any]) -> None:
    ...
    if not isinstance(rows, Sequence) or isinstance(rows, (str, bytes)):
        raise ValueError("dell_03B_R7_implementation_bindings_invalid")
    seen: set[str] = set()
    for row in rows:
        ...
        if ref in seen:
            raise ValueError("dell_03B_R7_implementation_binding_duplicate")
        seen.add(ref)
        path = base._resolve(ref)  # noqa: SLF001
        if not path.is_file() or base._sha256(path) != str(  # noqa: SLF001
            row.get("sha256") or ""
        ):
            raise ValueError(f"dell_03B_R7_implementation_binding_drift:{ref}")
...
def _validate_canonical_output_paths() -> None:
    ...
    if private_resolved == public_resolved or len(
        ...
        raise ValueError("dell_03B_R7_output_paths_must_be_distinct")
    if private_resolved.parent != receipt_resolved.parent:
        raise ValueError("dell_03B_R7_attempt_receipt_private_parent_mismatch")


def _require_output_disk_capacity() -> dict[str, int]:
    usage = shutil.disk_usage(ROOT)
    if usage.free < MIN_FREE_BYTES_BEFORE_ATTEMPT:
        raise RuntimeError("dell_03B_R7_minimum_free_disk_capacity_required")
    return {
...
def _write_attempt_consumption_receipt(
    *,
    ...
    if ATTEMPT_RECEIPT.exists() or ATTEMPT_RECEIPT.parent.exists():
        raise FileExistsError("dell_03B_R7_attempt_already_consumed")
    body = {
        "schema_version": (
...
def _publish_atomic_pair(*, private_bytes: bytes, public_bytes: bytes) -> None:
    if DEFAULT_PRIVATE.exists() or DEFAULT_PUBLIC.exists():
        raise FileExistsError("dell_03B_R7_output_collision")
    ...
    if private_tmp.exists() or public_tmp.exists():
        raise FileExistsError("dell_03B_R7_temporary_output_collision")
...
def run() -> dict[str, Any]:
    ...
    if not POLICY.is_file():
        raise FileNotFoundError("dell_03B_R7_canonical_policy_missing")
    policy = _read_json(POLICY)
    if policy.get("schema_version") != POLICY_SCHEMA_VERSION:
        raise ValueError("dell_03B_R7_policy_schema_invalid")
    ...
    if (
        ...
        raise FileExistsError("dell_03B_R7_attempt_or_output_exists")
    ...
    legacy_policy = validate_dell_report_internal_chain_ceiling_r7_policy(
        policy,
    ...
    if len(request_payloads) != 5 or {
        ...
        raise ValueError("dell_03B_R7_request_payload_population_invalid")
    ...
    if base._sha256(objects_path) != str(  # noqa: SLF001
        ...
        raise ValueError("dell_03B_R7_compiled_object_sha_drift")
    if base._sha256(sources_path) != str(  # noqa: SLF001
        bindings.get("source_records", {}).get("sha256") or ""
    ):
        raise ValueError("dell_03B_R7_source_record_sha_drift")
    ...
    private_result = compile_dell_report_internal_chain_ceiling_r7_result(
    ...
    public_result = build_dell_report_internal_chain_ceiling_r7_public_projection(
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Run the exact-once canonical DELL 03B R7 raw-position, typed-anchor "
