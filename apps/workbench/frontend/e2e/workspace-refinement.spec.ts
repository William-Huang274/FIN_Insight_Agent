...
for (const width of [1440,1024,390]) test(`research start, organization, panel and rendered revision at ${width}`, async ({page}) => {
  ...
  await page.route("**/api/v1/**",async route=>{
    ...
    let body:any={};
    if(path.endsWith("research-session-config"))body={fresh_research_enabled:true};
  ...
  await nav("研究配置");await page.getByRole("button",{name:"编辑草稿",exact:true}).click();await page.getByLabel("配置草稿").fill("# 新方法草稿\n\n核对期间、单位和反证。");await page.getByRole("button",{name:"保存草稿",exact:true}).click();await expect(page.getByRole("status")).toContainText("未应用到运行服务");
  await page.reload();await expect(page.getByRole("heading",{name:"新方法草稿",exact:true})).toBeVisible();
  await page.getByRole("button",{name:"Agent 图与编排"}).click();await page.getByRole("button",{name:"writer",exact:true}).click();await expect(page.locator(".fs-studio-edges")).toContainText("verifier");
  await page.getByRole("button",{name:"编辑草稿",exact:true}).click();await page.getByLabel("配置草稿").fill("{invalid");await page.getByRole("button",{name:"保存草稿",exact:true}).click();await expect(page.getByRole("status")).toContainText("JSON 语法有误");
