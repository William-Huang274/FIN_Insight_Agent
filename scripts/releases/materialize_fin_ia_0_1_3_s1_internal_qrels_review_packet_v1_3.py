...
def main() -> int:
    if OUTPUT_PATH.exists():
        raise RuntimeError("internal_qrels_successor_v1_3_already_exists")
    preflight = run_project_os_preflight(ROOT, run_scope=RUN_SCOPE)
    if preflight.get("status") != "pass":
        raise RuntimeError("internal_qrels_successor_v1_3_preflight_blocked")
