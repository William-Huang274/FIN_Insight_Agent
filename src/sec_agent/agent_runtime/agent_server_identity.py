"""Persist FIN task and Agent Server identity bindings in PostgreSQL."""
...
from .agent_server_recovery import (
    AgentServerRecoveryCase,
    AgentServerRecoveryError,
...
IDENTITY_SCHEMA_RESOURCE = "sql/001_agent_server_identity_v1_0.sql"
...
REMOTE_CREATE_LIFECYCLE_SCHEMA_RESOURCE = (
    "sql/002_agent_server_remote_create_lifecycle_v1_1.sql"
)
REMOTE_CREATE_LIFECYCLE_SCHEMA_SHA256 = (
    "9e9f1e324c07bd767f71c8e870d736d44892b7b5614a4e0ffb1d557491218d25"
)
AGENT_SERVER_ASSISTANT_ID = "dell_reference_vertical"
...
class AgentServerIdentityStoreError(RuntimeError):
...
class AgentServerIdentityConflict(AgentServerIdentityStoreError):
...
class AgentServerIdentityRepository(Protocol):
    ...
    def bind_agent_session(
        ...
        *,
        ...
        server_thread_id: str,
        assistant_id: str = AGENT_SERVER_ASSISTANT_ID,
...
def load_identity_schema_sql() -> str:
    ...
    if sha256(normalized.encode("utf-8")).hexdigest() != IDENTITY_SCHEMA_SHA256:
        raise AgentServerIdentityStoreError(
...
class PostgresAgentServerIdentityRepository:
        ...
        try:
            ...
            with connection_context as connection:
                ...
                with connection.transaction():
                    yield connection
                _require_idle_connection(connection)
        except AgentServerIdentityStoreError:
            raise
        except Exception:
            raise AgentServerIdentityStoreError(failure_code) from None
    ...
    def bind_agent_session(
        ...
        *,
        ...
        with self._transaction(
            ...
            if len(rows) != 1:
                raise AgentServerIdentityConflict(
    ...
    def begin_run_create(
        ...
        *,
        ...
        if invocation.run_id != run.run_id:
            raise AgentServerIdentityStoreError("run_invocation_run_mismatch")
        ...
        except AgentServerRecoveryError as exc:
            raise AgentServerIdentityStoreError(exc.code) from None
...
class PostgresAgentServerRecoveryOperatorRepository:
...
def _required_run_create_lifecycle(
    ...
    *,
    ...
    lifecycle = _run_create_lifecycle_from_rows(rows)
    if lifecycle is None:
        raise AgentServerIdentityStoreError("run_create_pending_missing")
    return lifecycle
...
def _append_orphan_observation(
    ...
    *,
    ...
    if lifecycle.dispatched is None:
        raise AgentServerIdentityConflict("run_create_dispatched_missing")
    for observation in lifecycle.orphan_observations:
...
def _json_object(value: Any, *, code: str) -> dict[str, Any]:
    if isinstance(value, str):
        try:
            ...
            raise AgentServerIdentityStoreError(code) from None
    if not isinstance(value, dict):
        raise AgentServerIdentityStoreError(code)
    return value
...
def _recovery_case_from_row(row: Sequence[Any]) -> AgentServerRecoveryCase:
    if len(row) != 17:
        raise AgentServerIdentityStoreError(
    ...
    try:
        ...
        recovery_case = AgentServerRecoveryCase(
...
def _session_from_row(row: Sequence[Any]) -> PersistedAgentSessionBinding:
    if len(row) != 6:
        raise AgentServerIdentityStoreError("identity_session_row_invalid")
...
def _run_from_row(row: Sequence[Any]) -> PersistedResearchRunIdentity:
    if len(row) != 5:
        raise AgentServerIdentityStoreError("identity_run_row_invalid")
...
def _invocation_from_row(row: Sequence[Any]) -> PersistedRunInvocationBinding:
    if len(row) != 12:
        raise AgentServerIdentityStoreError("identity_invocation_row_invalid")
...
def _assistant_id(value: Any) -> str:
    ...
    if assistant != AGENT_SERVER_ASSISTANT_ID:
        raise AgentServerIdentityStoreError(
...
__all__ = [
    ...
    "AgentServerIdentityConflict",
    "AgentServerIdentityRepository",
