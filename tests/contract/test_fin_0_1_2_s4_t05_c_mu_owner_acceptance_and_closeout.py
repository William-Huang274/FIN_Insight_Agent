...
from scripts.releases.materialize_fin_ia_0_1_2_s4_t05_c_mu_owner_acceptance_and_closeout import (  # noqa: E402
    DEFAULT_OUTPUT,
    EXPECTED_ASSESSMENT_DIGEST,
    T05CMUOwnerAcceptanceError,
    materialize,
...
def test_explicit_owner_acceptance_closes_T05_C_and_sets_MU_R2() -> None:
    ...
    assert decision["acceptance_effect"]["S4_T05_C"] == (
        "pass_closed_owner_accepted"
    )
    assert decision["acceptance_effect"]["MU_current_R2"] is True
    assert decision["acceptance_effect"]["S4_T05_D_entry"] == (
        "authorized_not_started"
    )


def test_acceptance_preserves_quality_release_and_production_boundaries() -> None:
    decision = _load()
    assert decision["acceptance_effect"]["post_transfer_NVDA_R2"] is False
    assert decision["preserved_boundaries"]["RC_P36_119"].startswith("open_")
    assert decision["preserved_boundaries"]["RC_P36_122"].startswith("open_")
    assert decision["preserved_boundaries"]["strong_MU_investment_thesis"] is False
    assert decision["preserved_boundaries"]["release"] == "not_qualified"
    assert decision["preserved_boundaries"]["production"] == "not_qualified"
    assert decision["observed_counts"]["T05_D_runs"] == 0
...
def test_non_accept_message_cannot_retain_acceptance_semantics() -> None:
    ...
    with pytest.raises(
        T05CMUOwnerAcceptanceError,
        match="s4_t05_c_mu_owner_acceptance_semantics_invalid",
    ):
...
@pytest.mark.parametrize(
    ("field", "value"),
    [("T05_D_execution_started", True), ("T05_D_entry_authorized", False)],
)
def test_owner_acceptance_does_not_silently_execute_T05_D(
    ...
    with pytest.raises(
        T05CMUOwnerAcceptanceError,
        match="s4_t05_c_mu_owner_acceptance_boundary_invalid",
    ):
        validate_owner_acceptance(changed)
...
def test_quality_or_release_claim_mutation_fails_closed() -> None:
    changed = deepcopy(_load())
    changed["preserved_boundaries"]["strong_MU_investment_thesis"] = True
