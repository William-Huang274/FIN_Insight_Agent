...
19. [ ] **当前唯一下一项**：执行 `S1-08-P2D / S1_08_V3_DELL_R3_EXACT_LIVE_ISSUANCE_AUTHORITY_PROJECTION_DECISION`。它只重核 v1.1 proof、当前 Runtime tree、contact presence、16-call budget 与 no-R4 stop rule；不得在同一项签发或执行 live。P2D 通过后才以独立 Attempt 执行唯一 DELL R3。R3 通过才进入 MU/NVDA common transfer 与 ranking；失败则执行 `S1-08-P3` Provider／产品来源范围决策，明确禁止 R4。
