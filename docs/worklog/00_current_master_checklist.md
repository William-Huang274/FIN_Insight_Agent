...
- [ ] Owner 已授权的 bounded engineering 已形成 implementation candidate，但最终签发待完成：`pyproject.toml + uv.lock`单一依赖源（157 lock records，含setuptools artifact hashes）；core/control-plane/qualification与locked supply tooling分层；PostgreSQL 16.15 transaction/UNIQUE/advisory lock/restart/host-roundtrip dump-restore；Dagster PostgreSQL run/event storage；真实DELL/MU/NVDA S2 fact-mart 1,319 observations、24/24 qrels legacy/Dagster shadow exact。两个镜像已候选化为non-root、OCI revision-bound、Compose secret file与只读data/internal-network合同；前端nanoid high已在lock层修复。旧34/76、pre-supply 33/86/88 manifests与034026/040515均非final；必须从干净候选commit和全新combined locked env重跑。
- [x] thin Dagster adapter只编排现有 CLI，legacy入口与 rollback保留、删除量0；LangGraph因本纵切面无 checkpoint/HITL/Agent graph需求而继续 HOLD。
- [ ] 先提交候选实现与“最终证据待跑”状态；再完成fresh locked qualification、最终Docker只读真实数据job、两个final image SBOM/CVE、全仓与frontend回归、independent read-only review和第二次evidence/Project OS commit/push。已暴露EIA key的外部轮换仍由Owner完成；之后由Owner决定下一条有用户价值的产品vertical，禁止自动全仓Dagster化、PostgreSQL cutover或LangGraph引入。
