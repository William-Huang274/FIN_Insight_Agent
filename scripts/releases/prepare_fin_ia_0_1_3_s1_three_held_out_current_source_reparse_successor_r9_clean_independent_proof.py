...
ATTEMPT_ID = "S1-THREE-HELD-OUT-CURRENT-SOURCE-REPARSE-R9-CLEAN-PROOF-A1"
PREDECESSOR_FAILURE_REFS = (
    Path(
        "configs/releases/fin_ia_0_1_3_s1_three_held_out_current_source_"
        "reparse_successor_r5_business_audit_failure_v1_0.json"
    ),
    Path(
        "configs/releases/fin_ia_0_1_3_s1_three_held_out_current_source_"
        "reparse_successor_r6_business_audit_failure_v1_0.json"
    ),
    Path(
        "configs/releases/fin_ia_0_1_3_s1_three_held_out_current_source_"
        "reparse_successor_r7_business_audit_failure_v1_0.json"
    ),
    Path(
...
class CleanReparseR9ProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise CleanReparseR9ProofError(code)
    ...
    with tarfile.open(archive_path, "r") as archive:
        for member in archive.getmembers():
            ...
            try:
                ...
                raise CleanReparseR9ProofError(
...
def _blocked_network(*args: Any, **kwargs: Any) -> Any:
    raise CleanReparseR9ProofError("clean_reparse_r9_proof_network_forbidden")
...
def _worker_payload(runtime_root: Path) -> dict[str, Any]:
    ...
    execution_root = runtime_root / (
        ...
        "fin_0_1_3_s1_three_held_out_current_source_reparse/clean-proof-r9"
    )
    ...
    return {
        "schema_version": (
            "fin_ia_0_1_3_s1_three_held_out_current_source_reparse_"
            "r9_clean_worker_v1_0"
        ),
        "status": "pass",
...
def build_result() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin013-s1-held-out-reparse-r9-clean-proof-"
    ) as parent:
        parent_path = Path(parent).resolve()
        _require(
            parent_path.name.startswith(
                "fin013-s1-held-out-reparse-r9-clean-proof-"
            ),
    ...
    body = {
        "schema_version": (
            "fin_ia_0_1_3_s1_three_held_out_current_source_reparse_"
            "successor_r9_clean_independent_proof_v1_0"
        ),
        "contract_ref": (
            "fin_0_1_3.S1.three_held_out_current_source_reparse_"
            "successor_r9_clean_independent_proof:v1"
        ),
        ...
        "stage_acceptance": {
            ...
            "flow_stock_and_business_coordinate_engineering": True,
        ...
        "decision_zh": (
            ...
            "原始响应，均逐字节重现 R9 result。R5-R7 与 manifest R3 业务失败继续保留；"
            "下一步只准入把 CandidateBundle-only manifest 改绑到 R9，不准入真实 BGE、Milvus、"
            "ranking、Evidence、外源补源或 DeepSeek。"
        ),
        "known_boundary": (
            "This proof establishes clean reproducibility of the R9 row/column/table-axis "
            "financial-time successor only. It does not establish a physical "
            "index, ranking quality, Evidence Pack completeness, external coverage, "
            "model synthesis, report quality or release readiness."
        ),
    }
