...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'fin_runtime_operator') THEN
        EXECUTE format(
            'CREATE ROLE fin_runtime_operator LOGIN NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION PASSWORD %L',
            '$FINSIGHT_FIN_RUNTIME_OPERATOR_POSTGRES_PASSWORD'
        );
    ELSE
        EXECUTE format(
            'ALTER ROLE fin_runtime_operator PASSWORD %L',
            '$FINSIGHT_FIN_RUNTIME_OPERATOR_POSTGRES_PASSWORD'
        );
    END IF;
END
...
ALTER ROLE fin_runtime_operator NOSUPERUSER NOCREATEDB NOCREATEROLE NOREPLICATION;
...
GRANT CONNECT ON DATABASE postgres TO fin_runtime_operator;
REVOKE CREATE ON SCHEMA public FROM fin_runtime_app;
REVOKE CREATE ON SCHEMA public FROM fin_runtime_operator;
SQL
