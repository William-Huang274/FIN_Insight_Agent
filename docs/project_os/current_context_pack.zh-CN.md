...
Research Context Closure 的结构门和当前 profile 容量门已通过，但唯一 Chat R1 已以 `IncompleteRead` terminal failure 消费，不能自动重跑。当前下一步先返回 Owner 本次 L1／内容不可评结果。若 Owner 之后授权 replacement，最早责任层是同一 S3 transport/capture：单独保存 `IncompleteRead` partial response、已取得的 HTTP status／headers 和长度差，做零调用回放与 mutation，再决定是否需要对直连／本地代理做有界处置并签发新 authority。第 7 步五单元、其他 RoleMethodPack、Responses 和完整图 handler仍不自动开始。
