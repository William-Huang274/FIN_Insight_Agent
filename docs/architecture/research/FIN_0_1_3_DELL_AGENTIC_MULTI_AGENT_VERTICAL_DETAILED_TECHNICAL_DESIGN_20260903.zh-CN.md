...
当前执行入口（2026-09-07）：本文件 [原五项＋新增需求剩余工作](#remaining-work-20260907) 与 S3/190 文末。Owner批准的3元内局部修订对照已结束：A整稿3调用/80,734 tokens/估0.2933814元，B局部2调用/50,775 tokens/估0.2182641元，合计0.5116455元；两组42引用/3图及无关正文不变，5个LangSmith LLM用量完整。**接口与成本实测成立，语义未通过，不晋升候选，不再追加本批调用。** 原稿“未单独披露→非已实现”歧义被修订参照传播，B还未经证明合并第三方口径；同层补了通用方法指引及零模型输入验证，不加NLP规则/框架。资格入口`a8d70ea1`、方法修正`1dbfcd91`，50项相关测试通过，代码未部署。下一沿步骤二核P02、原稿责任与方法应用，并保留步骤一上下文真实资格/摘要/路由待办；前端、上传视觉、交付引用、公开和新场景不消失。旧6run/265请求/264已知/17,060,539tokens/估28.092715元与LangSmith历史缺190815均保持原记录，新5调用单列资格账。原v3仍needs_revision，报告在 `D:/temp/finsight-dell-final-20260907-a1`，SQL/四格式/历史run不改，相关API/PG/Redis均停止。下方旧日期/运行状态仅历史。
...
- [ ] 真实小批已执行且保留失败，不是全绿：局部编辑语义问题仍开；长历史接续暴露旧观察回读接口遗漏并已修，四个真实CALC及原生循环零模型重放通过，修后模型验收仍待做。旧文字CALC现在可回读原成功窗口，但不是可执行新对象；继续计算须补读原操作数后重算，不猜造。
- [ ] 原生摘要已接并真实试验；完整原messages/SQL证据/配对/任务及独立摘要费用有证据，但长接续未通过，故`context_summarization.enabled=false`。摘要中的历史判断不能晋升事实；不凭输入从200k变74k称同质量省费。回读补丁之后尚无付费长接续复验。
