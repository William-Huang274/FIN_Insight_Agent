...
from test_external_sources import _run_scope, _FakeFetcher, _public_guard
...
def test_paid_web_permission_is_explicit_and_requires_source_tool(tmp_path):
    ...
    with pytest.raises(ValidationError, match="live_web_requires_source_read"):
        ModelExecutionAuthority.model_validate_json(json.dumps({**body, "decision_digest": canonical_sha256(body)}))
    body["source_read_enabled"] = True
    parsed = ModelExecutionAuthority.model_validate_json(json.dumps({**body, "decision_digest": canonical_sha256(body)}))
...
def test_live_exa_mcp_search_and_source_read():
    from test_agent_server_data_composition import DEFAULT_ARTIFACT_ENV, _execute_evidence
    from sec_agent.agent_runtime.agent_server_data_composition import open_approved_data_composition
    ...
    with open_approved_data_composition(run_invocation_id=output.name, environment=DEFAULT_ARTIFACT_ENV,
                                            source_read_enabled=True, live_web_read_enabled=True) as composition:
