...
Windows Docker 绑定挂载上的大知识库查询偏慢时，可按[不可变知识库原生卷部署](../engineering/immutable_library_volume.zh-CN.md)复制同一发布版本并校验后切换；保留原文件与回退方式，不重新生成付费向量。

0.1.4 开发分支增加 `/workspace/assets` 独立资产工作区：默认浏览已配置的知识库，也可切换财务数据库和项目资料；研究页顶部“项目资料”可打开右侧阅读面板，进入完整编辑页后返回原任务。资产区沿用研究区字体、配色和明暗主题。可将知识库所选章节、财务表格当前页所选记录加入项目，单独维护批注，并在“围绕资料提问”中勾选多份资产准备研究；保存后原始来源保持固定。也可阅读和修订项目资料，维护个人偏好，保存问题并携带所选版本进入助手对话或研究草稿。准备草稿不调用模型；实际发送或启动才进入原有运行流程。首版仍有本地容量限制，记忆由用户主动维护，运行中不会自动采用资产新版。协议、数据库关系与项目库备份恢复命令见[资产工作区说明](../architecture/asset_workspace_protocol.zh-CN.md)。

0.1.4 开发分支的项目资料页提供“版本、差异与关联成果”：可追加修订或新期间文件、比较保存文本、查询记录了旧版来源的任务/成果，再明确选版准备新任务。SEC 页面支持同一公司两个保存快照的原始观测差异。旧任务和报告保留原版；未记录的依赖会提示未知，不自动推断金融影响或调用模型。[操作与范围说明](../engineering/project_asset_versions.zh-CN.md)。
