...
WORKER_REF = (
    "scripts/releases/"
    "prove_fin_ia_0_1_3_s2_selected_evidence_numeric_natural_node_canary_"
    "worker.py"
)
DEFAULT_OUTPUT = ROOT / (
    "configs/releases/"
...
class NaturalNodeCanaryCleanProofError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise NaturalNodeCanaryCleanProofError(code)
...
def _require_clean_synced() -> str:
    _require(
        ...
        "natural_node_canary_clean_proof_requires_clean_worktree",
    )
    head = _git("rev-parse", "HEAD")
    _require(
        head == _git("rev-parse", "@{upstream}"),
        "natural_node_canary_clean_proof_requires_synced_head",
    )
    return head
...
def _load_json(path: Path, code: str) -> dict[str, Any]:
    try:
        ...
        raise NaturalNodeCanaryCleanProofError(code) from exc
...
def _private_artifacts() -> list[dict[str, Any]]:
    ...
    fixed_contract = _load_json(
        ...
        "natural_node_canary_clean_proof_fixed_contract_invalid",
    )
    ...
    historical_result = _load_json(
        ...
        "natural_node_canary_clean_proof_historical_result_invalid",
    )
    ...
    for row in rows:
        ...
        _require(
            ...
            f"natural_node_canary_clean_proof_private_artifact_drift:{row['kind']}",
        )
    ...
    with tarfile.open(archive_path, "r") as archive:
        ...
        for member in archive.getmembers():
            ...
            _require(
                ...
                "natural_node_canary_clean_proof_archive_path_escape",
            )
...
def _run_worker(
    *, archive_root: Path, implementation_commit: str
    ...
    output_path = archive_root / ".natural-node-canary-clean-output.json"
    completed = subprocess.run(
        [
            sys.executable,
            "-I",
    ...
    if completed.returncode != 0:
        raise NaturalNodeCanaryCleanProofError(
            "natural_node_canary_clean_worker_failed:"
            + completed.stderr[-4000:]
            + completed.stdout[-1500:]
        )
    _require(
        output_path.is_file(),
        "natural_node_canary_clean_worker_output_missing",
    )
    ...
    _require(
        ...
        "natural_node_canary_clean_worker_output_invalid",
    )
    summary = {
...
def _validate_proof(proof: Mapping[str, Any]) -> None:
    _require(
        ...
        "natural_node_canary_clean_proof_identity_invalid",
    )
    _require(
        ...
        "natural_node_canary_clean_proof_isolation_invalid",
    )
    ...
    _require(
        ...
        "natural_node_canary_clean_proof_nonzero_external_calls",
    )
...
def main() -> int:
    ...
    if args.output.exists():
        raise NaturalNodeCanaryCleanProofError(
            "natural_node_canary_clean_proof_output_already_exists"
        )

    head = _require_clean_synced()
    implementation = _load_json(
        IMPLEMENTATION_PATH,
        "natural_node_canary_clean_proof_implementation_invalid",
    )
    _require(
        ...
        "natural_node_canary_clean_proof_implementation_digest_invalid",
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix="fin013-s2-natural-node-canary-clean-", dir=temp_parent
    ) as directory:
        ...
        _require(
            ...
            "natural_node_canary_clean_worker_outputs_differ",
        )
        _require(
            injections[0] == injections[1] == public_artifact_manifest,
            "natural_node_canary_clean_worker_injections_differ",
        )
    _require(
        temporary_path is not None,
        "natural_node_canary_clean_proof_temporary_root_missing",
    )
    ...
    body = {
        ...
        "status": (
            "pass_two_clean_archives_two_fresh_processes_zero_external_call_reproducible"
        ),
        ...
        "known_boundary": (
            ...
            "canary compiler, fixture runtime and local gates. It does not register "
            "live scope, issue a live admission, call DeepSeek, prove natural research "
            "quality, rerun Dell, grant Owner acceptance or authorize release."
        ),
        "current_next": (
            "SEPARATE_ZERO_CALL_ONE_CALL_LIVE_CANARY_VALUE_COST_RISK_AUTHORITY_DECISION"
        ),
    }
