"""Validate model-call budgets, allowed modes and audit destinations."""
...
from .research_contracts import COVERAGE_OBLIGATION_IDS as FULL_RESEARCH_BRANCHES


Q1_PAID_SHADOW_SERVING_MODE = "q1_specialist_paid_shadow_v1"
Q1_REVIEW_SERVING_MODE = "q1_workpaper_review_repair_v1"
LEAD_RESEARCH_SERVING_MODE = "lead_research_delegation_v1"
CASE_REVIEW_SERVING_MODE = "case_workpaper_review_v1"
CASE_CONVERGENCE_SERVING_MODE = "case_convergence_v1"
CASE_REPAIR_PAPERS = ("P01", "P04", "P05", "P06", "P07", "P08")
Q1_PAID_SHADOW_AUTHORITY_ENV = "FINSIGHT_DELL_PAID_SHADOW_AUTHORITY_PATH"
IMPLEMENTATION_COMMIT_ENV = "FINSIGHT_DELL_IMPLEMENTATION_COMMIT"
...
class ModelExecutionPolicyError(RuntimeError):
...
class LeadResearchScope(BaseModel):
    ...
    def validate_scope(self) -> "LeadResearchScope":
        ...
        targeted = (self.targeted_completion and bool(scope)
                    and scope.issubset(set(FULL_RESEARCH_BRANCHES) - {"Q1_ISSUER_TRUTH"}))
        if (len(scope) != len(self.allowed_branch_ids) or not (targeted or (not self.targeted_completion and scope in (
                {"Q5_SUPPLY_AND_PRICE", "Q6_MODEL_COMPUTE_DEMAND"}, set(FULL_RESEARCH_BRANCHES))))):
...
class CaseReviewScope(BaseModel):
    model_config = ConfigDict(extra="forbid", strict=True, frozen=True)
    seed_state_host_path: str
    seed_state_sha256: str = Field(pattern=_DIGEST_PATTERN)
    node_budgets: dict[Literal["counter", "verifier"], TokenBudgetBasis]
    max_reviewer_model_turns: int = Field(ge=10, le=36)
    max_reviewer_tool_actions: int = Field(ge=24, le=100)

    @model_validator(mode="after")
    def validate_nodes(self):
        if set(self.node_budgets) != {"counter", "verifier"}:
            raise ValueError("case_review_budget_roles_invalid")
        return self
...
class CaseConvergenceScope(BaseModel):
    ...
    def validate_nodes(self):
        if (set(self.node_budgets) != {"repair", "writer", "verifier"}
                ...
                or set(self.repair_paper_ids) != set(CASE_REPAIR_PAPERS)):
            raise ValueError("case_convergence_roles_or_papers_invalid")
        return self


class ModelExecutionAuthority(BaseModel):
    ...
    @model_validator(mode="after")
    def validate_authority(self) -> "ModelExecutionAuthority":
        ...
        if (is_review != (self.serving_mode == Q1_REVIEW_SERVING_MODE)
                or is_lead != (self.serving_mode == LEAD_RESEARCH_SERVING_MODE)
                or is_case != (self.serving_mode == CASE_REVIEW_SERVING_MODE)
                or is_convergence != (self.serving_mode == CASE_CONVERGENCE_SERVING_MODE)):
...
def load_q1_paid_shadow_authority(
    path: str | Path,
) -> ModelExecutionAuthority:
    try:
        return ModelExecutionAuthority.model_validate_json(
...
def build_private_model_audit_sink(authority: ModelExecutionAuthority):
