"""Research workflow with planning, source tools and independent review."""
...
def _plain_mapping(value: Any, *, label: str) -> PlainDict:
    if not isinstance(value, Mapping):
        raise ResearchGraphError(f"{label}_must_be_mapping")
...
def _validate_model(model: type[ModelT], value: Any, *, label: str) -> ModelT:
    ...
    try:
        ...
        raise ResearchGraphError(f"{label}_invalid") from exc


def _require_text(value: Any, *, label: str, maximum: int = 2_000) -> str:
    if not isinstance(value, str) or not value.strip():
        raise ResearchGraphError(f"{label}_missing_or_empty")
    normalized = value.strip()
    if len(normalized) > maximum:
        raise ResearchGraphError(f"{label}_too_long")
    return normalized
...
def _validate_receipt(
    ...
    *,
    ...
    if receipt.kind != kind:
        raise ResearchGraphError("runtime_receipt_kind_mismatch")
    if receipt.actor != actor:
        raise ResearchGraphError("runtime_receipt_actor_mismatch")
    ...
    if receipt.request_digest != canonical_sha256(request):
        raise ResearchGraphError("runtime_receipt_request_digest_mismatch")
...
def _validate_agent_step_budget(
    ...
    *,
    ...
    if any(
        ...
        raise ResearchGraphError(f"{label}_result_limit_exceeded")
    if sum(row.intent.limit for row in requests) > ceiling.maximum_sources_visible_per_agent_step:
        raise ResearchGraphError(f"{label}_visible_source_limit_exceeded")
    ...
    if (
        ...
        raise ResearchGraphError(f"{label}_external_round_limit_exceeded")
    if captured_pages > ceiling.maximum_captured_pages_per_branch:
        raise ResearchGraphError(f"{label}_capture_limit_exceeded")
...
def _validate_specialist_round_authority(
    *,
    ...
    if requested_round != completed_rounds + 1:
        raise ResearchGraphError("specialist_round_sequence_invalid")
    if requested_round > binding.scope_ceiling.maximum_specialist_model_rounds:
        raise ResearchGraphError("specialist_round_limit_exceeded")
...
def _validate_tool_result(
    ...
    *,
    ...
    if actual != expected:
        raise ResearchGraphError("tool_lane_result_binding_mismatch")
    _validate_receipt(
...
def _index_lane_results(
    ...
    *,
    ...
    for value in values:
        ...
        if key in indexed:
            raise ResearchGraphError(f"{label}_duplicate")
        indexed[key] = result
    return indexed
...
def _join_initial_lane_results(
    *,
    ...
    if set(evidence) != expected_evidence:
        raise ResearchGraphError("initial_evidence_result_set_mismatch")
    if set(finance) != expected_finance:
        raise ResearchGraphError("initial_finance_result_set_mismatch")
...
def _validate_workpaper(
    ...
    *,
    ...
    if actual != expected:
        raise ResearchGraphError("branch_workpaper_binding_mismatch")
    expected_receipts = {
    ...
    if set(workpaper.tool_receipt_ids) != expected_receipts:
        raise ResearchGraphError("branch_workpaper_tool_receipt_mismatch")
    ...
    if unknown_evidence:
        raise ResearchGraphError("branch_workpaper_unknown_evidence_id")
    if unknown_facts:
        raise ResearchGraphError("branch_workpaper_unknown_fact_id")
    has_tool_failure = (
    ...
    *,
    ...
    for value in values:
        ...
        if row.revision != revision:
            raise ResearchGraphError("branch_workpaper_revision_invalid")
        if row.branch_id in indexed:
            raise ResearchGraphError("branch_workpaper_duplicate")
        indexed[row.branch_id] = row
    if set(indexed) != expected_branch_ids:
        raise ResearchGraphError("branch_workpaper_set_mismatch")
...
def _citation_index(state: Mapping[str, Any]) -> PlainDict:
    ...
    if not isinstance(effective_raw, Mapping):
        raise ResearchGraphError("citation_workpapers_missing")
    workpapers = {
...
def build_research_state_graph(
    *,
    ...
    except ExecutionProfileCheckError as exc:
        raise ResearchGraphError(str(exc)) from None
    ...
    def validate_source_route_requests(
        ...
        *,
        ...
        if len(requested_ids) != len(set(requested_ids)):
            raise ResearchGraphError(f"{label}_route_duplicate")
        for request in requests:
            route = source_routes_by_id.get(request.minimum_route_obligation_id)
            if route is None:
                raise ResearchGraphError(f"{label}_route_unknown")
            if route["coverage_obligation_id"] != branch_id:
                raise ResearchGraphError(f"{label}_route_branch_mismatch")
            if route["intent_kind"] != request.intent.intent_kind:
                raise ResearchGraphError(f"{label}_route_lane_mismatch")
    ...
    def bind_case(state: ResearchGraphState) -> ResearchGraphState:
        ...
        if len(foundation_digest) != 64 or any(
            ...
            raise ResearchGraphError("foundation_digest_invalid")
        request = {
    ...
    def plan(state: ResearchGraphState) -> ResearchGraphState:
        binding = _validate_model(
            CaseFoundationBinding,
            state.get("foundation_binding"),
            label="foundation_binding",
    ...
    def validate_plan(state: ResearchGraphState) -> ResearchGraphState:
        binding = _validate_model(
            CaseFoundationBinding,
            state.get("foundation_binding"),
            label="foundation_binding",
        ...
        if unknown:
            raise ResearchGraphError("planner_selected_unknown_branch")
        if not set(binding.required_branch_ids).issubset(drafts):
            raise ResearchGraphError("planner_required_branch_missing")
    ...
    def dispatch_initial_tools(
        ...
        if not tasks:
            raise ResearchGraphError("branch_tasks_missing")
        sends: list[Send] = []
        for task in tasks:
            sends.append(
                Send(
    ...
    def initial_evidence_lane(value: Mapping[str, Any]) -> ResearchGraphState:
    ...
    def initial_finance_lane(value: Mapping[str, Any]) -> ResearchGraphState:
    ...
    def dispatch_specialists(
        ...
        if not isinstance(values, Mapping) or not values:
            raise ResearchGraphError("initial_branch_inputs_missing")
        return [
            Send("specialist_agent", _plain_mapping(values[branch_id], label="branch_input"))
            for branch_id in sorted(values)
        ]

    def specialist_agent(value: Mapping[str, Any]) -> ResearchGraphState:
    ...
    def counter_agent(state: ResearchGraphState) -> ResearchGraphState:
        ...
        if not isinstance(workpapers_raw, Mapping):
            raise ResearchGraphError("initial_workpapers_missing")
        request_base = {
            "agent_id": actor,
            "run_id": state["run_id"],
            "case_id": state["case_id"],
        ...
        if {key: getattr(decision, key) for key in expected} != expected:
            raise ResearchGraphError("counter_decision_binding_mismatch")
        _validate_receipt(
        ...
        if target is None:
            raise ResearchGraphError("counter_reroute_branch_unknown")
        binding = _validate_model(
            CaseFoundationBinding,
            state.get("foundation_binding"),
            label="foundation_binding",
        ...
        if int(state.get("reroute_count", 0)) >= (
            ...
            raise ResearchGraphError("counter_reroute_limit_exceeded")
    ...
    def rework_evidence_lane(value: Mapping[str, Any]) -> ResearchGraphState:
        result = _execute_tool(value, executor=dependencies.evidence_tool)
        return {"rework_evidence_result": result.model_dump(mode="json")}

    def rework_finance_lane(value: Mapping[str, Any]) -> ResearchGraphState:
        result = _execute_tool(value, executor=dependencies.finance_tool)
    ...
    def join_rework_tools(
        ...
        if not isinstance(prior_raw, Mapping) or task.branch_id not in prior_raw:
            raise ResearchGraphError("rework_prior_workpaper_missing")
        ...
        if decision.reroute is None:
            raise ResearchGraphError("rework_counter_reroute_missing")
        agent_input = _branch_context(
            run_id=state["run_id"],
            task=task,
    ...
    def specialist_rework(state: ResearchGraphState) -> ResearchGraphState:
        value = state.get("rework_branch_input")
        if not isinstance(value, Mapping):
            raise ResearchGraphError("rework_branch_input_missing")
        task = _validate_model(
            BoundBranchTask,
    ...
    def merge_rework(state: ResearchGraphState) -> ResearchGraphState:
        task = _validate_model(
            BoundBranchTask, state.get("rework_task"), label="rework_task"
        )
        ...
        if workpaper.branch_id != task.branch_id or workpaper.revision != 1:
            raise ResearchGraphError("rework_workpaper_binding_mismatch")
        initial = state.get("initial_workpapers_by_branch")
    ...
    def lead_agent(state: ResearchGraphState) -> ResearchGraphState:
        effective_raw = state.get("effective_workpapers_by_branch")
        if not isinstance(effective_raw, Mapping) or not effective_raw:
            raise ResearchGraphError("effective_workpapers_missing")
        ...
        if {key: getattr(output, key) for key in expected} != expected:
            raise ResearchGraphError("lead_output_binding_mismatch")
        _validate_receipt(
            output.runtime_receipt,
            kind="model",
            actor=actor,
        ...
        for conclusion in output.branch_conclusions:
            ...
            if not set(conclusion.evidence_ids).issubset(source.evidence_ids):
                raise ResearchGraphError("lead_unknown_evidence_id")
            if not set(conclusion.fact_ids).issubset(source.fact_ids):
                raise ResearchGraphError("lead_unknown_fact_id")
        return {
            "lead_output": output.model_dump(mode="json"),
            "phase": "lead_completed",
        }

    def verify(state: ResearchGraphState) -> ResearchGraphState:
    ...
    def human_review(state: ResearchGraphState) -> ResearchGraphState:
        raw = interrupt(
            {
    ...
    def render(state: ResearchGraphState) -> ResearchGraphState:
        errors = _verification_errors(state)
        if errors:
            raise ResearchGraphError("render_verification_failed")
        citation_index = _citation_index(state)
        if citation_index != state.get("citation_index"):
            raise ResearchGraphError("citation_index_changed_after_review")
