...
def context_status(state, harness='native'):
    ...
    return {
        ...
        'summary_status':'unavailable' if harness=='hermes' else 'failed' if failure else 'active',
        ...
        'notice':('Hermes 原生会话保存并支持原文回读；自动压缩尚未启用。下方目录展示工作台已保存的公开消息；工具可读取 Hermes 会话原文。接近输入上限时请使用交接入口。'
            if harness=='hermes' else '摘要只用于定位与接续，原始消息、数字凭证和来源独立保留。新用户轮次按需摘要，每轮最多一次；失败不自动重试。'),
