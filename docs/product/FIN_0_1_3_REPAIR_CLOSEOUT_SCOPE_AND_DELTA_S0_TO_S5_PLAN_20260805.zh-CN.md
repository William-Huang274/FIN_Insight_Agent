...
4. [ ] `S1-08-P2C / S1_08_V3_DELL_R3_SUCCESSOR_CLEAN_ZERO_CALL_PREFLIGHT`：proof 前发现并修复 `preflight source_commit == future HEAD` 的提交自引用；现绑定 proven source commit，且未来 execution commit 必须为其后代并保持 Runtime tree 零漂移。修复提交后仍须从 clean archive/fresh process 复现 source SHA、完整 S1-08 contracts、mutation、exact-once 和零外部调用；通过后才另行推进一次 R3 issuance/live scope。
