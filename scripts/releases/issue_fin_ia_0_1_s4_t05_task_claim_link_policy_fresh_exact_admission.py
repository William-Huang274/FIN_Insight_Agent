...
EXPECTED_PROOF_SHA256 = (
    "cc5de889fff7a708afb000d4baae42cefed571cb2f0eb5fdd624f81676bf2b5d"
)
...
class S4T05TaskClaimAdmissionIssuanceError(RuntimeError):
    pass


def _require(condition: bool, code: str) -> None:
    if not condition:
        raise S4T05TaskClaimAdmissionIssuanceError(code)
...
def verify_issued_admission() -> dict[str, Any]:
    ...
    _require(
        ...
        "s4_t05_task_claim_frozen_proof_byte_drift",
    )
    _require(
        proof["status"] == EXPECTED_PROOF_STATUS,
        "s4_t05_task_claim_frozen_proof_status_mismatch",
    )
    _require(
        payload == proof["prospective_admission"]["payload"],
        "s4_t05_task_claim_issued_payload_not_frozen_payload",
    )
    ...
    _require(
        ...
        == EXPECTED_ADMISSION_DIGEST,
        "s4_t05_task_claim_admission_digest_mismatch",
    )
    _require(
        admission.task_claim_link_policy_ref
        == S3_TASK_CLAIM_LINK_POLICY_REF,
        "s4_t05_task_claim_policy_not_explicitly_bound",
    )
    _require(
        issuance["status"] == EXPECTED_ISSUANCE_STATUS,
        "s4_t05_task_claim_issuance_status_mismatch",
    )
    _require(
        issuance["source_proof_decision_sha256"]
        == EXPECTED_PROOF_SHA256,
        "s4_t05_task_claim_issuance_proof_binding_mismatch",
    )
    _require(
        issuance["issued_admission"]["admission_digest"] == digest,
        "s4_t05_task_claim_issuance_admission_binding_mismatch",
    )
    _require(
        issuance["next_action"] == NEXT_ACTION,
        "s4_t05_task_claim_issuance_next_action_mismatch",
    )
    ...
    _require(
        ...
        "s4_t05_task_claim_runner_load_payload_mismatch",
    )
    ...
    _require(provider_calls == 0, "provider_called_during_admission_issuance")
    ...
    _require(
        ...
        "s4_t05_task_claim_work_unit_consumed_before_authority",
    )
    _require(
        identity["attempt_id"] not in snapshot["attempt_ids"],
        "s4_t05_task_claim_attempt_consumed_before_authority",
    )
    _require(
        identity["research_run_id"] not in snapshot["research_run_ids"],
        "s4_t05_task_claim_research_run_consumed_before_authority",
    )
    for prior_run_id in proof["freshness_and_nonreuse"][
        ...
        _require(
            ...
            f"s4_t05_historical_failed_run_missing:{prior_run_id}",
        )
    _require(
        _sha256(database_path) == database_sha_before
        and _tree_digest(object_root) == object_sha_before,
        "s4_t05_task_claim_issuance_changed_target_runtime",
    )
    _require(
        ...
        "s4_t05_task_claim_implementation_binding_drift",
    )
    for relative_path, expected_digest in proof[
        ...
        _require(
            ...
            f"s4_t05_task_claim_runtime_binding_drift:{relative_path}",
        )
    ...
    _require(
        ...
        "s4_t05_task_claim_issuance_boundary_mismatch",
    )
    _require(
        ...
        "s4_t05_task_claim_issuance_counts_mismatch",
    )
    return {
