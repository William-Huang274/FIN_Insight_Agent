...
from retrieval.dell_report_internal_chain_ceiling_r11 import (
    ATTEMPT_ID,
    ATTEMPT_RECEIPT_REF,
    AUTHORITY,
    EXECUTION_CONTRACT,
    ...
    DellReportInternalChainCeilingR11Error,
    assess_dell_report_internal_chain_r11_packages,
    build_dell_report_internal_chain_ceiling_r11_public_projection,
    validate_dell_report_internal_chain_ceiling_r11_policy,
)
from retrieval.dell_report_predicate_frames_r11 import ASP_TARGET
from retrieval.query_plan import canonical_digest
from scripts.data_retrieval import (
    run_dell_report_internal_chain_ceiling_r11 as r11_runner,
)
...
def test_r11_assessment_emits_lossless_complete_frame_transformation() -> None:
    source_id = "SOURCE::DELL::ASP::R11"
    source_rows = [
        _source(
            source_id,
    ...
    result = assess_dell_report_internal_chain_r11_packages(
...
def _r11_projection_fixture() -> dict:
    path = ROOT / (
        "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
        "candidate_ceiling/dell-rsq-03b-internal-chain-r10/full_result.json"
    )
    ...
    bindings = {
        ...
        for binding_id in EXPECTED_BOUND_INPUT_IDS | {"R11_policy"}
    }
...
def test_r11_public_projection_drops_transformation_private_rows() -> None:
    private = _r11_projection_fixture()
    public = build_dell_report_internal_chain_ceiling_r11_public_projection(
...
def _r11_policy_fixture() -> tuple[dict, dict[str, dict]]:
    paths = {
        "R10_policy": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_policy_v1_9.json"
        ),
        "R10_public": ROOT / (
            "configs/retrieval/fin_ia_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling_result_v1_9.json"
        ),
        "R10_private": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r10/full_result.json"
        ),
        "R10_attempt_receipt": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r10/"
            "attempt_consumption_receipt.json"
        ),
        "R10_raw_execution_capture": ROOT / (
            "data/workbench_private/fin_0_1_3_s1_dell_03b_internal_chain_"
            "candidate_ceiling/dell-rsq-03b-internal-chain-r10/"
            "raw_execution_capture.json"
        ),
        "R10_fresh_audit": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_b4a04fec_dell_03b_"
            "r10_fresh_dual_audit_fail_v1_0.json"
        ),
        "R10_fixed_audit_manifest": ROOT / (
            "configs/audits/fin_ia_0_1_3_commit_b4a04fec_dell_03b_"
            "r10_fixed_dual_audit_manifest_v1_0.json"
        ),
    }
    r10_private = json.loads(paths["R10_private"].read_text(encoding="utf-8"))
    inherited = r10_private["input_bindings"]
    for binding_id in (
        "R17_report_audit",
    ...
    policy = {
        ...
        "decision_target": "One bounded R11 exact attempt.",
        "owner_basis": "Owner-approved same-stage R11 implementation.",
        ...
        "TokenBudgetBasis": {
            "node_purpose": "Bounded R11 qualification.",
            ...
            "comparable_run_evidence": "R10 and zero-call R11 preview.",
...
def test_r11_policy_binds_r10_failure_and_r17_14_file_carry_forward() -> None:
    policy, values = _r11_policy_fixture()
    predecessor = validate_dell_report_internal_chain_ceiling_r11_policy(
        policy,
        **values,
    )
    assert predecessor["attempt_id"] == "dell-rsq-03b-internal-chain-r10"


def test_r11_policy_rejects_changed_r17_bundle_population() -> None:
    policy, values = _r11_policy_fixture()
...
def test_r11_runner_requires_explicit_mode() -> None:
    with pytest.raises(SystemExit) as exc_info:
        r11_runner.main([])
    assert exc_info.value.code == 2


def test_r11_formal_fails_before_receipt_when_policy_is_missing(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    monkeypatch.setattr(r11_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r11_runner, "DEFAULT_PUBLIC", tmp_path / "public.json")
    monkeypatch.setattr(
        r11_runner,
        "ATTEMPT_RECEIPT",
    ...
    monkeypatch.setattr(r11_runner, "POLICY", tmp_path / "missing.json")
    monkeypatch.setattr(
        r11_runner,
    ...
    with pytest.raises(FileNotFoundError, match="canonical_policy_missing"):
        r11_runner.run_authorized_formal()
    assert not r11_runner.ATTEMPT_RECEIPT.exists()


def test_r11_raw_capture_precedes_redacted_terminal_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    ...
    monkeypatch.setattr(r11_runner, "RAW_EXECUTION_CAPTURE", raw_path)
    monkeypatch.setattr(r11_runner, "TERMINAL_FAILURE_RECEIPT", terminal_path)
    execution = {"request_results": [{"request_id": "REQ::DELL::ASP::V1"}]}
    sha = hashlib.sha256(
        r11_runner.base._canonical_json_bytes(execution)
    ).hexdigest()
    r11_runner._write_raw_execution_capture(
        policy={"result_digest": "a" * 64},
        execution=execution,
        execution_sha256=sha,
        recorded_at="2026-08-27T00:00:00+00:00",
    )
    receipt = r11_runner._write_terminal_failure_receipt(
        policy={"result_digest": "a" * 64},
...
def test_r11_attempt_receipt_is_exclusive_exact_once(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    receipt = tmp_path / "attempt" / "attempt_consumption_receipt.json"
    monkeypatch.setattr(r11_runner, "ATTEMPT_RECEIPT", receipt)
    kwargs = {
        "policy": {"result_digest": "a" * 64},
        "git_receipt": {
            "head": "b" * 40,
    ...
    first = r11_runner._write_attempt_consumption_receipt(**kwargs)
    ...
    with pytest.raises(FileExistsError, match="attempt_already_consumed"):
        r11_runner._write_attempt_consumption_receipt(**kwargs)


def test_r11_atomic_pair_rolls_back_second_publish_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    private = tmp_path / "private" / "full_result.json"
    ...
    monkeypatch.setattr(r11_runner, "DEFAULT_PRIVATE", private)
    monkeypatch.setattr(r11_runner, "DEFAULT_PUBLIC", public)
    real_link = r11_runner.os.link
    calls = 0
    ...
    monkeypatch.setattr(r11_runner.os, "link", fail_second)
    with pytest.raises(OSError, match="synthetic_second_publish_failure"):
        r11_runner._publish_atomic_pair(
