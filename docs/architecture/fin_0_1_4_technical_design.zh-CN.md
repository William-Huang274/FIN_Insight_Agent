...
2026-09-19用户补充（设计约束，尚未实现验证）：下一步接入写作时，应考虑由已参与研究的上游负责人直接执笔；若使用独立Writer，必须交接上游必要的工作记录，不能让几乎白板的角色仅凭若干结论摘要重新猜研究逻辑。角色上下文保持职责清晰，同时保留与成文有关的研究连续性。直接执笔也须消费当前有效研究状态，不能沿用已失效的判断。
