...
def _make_boundary_decision(
    ...
    *,
    ...
    right_subject = _FRAME_RIGHT_SUBJECT.match(right)
    ...
    elif left_predicate and right_subject and right_predicate:
        decision = "split"
        reason = "coordinator_with_left_predicate_and_new_right_subject_predicate"
    elif not left_predicate and right_predicate:
        left_companies = list(_COMPANY.finditer(left))
        if left_companies and right_subject and _COMPANY.fullmatch(
            right_subject.group(0)
        ):
    ...
    body = {
        ...
        "right_subject_span": _absolute_match_span(
            right_subject,
            sentence_start=sentence_start,
...
def frame_boundary_decisions(text: str) -> list[FrameBoundaryDecision]:
    ...
    for sentence_index, raw_sentence in enumerate(raw_sentences):
        ...
        if sentence_start < 0:
            raise ValueError("R9_normalized_sentence_provenance_lost")
...
def _modifier_frame_id(
    *,
    ...
    return f"MODIFIER::R9::{digest[:24].upper()}"
    ...
    for index, price in enumerate(prices):
        ...
        output.append(
            ArgumentGroupBinding(
                group_id=f"ARG::R9::{index:02d}::{digest[:20].upper()}",
                span_start=body["span_start"],
                span_end=body["span_end"],
                raw_text=text,
...
def _finalize(
    *,
    ...
    body = {
        ...
        # Compatibility aliases are intentionally explicit in the R9 public IR.
        "modality": actuality,
    ...
    return PredicateFrame(
        frame_id=f"FRAME::R9::{digest[:24].upper()}",
        frame_digest=digest,
        assertion_frame_id=assertion_frame_id,
        target_id=target_id,
        sentence_index=record.sentence_index,
...
def _asp_frame(record: FrameRecord) -> PredicateFrame | None:
    ...
    hardware_prices = [
        ...
        if row.object_class == "hardware" and row.ambiguity is None
    ]
    selected_prices = hardware_prices if len(hardware_prices) == 1 else []
    if prices and not selected_prices and any(
...
def extract_predicate_frames(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
    ...
    if target_id not in TARGET_IDS:
        raise ValueError(f"unsupported_R9_target:{target_id}")
...
def classify_package(
    *, target_id: str, text: str, metadata: Mapping[str, Any]
    ...
    assessment["semantic_guard_revision"] = "R9_SPAN_BOUND_PREDICATE_FRAME"
    ...
    if accepted and assessment.get("in_period") is True:
        ...
        limitations.update(
            f"missing_R9_role:{role}" for role in selected.missing_required_roles
        )
        limitations.update(f"R9_ambiguity:{row}" for row in selected.ambiguities)
        ...
        assessment["limitations"] = sorted(
            ...
            | {"no_single_R9_span_bound_target_frame"}
        )
    return assessment
