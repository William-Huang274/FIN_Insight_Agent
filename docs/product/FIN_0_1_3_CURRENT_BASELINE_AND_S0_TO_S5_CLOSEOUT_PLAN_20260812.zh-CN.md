...
25. **DELL 五单元动态完整案例（上下文工程门已通过，自然 live 待执行）**：有限 S2 回归已证明法说叙事不会越权成为 NumericFact；consumer policy v1.3 又为需求真实性、经营表现、价值捕获、现金／资产负债表、瓶颈／反方／WWC 五单元建立最小 RoleMethodPack 和本案即时 GraphContextPack。formal zero-call R2 已确认五个包均为 current-case、cell-local、case-neutral method，跨案／跨单元／未知引用／方法或图未消费均 fail closed。下一步建立一个稳定 exact-once natural runner 和 fresh authority，执行自然五单元，并依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容。一次完整运行不能自动等同 S3 acceptance。
