...
21. **DeepSeek GA Agent loop 资格路线（JSON 单节点通过，标准工具 R1 暴露项目兼容缺陷）**：干净提交 `ae86d8bc...` 已建立标准／JSON 对照／strict Beta 三个可替换 GA profile、capture-first tool-step transport，以及 reviewed Evidence、NumericFact、EvidenceRequest、Judgment 四工具循环。paired R1 的 5,000-token reasoning exhaustion 已由 v1.1 JSON R2 关闭；R2 JSON Judgment 通过本地合同与 L1/L2，节点适用维度 18/24，strict Beta 在 HTTP 前 `URLError` 后停放。标准 Tool Calls R1 则取得 HTTP 200：DeepSeek 正确同时请求 reviewed Evidence 与 NumericFact，但项目归一化器拒绝响应中的 tool-call `index`，循环也只允许一个工具，故没有工具执行或 Judgment。R1 不重试、不追认；当前先做 provider-neutral wire 精确归一化、唯一安全只读并行和 terminal capture ref 修复，再做 clean zero-call proof 与一个 replacement single-cell live。EvidenceRequest 仍只是提案，不能自行检索、关闭 gap 或把候选晋升为 Evidence；五单元继续阻断。
