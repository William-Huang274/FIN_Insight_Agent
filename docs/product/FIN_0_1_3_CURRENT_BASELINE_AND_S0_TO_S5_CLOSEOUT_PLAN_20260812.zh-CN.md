...
25. **DELL 五单元动态完整案例（runner engineering pass，正式零调用与自然 live 待执行）**：有限 S2 回归已证明法说叙事不会越权成为 NumericFact；consumer policy v1.3 又为需求真实性、经营表现、价值捕获、现金／资产负债表、瓶颈／反方／WWC 五单元建立最小 RoleMethodPack 和本案即时 GraphContextPack。formal context zero-call R2 已确认五个包均为 current-case、cell-local、case-neutral method。稳定 runner 现采用一次 planner／S1／S2、五组 cell-local 分析与严格交卷、全单元通过后才做模型自有跨单元综合；单元失败仍继续暴露其余问题，公开结果不保存模型正文。下一步完成 formal runner zero-call、clean authority 和自然 DELL 五单元，再依次验收身份／期间／来源／数值 L1、逐单元判断、跨单元综合、八维绝对质量、paired gain 和 qualified-human 内容。一次完整运行不能自动等同 S3 acceptance。
