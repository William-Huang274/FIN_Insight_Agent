...
21. **DeepSeek GA Agent loop 资格路线（JSON 单节点通过，标准工具循环待资格化）**：干净提交 `ae86d8bc...` 已建立标准／JSON 对照／strict Beta 三个可替换 GA profile、capture-first tool-step transport，以及 reviewed Evidence、NumericFact、EvidenceRequest、Judgment 四工具循环。零调用 R1 已通过：单单元 4 step、五单元 15 step，两次 fresh process 等价，EvidenceRequest 后 gap 仍 open。paired R1 的 5,000-token reasoning exhaustion 已由 v1.1 JSON R2 关闭；R2 JSON Judgment 通过本地合同与 L1/L2，节点适用维度 18/24，strict Beta 则在 HTTP 前 `URLError`，不作 schema 或内容结论且不再阻断核心路线。下一门改为一次标准 API 四工具 DELL 单 cell live；通过后才允许 DELL 五单元。EvidenceRequest 仍只是提案，不能自行检索、关闭 gap 或把候选晋升为 Evidence；私有 reasoning 只在同一次工具 continuation 中瞬时使用。
    - 标准 Tool Calls successor 已在当前工作树实现：复用唯一 canary runner 与唯一核心四工具循环，单 cell 缩到 6-step 硬上限，要求每 cell 先读取 Evidence 与 NumericFact，再提交 Judgment；成功前缀和中途失败均物化 typed receipt/terminal。该实现尚需 clean commit、fresh zero-call successor 和一次真实 single-cell authority，不能因工作树测试全绿提前写成 live pass。五 cell 必须绑定新的 `five_cell_live_authorized=true` 决策，不能复用单 cell R2 权限。
