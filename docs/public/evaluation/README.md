...
2026-09-11 · Open-label local evaluation, not a blind benchmark.
...
These records allow checking the published Hit@5 and anchor Recall@5 arithmetic without an API key. MRR@20 is retained from the original measured receipt; only top-five candidates are distributed here, so this bundle does not independently reconstruct MRR@20. For an exact historical replay, use the [original implementation](https://github.com/William-Huang274/FIN_Insight_Agent/tree/7b8287ab6c4d0af5dec82800f3fd2fa9ef0d4773/scripts/qualification) with its corresponding source corpus. A new corpus requires newly labeled anchors; source IDs cannot be reused against unrelated chunks.
...
For current code checks, follow the [quickstart](../quickstart.en.md). 当前代码的运行与检查见[快速开始](../quickstart.zh-CN.md)。
