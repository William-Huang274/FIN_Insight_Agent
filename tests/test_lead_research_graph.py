...
def _graph(model, worker, *, seed=None, **kwargs):
    ...
    return build_lead_research_graph(expected_input=value, research_question="Dell 供给价格与行业需求的关联是什么？",
...
def test_fresh_lead_uses_current_question_and_only_lead_role_method_without_old_workpapers():
    ...
    graph = build_lead_research_graph(
...
def test_full_scope_requires_eight_new_obligations_and_reuses_q1_without_rebilling():
    from sec_agent.agent_runtime.research_contracts import COVERAGE_OBLIGATION_IDS
    seed, seen, ran = _seed(), [], []
    value = SpecialistAgenticInput.model_validate_json(json.dumps(_input()))
    ...
    def model(request):
        ...
        if len(seen) == 2:
            ...
            tasks = [_task(str(i), branch, (seed["task"]["task_id"],))
                     for i, branch in enumerate(COVERAGE_OBLIGATION_IDS[1:])]
    ...
    graph = build_lead_research_graph(expected_input=value, research_question="Full-scope plumbing fixture, not research.",
        branch_catalog=[{"branch_id": b} for b in COVERAGE_OBLIGATION_IDS],
        allowed_branch_ids=COVERAGE_OBLIGATION_IDS, seed_workpapers={seed["task"]["task_id"]: seed},
    ...
    assert set(ran) == set(COVERAGE_OBLIGATION_IDS[1:])
...
def test_all_nine_existing_branch_methods_can_open_the_same_real_source_tool_composition():
    ...
    _assert_assets()
    for branch in COVERAGE_OBLIGATION_IDS:
        with open_specialist_scripted_qualification_composition(run_id="full-scope-host-check", run_invocation_id="full-scope-host-check-1",
...
def test_actual_a5_to_two_delegated_real_mcp_loops_zero_provider():
    ...
    def worker(task, dependencies, config):
        ...
        with open_specialist_scripted_qualification_composition(run_id="lead-local-qualification", run_invocation_id="lead-local-qualification-1",
    ...
    with open_specialist_scripted_qualification_composition(run_id="lead-local-qualification", run_invocation_id="lead-local-qualification-1",
        ...
        graph = build_lead_research_graph(expected_input=parent.graph_input, research_question="零模型资格：验证双研究任务实际 MCP 流转。",
            branch_catalog=CATALOG, allowed_branch_ids=BRANCHES, seed_workpapers={seed["task"]["task_id"]: seed},
...
def test_q6_actual_source_passage_can_reach_review_without_claiming_reviewed_route_completion():
    from test_specialist_composition import RUNTIME_ENVIRONMENT, _assert_assets
    from sec_agent.agent_runtime.specialist_composition import open_specialist_scripted_qualification_composition
    from sec_agent.agent_runtime.specialist_graph import SpecialistNotebook, SubmitWorkpaperAction, _submission_errors
    _assert_assets()
    calls = []
    def model(request):
        calls.append(request)
    ...
    with open_specialist_scripted_qualification_composition(run_id="lead-source-binding", run_invocation_id="lead-source-binding-1",
...
def test_actual_lead_a1_q6_tool_replay_returns_submission_errors_without_runtime_crash():
    ...
    with open_specialist_scripted_qualification_composition(run_id="lead-a1-q6-local-replay", run_invocation_id="lead-a1-q6-local-replay-1",
...
def test_full_runtime_feedback_is_preserved_and_citable_identity_conflicts_still_fail():
    from sec_agent.agent_runtime.specialist_graph import SpecialistNotebook, SubmitWorkpaperAction, _feedback, _submission_errors
...
def test_provider_task_schema_matches_executable_outputs_and_shared_catalog():
    from sec_agent.agent_runtime.lead_research_graph import DelegateResearchTasksAction, lead_capability_catalog
...
def test_real_q8_rejected_output_is_identified_at_the_exact_field():
    from sec_agent.agent_runtime.lead_research_graph import DelegateResearchTasksAction
...
def test_historical_route_receipt_not_promoted_or_new_question_delivery_gate(current_question):
    ...
    def model(request):
        ...
        return _stop(request, ready=True)
    graph = build_lead_research_graph(expected_input=value, research_question='Current bounded request',
        branch_catalog=CATALOG, allowed_branch_ids=BRANCHES, seed_workpapers={seed['task']['task_id']:seed},
