...
clean proof 的文本 binding 使用 `SHA256(UTF8(universal_newline_text))`，不得直接绑定 checkout-specific raw bytes。Git 管理的 JSON、Markdown、Python 与其他文本均遵循此规则；PDF、压缩包、原始 HTTP body 和 private evidence object 等二进制／审计 bytes 仍按 raw SHA。R1 因 CRLF/LF 差异在 policy binding 阶段终止，说明该规则属于可移植 authority contract，而非 proof runner 的可忽略便利项。

R2 在两个 clean archive 中重新加载 normalized bindings 和 byte-exact private artifacts，输出 byte-equivalent。worker 使用共享 ledger 分别终态化 success、transport、length 和 invalid JSON fixture；duplicate admission 在新 attempt root 仍被拒绝。proof 只记录 normalized result、capture digest／存在性和 typed code，不持久化临时 Pack 或 raw model fixture content。该结果允许下一项做 live authority 的零调用 value／cost／risk 决策，不允许 runner 自行从 fixture admission 切换到 live。

live path 必须使用与 zero-call scope 不同的 `FIN_0_1_3_S2_SELECTED_EVIDENCE_NUMERIC_NATURAL_NODE_CANARY_LIVE`，并由新的 authority artifact 绑定 implementation commit、clean proof digest、policy/profile normalized SHA、compiled input digest、request digest 和 runner SHA。live admission 的 `authority_kind／execution_mode／live_authority` 不能由 fixture 字段局部修改得到；validator 必须同时看到签发 authority。runner 只做 credential presence 检查，值只在 transport 内存中使用；shared ledger 位于 attempt root 之外。当前 decision 只授权实现和签一份未消费 admission，不授权 Provider invocation。
