...
class TencentWSAExactCopyR3RunnerError(RuntimeError):
...
def _validate_authority_bindings(
    ...
    required = {
        ...
        "support_sha256": ROOT
        / "src/sec_agent/s1_08_tencent_wsa_exact_copy_ak_sk_r3.py",
        ...
        / "configs/releases/fin_ia_0_1_3_s1_08_tencent_wsa_query_only_replacement_result_v1_0.json",
        "predecessor_assessment_sha256": ROOT
        / "configs/releases/fin_ia_0_1_3_s1_08_tencent_wsa_query_only_replacement_failure_assessment_v1_0.json",
        "zero_call_proof_sha256": ROOT
        / "configs/releases/fin_ia_0_1_3_s1_08_tencent_wsa_exact_copy_ak_sk_r3_zero_call_proof_v1_0.json",
    }
    for field, path in required.items():
        if not path.is_file() or bindings.get(field) != _sha256(path):
            raise TencentWSAExactCopyR3RunnerError(
                "tencent_wsa_exact_copy_r3_authority_binding_invalid"
            )
    ...
    if (
        ...
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_proof_binding_invalid"
        )
    if authority_path.resolve() == required["zero_call_proof_sha256"].resolve():
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_authority_path_invalid"
        )
...
def run_diagnostic(
    *,
    ...
    if output_path.exists() or terminal_path.exists():
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_exact_once_result_already_exists"
        )
    if _git("status", "--porcelain"):
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_clean_worktree_required"
        )
    source_commit = _git("rev-parse", "HEAD")
    authority = load_exact_copy_r3_authority(authority_path)
    ...
    if canonical_digest(profile) != (
        ...
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_profile_digest_invalid"
        )
    preflight = run_project_os_preflight(ROOT, run_scope=RUN_SCOPE)
    if preflight.get("status") != "pass":
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_project_os_preflight_failed"
        )
    ...
    if not secret_id or not secret_key:
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_runtime_credentials_missing"
        )
    ...
    try:
        ...
        terminal = build_exact_copy_r3_terminal_result(
            ...
            status="completed",
            terminal_code="tencent_wsa_exact_copy_r3_response_materialized",
        ...
        terminal = build_exact_copy_r3_terminal_result(
            ...
            source_commit=source_commit,
            status="failed",
            terminal_code="tencent_wsa_exact_copy_r3_typed_failure",
            request_capture=request_capture,
    ...
    if secret_id in serialized or secret_key in serialized:
        raise TencentWSAExactCopyR3RunnerError(
            "tencent_wsa_exact_copy_r3_secret_redaction_failed"
        )
...
def main() -> int:
    ...
    except (TencentWSAExactCopyR3RunnerError, TencentWSAExactCopyR3Error) as exc:
